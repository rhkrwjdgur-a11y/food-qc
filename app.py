...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V109.0 - 교차오염 스나이퍼 추가)")
    ...
    with tab1:
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("주표시면 기계적 텍스트 추출 및 정밀 대조 중..."):
                ...
                4. 🚨 [수량 2-Track 검증 (연산 & 텍스트)]: 1) [연산]: '단위용량 x 개수 = 총용량' (예: 190mL x 24 = 4560mL)의 산수가 정확한지 무조건 곱하기를 수행하여 적발하십시오. 2) [텍스트]: 계산이 맞더라도, 다른 면에 적힌 수량 텍스트 형태(개입/팩, 소문자x/대문자X)와 단 한 글자라도 다르면 무조건 🚨부적합 처리하십시오.
                </thinking>
            ...
            with st.spinner("서류 유형 분석 및 창의성 배제 100% 교차 검증 중..."):
                ...
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        3. 🚨 [누락 서류 조작(Auto-fill) 절대 금지]: 서류가 업로드되지 않은 경우, 해당 열에는 반드시 "제공된 서류 없음" 또는 "-"로 기재하십시오. 빈칸을 채우려고 시안의 글자를 복사해 넣는 환각을 엄격히 금지합니다.
                        4. 🚨 [기계적 1:1 대조 절대 명령]: 업로드된 서류, 박스, 팩의 텍스트를 기계적으로 1:1 검증하십시오.
                        5. 🚨 [수량 2-Track 검증 (연산 & 텍스트)]: 하단 구석의 수량을 추출하여 수학적 곱하기 연산 및 텍스트 형태(대소문자, 단어)가 다른 면과 완벽히 일치하는지 둘 다 검증하십시오.
                        </thinking>
                        ...
                        3. 🚨 [오류 유형별 그룹화 절대 금지]: 에러 종류별로 묶어서 표를 쪼개지 마십시오. 반드시 시안에 적힌 원재료 순서 그대로 순차적으로 작성하십시오.
                    ...
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        3. 🚨 [누락 서류 조작(Auto-fill) 절대 금지]: 서류가 업로드되지 않은 원료의 경우, 서류 열에는 반드시 "제공된 서류 없음" 또는 "-"로 기재하십시오. 
        ...
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("오차 계산 및 100% 전체 영양성분 3방향 대조 렌더링 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    ...
                    3. 🚨 [누락 서류 조작(Auto-fill) 절대 금지]: 성적서 등 기준 서류가 업로드되지 않은 경우, 해당 열에는 반드시 "제공된 서류 없음" 또는 "-"로 기재하십시오. 빈칸을 채우려고 시안의 수치를 복사해서 붙여넣는 조작(환각)을 엄격히 금지합니다.
                    4. 오차 연산: 서류 실측값 대비 표시량이 오차 범위 내인지 계산하십시오. (서류가 없으면 생략)
                    5. 일치 여부 확인: 팩과 박스의 숫자가 100% 동일한지 대조하십시오.
                    6. 🚨 [수량 2-Track 검증 (연산 & 텍스트)]: 총 내용량 정보를 추출하여 1) 수학적 곱하기 연산이 정확한지 검증하고, 2) 텍스트 형태(팩/개입, 대소문자 X)가 다른 면과 완벽히 일치하는지 둘 다 검증하십시오.
                    </thinking>
                    ...
                    2. 🚨 [소수점 및 행열 스나이퍼 절대 명령]: 영양성분표의 숫자를 추출할 때, **소수점(.)**을 절대 놓치지 마십시오. 행열 이탈을 절대 금지합니다. 
                    3. 🚨 [누락 서류 조작(Auto-fill) 절대 금지]: 기준 서류가 업로드되지 않은 경우, 해당 열에는 반드시 "제공된 서류 없음" 또는 "-"로 기재하십시오. 시안의 수치를 복사해 넣는 환각을 엄격히 금지합니다.
                    4. 오차 연산: 서류 대비 표시량이 오차 범위 내인지 계산하십시오. (서류 없으면 생략)
                    5. 🚨 [수량 2-Track 검증 (연산 & 텍스트)]: 총 내용량 정보를 추출하여 수학적 연산 및 텍스트 형태가 다른 면과 완벽히 일치하는지 둘 다 검증하십시오.
                    </thinking>
    ...
    with tab4:
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("기타면/측면 기계적 텍스트 검토 중..."):
                ...
                3. 🚨 [수량 2-Track 검증 (연산 & 텍스트)]: 기재된 수량 정보가 1) 수학적 곱하기 연산이 정확한지 검증하고, 2) 텍스트 형태(글자, 기호)가 다른 모든 면과 100% 일치하는지 둘 다 기계적으로 검증하십시오.
                4. 🚨 [Rule 38 알레르기 교차오염 스캔 명령]: 기타면에 기재된 주의사항 중 '제조시설 공유(교차오염)' 안내 문구(예: "...같은 시설에서 제조하고 있습니다")를 샅샅이 스캔하고 필사하십시오. 이 문구 안의 알레르기 물질이 제품의 원재료명 리스트에 이미 들어있는 물질과 중복되지는 않는지(Rule 38 위반) 기계적으로 대조하여 적발하십시오.
                </thinking>
                
                ## 6️⃣ [기타면/측면 표시사항 및 마케팅 뱃지]
                - 결론: (✅ 적합 또는 🚨 부적합/확인요망)
                - 🚨 [Rule 38] 알레르기 교차오염 문구 적합성: (기재 여부 및 원재료와 중복 기재 등 모순이 없는지 명시)
        ...
        if st.button("▶️ 최종 종합 리포트 생성", key="btn_summary"):
            if not any([st.session_state["result_tab1"], st.session_state["result_tab2"], st.session_state["result_tab3"], st.session_state["result_tab4"]]):
                ...
                with st.spinner("모든 분석 데이터를 병합하여 최종 수정 지시서를 작성 중입니다..."):
                    ...
                    ### 📌 [핵심 지적 사항 및 수정 지시]
                    (위 분석 데이터에서 '부적합(🚨)' 또는 '확인요망'이 나온 내용들만 뽑아서 1, 2, 3번 불릿 포인트로 요약하십시오. 특히 '포장 단위 충돌(산수/텍스트)', '띄어쓰기 오탈자', '고형분 누락'은 가장 중요하게 다루십시오.)
