...
🔥 [오탈자 무관용 원칙]: 단어의 의미가 통하더라도 글자(자음/모음)가 단 하나라도 다르면 무조건 부적합 처리하십시오. 단, 단순 띄어쓰기 차이로 억지 부적합을 남발하여 핵심 오타를 가리는 행위는 자제하십시오.
...
🔥 **Rule 3. [주표시면 vs 영양성분표 수치 100% 일치 및 총열량/식이섬유 열량 계산 강제 룰]**
   - 주표시면(앞면)에 열량(kcal)이나 특정 영양소 함량(예: 칼슘 200mg)이 강조되어 있다면, 반드시 뒷면 영양성분표의 수치와 단 단 1의 오차도 없이 100% 일치하는지 교차 대조하십시오.
...
🔥 **Rule 28. [원산지 3순위 완벽 필터링 및 과잉 표기 경고 룰]**
   - 1) **[3순위 강제 도출]**: 정제수, 주정, 당류, 식품첨가물을 원산지 산정 순위에서 강제로 제외하십시오. 남은 진짜 원료 중 상위 1위, 2위, 3위 원료를 정확히 도출하고, 이 3개 중 단 하나라도 라벨에 원산지가 누락되었다면 🚨부적합 처리하십시오.
   - 2) **[과잉 도출 경고]**: 만약 의무 3순위에도 해당하지 않고, 제품명 강조 대상도 아닌 '하위 부원료'에 원산지가 적혀 있다면 법적 위반은 아니나, 무조건 🚨**[확인 요망]**을 띄우고 "의무 3순위 외의 원료(OOO)에 원산지가 자발적으로 과잉 표기되었습니다. 향후 원산지 변경 시 리스크가 있으므로 실제 서류와 100% 일치하는지 실무자 재검증이 필요합니다"라고 명시하여 사용자에게 경고하십시오.

🔥 **Rule 29. [복합원재료 원산지 표시 한계]**
   - 복합원재료 자체의 원산지만 확인하십시오. 하위 성분 각각의 원산지까지 강제하지 마십시오. (단, Rule 28에 따라 강조 원료 도출 시에는 예외)
...
🔥 **Rule 44. [혼합제제 전개 및 해체 병합(Merge) 완벽 허용 룰]**
   ...
   - [방식 B (완전 해체 병합 - 통합 룰)]: 혼합제제의 괄호를 완전히 없애고, 하위 부형제(포도당, 전분, 아라비아검 등)들을 다른 원료들과 섞어 최종 투입량(%) 기준 내림차순으로 뿔뿔이 흩어지게 재배열(Merge)하여 기재하는 것도 식약처 고시상 100% 합법입니다. 
   - ⭐ **[가장 중요한 금지 명령]:** 방식 B처럼 부형제나 비타민 성분들이 괄호 없이 개별 원료처럼 흩어져 있다고 해서 "혼합제제명 뒤에 괄호로 안 묶었다"며 부적합(🚨)을 때리는 멍청한 짓을 절대 금지합니다. 해체되어 흩어져 있으면 완벽한 합법(✅)으로 인정하십시오.
...
🔥 **Rule 61. [모든 복합원재료 원산지 표기 범용 타겟팅 룰 (가공국 vs 원물국 분리 엄격 적용)]**
   - 명칭이 '페이스트, 농축액, 추출액, 분말, 유지류(기름)' 등으로 끝나는 모든 가공 원료의 원산지 표기 시, 이름에 원물명이 들어있더라도 반드시 괄호 안에 '원물명-국가명' 형태로 정확히 타겟팅하여 병기해야 합니다. (외국산 원료를 국내에서 가공했을 때의 가공국 오인 방지 목적)
   - [적합✅ 예시]: `검은참깨페이스트(검은참깨: 미얀마산)`, `검은콩농축액(검은콩: 중국산)`, `옥배유(옥수수: 외국산)`
   - [부적합🚨 예시]: `검은참깨페이스트(미얀마산)`, `검은콩농축액(중국산)`, `옥배유(외국산)` (원물명 병기 누락 시 무조건 부적합 처리)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V156.2 - 오탈자 무관용/뇌분리 완결판)")
    ...
    def run_qc_3pass(tab_rules: str, judgment_prompt: str):
        ...
        # ── PASS 1: 순수 텍스트 추출 및 증빙 서류 마스터링 (뇌 구조 분리 패치) ──
        pass1_prompt = """
[PASS 1 - 텍스트 추출 및 서류 파싱 전용 명령]
⭐ 이 단계에서는 판정이나 평가를 절대 금지합니다. 오직 아래 두 가지 미션만 수행하십시오.

🎯 [미션 A: 라벨 시안 이미지 추출 (환각 및 오타 교정 절대 차단)]
- 당신은 인간의 맥락 이해 능력이 없는 '바보 광학 스캐너(OCR)'입니다. 시안 이미지(JPG, PNG)에 오타(예: 토코페일아세티이트)가 있더라도 절대 올바른 단어로 교정하지 말고 기괴한 글자 그대로 100% 필사하십시오.
- 물리적 배열 순서(Top-to-Bottom, Left-to-Right)를 반드시 유지하십시오.
- 이 작업을 할 때 '미션 B'에서 읽은 서류 내용에 절대 영향받지 마십시오.

🎯 [미션 B: PDF 증빙 서류 파싱 (마스터 정답지 선행 구축)]
- 사용자가 업로드한 '증빙 서류(한글라벨, 배합비 엑셀, 성적서 PDF 등)'를 샅샅이 분석하여, 시안 검토의 기준이 될 [절대 정답지 체크리스트]를 구축하십시오.
- 특히 혼합제제의 하위 성분(예: 비타민D3혼합제제 안의 '가공유지', '팜유' 등)을 단 하나도 빠짐없이 100% 추출하여 표 형태로 정리하십시오. 
- 서류에 등장하는 각 원료별로 1) 식품유형, 2) 원료 제품명, 3) 한글표시사항(하위 전개 성분 전부 포함), 4) 원산지를 빠짐없이 추출하십시오.
- ⭐ [가장 중요한 명령]: 이 마스터 정답지를 구축하는 동안, 시안 이미지는 절대 참고하거나 컨닝하지 마십시오. 오직 서류에 있는 사실만으로 완전 무결한 리스트를 만드십시오.
...
"""
        ...
        # ── PASS 1.5: 자체 검증 (배열 순서 및 서류 누락 정밀 대조) ──
        pass15_prompt = f"""
...
검증 규칙:
1. ⭐ [미션 A (시안) 검증]: Pass 1 텍스트가 '올바른 맞춤법'으로 적혀 있다면 원본 이미지와 다시 대조하십시오. AI가 마음대로 오타를 교정했다면 원본 이미지의 오타 그대로 훼손시켜 복구하십시오. 배열 순서가 시안의 실제 흐름과 일치하는지 강제 정렬하십시오.
2. ⭐ [미션 B (서류) 검증]: 사용자가 업로드한 PDF/엑셀 서류 내용 중, Pass 1이 실수로 빼먹은 원료나 하위 성분(예: '가공유지' 등 구석에 숨은 부형제), 원산지가 없는지 샅샅이 뒤져서 100% 채워 넣으십시오.
...
🔥 [마스터 표 절대 독립 및 누락 적발 강제 명령 (가장 중요)] 🔥
1. [마스터 표 작성 시 시안 컨닝 금지]: 1번 마스터표를 작성할 때 '시안 텍스트'는 절대 쳐다보지도 마십시오. 오직 '증빙 서류(미션 B)'만 분석하여 혼합제제의 하위 성분(예: 가공유지 등)까지 단 하나도 빠짐없이 100% 영혼까지 끌어모아 완벽한 정답지(마스터표)를 먼저 완성하십시오.
2. [체크리스트 기반 누락 적발]: 2번 대조 표를 작성할 때, 1번 마스터표에 존재하는 원료/성분이 시안 텍스트에 단 하나라도 빠져있다면(누락), 즉시 🚨부적합(서류상 존재하는 원료 누락)으로 판정하고 팩트를 폭격하십시오!
...
1. ⭐ [원산지 도출 로직 명시 (가장 중요)]: 표의 '원산지 산정 순위' 열에 이 원료가 왜 원산지가 적혔는지(또는 안 적혔는지) 명확히 기재하십시오. (출력 예시: `1순위(의무)`, `제외대상(당류)`, `제외대상(첨가물)`, `순위외(제품명 강조)`, `순위외(과잉표기 경고)`)
2. [순서 검증]: 해당 원료가 규격서 배합비 투입량에 맞게 내림차순으로 나열되었는가? (단, 2% 미만 소량 원료는 순서 유연성 인정)
3. [원산지 검증]: 해당 원료가 농수산물인 경우, 원산지가 원재료명 '바로 뒤 괄호' 안에 합법적으로 표기되었는가? (Rule 28 연계)
4. 🔥 [혼합제제 강제 합법 판정]: 시안에 비타민이나 부형제가 괄호 없이 뿔뿔이 흩어져 적혀 있다면, 그것은 Rule 44의 [방식 B]를 따른 완벽한 합법(✅적합)입니다. 당신은 절대로 "괄호()로 묶어라", "구분 없이 나열되었다"라는 지적을 출력해서는 안 됩니다.
5. 🔥 [표 작성 절대 규칙]: 귀찮다고 여러 원료를 한 줄(Row)에 쉼표로 묶어서 표기하지 마십시오. 비타민류 10개가 해체되어 있다면 표의 행(Row)도 반드시 10개를 생성하여 1:1로 판정하십시오.
6. ⭐ [가공국 vs 원물국 범용 검증 (Rule 61)]: 명칭이 '페이스트, 농축액, 분말' 등으로 끝나는 모든 복합원재료에 대해 '원물명-국가명' 형태(예: `검은콩농축액(검은콩:중국산)`)로 정확히 타겟팅되어 적혀있는가?
7. 🔥 [글자 단위 완전 일치 강제 명령 (오탈자 무관용)]: 당신은 글자의 '의미'를 이해하는 AI가 아니라, 글자의 '형태'만 대조하는 시각 판독기입니다. 서류 정답지와 시안의 글자를 1:1로 대조할 때, "토코페일"과 "토코페릴"처럼 단 한 글자의 자음/모음이라도 다르면 의미가 같더라도 절대 합격(✅)시키지 마십시오. 무조건 🚨부적합(오탈자 적발)으로 판정하고 어느 글자가 틀렸는지 지적하십시오. (단, 단순 띄어쓰기 차이는 유연하게 넘어갈 것)
"""
...
| 통합 서류 기준 원재료명 | 원산지 산정 순위 | 타겟(박스) 시안 표기 | 비교용(팩) 시안 표기 | 판정 (Rule 기반 상세 사유 필수 포함) |
|---|---|---|---|---|
| (1행 1원료, 100% 나열) | (예: 1순위, 제외(당류), 순위외(강조) 등) | [내용 작성] | [내용 작성] | [내용 작성] |
...
| 통합 서류 기준 원재료명 | 원산지 산정 순위 | 시안 대조 결과 | 판정 (Rule 기반 상세 사유 필수 포함) |
|---|---|---|---|
| (1행 1원료, 100% 나열) | (예: 1순위, 제외(당류), 순위외(강조) 등) | [내용 작성] | [내용 작성] |
...
|---|---|---|---|---|---|
| (1행 1원료, 100% 나열) | [내용 작성] | [내용 작성] | [내용 작성] | [내용 작성] | [내용 작성] |

## 2️⃣ [원재료명 3방향 정밀 교차 검증 (위 마스터 표 vs 박스 시안 vs 팩 시안)]
- 결론: (✅ 적합 또는 🚨 부적합 또는 🚨 확인 요망)

| 위 마스터 표 기준 원재료명 | 원산지 산정 순위 | 타겟(박스) 시안 표기 | 비교용(팩) 시안 표기 | 판정 (Rule 기반 상세 사유 필수 포함) |
|---|---|---|---|---|
| (100% 전체 나열) | (예: 1순위, 제외(당류), 순위외(강조) 등) | [내용 작성] | [내용 작성] | [내용 작성] |
...
| 위 마스터 표 기준 원재료명 | 원산지 산정 순위 | 시안 대조 결과 | 판정 (Rule 기반 상세 사유 필수 포함) |
|---|---|---|---|
| (100% 전체 나열) | (예: 1순위, 제외(당류), 순위외(강조) 등) | [내용 작성] | [내용 작성] |
