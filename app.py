...
🔥 **Rule 11. [영양정보 단방향 허용오차 법칙 (수학적 역산 절대 금지!)]**
   - **[⭐ 하한선 그룹 (비타민, 미네랄, 단백질, 탄수화물, 식이섬유 등)]**: 
     반드시 `(성적서 실측값) >= (시안 표시량 × 0.8)` 공식만 대입하십시오. 
     예를 들어 실측값이 4.22g이고 시안 표시량이 2.6g이라면, 4.22 >= (2.6 * 0.8 = 2.08) 이므로 제조사의 보수적 표기로 완벽한 합법(✅)입니다! 절대 `(시안 표시량 ÷ 실측값)`으로 거꾸로 나누어 부적합을 남발하는 바보 같은 수학적 환각을 엄격히 금지합니다.
   - **[⭐ 상한선 그룹 (열량, 나트륨, 당류, 지방, 트랜스지방, 포화지방, 콜레스테롤 등)]**: `(실측값) <= (시안 표시량 × 1.2)` 이면 합법(✅).
...
🔥 **Rule 28. [원산지 3순위 완벽 필터링 및 강제 재정렬 룰 (숫자 세기 환각 절대 금지!)]**
   - 1) **[제외 대상 100% 삭제]**: 정제수(물), 주정, 당류, 식품첨가물은 물론이고 **유산균, 효모, 사균체 분말, 포스트바이오틱스 등 '미생물류' 역시 농수산물이 아니므로 원산지 산정 대상에서 100% 강제 제외(삭제)**하십시오.
   - 2) **[법적 순위 강제 재정렬]**: 텍스트의 단순 나열 순서(앞에서 몇 번째인지)를 원산지 순위로 착각하는 멍청한 카운팅을 절대 금지합니다! 위 1)번의 제외 대상을 완전히 소거한 후, **남은 '진짜 농수산물 및 가공품' 원료들만 모아서 배합량(표기 순서)대로 새롭게 [법적 1순위], [법적 2순위], [법적 3순위] 계급을 다시 부여**하십시오.
   - 3) 이 새롭게 도출된 1, 2, 3순위 원료 중 시안에 원산지가 누락된 것이 있다면 가차 없이 🚨부적합 처리하십시오. 의무 대상이 아닌 4순위 이하 하위 부원료에 원산지가 적혀 있다면 무조건 🚨**[확인 요망]**을 띄워 경고하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V307.0 - 원산지 족쇄 최종 완성판)")
    ...
    def display_result(result, tab_name=""):
        ...
        if pass15_match:
            ...
            with st.expander(f"✅ Pass 1.5 자체검증 완료본 보기 ({tab_name}) ← 실제 판정에 사용된 텍스트"):
                st.info("💡 Pass 1.5는 Pass 1 추출본을 이미지와 재대조하여 오독/환각을 제거한 최종 확정 텍스트입니다. (만약 여기에 '등 다수'라고 뭉뚱그려져 있다면 기계가 스킵 에러를 낸 것입니다!)")
...
## 🔍 [주표시면 전용: 오탈자 검증]
- ⭐ 띄어쓰기 및 오탈자 적발: (오직 주표시면 텍스트에 한정하여 100% 무관용 전수 검사 수행. 다른 구역 지적 금지.)
"""
        ...
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【분할 미션 스캔 중... (시간이 다소 소요됩니다)】"):
                missions = [
                    ...
                    "정보표시면의 행정 정보(제조원, 포장재질, 품목제조보고번호) 추출.",
                    "증빙 서류의 모든 원료명, 하위 성분, 원산지를 표로 추출. (절대 '등 다수'로 축약 금지!)",
...
⭐ [강제 지시]: 팩 시안이 존재한다면 박스와의 차이점을 반드시 아래 표 양식으로 출력하십시오.
| 구분 항목 | 타겟(박스) 시안 | 비교용(팩) 시안 | 판정 사유 및 수정 제안 |
|---|---|---|---|

## 3️⃣ [마스터 서류 vs 시안 법적 교차 검증]
⭐ [강제 지시]: 1:1 대조 결과를 반드시 아래 표 양식으로 출력하십시오. '등 다수'로 묶어 표를 끊어버리면 시스템이 붕괴됩니다.
...
## 🏛️ 6️⃣ [행정 정보 교차 검증]
## 🔍 [정보표시면 전용: 오탈자 검증]
- ⭐ 띄어쓰기 및 오탈자 적발: (오직 정보표시면 텍스트에 한정하여 무관용 전수 검사. 영양성분 등 다른 구역은 절대 지적 금지.)
"""
                # 통합 자료인 경우 마스터표 생략
    ...
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【분할 미션 스캔 중... (잠시만 기다려주세요)】"):
                ...
                # 모드에 따른 다이나믹 프롬프트 설정 (단품 검사 시 쓸데없는 칸 날림)
...
⭐ [강제 지시]: 사용자가 성적서 서류를 업로드하지 않았더라도 핑계 대지 말고 아래 표는 100% 그려야 합니다! 성적서가 없으면 '성적서 환산값(A)' 란에 '서류 없음'이라고 적으십시오. 단품 모드이므로 팩/박스 교차 대조는 생략합니다.
⭐ [부적합 사유 증명 필수]: 부적합(🚨) 판정 시 반드시 "왜 부적합인지" (성적서 실측값 A)와 (법적 기준선)을 비교한 수학적 부등호 수식(예: 9.74 < 14.4)을 <판정 및 상세 사유> 란에 기재하여 증명하십시오.
...
## 🔍 [영양성분표 전용: 오탈자 검증]
- ⭐ 띄어쓰기 및 오탈자 적발: (오직 영양성분표 텍스트에 한정하여 무관용 전수 검사)
"""
                else:
                    judgment_prompt = """
...
- 결론: (✅ 적합 또는 🚨 부적합)
⭐ [계산 검증 원칙]: 하한선 그룹(비타민/단백질 등)은 무조건 (성적서 실측값) >= (법적 기준선) 이면 ✅적합입니다. 절대 역산하지 마십시오.
⭐ [강제 지시]: 사용자가 성적서 서류를 업로드하지 않았더라도 핑계 대지 말고 아래 표는 100% 그려야 합니다! 성적서가 없으면 '성적서 환산값(A)' 란에 '서류 없음'이라고 적으십시오.
⭐ [부적합 사유 증명 필수]: 부적합(🚨) 판정 시 반드시 "왜 부적합인지" (성적서 실측값 A)와 (법적 기준선)을 비교한 수학적 부등호 수식(예: 9.74 < 14.4)을 <판정 및 상세 사유> 란에 기재하여 증명하십시오.

| 영양성분 | 성적서 환산값(A) | 비교용(팩) 시안(B) | 타겟(박스) 시안(C) | 팩/박스 일치 여부 | 법적 기준선 (표시량의 80% 또는 120%) | 🎯 % 계산 검증 | 판정 및 상세 사유 (수식 증명 필수) |
|---|---|---|---|---|---|---|---|
        ...
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("【분할 미션 스캔 중... (잠시만 기다려주세요)】"):
                missions = [
                    ...
                    "포장재질 표기(뚜껑, 본체 등 세부 재질 포함) 및 CS 방어 주의문구 추출."
                ]
                judgment_prompt = """
...
## 🔍 [기타면/측면 전용: 오탈자 검증]
- ⭐ 오탈자 및 띄어쓰기 적발: (오직 기타면/측면 텍스트에 한정하여 100% 무관용 전수 검사 수행. 다른 구역 지적 금지.)
"""
