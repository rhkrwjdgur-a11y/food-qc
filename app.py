...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V77.0 - 셧다운 방지 리스트+표 하이브리드)")
    ...
    with tab2:
        st.info("사용자의 요청에 따라 모든 원료를 생략 없이 보여주되, 시스템 셧다운을 막기 위해 '체크리스트 + 에러 상세표' 구조로 출력합니다.")
        if st.button("▶️ 정보표시면 원재료 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("모든 원재료를 생략 없이 100% 나열하면서 에러를 추출하는 중입니다... (최대 40초 소요)"):
                
                if inspection_mode == "선물세트 교차 검토 (팩 vs 박스 비교)":
                    prompt = """
                    [지시]: '선물세트 교차 검토' 모드입니다. 업로드된 **[비교용 서류(한글라벨 PDF)]**를 기준으로 **[팩(내포장) 시안]**과 **[박스(외포장) 시안]**이 일치하는지 교차 검증하십시오.
                    
                    🚨 [사용자 절대 명령: 시스템 셧다운 방지 '하이브리드 출력' 적용] 🚨
                    원재료가 너무 많아 표(Table)로 50줄을 그리면 글자 수 초과로 뻗습니다! 따라서 반드시 아래의 **[1. 완벽 일치 항목 리스트]**와 **[2. 불일치/에러 항목 상세 표]** 두 가지로 나누어서 출력하십시오. 절대로 50줄짜리 3단 표를 통째로 그리지 마십시오!
                    
                    ## 2️⃣ [서류 기준: 팩(내포장) vs 박스(외포장) 전수 대조 결과]
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    
                    ### 🟢 [100% 완벽 일치 항목 리스트] (생략 금지)
                    (서류, 팩, 박스가 토씨 하나 안 틀리고 완벽하게 일치하는 원료들은 표를 그리지 말고, 아래처럼 쉼표(,)로 연결하여 빠짐없이 나열하십시오. 사용자가 눈으로 확인할 수 있도록 1번부터 끝까지 단 한 개도 빼먹으면 안 됩니다!)
                    - **일치 항목:** (예: 정제수, A2단백원유(국산), 분리유단백(외국산:캐나다, 스페인 등), 분리대두단백, 탈지분유, 농축우유단백, 말토덱스트린, 알룰로스, 채종유 ... 모든 일치 원료를 끝까지 쉼표로 나열할 것)
                    
                    ### 🔴 [🚨 불일치 / 누락 / 오타 / 룰 위반 항목 상세 표]
                    (오직 팩이나 박스에서 글자가 틀렸거나, 누락되었거나, 순서가 바뀌었거나, Rule 44 등을 위반한 '문제 원료'들만 뽑아서 아래 표로 상세히 전개하십시오. 문제가 하나도 없다면 "➖ 전 항목 완벽 일치"라고 적으십시오.)
                    
                    | 서류 매칭 원료 (비교용 기준) | 팩(내포장) 시안 텍스트 | 박스(외포장) 시안 텍스트 | 대조 결과 (순서/누락/오타) | 판정 |
                    |---|---|---|---|---|
                    | (문제 원료 1) | ... | ... | ... | 🚨 |
                    | (문제 원료 2) | ... | ... | ... | 🚨 |
                    ...
                    🚨 [사용자 절대 명령: 시스템 셧다운 방지 '하이브리드 출력' 적용] 🚨
                    원재료가 너무 많아 표(Table)로 50줄을 그리면 글자 수 초과로 뻗습니다! 따라서 반드시 아래의 **[1. 완벽 일치 항목 리스트]**와 **[2. 불일치/에러 항목 상세 표]** 두 가지로 나누어서 출력하십시오. 절대로 거대한 표를 통째로 그리지 마십시오!
                    
                    ## 2️⃣ [원재료명 1:1 정밀 대조 및 원산지 검증]
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    
                    ### 🟢 [100% 완벽 일치 항목 리스트] (생략 금지)
                    (서류와 시안이 토씨 하나 안 틀리고 완벽하게 일치하는 원료들은 표를 그리지 말고, 아래처럼 쉼표(,)로 연결하여 빠짐없이 나열하십시오. 사용자가 눈으로 확인할 수 있도록 단 한 개도 빼먹으면 안 됩니다!)
                    - **일치 항목:** (예: 정제수, 유함유가공품(프랑스산/버터밀크 등), 설탕, 기타가공품 ... 모든 일치 원료를 끝까지 쉼표로 나열할 것)
                    
                    ### 🔴 [🚨 불일치 / 누락 / 오타 / 룰 위반 항목 상세 표]
                    (오직 시안에서 글자가 틀렸거나, 누락되었거나, Rule 44 등을 위반한 '문제 원료'들만 뽑아서 아래 표로 상세히 전개하십시오. 문제가 하나도 없다면 "➖ 전 항목 완벽 일치"라고 적으십시오.)
