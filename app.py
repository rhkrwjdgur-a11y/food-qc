...
🔥 **Rule 35. [🌟 범용 간략명/동의어 허용 및 N종 묶음 예외 보장 (강력 예외 룰 V194.0)]**
   ...
   - **[🚨 'N종' 묶음 간략화 금지 및 예외]**: '비타민 3종', '야채농축액 5종'처럼 하위 성분을 '숨길 목적'으로 숫자로 묶는 것만 🚨부적합입니다. **단, 1) '향료 2종'처럼 향료에 숫자가 붙거나, 2) '영양강화제 2종'처럼 표 6(원래 일괄 표기 가능) 항목에 숫자를 붙인 경우, 또는 3) 숫자를 붙였으나 괄호나 하단에 개별 성분이 100% 전개되어 있는 경우에는 은폐가 아니므로 절대 부적합으로 잡지 말고 완벽한 합법(✅) 처리하십시오.**
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V194.0 - 하이브리드 수동입력 연동판)")
