...
🔥 **Rule 7. 감미료 주의문구 (엄격한 조건부 발동)**
   ...
   - 단, 일반 첨가물 감미료(수크랄로스 등)를 썼다는 이유만으로 14pt 주의문구를 강요하지 마십시오. (Rule 24 참조)
...
def main():
    ...
    if st.button("🔍 서류 추출 및 QC 정밀 진단 시작", type="primary"):
        ...
        with st.spinner("수십 장의 서류를 스캔하여 엑셀 표로 추출하고 시안과 대조 중입니다..."):
            ...
            [🚨치명적 오류 방지 2🚨]: Rule 33에 따라 표를 그릴 때 '제품 내 원재료명' 열은 반드시 '시안'에서 따오고, 나머지 열은 '서류'에서 추출하십시오. CSV 데이터를 무시하지 말고 표에 반영하십시오.
            [🚨치명적 오류 방지 3🚨]: 서류 vs 시안 대조 시 Rule 33을 적용하여, 환자식일 경우 서류의 '하위성분 풀전개' 내용 전체가 시안 괄호 안에 100% 동일하게 풀려 있는지 극도로 깐깐하게 대조하십시오.
            [🚨치명적 오류 방지 4🚨]: Rule 24에 따라, 시안에 "ZERO, 무당, 무가당, 당류무첨가" 강조가 없다면 감미료 주의문구를 강제하지 마십시오.
            """
