...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V107.0 - 수량 2-Track 완결판)")
    ...
    with tab1:
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("주표시면 기계적 텍스트 추출 및 정밀 대조 중..."):
                ...
                <thinking>
                1. 🚨 [시각 판독 및 필사 절대 명령 (창의성 0%)]: 이미지의 맨 위에서부터 맨 아래까지 줄 바꿈을 기준으로 '모든 텍스트'를 순차적으로 100% 스캔하고 필사하십시오.
            ...
            with st.spinner("서류 유형 분석 및 창의성 배제 100% 교차 검증 중..."):
                ...
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        4. 🚨 [수량 2-Track 검증 (연산 & 텍스트)]: 하단 구석의 수량 정보를 추출하여 수학적 곱하기 연산이 맞는지, 그리고 텍스트 형태(대소문자, 단어)가 다른 면과 완벽히 일치하는지 둘 다 검증하십시오.
                        </thinking>
                        ...
                        4. 🚨 [수량 2-Track 검증 (연산 & 텍스트)]: 수량 정보를 추출하여 수학적 곱하기 연산이 맞는지, 그리고 텍스트 형태가 다른 면과 완벽히 일치하는지 둘 다 검증하십시오.
                        </thinking>
                    ...
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        3. 🚨 [기계적 3방향 교차 대조]: 도출된 정답 리스트를 바탕으로 박스/팩 시안과 대조하십시오. 
                        4. 🚨 [수량 2-Track 검증 (연산 & 텍스트)]: 수량 정보를 추출하여 수학적 곱하기 연산이 맞는지, 그리고 텍스트 형태가 다른 면과 완벽히 일치하는지 둘 다 검증하십시오.
                        </thinking>
                        ...
                        3. 🚨 [기계적 교차 대조]: 도출된 정답 리스트를 시안과 대조하십시오. 
                        4. 🚨 [수량 2-Track 검증 (연산 & 텍스트)]: 수량 정보를 추출하여 수학적 곱하기 연산이 맞는지, 그리고 텍스트 형태가 다른 면과 완벽히 일치하는지 둘 다 검증하십시오.
                        </thinking>
        ...
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("오차 계산 및 100% 전체 영양성분 3방향 대조 렌더링 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    ...
                    2. 🚨 [소수점 및 행열 스나이퍼 절대 명령]: 영양성분표의 숫자를 추출할 때, **소수점(.)**을 절대 놓치지 마십시오. (예: 2.5mg을 250mg으로 오독하는 시각적 환각 금지). 표 안에서 특정 성분을 건너뛰거나 다른 줄의 숫자를 끌어오는 행열 이탈을 절대 금지합니다.
                    3. 오차 연산: 서류 실측값 대비 표시량이 오차 범위 내인지 계산하십시오.
                    4. 일치 여부 확인: 팩과 박스의 숫자가 100% 동일한지 대조하십시오.
                    5. 🚨 [수량 2-Track 검증 (연산 & 텍스트)]: 총 내용량 정보를 추출하여 1) 수학적 곱하기 연산이 정확한지 검증하고, 2) 텍스트 형태(팩/개입, 대소문자 X)가 다른 면과 완벽히 일치하는지 둘 다 검증하십시오.
                    </thinking>
                    
                    🚨 [표 작성 필수 명령 절대 강제 - 출력 단축(Lazy Output) 원천 봉쇄] 🚨
                    1. 🚨 [생략 절대 금지]: 표 안에 '...', '중략' 등으로 축약하는 것을 엄격히 금지합니다. 1번부터 끝까지 100% 전부 타이핑하여 완성하십시오. 
                    ...
                    2. 🚨 [소수점 및 행열 스나이퍼 절대 명령]: 영양성분표의 숫자를 추출할 때, **소수점(.)**을 절대 놓치지 마십시오. 표 안의 성분을 건너뛰거나 줄을 헷갈리는 행열 이탈을 절대 금지합니다. 
                    3. 오차 연산: 서류 대비 표시량이 오차 범위 내인지 계산하십시오.
                    4. 🚨 [수량 2-Track 검증 (연산 & 텍스트)]: 총 내용량 정보를 추출하여 1) 수학적 곱하기 연산이 정확한지 검증하고, 2) 텍스트 형태가 다른 면과 완벽히 일치하는지 둘 다 검증하십시오.
                    </thinking>
