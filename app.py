...
🔥 **Rule 93. [수출 겸용 소재지 '대한민국' 표기 감지 및 예외 룰]**
   - 시안의 제조원 또는 유통전문판매원 주소지에 "대한민국"이라는 국가명이 추가로 표기되어 있다면 절대 부적합 처리하지 마십시오.
...
ALL_RULES_NUMBERS = list(range(1, 94))
...
def main():
    ...
    # ⭐ 상단 고정 헤더 영역
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V333.00 - 축산물 종속성 패치 완결판)")
...
| **소재지 '대한민국' 수출용 예외 검증** | [Rule 93] | (※ 제조원/판매원 주소지에 '대한민국' 표기가 있는지 스캔) | |
...
(단순 주의문구 체크를 넘어서, 앞서 적용된 기계적 룰(1~93)이 놓칠 수 있는 패키지 전반의 법적 이상 징후를 업로드된 고시 원문을 바탕으로 자유롭게 찾아내십시오. 
