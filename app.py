...
🔥 **Rule 35. [🌟 범용 간략명/동의어 허용 및 괄호 상세명칭 생략 보장 (강력 예외 룰)]**
   ...
   - **[🌟 괄호 상세명칭 생략 (V190.0 업그레이드)]**: 서류에 `유성비타민A지방산에스테르(비타민A아세테이트)` 처럼 괄호로 이명이나 구체적 명칭이 부연 설명된 경우, 시안에서 괄호를 통째로 날리고 `유성비타민A지방산에스테르`라고만 적는 것은 완벽한 합법(✅)입니다. 누락이라고 억지 지적하지 마십시오.
   - **[🌟 화학식/기호/영문 약어 범용 예외]**: 식약처 공전에 등재된 공식 이명(예: 카복시메틸셀룰로스나트륨 = CMC-Na, CMC, 셀룰로스검)은 100% 합법입니다. 🚨부적합 처리 절대 금지. α = ALPHA 등도 모두 일치 처리.
   - **[🚨 'N종' 묶음 간략화 금지 및 향료 예외]**: 복합원재료나 혼합제제 하위 성분을 '비타민 3종', '야채농축액 5종'처럼 임의로 숫자로 묶어 기재하는 것은 명백한 원료 은폐이므로 🚨부적합입니다. **단, '향료'의 경우는 절대 예외입니다.** '향료(2종)', '합성향료(밀크향 2종)'처럼 향료에 숫자가 붙는 것은 실무적으로 통용되는 합법 표기이므로 절대 부적합으로 잡지 말고 ✅적합 처리하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V191.0 - 환각 완전 차단 및 수동 입력 탑재)")
...
2. ⭐ [누락/증발 텍스트 복구 강제 (가장 중요!)]: 원본 이미지(또는 사용자가 입력한 수동 텍스트)에는 분명히 적혀 있는데, Pass 1 텍스트에서 누락된 단어(특히 괄호 안의 하위 성분, 예: 가공유지 등)가 없는지 글자 단위로 대조하십시오. 단 하나라도 빼먹은 단어가 있다면 무조건 텍스트를 수정하여 완벽하게 복구해 내십시오. 빼먹고 넘어가는 것은 치명적인 시스템 오류입니다.
...
"""
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【3-Pass】 분석 진행 중..."):
                extract_mission = """
🎯 [미션 A: 정보표시면 시각 데이터 '독립' 강제 추출 (초정밀 OCR)]
...
3. ⭐ [혼합제제 괄호 안 텍스트 누락 금지]: 텍스트가 빽빽하더라도 '비타민D3혼합제제(...)' 같은 괄호 안의 하위 성분들을 절대 임의로 스킵하거나 생략하지 마십시오. 100% 있는 그대로 모두 추출해야 합니다. 
