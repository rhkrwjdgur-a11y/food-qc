...
def main():
    ...
    st.title("식품 표시사항 정밀 검토 시스템 (V4.2.9 Ultimate Master)")
    ...
    with tab3:
        if st.button("영양성분표 수치 자동 환산 및 대조", key="btn_nutri"):
            with st.spinner("영양성분 수치 역산 및 오차 매트릭스 검증 중..."):
                ...
                judgment_prompt_tab3 += f"## [사전 연산 3: 영양성분 수치 역산 스크래치패드 및 다단 0표기 연동 검증]\n"
                judgment_prompt_tab3 += "🛑 [다단 0표기 절대 룰 점검]: 기둥이 2개 이상일 경우, 각 성분별로 '총 내용량당'의 수치가 0이 아닌데 '다른 단위당(예: 1컵당)' 수치를 0으로 표기한 꼼수가 없는지 샅샅이 대조하십시오. 총 내용량이 0이 아니면 다른 기둥은 무조건 '실제수치' 또는 'OO 미만'으로 표기해야 합니다!\n"
                judgment_prompt_tab3 += "🛑 [사칙연산 강제 출력]: 각 영양소별 실측값 vs 표시량 대비 허용오차(80% 하한선, 120% 상한선) 계산식과 결과값을 이곳에 반드시 숫자로 풀어서 적으십시오.\n"
                if has_any_doc:
                    judgment_prompt_tab3 += "(이곳에 1일 기준치 % 및 성적서 환산치 수학적 계산 과정을 자유롭게 작성하십시오. 위 '다단 0표기 연동 법칙'을 적용하여 디자이너가 잘못 적은 0이 없는지도 철저히 메모하십시오.)\n"
                else:
                    judgment_prompt_tab3 += "(※ 증빙 서류가 없으나, 시안 내부의 숫자만 보고도 '총 내용량당'은 0이 아닌데 '단위당'은 0으로 표기된 불법 모순이 없는지 교차 검증하십시오. 열량 역산(탄4단4지9) 결과도 수식으로 적으십시오.)\n"
                judgment_prompt_tab3 += "</pre_calc>\n\n"
