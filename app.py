...
# ==========================================
# 📚 2. 통합 전문가 프롬프트 (비즈니스 용어 최적화 및 Rule 53 추가)
# ==========================================
...
## ⚠️ 검토 대원칙: 53대 품질관리 지침 (단 한 글자도 생략 없이 엄수할 것)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V17.0 - 제품명 연동 함량/원산지 추적판)")
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        최종 7단계 리포트를 작성하기 전에, 반드시 `<thinking>` 태그를 열고 업로드된 시안과 서류를 53대 룰에 맞춰 어떻게 교차 검증했는지 당신의 '의사결정 논리'를 먼저 상세히 서술하십시오.
        
        <thinking>
        (이곳에 53대 룰과 긴급 차단 명령을 적용하여 시안과 서류를 대조하는 당신의 산술 연산 및 논리적 판단 과정을 먼저 출력할 것)
        </thinking>
        ...
        ## 2️⃣ [원재료명 및 원산지 대조 (Rule 48, 49, 50, 51, 52, 53 적용)]
        - 결론: (✅ 또는 🚨)
    ...
    if st.button("🔍 정밀 QC 검수 시작", type="primary"):
        ...
        with st.spinner(f"53대 품질관리 룰셋 및 데이터 정합성 검증 중... [{inspection_mode}]"):
