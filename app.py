...
🔥 **Rule 44. [🌟 식품첨가물(혼합제제) 중복 시 '명칭 유지 병합' 전면 금지 및 해체 강제 룰]**
   - **혼합제제는 일반 복합원재료가 아니라 '식품첨가물'이므로 껍데기 명칭 유지가 절대 불가합니다.** 제품에 A원료 안에도 '혼합제제'가 있고, B원료 안에도 '혼합제제'가 있을 때, 이를 통합 표기하겠다며 **"OO혼합제제"라는 복합 묶음 명칭(껍데기) 자체를 그대로 살려서 한 번만 기재하는 것은 명백한 위법(🚨부적합)입니다.**
   - ⭐ **[최하위 단일 성분 전개 필수]**: 여러 경로로 중복 투입된 혼합제제를 합치려면, 반드시 그 껍데기를 부수고 하위의 '최하위 단일 성분(개별 첨가물)' 단위로 낱낱이 해체(전개)하여 흩뿌려 적어야만 합법(✅)입니다. 
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V312.70 - 무결점 방어막 융합 패치)")
        ...
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                missions = [
                    ...
                    "⭐ [절대 미션: 개별 단위 쪼개기 및 괄호 보존 법칙]: 추출한 원재료명을 쉼표(,)를 기준으로 개별 리스트로 쪼개되, **괄호 `()`나 대괄호 `[]` 안에 있는 쉼표는 절대 쪼개지 말고 한 덩어리로 무조건 유지하십시오.** 부모 명칭에 하위 성분을 괄호로 다 적었다면 (예: `혼합제제(A, B)`), 밑에 하위 성분(A, B)을 위한 개별 행을 중복해서 파생 생성하지 마십시오. 시안 텍스트를 `[...]`로 절대 축약하지 말고 100% 타이핑하십시오."
                ]
                
                tab2_special_rules = RULES_TAB2 + """
                \n\n🔥 [Tab 2 특별 지시사항 (반드시 지킬 것 - 화면 출력 금지)] 🔥
                1. [마스터표 강제 완성 및 절대 생략 금지]: 서류의 데이터가 아무리 길어도 마스터표 작성 시 절대 중간에 끊거나 `(...)` 기호를 사용하여 요약하지 마십시오. 원본 데이터의 1행부터 마지막 행까지 100% 전수 조사하여 끝까지 표를 완성하십시오.
