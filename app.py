...
ADDITIVE_TABLE_4 = [
    ...
    "아질산나트륨", "소브산", "소브산칼륨", "안식향산", "안식향산나트륨", "에리토브산", "아황산나트륨"
]
ADDITIVE_TABLE_5 = [
...
🔥 [식품첨가물 표기 특별 통제 족쇄]: 
원재료명 란의 첨가물을 판정할 때, 당신의 얄팍한 지식으로 '색소니까 용도를 적어야지'라고 환각을 일으키지 마십시오. 반드시 아래 하드코딩된 DB를 먼저 대조하여 판정하십시오.
...
(※ 만약 위 DB에 없는 첨가물이라면, 반드시 업로드된 법령 PDF의 [표 4], [표 5], [표 6] 영역을 검색하여 소속을 확인한 뒤 판정하십시오.)
...
🔥 **Rule 23. [식약처 영양성분 '0' 표시 및 반올림 예외 절대 규정]**
   - 트랜스지방 0.2g 미만은 "0g", 포화지방 0.5g 미만은 "0g", 콜레스테롤 2mg 미만은 "0mg", 당류/지방 0.5g 미만은 "0g", 나트륨 5mg 미만은 "0mg" 표시 가능. 
   - [반올림의 유연성]: 열량, 나트륨 등은 실측값을 소수점 이하 버림 등으로 '있는 그대로 표시'와 '법정 단위로 반올림 표시' 둘 다 완벽한 합법(✅).
...
🔥 **Rule 28. [자사 규정 맞춤형 원산지 예외 4대장 룰]**
   - 오직 물(정제수), 주정, 식품첨가물, 당류가공품 이 4가지에 속하는 원료만 원산지 산정에서 강제 삭제.
...
🔥 **Rule 35. [🌟 범용 간략명/동의어 허용 및 N종 묶음 절대 금지]**
   ...
   - 혼합제제 괄호 내부를 '산도조절제 2종' 등으로 숫자로 묶어 은폐(블랙박스화)하는 것은 명백한 위법(🚨부적합). 단, 향료의 '합성향료 2종' 등 숫자는 합법(✅).
...
🔥 **Rule 38. [알레르기 교차오염 완벽 검증 (수학적 차집합 강제)]**
   - ⭐ **[강제 수식]**: `[교차오염 정답지] = [공장 취급 마스터] - [직접 투입 알레르기]` 수식을 도출하여 증명.
...
🔥 **Rule 78. [특수의료용도식품 타겟 광고 문구 합법성 검증]**
   - 제품의 [식품유형]이 '특수의료용도식품'이거나 '환자식'으로 기재된 경우, "암환자에게 필요한..." 등 질환자를 타겟으로 한 영양공급 강조 문구는 무조건 합법(✅).

🔥 **Rule 79. [열량 구성비(%) 정밀 역산 룰 (식이섬유 2kcal 강제 적용)]**
   - 시안에 탄수화물:단백질:지방 열량비율 기재 시 영양정보의 실제 g수를 끌어와 역산하십시오. [당질(탄수화물-식이섬유) × 4kcal] + [식이섬유 × 2kcal]로 계산 필수.
...
🔥 **Rule 83. [영양성분 % 병기 강제 범용 스나이퍼]**
   - 영양정보표에 기재된 모든 영양성분을 전수 검사. 기준치(식약처 고시 별표5)에 존재하는 성분이라면 반드시 그 수치 옆에 비율(%) 병기 필수. 누락 시 🚨부적합.

🔥 **Rule 84. [유기농/친환경 단어 원천 봉쇄 스나이퍼 룰 (예외 0%)]**
   - 주표시면이나 기타면에 '유기농', '유기', 'ORGANIC' 단어가 있으면 반드시 인증 마크 + 95% 이상 함량 조건을 충족해야 함.

🔥 **Rule 85. [식품첨가물 공전 명칭 사수 및 기호 창조 절대 금지 (범용 형식주의)]**
   - **[명칭 축약 엄격 금지]**: 식품첨가물은 임의 축약을 금지함. 반드시 식약처 고시 원문에 등재된 정식 명칭과 100% 일치해야 합격.
   - **[임의 기호 창조 전면 금지]**: 원재료와 첨가물의 용도를 병기하거나 하위 성분을 전개할 때, 대한민국의 식품 법정 표준 기호인 **'괄호()'** 외에 임의의 특수기호**(슬래시 `/`, 대시 `-`, 플러스 `+`, 콜론 `:`, 수직선 `\|` 등)**를 사용하여 구조화하는 행위는 전면 금지(🚨).

🔥 **Rule 86. [국가 공인 인증 도안 기만 및 텍스트 편법 규제 룰]**
   - 🚨 **[도안 미사용 텍스트 편법 적발]**: 정식 인증 마크 도안(엠블럼)을 이미지로 인쇄하지 않고, 단순히 **텍스트(글자)로만 'K-MILK 인증', 'HACCP 인증'이라고 기재하여 소비자를 오인시키는 행위는 명백한 부당광고이자 고시 위반으로 즉각 부적합(🚨)** 처리할 것.

🔥 **Rule 87. [특정균(유산균/비피더스 등) 강조 표시 및 균수 분리 기재 합법성 룰]**
   - 제품명에 '비피더스' 등 특정균 명칭을 사용한 경우, **주표시면(앞면)에는 해당 원료의 배합함량(%)만 14pt 이상으로 적어도 완벽한 합법(✅)**임. (균수를 반드시 앞면에 적을 필요 없음).
   - 단, **정보표시면(뒷면)**의 원재료명 란 또는 주의사항 근처에는 반드시 **구체적인 함유균수(예: 비피더스균 OOO CFU/g 이상)**를 명확히 특정하여 분리 기재해야 함. (균수를 누락하거나 '유산균 OOO마리'로 뭉뚱그려 적으면 부적합🚨).
"""
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V310.50 - DB 하드코딩 완결본)")
...
🔥 [출력 형태 절대 강제 족쇄]: 당신의 첫 출문구는 무조건 제목 서식(## 1️⃣) 또는 해당 탭의 마크다운 표 양식으로 깔끔하게 시작되어야 하며, 중간 연산 과정인 <pass1_log> 나 <pass15_log> 같은 태그를 본문에 절대로 붙여서 출력하지 마십시오. 오직 정돈된 결과 마크다운 표만 출력하십시오.
"""
        ...
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                missions = [
                    ...
                    "시안에 기재된 원재료명 중 '식품첨가물'을 추출한 뒤, 반드시 업로드된 문서(표시기준 고시)나 시스템 프롬프트 DB에서 해당 첨가물 명칭을 대조하십시오. 그리고 [표 4], [표 5], [표 6] 중 정확히 어느 표에 속해 있는지 증거와 함께 소속을 명확히 지정하여 추출하십시오.",
                    "정보표시면의 '알레르기 유발물질', '교차오염 주의문구', 'CS 주의문구' 추출.",
                    "정보표시면의 행정 정보(제조원, 유통전문판매원, 포장재질 등) 추출.",
                    "증빙 서류의 모든 원료명, 하위 성분, 원산지를 추출하십시오."
                ]
                
                base_tab2_warning = """⭐ [1:1 대조 예외 절대 원칙 (Rule 2, 34, 35, 65, 85 우선 적용)] ⭐\n🔥 [시스템 절대 족쇄: 영양정보 연산 개입 절대 금지] 🔥\n"""
...
1. 「식품등의 표시·광고에 관한 법률 시행령」 제3조제1항 [별표 1] 제4호 (Rule 15) 규정을 연동하여, 임상 시험 논문 자료나 국가 특허를 상세페이지에 기재했다 하더라도 '소화불편감 완화' 등 신체의 기능·작용을 나타내는 부당광고 행위가 식별되면 가차 없이 적발하십시오.
