...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V346.00 - 다중레이아웃 분할 강제 족쇄 패치)")
    ...
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【영양성분표 수치 환산 및 교차 검증 중...】"):
                ...
                7. ⭐ [다중 복합 레이아웃 강제 전개 족쇄]: 
                   - 시안에 '여러 맛'과 '여러 단위'가 병기되어 있을 경우 1개의 표로 압축하는 짓을 절대 금지합니다.
                   - 아래의 [Target 1], [Target 2] 양식 뼈대를 활용하여 반드시 모든 경우의 수만큼 독립된 표를 생성하십시오.
                """
                ...
                if has_any_doc:
                    ...
                    judgment_prompt_tab3 += f"## {title_prefix} [영양표시 오차 검증 매트릭스 (★다중 레이아웃 강제 전개★)]\n"
                    judgment_prompt_tab3 += "🔥 [AI 게으름 방지 강제 족쇄]: 시안에 여러 맛(예: 플레인, 딸기)과 여러 단위(예: 1개당, 총 내용량당)가 병기된 다단 레이아웃인 경우, 하나의 표에 뭉뚱그리거나 특정 단위를 누락하는 것을 엄격히 금지한다.\n"
                    judgment_prompt_tab3 += "반드시 아래의 **[Target 1], [Target 2]... 뼈대를 필요한 개수만큼 복사하여, '모든 맛'의 '모든 용량 기준(1개당, 총내용량당 등)'에 대해 개별적인 표를 100% 전수 작성**하라. (예: 맛 2개 × 기준 2개 = 총 4개의 표 작성)\n\n"
                    judgment_prompt_tab3 += "### 🔹 Target 1: [맛 이름 및 단위 - 예: 플레인 1컵(80g)당]\n"
                    judgment_prompt_tab3 += "| 영양성분 | 🧪 성적서 환산값(A) | 📦 시안 표시량(B) | ⚖️ 허용오차 커트라인 | 📊 1일 기준치 % 검증 | 🎯 상세 사유 | 판정 |\n|---|---|---|---|---|---|---|\n\n"
                    judgment_prompt_tab3 += "### 🔹 Target 2: [맛 이름 및 단위 - 예: 플레인 총 내용량(640g)당]\n"
                    judgment_prompt_tab3 += "| 영양성분 | 🧪 성적서 환산값(A) | 📦 시안 표시량(B) | ⚖️ 허용오차 커트라인 | 📊 1일 기준치 % 검증 | 🎯 상세 사유 | 판정 |\n|---|---|---|---|---|---|---|\n\n"
                    judgment_prompt_tab3 += "(※ 위 Target 1, Target 2 형식을 시안에 존재하는 모든 맛과 단위 개수만큼 계속 이어서 작성할 것. 절대 중간에 생략하지 마라!)\n\n"
