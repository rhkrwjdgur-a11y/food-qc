...
# 1. API 키 및 모델 설정 (2026 유료 티어 최적화)
...
# 2. 통합 전문가 프롬프트 (Rule 1~45 무삭제 원문 100% 복원)
...
---
# [식품 패키지 표시사항 QC 자동화 검수 시스템 룰북 6.25]
...
✅ **Rule 14. 첨가물 용도명 병기 스캔**
   - 감미료, 보존료 등은 원재료명 란에 명칭과 용도명 괄호 병기(예: 아스파탐(감미료))가 필수입니다. 단, '감미료(A, B)' 형태의 묶음 표기는 100% 합법이므로 지적하지 마십시오.

✅ **Rule 15. 강조표시 및 효능/기능성 연쇄 불합격 스캔**
   - 식약처 세부 기준을 엄격히 대조하여 단 하나라도 미달 시 부적합 판정하십시오.

✅ **Rule 16. [원산지 100%]** - 물은 산정 제외, 농산물이 국산이면 100% 합법입니다.
✅ **Rule 17. ['無첨가' 강조 룰]** - 금지된 첨가물을 안 넣었다고 강조할 때만 지적하십시오.
✅ **Rule 18. [영유아 타겟 명칭]** - '베베', '키즈' 명칭 사용은 일반식품에서도 합법입니다.
✅ **Rule 19. ['무당/무가당/저당' 엄격 적용]** - 저당(5g 미만), 무당(0.5g 미만) 기준 준수 여부.
✅ **Rule 20. [용기·포장재질 표기법]** - "재질명(포장부위)" 형태 및 내면재질 표기를 인정하십시오.
...
🔥 **Rule 25. [다중 포장 듀얼 컬럼]** - 1개당 / 총 내용량 수치 혼동 금지.
🔥 **Rule 26. [고체 vs 액체 단위 엄격 구분]** - g과 mL 기준 혼용을 금지합니다.
🔥 **Rule 27. [제한 영양성분 100kcal 적용 절대 금지 룰]** - 무/저 강조 시 100g/100mL 기준만 적용.
...
🔥 **Rule 32. [균형 열량 구성비 역산 금지]** - 환자식 탄:단:지 비율은 설계치이므로 오차 지적 금지.
🔥 **Rule 33. [데이터 출처 완벽 분리 및 100% 필사본 강제 룰]**
   - 시안에 적힌 텍스트를 토씨 하나 빼먹지 말고 100% 타이핑하여 보고하십시오. 임의 요약 금지.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (V6.25 - 2026 유료 티어 최적화)")
    st.markdown("---")
