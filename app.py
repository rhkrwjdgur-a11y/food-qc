...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V320.00 - 가독성/HTML 렌더링 패치)")
...
(예시: **✅ 적합**<br>- **안전율**: 104.7% (157 / 150)<br>- **% 검증**: 4% (13.5 / 324))
        ...
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("【정밀 법리 검수 매트릭스 연산 중...】"):
                missions = [
                    ...
                    "⭐ [시야 차단 명령]: 뒷면의 원재료명이나 첨가물 리스트는 절대 스캔하지 마십시오. 당신은 1번 탭 담당이므로 오직 앞면의 마케팅 뱃지와 문구만 보십시오."
                ]
                judgment_prompt = """## 1️⃣ [주표시면 및 마케팅 뱃지 정밀 검증]
| 검토 항목 | 검토 룰(Rule) | 검토 결과 및 사유 (오탈자 무관용, 말줄임표 금지, <br> 태그로 줄바꿈 필수) | 판정 |
| :--- | :--- | :--- | :--- |
        ...
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                if "박스" in ins_mode:
                    ...
                    judgment_prompt += "| 타겟(박스) 표기 개별 원재료명 (행정정보 제외) | 비교용(팩) 표기 개별 원재료명 | 대조 검증 결과 (<br> 태그 활용 상세 서술) | 최종 판정 |\n|---|---|---|---|\n\n"
                ...
                if has_any_doc:
                    ...
                    judgment_prompt += "| 시안 표기 개별 원재료명 (생략 금지) | 매칭된 서류 원료명 | ⚖️ 배합비(%) 및 순위 | 🌍 원산지 룰 검증 | 최종 대조 결과 및 사유 (<br> 태그로 줄바꿈 필수) | 판정 |\n|---|---|---|---|---|---|\n\n"
        ...
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【영양성분표 수치 환산 및 교차 검증 중...】"):
                ...
                5. ⭐ [수학적 투명성 강제 (수석님 특별 지시사항)]: 
                   ...
                   - 성적서 환산값(A) 칼럼: `원본 실측값(100g당) × 환산비율 = 최종 환산값(A)`
                   - 커트라인 칼럼: `표시량(B) × 0.8 = X 이상` 또는 `표시량(B) × 1.2 = Y 이하`
                   - 판정 및 사유 칼럼 예시:
                     **✅ 적합**<br>- **실제 안전율**: 104% (157.02 / 150)<br>- **1일 기준치 % 검증**: 4% (13.5 / 324) 일치
                ...
                if has_any_doc:
                    ...
                    judgment_prompt_tab3 += "| 영양성분 | 🧪 성적서 환산값(A) | 📦 시안 표시량(B) | ⚖️ 허용오차 커트라인 | 🎯 최종 판정 및 상세 사유 (<br> 태그 필수 사용, 볼드체 요약) |\n|---|---|---|---|---|\n\n"
...
| 검토 항목 | 검토 룰(Rule) | 검토 결과 및 사유 (<br> 태그 사용 생략 없이 100% 서술) | 판정 |
| :--- | :--- | :--- | :--- |
