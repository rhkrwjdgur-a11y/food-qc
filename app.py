...
🔥 **Rule 19. ['무당' vs '무가당' 범용 검증 및 🌟혼합제제 내 당류 무조건 알림 룰 (V344 패치)]**
...
🔥 **Rule 95. [⭐ 특정 지역명 마케팅(예: 시칠리아산) 및 원재료명 국가명 강제 룰]**
   - **[주표시면/마케팅]**: 시안에 '시칠리아산', '보르도산' 등 특정 지역명이 강조된 경우, 무조건 **⚠️(실무 확인 권장)** 처리하십시오.
   - 🛑 **[AI 환각 방지 족쇄]**: 제조사의 공장 주소나 회사 이름에 해당 지역명이 들어있다고 해서, 원물의 산지가 증명된 것으로 절대 착각(합법 처리)하지 마십시오. 오직 "원물이 해당 지역에서 재배되었다"는 명시적인 증명서(원산지 증명서 등)가 별도로 존재하는지 실무자가 최종 확인해야 한다고 강력히 경고하십시오.
   - **[정보표시면(원재료명)]**: 단, 뒷면 원재료명 란에는 무조건 **법정 국가명(예: 이탈리아산, 프랑스산)**으로만 기재해야 합니다. 지역명(예: 시칠리아산)으로 적혀있다면 즉시 **🚨부적합** 처리하십시오.

🔥 **Rule 96. [⭐ 유기가공식품 인증 마크 법정 CMYK 배색 비율 강제 점검 룰]**
   - 시안에 유기가공식품 인증 마크가 존재할 경우, AI는 화면상 색상(녹/적/청/흑)이 맞아 보이더라도 절대 단독으로 적합 판정을 내리지 마십시오. (JPG 한계)
   - 무조건 **⚠️(실무 확인 권장)**으로 판정하고, 사유에 "⚠️ [확인바람]: JPG 화면상 허용 색상으로 보이나, 친환경농어업법에 따라 인증 마크는 지정된 CMYK 비율(예: 검은색은 C20+K100)을 1% 오차 없이 준수해야 합니다. 디자인 원본 파일(AI)을 열어 색상값을 직접 확인바람!" 이라고 강력하게 출력하십시오.

🔥 **Rule 97. [⭐ 영양성분 표시단위 결정 프로세스 스마트 DB 내장 룰]**
   - 시안의 '식품유형'을 파악하여 아래 식약처 1회 섭취참고량 DB를 자동 대입하라.
     * 우유, 가공유, 두유류, 과채주스/음료, 탄산, 액상차(당류포함): 200ml
     * 발효유: 80ml (또는 80g)
     * 농후발효유(발효유 외): 액상 150ml, 호상 100ml(g)
     * 환자용/당뇨/신장질환자용 조제식품(액상), 임산수유부용(액상): 200ml
     * 커피: 240ml
   - 시안의 총 내용량이 100g(ml)을 초과하고, 위에서 도출한 [1회 섭취참고량]도 초과하는데 오직 '총 내용량당' 단독으로만 영양정보표를 표기했다면 명백한 불법(🚨부적합) 처리하라.

🔥 **Rule 98. [⭐ 다단 표기 & 단위포장 프리패스 룰]**
   - 1) 제품이 '1포당, 1개당' 등 낱개 개별 포장된 경우 1회 섭취참고량과 무관하게 '1단위 포장당' 표기는 100% 합법(✅)이다. 참고량 초과 여부를 따지지 말고 즉시 합법 처리하라.
   - 2) '1포당'과 '총 내용량당'이 2단으로 병기된 다단 레이아웃은 모범적인 합법 표기이다. 단, 두 수치 간의 환산(곱하기) 오차가 없는지만 수학적으로 교차 검증하라.

🔥 **Rule 99. [⭐ 성역 없는 마케팅 vs 원재료 전체 크로스체크 룰]**
   - 세트포장(박스) 모드이든 단품 모드이든 상관없이, 패키지 어디에서든 '무가당, 무첨가, 100%' 등 성분 통제 마케팅 문구가 발견되면, 즉시 업로드된 모든 이미지(박스+팩 전체)의 정보표시면 원재료명 리스트를 샅샅이 뒤져 모순을 엮어내어 지적하라.
"""
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V345.00 - 복합 다단 레이아웃 분해 패치)")
    ...
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【영양성분표 수치 환산 및 교차 검증 중...】"):
                ...
                7. ⭐ [다중 복합 레이아웃(여러 맛 + 1개당/총내용량당 병기) 분리 작성 강제]: 
                   - 시안이 여러 가지 맛(예: 플레인, 딸기)의 영양정보를 한 표에 담고 있고, 각각 '1개당' 수치와 '총 내용량당' 수치를 모두 병기한 매우 복잡한 구조일 수 있습니다.
                   - 이런 경우 억지로 하나의 표에 구겨 넣다가 '총 내용량당' 수치를 누락하는 멍청한 실수를 절대 하지 마십시오.
                   - 반드시 **"플레인 (1컵당)", "플레인 (총 내용량당)", "딸기 (1컵당)", "딸기 (총 내용량당)"** 처럼 대분류를 나누어, 표(Table)를 여러 개 생성하더라도 모든 칼럼의 수치를 빠짐없이 100% 독립적으로 연산하고 검증하십시오.
                """
                ...
                if has_any_doc:
                    ...
                    judgment_prompt_tab3 += f"## {title_prefix} [영양표시 오차 검증 매트릭스]\n"
                    judgment_prompt_tab3 += "(※ 시안에 여러 맛(플레인, 딸기 등)과 여러 단위(1개당, 총 내용량당)가 존재할 경우, 아래 표 형식을 계속 복사하여 **모든 맛과 모든 단위에 대해 100% 각각 독립된 표를 생성**할 것. 절대 '총 내용량당'을 누락하지 마라!)\n\n"
                    judgment_prompt_tab3 += "### 🔹 타겟: [여기에 맛 이름 및 표시단위 기재 (예: 플레인 - 1컵(85g)당)]\n"
