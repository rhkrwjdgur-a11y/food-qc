...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V319.00 - 부분 캡처 누락 보류 패치)")
        ...
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                if "박스" in ins_mode:
                    ...
                    judgment_prompt += "| 타겟(박스) 표기 개별 원재료명 (소비기한, 보관방법 등 행정정보 절대 제외) | 비교용(팩) 표기 개별 원재료명 | 대조 검증 결과 (픽셀 100% 일치 여부 상세 서술, 말줄임표 절대 금지) | 최종 판정 |\n|---|---|---|---|\n\n"
        ...
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【영양성분표 수치 환산 및 교차 검증 중...】"):
                ...
                if has_any_doc:
                    ...
                    judgment_prompt_tab3 += "| 영양성분 | 🧪 성적서 환산값(A) (원본값×비율=A 필수 기재) | 📦 시안 표시량(B) | ⚖️ 허용오차 커트라인 (80% 또는 120%) | 🎯 1일 기준치 % 검증 (수식 포함) | 최종 판정 및 사유 (절대 생략 금지) |\n|---|---|---|---|---|---|\n\n"
