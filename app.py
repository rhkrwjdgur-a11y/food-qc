...
ADDITIVE_TABLE_6 = [
    # ⭐ [표 6 전용 주용도 명칭 (이 단어 하나만 단독으로 적어도 합법)]
...
🔥 **Rule 35. [🌟 범용 간략명/관용명 허용 및 혼합제제 괄호 내부 N종 은폐 금지 룰]**
   ...
   - ⭐ **[혼합제제 괄호 내부 N종 은폐 스나이퍼]**: 패키지 시안에 `혼합제제(산도조절제 2종)`처럼 **'혼합제제의 괄호 안'**에 첨가물을 숫자로 묶어 은폐한 경우에만 위법(🚨부적합)으로 처리하십시오. (단, 향료의 '합성향료 2종' 등은 합법)
   - 🚫 **[과잉 단속 금지]**: 증빙 서류에 단순 분류 타이틀로 '유화제 2종'이라고 적혀 있거나, 시안이더라도 혼합제제의 괄호 내부가 아니라면 N종이라는 단어만 보고 기계적으로 부적합 처리하지 마십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.48 - 표 6 DB 보완 및 전수 오탈자 검사 패치)")
        ...
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                missions = [
                    ...
                    "⭐ [절대 미션: 개별 단위 쪼개기]: 대조 매트릭스 표를 작성할 때, 쉼표(,)나 띄어쓰기를 기준으로 '모든 원재료를 개별 원료 단위로 완벽하게 쪼개어' 행(Row)을 분리하십시오. 무조건 1줄에 1개씩 1:1 매칭되도록 출력하십시오."
                ]
                ...
                if "박스" in ins_mode:
                    ...
                    common_tab2_prompts += """## 2️⃣-1. [박스(타겟) vs 팩(비교용) 내외포장 100% 일치 대조 매트릭스 (Rule 70)]
⭐ **[내/외포장 1:1 대조 족쇄]**: 오직 타겟(박스)의 원재료명과 비교용(팩)의 원재료명을 픽셀 단위로 대조하여 단 하나의 글자, 기호 띄어쓰기라도 다르면 무조건 🚨부적합 처리하십시오. (단, 내용량/열량 표기는 예외로 다를 수 있음)
| 타겟(박스) 표기 개별 원재료명 (1줄에 1개씩) | 비교용(팩) 표기 개별 원재료명 (1줄에 1개씩) | 대조 검증 결과 (픽셀 100% 일치 여부) | 최종 판정 |
|---|---|---|---|
...
"""
                ...
                # 2. 서류 대조 (박스 대조 다음 또는 단독 실행)
                if has_any_doc:
                    if "무더기" in doc_mode:
                        missions.append("업로드된 '개별 원료 한글라벨 무더기' 데이터를 분석하여 [마스터 배합비 표]로 합성하십시오.")
                        doc_table = "## 2️⃣-2. [AI 종합 마스터 배합비 표] (한글라벨 무더기 기반 합성)\n| 통합 원료명 (1줄에 1개씩) | 하위 전개 성분 | 원산지 | 추정 배합비(%) / 순위 |\n|---|---|---|---|\n\n"
                    else:
                        missions.append("업로드된 증빙 서류(마스터 엑셀/PDF)에서 모든 원료명, 하위 성분, 원산지, 배합비(%)를 추출하십시오.")
                        doc_table = "## 2️⃣-2. [서류 기반 마스터 원재료 DB]\n| 서류상 원료명 (1줄에 1개씩) | 하위 전개 성분 | 원산지 | 배합비(%) / 순위 |\n|---|---|---|---|\n\n"
                    
                    title_prefix = "2️⃣-3." if "박스" in ins_mode else "2️⃣-2."
                    common_tab2_prompts += doc_table + f"""## {title_prefix} [마스터 서류 vs 시안 법적 대조 매트릭스]
⭐ **[복합원재료/시럽류 유연한 치환 맵핑 룰]**: 시안에 '당류가공품', '올리고당', '혼합제제' 등이 적혀있고 서류 1열에 동일 명칭이 없더라도, 서류의 'OO시럽', 'OO페이스트' 등 복합원재료의 하위 성분/비고란을 살펴보고 문맥상 일치하면 ✅적합으로 유연하게 매칭하십시오. 억지 누락 처리 금지.
⭐ **[원산지 98% 컷오프 및 예외 완벽 검증 족쇄 (Rule 1, 28 절대 준수)]**: 
1. [1단계]: 1순위 원료의 배합비가 98% 이상인지 확인!
2. [2단계]: 1순위 98% 이상이면 2순위, 3순위 누락은 합법(✅) 처리.
3. [3단계]: 1순위+2순위 98% 이상이면 3순위 누락 합법(✅) 처리.
4. 🚫 **[과잉 단속 절대 금지]**: 배합비 4순위 이하의 원료, 당류(설탕/시럽류), 식품첨가물(레시틴, 구연산 등)은 원산지 표시 대상이 아니므로 무조건 패스(✅).
⭐ **[가공품 원산지 이중 표기 족쇄 (Rule 89 절대 준수)]**: 국내 제조 가공품은 반드시 `원료명(원료명: 국가명)` 형태여야 합니다. `원료명(국가명)`은 🚨부적합.
| 시안 표기 개별 원재료명 | 매칭된 서류 원료명 (유연한 맵핑 적용) | 대조 검증 결과 (명칭 대조 및 상위 3순위 원산지 표기 여부) | 최종 판정 |
|---|---|---|---|
...
"""
                    ...
                    common_tab2_prompts += "## 2️⃣-1. [시안 표기 원재료명 리스트]\n(※ 증빙 서류 미제출로 서류 대조 및 원산지 검증 불가)\n\n## 2️⃣-2. [자체 형식 검토 매트릭스]\n| 시안 표기 개별 원재료명 | 형식 검토 결과 (오탈자 등) | 판정 |\n|---|---|---|\n"

                # [공통 하단 양식] (첨가물 룰, 2% 룰, 알레르기 등, 그리고 오탈자 전수검사)
                common_tab2_bottom = """
...
## 🔍 6️⃣ [전체 텍스트 오탈자 및 띄어쓰기 스캔 (전수 검사)]
...
⭐ **[판정 이모지 3단계 절대 규칙 및 안전율 인정 (반올림 지적 금지)]**:
   ...
   - ✅(적합): 오차 범위 내에 속하고 수치도 완벽히 일치할 때.
| 영양성분 | 성적서 환산값(A) (계산식 포함) | 시안 표시량(B) | 법적 기준선 (B의 80% 또는 120%) | 🎯 % 역산 검증 (수식 포함) | 판정 및 상세 사유 (오차/안전율 판단) |
|---|---|---|---|---|---|
