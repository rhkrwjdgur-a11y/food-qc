...
# ==========================================
# 📚 3. 65대 룰북 원문 (V155.6 - 원물/가공국 엄격분리 및 시각 환각 완전 차단 패치)
# ==========================================
RULE_BOOK_FULL = """
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V155.6 - 원물/가공국 엄격분리 및 시각 환각 완전 차단 패치)")
    ...
    def run_qc_3pass(tab_rules: str, judgment_prompt: str):
        ...
        # ── PASS 1: 순수 텍스트 추출 (시각 환각 완전 차단) ──
        pass1_prompt = """
[PASS 1 - 텍스트 추출 전용 명령]
⭐ 이 단계에서는 어떤 판정, 평가, 해석도 절대 금지입니다. 오직 텍스트 필사만 수행하십시오.
⭐ [환각 차단 절대 명령]: 당신은 OCR(광학 문자 인식) 기계입니다. 사용자가 업로드한 'PDF 증빙 서류'의 내용을 머릿속에서 완전히 지우십시오. 오직 '라벨 시안 이미지' 눈에 보이는 픽셀 글자만 그대로 베껴 써야 합니다. (시안에 '올리고당'이라 적혀있으면 서류에 '말토올리고당'이 있더라도 무조건 '올리고당'만 추출할 것!)
...
2. [원산지 검증]: 해당 원료가 농수산물인 경우, 원산지가 원재료명 '바로 뒤 괄호' 안에 합법적으로 표기되었는가? Rule 28에 따른 누락 또는 과잉 표기 여부 적발!
