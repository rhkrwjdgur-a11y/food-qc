...
🔥 **Rule 50. ['원액/100%' 명칭 기만광고 판별 룰 (단일 원료 vs 최종 제품 혼합 구분)]**
   - **[✅합법 조건]**: 투입되는 특정 원료 자체가 순수한 원액이라면, 최종 제품 공정에서 다른 첨가물(효소, 정제수 등)과 섞이더라도 그 원료를 'OO원액'이라고 부르는 것은 완벽한 합법입니다.
   - **[🚨기만광고 적발 조건]**: 단, 납품받은 특정 원료 **그 자체의 하위 성분(괄호 안)**에 이미 정제수나 덱스트린 등이 섞여서 들어왔음에도 불구하고, 그것을 묶어서 '100% 원액'이라고 부르는 경우에만 기만광고로 적발하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (V6.82 - 원클릭 종합 & 실무 논리 완벽판)")
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        AI의 누락(Lost in the Middle)을 방지하기 위해 반드시 아래 7단계 목차를 100% 준수하여 하나의 리포트로 출력하십시오.
        ...
        - 🚨 [긴급 차단 명령9 (식품 접촉면 & 정보 일치 강제 검증)]: 
          1) **식품 접촉면 검증**: 외포장/내포장 텍스트 란에 **'식품과 직접 닿는 면의 재질(예: 폴리에틸렌 등)'**이 제대로 표시되어 있는지 확인하십시오. (박스가 종이라고 해서 텍스트로 외포장 종이를 적으라고 지적하지 마십시오).
          2) **정보 100% 일치 검증**: 소비기한 위치, 재질 같은 물리적 차이를 제외하고, **나머지 모든 핵심 텍스트 정보(원재료명, 영양성분, 주의사항 등)가 내/외포장 간에 100% 일치하는지** 전수 대조하여 결과를 브리핑하십시오.
