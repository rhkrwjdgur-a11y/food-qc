...
🔥 **Rule 34. [🌟 배합비 전개 순서 100% 일치 강제 및 2% 미만 예외 룰]**
   - **[절대 원칙]**: 원재료명은 반드시 서류상의 배합비율(%)이 높은 순서대로 기재되어야 합니다. 배합비 중량 순서와 시안의 나열 순서가 다르면 명백한 표시기준 위반(🚨부적합)입니다.
   - **[2% 미만 예외]**: 단, 배합비율이 **2% 미만**인 원료들은 중량 순서에 상관없이 자유롭게 기재해도 완벽한 합법(✅)입니다.
   - ⭐ **[순서 역전 스나이퍼]**: AI는 서류에서 2% 이상인 원료들만 우선적으로 묶어 그들 간의 순서(1위, 2위, 3위...)가 시안에서도 정확히 지켜졌는지 확인하십시오. (예: 10.7%인 원료가 10.0%인 원료보다 반드시 앞에 적혀야 합니다. 이를 어기면 🚨부적합 처리하십시오.)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.65 - 2% 이상 원료 순서 역전 스나이퍼 패치)")
        ...
        def get_uploaded_content():
            ...
            def robust_upload(file_path, label):
                ...
                if file_path.lower().endswith(('.png', '.jpg', '.jpeg')):
                    # ⭐ 비전 API 작동 시각적 알림 (화면 우측 하단 팝업) 패치 적용
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                base_tab2_warning = "⭐ [1:1 대조 예외 절대 원칙 (Rule 35, Rule 90 범용 치환 맵핑 완벽 적용)] ⭐\n🔥 [초강력 테이블 분리 및 단답형 금지 족쇄]: 1. 원재료명 대조표를 출력할 때 **무조건 1줄(Row)에 딱 1개의 원료명만** 들어가게 쪼개십시오. 한 칸에 여러 원료를 뭉쳐서 적으면 치명적 오류입니다. 2. 사유를 '명칭 일치'처럼 성의 없게 단답형으로 적지 마십시오. 어떤 룰을 적용하여 어떻게 매칭했는지 **반드시 2~3문장 이상의 상세한 서술형**으로 답변하십시오. 3. 알레르기 유발물질 표시(예: 대두 함유)는 원재료가 아니므로 원재료명 대조표에 넣지 말고 하단 알레르기 섹션에서만 다루십시오.\n\n"
                common_tab2_prompts = ""
                ...
                if has_any_doc:
                    if "무더기" in doc_mode:
                        ...
                        common_tab2_prompts += "## 2️⃣-1. [AI 종합 마스터 배합비 표] (한글라벨 무더기 기반 합성)\n| 통합 원료명 (1줄에 1개씩) | 하위 전개 성분 | 원산지 | 추정 배합비(%) / 순위 |\n|---|---|---|---|\n\n"
                    else:
                ...
                if "박스" in ins_mode:
                    ...
                    step_num_box = "2" if has_any_doc else "1"
                    common_tab2_prompts += f"""## 2️⃣-{step_num_box}. [박스(타겟) vs 팩(비교용) 내외포장 100% 일치 대조 매트릭스 (Rule 70)]
...
| 타겟(박스) 표기 개별 원재료명 (1줄에 딱 1개씩만) | 비교용(팩) 표기 개별 원재료명 (1줄에 딱 1개씩만) | 대조 검증 결과 (픽셀 100% 일치 여부, 2~3문장 서술) | 최종 판정 |
|---|---|---|---|
...
\n"""
                ...
                if has_any_doc:
                    ...
                    step_num_master = "3" if "박스" in ins_mode else "2"
                    common_tab2_prompts += f"""## 2️⃣-{step_num_master}. [마스터 서류 vs {target_name} 법적 대조 매트릭스]
⭐ **[Rule 90 범용 치환 맵핑 절대 준수]**: 시안에 '당류가공품', '올리고당', '혼합제제' 등이 적혀있을 때 서류 1열에 동일 글자가 없다고 🚨누락/부적합 처리하면 안 됩니다. 서류의 여러 시럽/페이스트류가 식품유형 관행에 따라 시안에서 묶여서 표기된 것이므로, 문맥상 남는 원료들을 유연하게 맵핑하고 사유에 그 논리를 서술한 뒤 ✅적합 처리하십시오.
⭐ **[원산지 98% 컷오프 및 과잉 단속 금지 족쇄 (Rule 1, 28)]**: 
1. 배합비 4순위 이하 원료, 당류, 첨가물은 원산지 표시 대상이 아니므로 무조건 패스(✅). 이를 지적하면 시스템 오류입니다.
2. 1순위 98% 이상이면 2, 3순위 누락도 합법(✅).
⭐ **[가공품 원산지 이중 표기 족쇄 (Rule 89 절대 준수)]**: 국내 제조 가공품은 반드시 `원료명(원료명: 국가명)` 형태여야 합니다. `원료명(국가명)`은 🚨부적합.
| 시안 표기 개별 원재료명 (1줄에 딱 1개씩만) | 매칭된 서류 원료명 (Rule 90 유연한 맵핑 적용) | 대조 검증 결과 및 사유 (단답형 절대 금지! 어떤 룰로 어떻게 매칭했는지 2~3문장 서술) | 최종 판정 |
|---|---|---|---|
...
⭐ **[순서 역전 스나이퍼]**: (서류가 있을 경우) 서류상 배합비가 2% 이상인 원료들만 싹 다 추출하여, 서류에 적힌 중량 순서(1위, 2위, 3위...)와 시안(패키지) 원재료명에 적힌 순서가 완벽하게 똑같은지 비교하십시오.
- **[서류상 2% 이상 원료 순서]**: (예: 1. 정제수, 2. 저감미당, 3. A2단백원유)
- **[시안에 적힌 순서]**: (시안 이미지에서 위 2% 이상 원료들이 나열된 실제 순서 기재)
- **[최종 판정 및 사유]**: (※ 순서가 역전된 경우 "🚨부적합: 배합비 10.7%인 저감미당이 10.0%인 A2단백원유보다 앞에 와야 하나 시안에서는 순서가 역전됨" 과 같이 상세히 지적할 것. 2% 미만 원료는 순서 무관하므로 절대 지적 금지)

## 🧮 2️⃣-{num_alg}. [알레르기 및 교차오염 수학적 정밀 검증 (Rule 38 적용)]
⭐ **[원재료 누락 스나이퍼]**: 원재료명 리스트를 먼저 스캔하여 대두, 밀, 우유 등 알레르기 유발물질이 포함되어 있는지 파악하고, 이들이 시안의 알레르기 표시란(OO 함유)에 누락 없이 모두 기재되었는지 반드시 교차 검증하십시오!
- **[공장 마스터 목록]**: 
- **[직접 투입된 알레르기]**: (※ 시안에 명시된 원재료 기준 직접 투입 알레르기를 반드시 추출하여 적을 것)
- **[도출된 교차오염 정답지]**: (공장 마스터 - 직접 투입 = 정답지)
- **[시안 표기 주의문구]**: 
- **[최종 판정 및 사유]**: (※ 직접 투입 알레르기가 시안의 'OO 함유'란에 누락 없이 잘 표기되었는지, 교차오염 수식이 맞는지 판정할 것. 'OO 함유' 문구가 있다면 ✅적합 처리하되, 사유 끝에 반드시 "⚠️(실무 확인 권장): 시스템상 'OO 함유' 텍스트는 확인되었으나, 해당 문구의 바탕색이 원재료명 란과 다르게 음영 처리되어 확실히 구분되는지 육안으로 한 번 더 확인해 주십시오."라고 기재할 것)
