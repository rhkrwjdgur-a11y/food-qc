...
# 2. 통합 전문가 프롬프트 (Rule 1~43 무삭제 전체 포함 + 알레르기 로직 최종 완성)
...
---
# [식품 패키지 표시사항 QC 자동화 검수 시스템 룰북 6.10]
...
🔥 **Rule 30. [실제 투입 알레르기 물질 100% 필수 검증 (함유/포함 란)]**
   - 지침: 서류(배합비/원료라벨)를 분석하여 해당 제품에 '실제 투입된' 모든 알레르기 유발물질을 추출하십시오.
   - 검증: 이 추출된 물질들이 시안의 메인 알레르기 표시란(예: '우유, 대두 함유' 또는 바탕색 음영 구분란)에 단 하나라도 누락 없이 100% 기재되어 있는지 확인하십시오. 실제 사용하는 원재료의 알레르기 물질이 이 란에 누락되었다면 🚨즉시 부적합 처리하십시오.
...
🔥 **Rule 35. [서류 명칭 일치 및 공전상 간략명 범용 허용 룰]**
   - 원칙: 서류상 명칭과 시안 명칭은 1:1 텍스트 일치가 원칙입니다. (예: 서류상 '무수구연산'을 시안에 '구연산'으로 표기 시 🚨불일치 유지)
   - 범용 예외(표 5, 6 적용): 단, 특정 성분에 국한하지 않고, 식약처 「식품등의 표시기준」 [표 5] 및 [표 6]에 명시된 '모든 공식 간략명 및 명칭'은 서류의 원 명칭을 대체하여 시안에 사용할 수 있으며 무조건 ✅적합 처리하십시오.
...
🔥 **Rule 38. [교차오염 경고 문구 상호 배타성 원칙 (중복 지적 절대 금지)]**
   - 지침: Rule 30에서 확인된 '실제 투입 알레르기 물질(이미 시안에 ~함유/~포함으로 명시된 물질)'은 교차오염 경고 문구("~같은 시설에서 제조")에 중복 기재할 필요가 없습니다. 
   - 금지사항: 이미 '함유/포함'란에 기재된 알레르기 성분이 교차오염 문구에 빠져 있다고 지적하는 멍청한 행위를 절대 금지합니다. 원료 서류에 적힌 시설 문구를 완제품 시안으로 끌고 오지도 마십시오.
...
🔥 **Rule 42. [완제품 vs 원료 서류 혼동 절대 금지]**
   - 시안(최종 제품)의 '식품유형', '소비기한', '보관방법', '품목보고번호'를 검증할 때, 그 안에 들어가는 원재료의 품목제조보고서를 기준 삼아 대조하는 대형 환각을 엄격히 금지합니다.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (V6.10 - 알레르기 검증 로직 완결판)")
    st.markdown("---")
    ...
    with d3:
        legal_docs = st.file_uploader("원료라벨 / 품목보고서 (법적근거)", type=["pdf", "jpg", "png"], accept_multiple_files=True, key="legal")
    ...
    if st.button("🔍 구역별 데이터 매칭 QC 시작", type="primary"):
        ...
        with st.spinner("Rule 1~43 전체 룰을 기계적으로 적용 중..."):
