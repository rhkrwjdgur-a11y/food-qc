...
🔥 **Rule 70. [내/외포장 원재료명 100% 일치 강제 스나이퍼 룰]**
   - 세트포장(박스/트레이) 검토 시, 내포장(낱팩)의 원재료명과 외포장(박스)의 원재료명은 토시 하나 틀리지 않고 100% 똑같아야 합니다. 
   - 서류와의 대조 시에는 Rule 35(동의어 허용)가 적용되어 합법일지라도, **내포장과 외포장 시안끼리의 표기가 서로 다르다면(예: 낱팩은 '구연산나트륨', 박스는 '구연산삼나트륨') 소비자 오인 우려가 있으므로 무조건 🚨부적합(내외포장 불일치)** 처리하십시오.
"""
...
RULES_TAB2 = "[탭2 정보표시면/원재료명 관련 핵심 룰]\n" + get_sliced_rules([1, 2, 5, 6, 7, 8, 12, 13, 14, 25, 28, 29, 30, 34, 35, 38, 39, 44, 48, 52, 54, 57, 58, 59, 60, 61, 65, 69, 70] + COMMON_RULES)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V178.0 - 내/외포장 전 구간 분리 패치)")
...
"""
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【3-Pass】 분석 진행 중..."):
                extract_mission = """
🎯 [미션 A: 정보표시면 배열 강제 추출 (내포장/외포장 및 알레르기 완벽 분리!!)]
1. [원재료명 추출]: 시안의 '원재료명' 텍스트를 [1. 원액두유, 2. 정제수 ...] 처럼 번호가 매겨진 세로 리스트 형태로 추출하십시오. 
   - ⭐ 주의: 끝에 적힌 알레르기 유발물질(예: 우유 함유)은 원재료명이 아니므로 이 리스트에 절대 포함시키지 마십시오!
   - ⭐ 강력 지시: 만약 검토 모드가 '선물세트 박스(외포장) 교차 검토'라면, **반드시 [타겟(박스) 원재료명 리스트]와 [비교용(팩) 원재료명 리스트] 2개를 각각 따로 작성**하십시오! 둘 중 하나라도 누락하면 안 됩니다.
2. [알레르기 추출]: 시안의 "OO 함유" 박스 텍스트는 별도로 추출하십시오. (박스/팩 교차 검토 시 둘 다 추출)

🎯 [미션 B: 증빙 서류 파싱 (마스터 정답지 선행 구축)]
- 사용자가 업로드한 '증빙 서류'를 분석하여 아래 컬럼을 가진 마크다운 표로 정리하십시오.
  | 증빙 서류명 | 원료 제품명 | 식품유형 | 하위 전개 성분 (100% 나열) | 원산지 |
- 혼합제제 하위 성분을 100% 전부 타이핑하십시오.
"""
                base_tab2_warning = """
...
🔥 [공식 보고서 작성 절대 규칙] 🔥
1. 표의 가장 맨 왼쪽 열(Column 1)은 반드시 "[미션 A]에서 추출한 타겟(박스) 시안 원재료명 리스트"를 그대로 복붙해야 합니다. 서류에만 있는 묶음 단어를 임의로 1열에 창조해 내지 마십시오.
2. [원산지 도출 로직]: 표에 이 원료가 왜 원산지가 적혔는지(또는 안 적혔는지) 명확히 기재하십시오. (예: `1순위(의무)`, `제외대상(당류)`)
3. [최종 누락 스나이퍼]: 표 작성이 끝나면 서류에는 있지만 시안에서 통째로 누락된 원료를 차집합으로 계산하여 적발하십시오.
"""
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        judgment_prompt = base_tab2_warning + """
## 1️⃣ [Rule 70. 내포장(팩) vs 외포장(박스) 원재료명 100% 일치 스나이퍼]
- 검증 결과: (두 시안의 원재료명 리스트를 토시 하나까지 1:1 대조하여, '구연산나트륨' vs '구연산삼나트륨'처럼 단 한 글자라도 다르면 즉시 🚨부적합 처리하고 어느 부분이 다른지 명시하십시오. 완전히 동일하면 ✅적합)
...
| 시안 표기 원재료명 (순서대로) | 매칭된 마스터 서류 원료명 | 원산지 산정 순위 | 대조 검증 (오탈자/순서/합법 간략화) | 판정 (상세 사유) |
|---|---|---|---|---|
| (시안 기준 100% 나열) | [내용 작성] | [내용 작성] | [내용 작성] | [내용 작성] |
...
|---|---|---|---|---|
| [내용 작성] | [내용 작성] | [내용 작성] | [내용 작성] | [내용 작성] |

## 2️⃣ [Rule 70. 내포장(팩) vs 외포장(박스) 원재료명 100% 일치 스나이퍼]
- 검증 결과: (두 시안의 원재료명 리스트를 1:1 대조하여, '구연산나트륨' vs '구연산삼나트륨'처럼 다르면 즉시 🚨부적합 처리. 완전히 동일하면 ✅적합)
...
## 2️⃣ [원재료명 정밀 교차 검증 (시안 기준 대조 + 누락 적발)]
| 시안 표기 원재료명 (순서대로) | 매칭된 위 마스터 표 원료명 | 원산지 산정 순위 | 대조 검증 (오탈자/순서/합법 간략화) | 판정 (상세 사유) |
|---|---|---|---|---|
| (시안 기준 100% 나열) | [내용 작성] | [내용 작성] | [내용 작성] | [내용 작성] |
