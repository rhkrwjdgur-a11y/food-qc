...
# ==========================================
# 📚 3. 89대 마스터 룰북 원문 (V311.44 완결본 - 과잉단속 금지 및 방어막 패치)
# ==========================================
RULE_BOOK_FULL = """
...
🔥 **Rule 5. [복합원재료 5% 미만 전개 면제 및 5가지 컷오프 룰 (최우선 방어막)]**
   ...
   - 🌟 **[첨가물 과잉 단속 금지 족쇄]**: 위 조건 A에 따라, 5% 미만 복합원재료 내부에 [표 4, 5, 6] 소속 식품첨가물(예: 효소처리스테비아 등)이 들어있더라도 명칭/용도 표시 의무가 완전히 면제됩니다. Rule 14(첨가물 룰)보다 이 룰이 무조건 우선하며, 하위 첨가물 용도 누락을 절대 지적하지 마십시오.
...
🔥 **Rule 11. [영양정보 단방향 허용오차 법칙 및 단순 수치 차이 합법 안내 룰]**
   ...
   - ⭐ **[단순 수치 차이 안내 및 합법 판정]**: 실측 환산값이 시안의 표시량과 숫자상으로 완벽하게 일치하지 않더라도, 80%~120% 허용 오차 범위 안에 들어온다면 무조건 **✅(적합)**으로 판정하십시오. 단, 사유에는 "수치에 다소 차이가 있으나, 허용 오차 범위 내에 속하므로 합법입니다."라고 친절하게 명시하십시오.
...
🔥 **Rule 55. [영양성분 반올림 강박 금지 및 "그대로 표시" 허용 룰]**
   - 식약처 고시에 따라 열량을 포함한 9대 영양소 모두 "그 값을 그대로 표시하거나" 지정된 단위로 반올림하여 표시하는 것이 선택적으로 허용됩니다. 
   - 따라서 소수점이나 반올림 단위가 수학의 정석과 딱 맞지 않는다고 해서 기계적으로 부적합(🚨) 처리하지 마십시오. 
   - 수치가 달라도 **80% ~ 120% 오차 범위 내에 있다면 완벽한 합법(✅)**입니다.
...
🔥 **Rule 70. [내/외포장 100% 일치 강제 범용 스나이퍼]**
   - 내포장(팩)과 외포장(박스)의 원재료명, 주의문구, **특히 '1개당 영양성분 수치'** 등 공통 표시사항은 텍스트 픽셀 단위로 대조하여 단 하나의 기호나 숫자라도 틀리면 무조건 부적합(🚨) 처리하십시오.
...
ALL_RULES_NUMBERS = list(range(1, 90))
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.44 - 과잉 단속 금지 및 방어막 완결판)")
        ...
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                missions = [
                    ...
                    "⭐ [절대 미션: 개별 단위 쪼개기]: 대조 매트릭스 표를 작성할 때, 쉼표(,)나 띄어쓰기를 기준으로 '모든 원재료를 개별 원료 단위로 완벽하게 쪼개어' 행(Row)을 분리하십시오. 표의 한 칸에 여러 원료를 길게 줄글로 나열하는 행위를 절대 금지합니다. 무조건 1줄에 1개씩 1:1 매칭되도록 출력하십시오."
                ]
                
                base_tab2_warning = "⭐ [1:1 대조 예외 절대 원칙 (Rule 35. 관용명/동의어, 내부 식별코드 생략, 유기농 생략 완벽 합법 우선 적용)] ⭐\n🔥 [시스템 절대 족쇄: 월권행위 금지 및 1줄 1원료 원칙] 🔥\n이 탭은 오직 '원재료명, 첨가물, 알레르기, 행정정보'만 검토하는 탭입니다. 영양정보 등은 배제하십시오. 원재료명은 반드시 분해하여 한 행당 하나씩 기재하십시오.\n\n"

                # 🚀 [분기 1] 증빙 서류(레시피/라벨)가 제출된 경우
                if has_any_doc:
                    if "무더기" in doc_mode:
                        missions.append("업로드된 '개별 원료 한글라벨 무더기' 데이터를 면밀히 분석하고 종합하여 하나의 완벽한 [마스터 배합비 표]로 합성/정리하십시오. 배합비(%)와 원산지 순위를 명확히 도출해야 합니다.")
                        doc_prompt_section = "## 2️⃣-1. [AI 종합 마스터 배합비 표] (한글라벨 무더기 기반 합성)\n| 통합 원료명 (1줄에 1개씩) | 하위 전개 성분 | 원산지 | 추정 배합비(%) / 순위 |\n|---|---|---|---|\n"
                    else:
                        missions.append("업로드된 증빙 서류(마스터 엑셀/PDF)에서 모든 원료명, 하위 성분, 원산지, 배합비(%)를 추출하십시오.")
                        doc_prompt_section = "## 2️⃣-1. [서류 기반 마스터 원재료 DB]\n| 서류상 원료명 (1줄에 1개씩) | 하위 전개 성분 | 원산지 | 배합비(%) / 순위 |\n|---|---|---|---|\n"

                    common_tab2_prompts = doc_prompt_section + """
## 2️⃣-2. [마스터 서류 vs 시안 법적 대조 매트릭스]
⭐ **[원산지 98% 컷오프 및 예외 완벽 검증 족쇄 (Rule 1, 28 절대 준수)]**: 
1. [1단계]: 1순위 원료의 배합비가 98% 이상인지 숫자로 먼저 확인하십시오!
2. [2단계]: **만약 1순위가 98% 이상이라면**, 2순위와 3순위 원료는 원산지가 시안에서 누락되어도 절대로 지적하지 말고 **"✅(적합) - 1순위 98% 초과로 표기 생략 합법"**이라고 판정하십시오.
3. [3단계]: 1순위+2순위 합이 98% 이상이면 3순위 누락은 합법 처리하십시오. 98% 룰 방어막을 뚫은 누락에 대해서만 🚨부적합 처리하십시오.
4. 🚫 **[과잉 단속 절대 금지]**: 배합비 4순위 이하의 원료, 당류(설탕/시럽류), 식품첨가물(레시틴, 구연산 등)은 원산지 표시 대상이 아닙니다. 이들의 원산지 누락을 지적하면 즉시 시스템 오류로 간주됩니다. 묻지도 따지지도 말고 무조건 패스(✅적합)하십시오.
⭐ **[가공품 원산지 이중 표기 족쇄 (Rule 89 절대 준수)]**: 업로드된 원료 라벨에 '품목제조보고번호'가 있거나 '제조원'이 국내업체인 경우, 이는 수입 원물로 국내에서 제조된 가공품(예: 옥배유, 사과농축액)입니다. 이 경우 반드시 `원료명(원료명: 국가명)` 형태인지 확인하십시오. 시안에 단순히 `원료명(국가명)` 형태로 기재했다면 무조건 🚨부적합 처리하십시오.
| 시안 표기 개별 원재료명 (반드시 1줄에 1개씩 쪼개어 작성) | 매칭된 서류 원료명 (없으면 '제출 안 됨') | 대조 검증 결과 (명칭 대조 및 ⭐상위 3순위 원산지 표기 누락 여부⭐) | 최종 판정 |
|---|---|---|---|
...
"""
                # 🚀 [분기 2] 증빙 서류 없이 '박스 vs 팩' 1:1 비교만 하는 경우
                else:
                    if "박스" in ins_mode:
                        missions.extend([
                            "오직 '타겟(박스) 시안'의 원재료명 리스트를 1줄에 1개씩 나열하십시오.",
                            "오직 '비교용(팩) 시안'의 원재료명 리스트를 1줄에 1개씩 나열하십시오. 증빙 서류는 없으므로 원산지 산정 검증은 생략합니다."
                        ])
                        common_tab2_prompts = "## 2️⃣-1. [내포장(팩) 정답지 원재료 DB] (비교용 팩 시안 기준)\n| 비교용 팩 시안 개별 원재료명 (1줄에 1개씩) | 비고 (서류 미제출로 배합비 알 수 없음) |\n|---|---|\n\n## 2️⃣-2. [박스(타겟) vs 팩(비교용) 내외포장 100% 일치 대조 매트릭스 (Rule 70)]\n⭐ **[내/외포장 1:1 대조 족쇄]**: 서류가 없으므로 원산지 3순위 검증은 수행하지 않습니다. 오직 타겟(박스)의 원재료명과 비교용(팩)의 원재료명을 픽셀 단위로 대조하여 단 하나의 글자, 기호 띄어쓰기라도 다르면 무조건 🚨부적합 처리하십시오.\n| 타겟(박스) 표기 개별 원재료명 (반드시 1줄에 1개씩) | 비교용(팩) 표기 개별 원재료명 (반드시 1줄에 1개씩) | 대조 검증 결과 (픽셀 100% 일치 여부) | 최종 판정 |\n|---|---|---|---|\n\n### 🚨 [팩 시안 기준 최종 누락 스나이퍼 검증]\n- 적발 양식: \"🚨 [누락]: 팩 시안의 'OOO' 원료가 박스 시안에서 다르게 적혔거나 누락되었습니다.\"\n- 이상 없을 시: \"✅ 팩 시안 대비 누락된 원료 없음.\"\n"
                    # 🚀 [분기 3] 단품 모드인데 서류도 없는 경우 (형식 검토만)
                    else:
                        missions.append("증빙 서류가 없으므로 시안에 기재된 원재료명의 형식적 오류(첨가물 룰북 위반 여부 등)만 집중 검토하십시오.")
                        common_tab2_prompts = "## 2️⃣-1. [시안 표기 원재료명 리스트]\n(※ 증빙 서류 미제출로 서류 대조 및 원산지 검증 불가)\n\n## 2️⃣-2. [자체 형식 검토 매트릭스]\n| 시안 표기 개별 원재료명 (반드시 1줄에 1개씩 쪼개어 작성) | 형식 검토 결과 (오탈자 등) | 판정 |\n|---|---|---|\n"

                # [공통 하단 양식] (첨가물 룰, 2% 룰, 알레르기 등)
                common_tab2_bottom = """
...
(※ 서류가 없어 배합비를 모를 경우 "데이터 부재로 검증 불가" 기재, 있을 경우 결과 요약)
    ...
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【영양성분표 수치 환산 및 교차 검증 중...】"):
                ...
                base_tab3_warning = "⭐ [영양성분표 절대 원칙]: 시안의 수치, 단위(g, mg, μg, α-TE 등), % 기준치는 소수점 하나까지 정확히 추출하고 대조하십시오.\n\n"
                
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    common_tab3_prompts = """## 3️⃣-1. [박스(외포장) vs 팩(내포장) 영양정보 1:1 교차 검증]
⭐ **[Rule 68 & 70 족쇄]**: 박스의 '1개(팩)당' 영양 수치와 실제 팩의 영양 수치가 숫자, 기호, 띄어쓰기까지 100% 일치하는지 대조하십시오. 또한, 박스의 영양정보가 총 내용량 기준인지 1개당 기준인지 확인하여 '낱팩 용량 그대로 복붙(Rule 68)' 에러가 없는지 스캔하십시오.
| 영양성분명 | 타겟(박스) 1개당 표시량 | 비교용(팩) 표시량 | 일치 여부 (단위 포함 100% 픽셀 대조) | 판정 |
|---|---|---|---|---|

## 3️⃣-2. [영양표시 오차 검증 및 % 기준치 수학적 역산 (성적서 대조)]
(※ 성적서가 제출된 경우에만 작성, 없으면 '성적서 미제출로 실측 오차 검증 생략' 기재)
⭐ **[성적서 정밀 계산 족쇄]**: 3-1에서 교차 검증을 마쳤더라도, 성적서가 존재할 경우 절대 건너뛰지 마십시오. 반드시 '환산 수식(예: 실측값 x 1.8)'과 '80%/120% 컷오프 만족 여부', 그리고 '1일 기준치 대비 %(Rule 41)'를 직접 수학적으로 계산하여 시안의 숫자와 맞는지 증명하십시오.
⭐ **[사유 작성 지침]**: 수치가 다소 다르더라도 80~120% 오차 범위 내라면 반드시 '✅ 적합'을 부여하고, "단순 수치 차이가 나지만 오차 범위 내이므로 합법입니다"라고 친절히 안내만 하십시오. 기계적인 반올림 강박을 버리십시오.
| 영양성분 | 성적서 환산값(A) (계산식 포함) | 시안 표시량(B) | 법적 기준선 (B의 80% 또는 120%) | 🎯 % 역산 검증 (수식 포함) | 판정 및 상세 사유 (오차 판단) |
|---|---|---|---|---|---|
...
"""
                    ...
                    common_tab3_prompts = """## 3️⃣ [영양표시 오차 검증 및 % 기준치 수학적 역산 (성적서 대조)]
(※ 성적서가 제출된 경우에만 작성, 없으면 '성적서 미제출로 실측 오차 검증 생략' 기재)
⭐ **[성적서 정밀 계산 족쇄]**: 성적서가 존재할 경우 절대 건너뛰지 마십시오. 반드시 '환산 수식(예: 실측값 x 1.8)'과 '80%/120% 컷오프 만족 여부', 그리고 '1일 기준치 대비 %(Rule 41)'를 직접 수학적으로 계산하여 시안의 숫자와 맞는지 증명하십시오.
⭐ **[사유 작성 지침]**: 수치가 다소 다르더라도 80~120% 오차 범위 내라면 반드시 '✅ 적합'을 부여하고, "단순 수치 차이가 나지만 오차 범위 내이므로 합법입니다"라고 친절히 안내만 하십시오. 기계적인 반올림 강박을 버리십시오.
| 영양성분 | 성적서 환산값(A) (계산식 포함) | 시안 표시량(B) | 법적 기준선 (B의 80% 또는 120%) | 🎯 % 역산 검증 (수식 포함) | 판정 및 상세 사유 (오차 판단) |
|---|---|---|---|---|---|
        ...
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("【의무표시 및 인증마크 해독 중...】"):
                missions = [
                    ...
                    "기타면/측면 이미지에서 '단백질', '비타민', '칼슘' 등 특정 영양소를 강조하는 마케팅 뱃지나 텍스트, 그리고 '100%' 라는 수치 강조 문구가 있는지 빠짐없이 스캔하여 추출하십시오. (단순 정보 제공이라 자의적으로 판단하여 추출을 누락하지 마십시오)."
                ]
