...
🔥 **Rule 24. [감미료 14pt 의무 표기 조건 엄격 분리 및 다중 강조표시 규칙]**
   - 식약처 규정상 "감미료 함유" 문구 표시 의무는 오직 **'무당(Zero)'** 또는 **'무가당'**을 강조했을 때만 발동합니다.
   - **[🚨과잉 지적 절대 금지]**: 시안에 **'저당(Low)'**이라고 표시된 경우에는, 원재료에 감미료(수크랄로스, 알룰로스 등)가 들어있더라도 주표시면에 "감미료 함유" 문구를 기재할 법적 의무가 없으므로 무조건 적합(✅) 처리하십시오. '저당'을 '무당'과 혼동하여 부적합 처리하는 환각을 엄격히 금지합니다.
   - **[🚨다중 강조 시 위치 강제]**: 단, '무당'이나 '무가당' 표시가 시안에 2회 이상 반복되어 있는 경우에는, 반드시 "가장 큰 강조표시 주위"에 감미료 함유 문구를 기재해야 합니다.
...
✅ **Rule 32. [단순 역산에 의한 부적합 판정 금지 (열량 비율 등)]**
   - 영양성분표의 수치(탄수화물x4, 단백질x4, 지방x9)를 단순 계산하여 도출한 열량 비율과 마케팅 문구의 비율(예: 균형영양 48:22:30 등)이 일치하지 않는다고 부적합 처리하지 마십시오. 식이섬유(2kcal/g) 및 법적 반올림 규정으로 인해 단순 역산은 무조건 오차가 발생하므로 비율은 적합(✅)으로 인정하십시오.
...
🔥 **Rule 50. [원료사 명칭 꼼수 적발 및 원액/추출물 고형분 의무 표시 강제 룰]**
   - **[🚨꼼수 적발]**: 납품업체가 서류상 원료명에 '원액'을 썼더라도, 하위 성분에 단 하나의 첨가물(효소제, 알파갈락토시다아제 등)이나 정제수가 섞여 있다면 마케팅 면에 '원액/100%' 강조 시 기만광고(🚨확인요망)로 지적하십시오.
   - **[🚨고형분 누락 적발 절대 명령]**: 식약처 고시에 따라, 주표시면에 '원액', '추출물', '농축액'의 함량(%)을 강조하여 표시한 경우, **반드시 그 주변에 '고형분 함량(%)' 또는 '배합함량(%)'이 함께 기재되어 있어야 합법**입니다. 제품명에 해당 원료명이 명시되어 있지 않더라도, 패키지에 함량(%)을 표기한 순간 고형분 표시 의무가 발동합니다. 함량만 덩그러니 강조하고 고형분 수치를 누락했다면 즉시 규정 위반(🚨부적합)으로 처리하십시오.
...
🔥 **Rule 52. [논리적 모순 탐지 및 영양학적 팩트 기반 카운트 강제 룰 (🚨보수적 표기 허용)]**
   - 제품명이나 마케팅 문구에 특정 숫자(예: '비타민 15종', '미네랄 3종')가 명시되어 있다면 대조 카운트하십시오.
   - **[🚨카운트 헛소리 방지 1 (비타민)]:** 이름에 비타민이 없어도 나이아신, 엽산, 판토텐산, 바이오틴을 무조건 합산하십시오.
   - **[🚨카운트 헛소리 방지 2 (미네랄)]:** 나트륨(Sodium)은 카운트에서 무조건 제외(합산 금지)하십시오!
   - **[🚨보수적 표기 및 그룹핑 인정 절대 명령]**: 마케팅 문구에 강조한 숫자(예: 24종)가 실제 영양성분표나 원재료에 있는 숫자(예: 25종)보다 **작거나 같은 경우**는 영양소 그룹핑 및 보수적 표기에 해당하므로 소비자를 기만하지 않은 합법입니다. **무조건 적합(✅)** 처리하십시오. 실제 성분보다 마케팅 숫자를 과장(부풀림)한 경우에만 부적합(🚨) 처리하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V112.0 - 알레르기 차집합 마스터판)")
    ...
    with tab1:
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("주표시면 기계적 텍스트 추출 및 정밀 대조 중..."):
                ...
                2. 키워드 스캔: 추출된 텍스트 안에 '원액', '100%', '제로', '무당', 마케팅 숫자, 분말 등이 포함되어 있는지 확인하십시오.
                3. 🚨 [Rule 50 고형분 누락 스나이퍼]: 주표시면에 '원액', '추출물', '농축액'의 함량(%)이 강조되어 있다면 고형분 N%가 있는지 샅샅이 찾으십시오. 단, '추출분말'은 고체이므로 고형분 표시 대상이 아닙니다.
                4. 🚨 [수량 2-Track 검증 (연산 & 텍스트)]: 1) [연산]: '단위용량 x 개수 = 총용량'의 산수가 정확한지 곱하기를 수행하여 적발하십시오. 2) [텍스트]: 계산이 맞더라도 수량 텍스트 형태(개입/팩, 소문자x/대문자X)가 단 한 글자라도 다르면 🚨부적합 처리하십시오.
                </thinking>
            ...
            with st.spinner("서류 유형 분석 및 창의성 배제 100% 교차 검증 중..."):
                ...
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    ...
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        2. 🚨 [다중 라벨 병합 분석]: 원료 라벨들을 매칭하여 마스터 리스트 조립을 수행하십시오.
