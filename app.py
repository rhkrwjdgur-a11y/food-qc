...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V53.0 - 뇌정지 방지 숏-띵킹 패치)")
    ...
    # ==========================================================
    # 탭 2: 정보표시면 검토 (V53 뇌정지 방지 숏-띵킹 패치 적용)
    # ==========================================================
    with tab2:
        ...
        if st.button("▶️ 정보표시면 원재료 100% 대조 시작", key="btn_info"):
            with st.spinner("서류 분석 및 원재료 대조 표 렌더링 중... (최대 30초 소요)"):
                ...
                `<thinking>` 태그를 열고 전체 '배합비(%)'가 적혀 있는지 확인하십시오. 
                **[🚨뇌정지 방지 강제 명령]: 생각(Thinking) 과정은 반드시 3문장 이내로 극도로 짧게 요약하십시오. 길게 나열하면 시스템이 멈춥니다. %가 없다면 즉시 파악을 멈추고 `</thinking>` 태그를 닫은 후 2단계 표를 그리십시오. 절대 분석 중에 멈추지 마십시오.**
