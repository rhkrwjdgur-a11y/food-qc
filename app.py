...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V350.00 - CoT 다단생성 및 병렬 스레드 가속화 적용)")
    ...
    def run_qc_3pass(tab_rules: str, judgment_prompt: str, extract_missions_list: list = None):
        ...
        if extract_missions_list:
            # ⭐ [V350.00 유지]: Pass 1은 100% 무결점 보장을 위해 무조건 '순차적'으로 추출합니다.
            extracted_results = []
...
"""
            ...
            # ⭐ [V350.00 신규 최적화]: Pass 1.5와 Pass 1.8은 독립적이므로 ThreadPoolExecutor를 사용해 '동시(병렬)'에 실행시켜 속도를 2배 높입니다.
            def run_pass15():
                pass15_prompt = f"""
                    ...
                    try:
                        ...
                        if "504" in str(e) or "Deadline" in str(e) or "503" in str(e):
                            if attempt < max_retries - 1:
                                time.sleep(10)
                                continue
                        return extracted_text_combined # 에러 시 원본 텍스트를 그냥 리턴하여 셧다운 방지
...
| ⭐ **마케팅 강조 문구 팩트체크 (전면 Cross-Check)** | [Rule 17, 19, 88, 99] | (※ 앞면에 '무(ZERO)', '무첨가', '100%' 등의 마케팅 클레임이 단 하나라도 존재한다면, 반드시 뒷면 **'원재료명'에 나열된 모든 텍스트를 처음부터 끝까지(괄호 유무 상관없이) 전수 스캔**하여 모순을 적발할 것. <br> 1) 무가당/저당: 원재료명 어디에든 포도당/시럽/물엿/덱스트린 등이 섞여 있는지 추적 <br> 2) 무첨가: 해당 첨가물 존재 여부 추적 <br> 3) 100%: 0.01%의 타 원료 혼입 여부 추적 <br> 발견 시 즉시 기만행위(🚨부적합)로 사유 작성!) | |
...
"""
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                if "박스" in ins_mode:
                    missions.append("⭐ [내외포장 1:1 분할 매칭 강제명령]: 타겟(박스) 시안과 비교용(팩) 시안의 원재료명 전체 텍스트를 표의 한 칸(행)에 통째로 때려 넣는 행위를 절대 금지합니다. 반드시 쉼표(,)를 기준으로 각각의 개별 원재료를 쪼개어, 표의 1개 행(Row) 당 딱 1개의 원재료만 1:1로 나란히 매칭되도록 길게 표 창작하십시오.")
                ...
                if "무더기" in doc_mode:
                    judgment_prompt += "## 2️⃣-1. [서류 기반 원재료 마스터표 생성]\n| No | 배합비 순위 (Rank A) | 서류상 원재료명 | 배합비율(%) | 서류상 원산지 | 비고 (식품유형/혼합제제 하위성분 등) |\n|---|---|---|---|---|---|\n\n"
                
                if "박스" in ins_mode:
                    judgment_prompt += "## 2️⃣-2. [내외포장 원재료명 1:1 대조 검증 (절대 생략 금지)]\n"
    ...
    with tab3:
        if st.button("▶️ 영양성분표 수치 자동 환산 및 대조", key="btn_nutri"):
            with st.spinner("【영양성분 수치 역산 및 오차 매트릭스 검증 중...】"):
                ...
                if "박스" in ins_mode:
                    judgment_prompt_tab3 += "## 3️⃣-1. [내외포장 영양성분표 1:1 대조 (절대 생략 금지)]\n"
                ...
                if has_any_doc:
                    ...
                    # ⭐ [V350.00 유지] CoT 사전 연산: 생성해야 할 표 목록 확정
    ...
    with tab4:
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("【기타면 주의사항 및 텍스트 검증 중...】"):
                ...
                judgment_prompt = """## 4️⃣ [기타면/측면 법적 주의사항 검증]
...
"""
    ...
    with tab5:
        if st.button("▶️ 🤖 AI 법무팀장 자유 스캔 시작", key="btn_law"):
            with st.spinner("【AI 법률 엔진 가동: 표시광고법 및 판례 기반 심층 스캔 중...】"):
                # ⭐ [V350.00 유지]: 5번 탭은 족쇄 없이 완전히 자유로운 '법무팀장' 역할을 수행하도록 프롬프트 분리
                free_style_prompt = """## 🤖 5️⃣ [AI 법무팀장 특별 감사 리포트 (자율 심층 스캔)]
...
| 🚨 적발된 리스크 (Risk) | 🔍 발견 위치 및 문구 | ⚖️ 관련 법령 및 판정 근거 (상세 사유) | 🛡️ 방어 및 수정 제안 |
|---|---|---|---|
"""
    ...
    with tab6:
        if st.button("▶️ 종합 결과 요약 리포트 생성", key="btn_summary"):
            with st.spinner("【전체 검토 결과 집계 및 경영진 보고서 작성 중...】"):
                summary_prompt = """## 📊 6️⃣ [식품 표시사항 종합 검토 리포트]
...
3. **[⚠️ 실무 확인 요망]**: 규정 위반은 아니나, 폰트 크기, 마크 색상, 보수적 표기 안전율 등 인간의 눈으로 최종 확인해야 할 사항
