...
def main():
    ...
    st.title("식품 표시사항 정밀 검토 시스템 (V4.2.8 Ultimate Master)")
    ...
    with tab3:
        if st.button("영양성분표 수치 자동 환산 및 대조", key="btn_nutri"):
            with st.spinner("영양성분 수치 역산 및 오차 매트릭스 검증 중..."):
                ...
                judgment_prompt_tab3 += "- 🚨 [발효유/농후발효유 절대 주의]: 패키지 디자인(스푼 유무)과 제품명(떠먹는 등)을 스캔하여 '성상'을 파악할 것!\n"
                judgment_prompt_tab3 += "  * 일반 발효유 (액상): 80ml (또는 80g)\n"
                judgment_prompt_tab3 += "  * 농후발효유 (액상): 150ml (또는 150g)\n"
                judgment_prompt_tab3 += "  * 떠먹는 호상 발효유 및 농후발효유 (유형 불문 호상 형태): 100g\n\n"
                ...
                judgment_prompt_tab3 += f"## [사전 연산 3: 영양성분 수치 역산 스크래치패드]\n"
                if has_any_doc:
                    ...
                    judgment_prompt_tab3 += "(※ 증빙 서류가 없으므로 실측치 환산은 생략합니다. 대신 시안의 '1일 기준치 %' 산출이 정확한지, '열량 역산(탄4단4지9)'이 맞는지, '0표기 규정'을 지켰는지 이곳에서 꼼꼼히 역산 검증하십시오.)\n"
                judgment_prompt_tab3 += "</pre_calc>\n\n"
