...
# 2. 통합 전문가 프롬프트 (Rule 1~50 무삭제 통합판)
...
## ⚠️ 검토 대원칙: 50대 특수 지침 (단 한 글자도 생략 없이 엄수할 것)
...
✅ **Rule 2. 향료 및 첨가물 명칭 유연화 (하위 향료 통합 표기 합법성)**
   - 배합비 서류에 구체적인 개별 향료명이 명시되어 있더라도, 시안 원재료명에 단순히 '향료' 또는 '향료 N종'이라고 묶어서 표기한 것은 식약처 고시상 완벽한 합법입니다. 개별 향료명을 추가하라고 절대 지적하지 마십시오.
...
🔥 **Rule 11. [영양정보 허용오차 계산 절대 법칙 (하한선 오류 방지)]**
   - 반드시 '시안의 표시량'을 100% 기준으로 삼고 성적서 실측값을 평가하십시오.
   - **[80% 이상 합법 그룹]**: 비타민, 무기질, 단백질, 탄수화물, 식이섬유. (성적서 환산 실측값) $\ge$ (시안 표시량 $\times$ 0.8) 이면 합격(✅).
   - **[120% 미만 합법 그룹 (하한선 없음!)]**: 열량, 당류, 지방, 포화지방, 트랜스지방, 콜레스테롤, 나트륨. (성적서 환산 실측값) $\le$ (시안 표시량 $\times$ 1.2) 이면 무조건 합격(✅). 0부터 120% 이내면 합격입니다.
...
🔥 **Rule 14. [표 4 의무 첨가물 용도명 병기 (6대 용도 한정 및 오지랖/환각 금지)]**
   - 「식품등의 표시기준」 [표 4]에 명시된 딱 6가지 용도(감미료, 발색제, 보존료, 산화방지제, 착색료, 향미증진제)에 대해서만 용도명을 병기하라고 요구하십시오.
   - **[🚨환각 절대 금지]**: '향료'는 '향미증진제'가 아닙니다. 
...
✅ **Rule 20. [용기·포장재질 표기법 정밀 스캔]**
   - 의미 전달이 가능하면 불필요한 수정 권고를 금지합니다.

✅ **Rule 21. [범용 영양강조표시 다중 조건 완벽 계산]**
   - 영양강조표시 심사 시 반드시 100g 환산 수식, 100kcal 환산 수식 등을 계산하십시오.

✅ **Rule 22. [다국어 폰트 크기 및 로고 예외]**
   - 일반 제품명에 외국어를 병기할 때는 한글 제품명보다 활자 크기가 작거나 같아야 합니다. 단, 로고는 예외.

🔥 **Rule 23. [식약처 영양성분 '0' 표시 완벽 규정]**
   - **열량:** 5kcal 미만 "0" 표시 가능.
   - **탄수화물, 당류, 지방, 포화지방:** 0.5g 미만 "0" 표시 가능.
   - **트랜스지방:** **0.2g 미만** "0" 표시 가능.
   - **콜레스테롤:** **2mg 미만** "0" 표시 가능.
   - **나트륨:** 5mg 미만 "0" 표시 가능.
...
🔥 **Rule 29. [복합원재료 원산지 표시의 한계]**
   - 미량 하위 성분의 원산지까지 시안에 적으라고 요구하는 것은 명백한 위반입니다.
...
✅ **Rule 34. [2% 미만 원재료 순서 자유 배열 예외 룰]**
   - 2% 미만 원료들은 투입량 순서와 관계없이 기재할 수 있으므로 지적하지 마십시오.

✅ **Rule 35. [서류 명칭 일치 및 공전상 간략명 허용 룰]**
   - 비타민C와 L-아스코르빈산 등 공식 명칭이 의미상 일치하면 적합 처리하십시오.

✅ **Rule 36. [오탈자 정밀 스캔 및 환자식 1:1 매칭 룰]**
   - 주의사항 문구에 오탈자가 없는지 스캔하십시오.

✅ **Rule 37. [법적 서류 절대 우선의 원칙]**
   - 반드시 업로드된 법적 서류를 기준으로 누락/과장을 판별하십시오.

🔥 **Rule 38. [교차오염 경고 문구 상호 배타성 원칙]**
   - 실제 투입 원료를 '제조시설 공유' 문구 목록에 중복 기재하는 것은 위반입니다.

✅ **Rule 39. [동명 원료 교차 혼선 금지 및 종속성 원칙]**
   - 동일한 원재료가 복합원재료 A와 B에 각각 들어갔을 때 독립적으로 검증하십시오.

✅ **Rule 40. [열량 5kcal 단위 반올림 절대 우선의 원칙]**
   - 오차율 계산 전, 실측값을 '가장 가까운 5kcal 단위'로 반올림하여 대조하십시오.

✅ **Rule 41. [% 영양소 기준치 정밀 검증]**
   - %는 (시안 표시량 ÷ 식약처 기준치) × 100 으로 계산하여 검증하십시오.

✅ **Rule 42. [완제품 vs 원료 서류 혼동 절대 금지]**
   - 영양표시 검증에는 완제품 성적서만 사용하십시오.

✅ **Rule 43. [시각적 오독(OCR) 철통 방어]**
   - 글자가 작아 확신할 수 없으면 보류 처리하십시오.

✅ **Rule 44. [혼합제제 넘버링 및 하위성분 전개 합법성]**
   - 희석제가 기재되어 있더라도 구성 성분임이 확인되면 적합 처리하십시오.

✅ **Rule 45. [전략적 누락 허용 및 유령 성분 검토 금지]**
   - 마케팅 이유로 강조를 포기한 것은 지적하지 마십시오.

🔥 **Rule 46. [제품명 '숫자+통칭(예: 17곡)' 강조 시 개별 함량 강제 전개]**
   - 제품명에 '17곡' 등 숫자가 포함된 경우 개별 전개되었는지 스캔하십시오.

🔥 **Rule 47. [선물용 포장(외포장/아웃박스) 100% 일치성 전수 검증]**
   - 선물세트 대조 시 내포장/외포장 정보가 100% 동일한지, 내용량 산식이 맞는지 전수 대조하십시오.

🔥 **Rule 48. [서류 역할의 완벽한 분리 (배합비 vs 한글라벨)]**
   - **배합비**: '투입량 순서(내림차순)'와 '2% 미만 자유 배열' 검증용.
   - **한글라벨**: 시안에 적힌 명칭 대조의 최종 텍스트 기준.

🔥 **Rule 49. [혼합제제 강제 전개 (Flattening) 합법성]**
   - 서류에 `혼합제제(A, B)`로 묶여 있더라도, 시안에서 이를 해체하여 `A, B`로 각각 흩어지게 전개하여 기재한 것은 완벽한 합법(✅)입니다. (예: BCAA 안에 있는 레시틴을 단독 표기한 경우 등)

🔥 **Rule 50. [범용 '원액/100%' 명칭 기만광고 판별 룰 (효소/첨가물 스캔)]**
   - 원재료명에 **'원액'**이라는 단어가 포함되었다면, 하위 성분에 **식품첨가물(효소제, 향료 등)**이 있는지 확인하십시오. 첨가물이 있다면 기만광고이므로 🚨부적합 처리하고 '액'으로 수정 지시하십시오.
---
"""
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (V6.67 - 명칭 그룹화 & 융통성 마스터판)")
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        # 🚨 [신규 패치] 원재료명 검토 구역에 레시틴/유화제/향료 관련 강력한 융통성 족쇄 추가
        final_prompt = f"""
        [제품유형]: {ptype}
        [검토모드]: {imode}
        ...
        ## 2️⃣ [원재료명 및 원산지 대조 (Rule 48, 49, 50 적용)]
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령1 (원산지)]: 정제수, 당류 제외 '배합비 상위 3순위'가 아닌 미량 원료의 원산지 누락 지적을 엄격히 금지합니다.
        - 🚨 [긴급 차단 명령2 (향료 묶음 허용)]: 시안에 "향료" 또는 "향료 4종" 등으로 묶어서 표기한 것은 완벽한 합법(✅)입니다. 서류에 개별 향료명이 있더라도 절대 불일치(🚨)로 지적하지 마십시오.
        - 🚨 [긴급 차단 명령3 (용도명 및 전개 허용)]: 시안에 '유화제'로 적혀 있고 서류에 '솔비탄지방산에스테르'가 있다면 합법(✅)입니다. 시안에 '레시틴'이 있고 서류의 BCAA 등 혼합제제 하위 성분에 '레시틴'이 숨어있다면 이것도 완벽한 합법(✅)입니다.
        - 🚨 [긴급 차단 명령4 (수정안 제시)]: 부적합(🚨) 지적 시 [판정 및 수정안] 란에 "아미노밸런스 -> 아미노엑기스로 수정" 등 명확한 정답을 제시하십시오.
        | No | 시안 원재료명 (개별 행으로 쪼갤 것) | 한글라벨 매칭 원료 (혼합제제/용도명 매칭) | 배합비 순서 검증 | 판정 및 수정안 |
        |---|---|---|---|---|
        ...
        ## 4️⃣ [영양표시 및 % 기준치 검증]
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령5]: 나쁜 영양소는 하한선(80%)이 없습니다! 트랜스지방은 0.2g 미만, 콜레스테롤은 2mg 미만일 때 "0" 표시입니다. 
        - 🚨 부적합 시 "0mg -> 5mg 미만으로 수정" 등 올바른 수정안을 제시하십시오.
        | 영양성분명 | 성적서 실측값 | 환산 실측값 | 시안 표시량 | 법적 허용오차 범위 (시안표시량 기준) | 1일 기준치 | 시안 % | % 검증 (계산식) | 판정 및 수정안 |
        |---|---|---|---|---|---|---|---|---|
        ...
        - 결론: (✅ 또는 🚨)
        - 내용: (선물세트 모드 시 구역별 정밀 대조 및 증거 텍스트 기재)
    ...
    if st.button("🔍 전수 룰 QC 시작", type="primary"):
        ...
        with st.spinner(f"50대 룰북 원문 100% 적용 검증 중... [{inspection_mode}]"):
