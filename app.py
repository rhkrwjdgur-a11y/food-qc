...
🆕 **Rule 11. 시험성적서 vs 시안 표기값 엄격한 팩트 체크 및 1일 기준치(%) 검증 (자료 제공 시)**
   - **[절대 주의 - 환각 방지]:** 당신이 계산한 환산값을 시안에 적혀있다고 착각(거짓말)하지 마십시오! 반드시 시안(이미지)에 '실제로 적힌 숫자 및 퍼센트(%)'를 먼저 추출하여 고정해야 합니다.
   - **[숫자 크기 비교 주의]:** 환산값이 5 이상이면 절대 '5 미만'이라고 착각하지 마십시오. 열량이 5 이상이면 무조건 5kcal 단위로 반올림하여 표시해야 하며 절대 0으로 표시할 수 없습니다.
   - 검토 시 반드시 아래 항목들을 **표(Table) 형태**로 비교 보고하십시오:
     1. **시안 실제 표기값:** 이미지 영양정보란에 '현재 쓰여 있는' 숫자 및 %.
     2. **성적서 환산 정답:** 100mL 기준 성적서 값을 제품 총 용량으로 비례 계산하고 법적 반올림을 적용한 정답 숫자.
     3. **1일 영양성분 기준치 비율(%) 정답:** 환산된 영양성분 값을 법적 1일 영양성분 기준치로 나누어 계산된 올바른 비율(%).
     4. **일치 여부 판정:** 시안의 표기값이나 비율(%)이 정답과 다르면 무조건 **"🚨수정 필요(부적합)"**으로 강력히 지적하고 올바른 수정안을 제시하십시오.
...
## 0️⃣ [시험성적서 및 배합비 교차 검증] (해당 자료 제공 시에만 작성)
   - 📊 **영양성분 팩트 체크 표** (Rule 11 적용: 시안 표기값 vs 성적서 환산값 vs 1일 기준치 비율(%) 철저 비교)
    ...
    if uploaded_files:
        ...
        if st.button("🔍 QC 전문가 모드 진단 시작", type="primary"):
            try:
                ...
                # 안내 문구 및 AI 최면 프롬프트 강력하게 업데이트
                with st.spinner("영양성분 환산 및 퍼센트(%) 비율 검증, 시각적 음영 스캔 등 정밀 분석 중입니다..."):
                    prompt = """
                    업로드된 파일들을 종합적으로 검토해줘. 
                    특히 [Rule 11]을 철저히 지켜서, 시안에 쓰여있는 실제 숫자와 네가 계산한 성적서 환산값을 절대 헷갈리지 말고 반드시 표(Table)로 먼저 정리해!
                    1일 영양성분 기준치 비율(%) 계산이 시안에 적힌 퍼센트와 맞는지도 꼭 팩트 체크해. 
                    계산값이 5 이상이면 절대 0으로 표시할 수 없다는 수학적 팩트도 명심해.
                    또한 [Rule 13, Rule 14]에 집중하여, 알레르기 문구의 '시각적 음영/테두리', 첨가물 '용도명' 병기를 꼼꼼히 확인해.
                    """
