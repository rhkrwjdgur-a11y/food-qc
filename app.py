...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V155.8 - 순서 강제보존 및 원산지 시각화 완결판)")
    ...
    def run_qc_3pass(tab_rules: str, judgment_prompt: str):
        ...
        # ── PASS 1: 순수 텍스트 추출 (물리적 순서 절대 보존 및 오타 교정 차단) ──
        pass1_prompt = """
[PASS 1 - 텍스트 추출 전용 명령]
⭐ 이 단계에서는 어떤 판정, 평가, 해석도 절대 금지입니다. 오직 텍스트 필사만 수행하십시오.
⭐ [환각 차단 절대 명령]: 당신은 인간의 맥락 이해 능력이 없는 '바보 광학 스캐너(OCR)'입니다. 사용자가 업로드한 'PDF 증빙 서류'의 내용을 머릿속에서 완전히 지우십시오. 오직 '라벨 시안 이미지' 눈에 보이는 픽셀 글자만 그대로 베껴 써야 합니다.
⭐ [오타 자동 교정 절대 금지]: 시안에 오타(예: '토코페일아세티이트')가 있더라도, 절대로 올바른 단어로 자동 교정하지 마십시오. 기괴한 글자라도 있는 그대로 필사해야 합니다.
🔥 [물리적 순서 강제 보존 명령 (가장 중요)]: 텍스트를 추출할 때 반드시 시안 이미지의 맨 위에서부터 아래로(Top to Bottom), 왼쪽에서 오른쪽으로(Left to Right) 시선의 흐름에 따라 순서대로 추출하십시오. 원재료명의 나열 순서가 뒤바뀌면 법적 판정이 어긋나므로, 물리적인 배열 순서를 100% 그대로 유지해야 합니다. 쉼표(,)로 연결된 단어들의 순서를 임의로 섞지 마십시오.

각 이미지에 라벨이 붙어 있습니다. 라벨별로 추출 형식을 준수하여 필사하십시오.
출력 형식:
=== [라벨명] 추출 텍스트 ===
...
1. ⭐ [물리적 위치 기반 환각 삭제 (가장 중요)]: 픽셀상으로 명확히 찾을 수 없는 단어는 즉시 삭제하십시오.
2. ⭐ [오탈자 복구 강제]: Pass 1 텍스트가 '올바른 맞춤법'으로 적혀 있다면 의심하십시오. 원본 이미지에 오타가 있는데 Pass 1이 마음대로 정답으로 교정했다면, 즉시 '원본 이미지의 오타 그대로' 다시 훼손시켜서 추출하십시오.
3. 🔥 [배열 순서 정밀 대조]: Pass 1에서 추출한 원재료 등의 나열 순서가 실제 이미지의 왼쪽에서 오른쪽, 위에서 아래 흐름과 정확히 일치하는지 확인하십시오. 순서가 임의로 섞였거나 줄바꿈이 무시되었다면, 시안과 똑같은 순서로 재배열하여 복구하십시오.
4. ⭐ [숫자/단위/기호 검증]: 소수점, 단위, 괄호를 정확히 대조하십시오.

오직 검증 및 수정이 완료된 최종 텍스트만 화면에 출력하십시오.
"""
        try:
...
각 원료 행마다 다음 5가지를 끈질기게 추적하여 판정 사유에 기재하십시오:
...
4. [혼합제제 전개 검증]: 혼합제제의 경우, 마스터 서류에 있는 수많은 하위 성분이 시안에 단 단 하나도 빠짐없이 전개되었는가?
5. ⭐ [가공국 vs 원물국 범용 검증 (Rule 61)]: 명칭이 '페이스트, 농축액, 분말' 등으로 끝나는 모든 복합원재료에 대해 '원물명-국가명' 형태(예: `검은콩농축액(검은콩:중국산)`)로 정확히 타겟팅되어 적혀있는가?
"""
