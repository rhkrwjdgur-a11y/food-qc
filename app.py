...
def main():
        ...
        if st.button("▶️ 정보표시면 원재료 서류 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("서류 기준 하위성분 전개 및 100% 맵핑 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    ...
                    - **[Rule 47 주의사항 예외 적용]**: 박스에 고객상담실, 반품처, 추가 보관방법 등 행정 문구가 팩보다 더 추가된 것은 지극히 정상이며 합법(✅)입니다.
                    """
                else:
                    prompt = """
                    [지시]: 단품 검토 모드입니다.
    ...
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("오차 계산 및 100% 전체 영양성분 표를 렌더링 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    prompt = """
                    [지시]: '3번 탭: 영양성분표 검토' 모드입니다.
                    
                    <thinking>
                    (정확성을 위해 여기서 각 영양소의 오차율을 미리 계산하십시오. 수식은 자유롭게 쓰되 최대한 간결하게 작성하십시오. 예: 나트륨 146<216✅)
                    </thinking>
                    
                    🚨 [표 작성 필수 명령: 내부 설명 금지 & 5글자 룰] 🚨
                    영양성분이 25개가 넘더라도 **단 하나도 생략하지 말고 표에 모두 100% 기재**하십시오. 
                    단, 표 안에는 계산 과정이나 룰 설명을 단 한 글자도 쓰지 마십시오! 오직 `✅ 적합`, `🚨 0g 수정`, `✅ 100% 일치` 처럼 5글자 내외로만 답을 적으십시오.
                    
                    ## 4️⃣ [영양표시 오차 검증 및 팩/박스 교차 대조]
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    
                    | 영양성분명 | 실측값 vs 시안 오차 검증 | 타겟(박스) vs 비교용(팩) 일치 여부 | 판정 |
                    |---|---|---|---|
                    | 나트륨 | ✅ 적합 | ✅ 100% 일치 | ✅ |
                    | 콜레스테롤 | 🚨 5mg 미만 표기 수정 | ✅ 100% 일치 | 🚨 |
                    | 열량 | ✅ 표기 대상 아님 | ✅ 100% 일치 | ✅ |
                    ...
                    🚨 [표 작성 필수 명령: 내부 설명 금지 & 5글자 룰] 🚨
                    영양성분이 많더라도 **단 하나도 생략하지 말고 표에 모두 100% 기재**하십시오. 
                    단, 표 안에는 계산 과정이나 룰 설명을 단 한 글자도 쓰지 마십시오! 오직 `✅ 적합`, `🚨 0g 수정` 처럼 5글자 내외로만 답을 적으십시오.
                    
                    ## 4️⃣ [영양표시 및 % 기준치 검증]
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    
                    | 영양성분명 | 성적서 실측값 | 시안 표시량 | 법적 허용오차 | 시안 % | 오차 검증 결과 | 판정 |
                    |---|---|---|---|---|---|---|
                    | 나트륨 | 146.6mg | 180mg | 216mg 이하 | 9% | ✅ 적합 | ✅ |
                    | 콜레스테롤 | 4.99mg | 2mg | 2.4mg 이하 | 1% | 🚨 5mg 미만 표기 수정 | 🚨 |
                    | 열량 | 140.5kcal | 145kcal | 174kcal 이하 | - | ✅ 표기 대상 아님 | ✅ |
                    | (모든 영양성분을 생략 없이 기재할 것) | ... | ... | ... | ... | ... | ... |
                    """
        ...
        if st.button("▶️ 최종 종합 리포트 생성", key="btn_summary"):
            if not any([st.session_state["result_tab1"], st.session_state["result_tab2"], st.session_state["result_tab3"], st.session_state["result_tab4"]]):
                ...
                with st.spinner("모든 분석 데이터를 병합하여 최종 수정 지시서를 작성 중입니다..."):
                    ...
                    ### 📌 [핵심 지적 사항 및 수정 지시]
                    (위 분석 데이터에서 '부적합(🚨)' 또는 '확인요망'이 나온 내용들만 뽑아서 1, 2, 3번 불릿 포인트로 요약하십시오.)
