...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V331.00 - 마스터 룰 보존 완전체)")
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                8. 🔥 [서류 미제출 원료 하이브리드 우회 검증 로직 (수석님 특명)]: 
                   ...
                   - 단, 여기서 검토를 포기하지 말고, **오직 '시안에 적힌 텍스트 위치와 문맥'만을 단서로 활용하여** [Rule 1: 원산지 상위 3순위 룰 (시안 나열 순서 기반)], [Rule 50: 고형분 표기 룰] 등 **범용적인 표시기준에 위배됨이 없는지 시안 자체만으로 우회 추적(대체 검증)**하여 결과를 사유란에 적어주십시오.
                """
