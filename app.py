...
🔥 **Rule 52. [단순 명칭 강조 및 '함유/급원' 4조건 완벽 방어 룰]**
   ...
   - 제품에 **"무당", "당 ZERO", "당류 무첨가"** 등의 강조 표시를 하면서 감미료(수크랄로스, 알룰로오스 등)를 사용한 경우, 해당 강조 표시 주변에 반드시 **"감미료 함유"**를 병기해야 합니다. 누락 시 소비자 기만으로 🚨부적합.
...
🔥 **Rule 94. ['MADE IN KOREA' 전면 금지 및 은폐 텍스트 스캔 룰]**
   - **[대원칙]**: 대외무역관리규정 제86조 1항 2호에 의거, 1류~24류(식품)는 부가가치 51% 창출 혜택에서 배제되므로 수입 원료가 1%라도 포함된 가공식품에 'MADE IN KOREA'를 단독 표기하는 것은 명백한 위법(원산지 오인 기만)입니다.
   - **[AI 강제 스캔 명령]**: 패키지 시안(주표시면, 정보표시면, 기타면 전체)의 모든 텍스트를 픽셀 단위로 스캔하여 **'MADE IN KOREA', 'Made in Korea', '한국산'** 문구가 숨어있는지 이 잡듯이 뒤지십시오.
   - **[은폐 구역 주의]**: 특히 'K-MILK 로고 바로 아래', '바코드 주변', '주의사항 하단 구석' 등에 아주 작은 영문 글씨로 숨어있는 경우가 많으니 절대 놓치지 마십시오.
   - **[판정]**: 발견 즉시 🚨부적합 처리하고, 사유에 "⚠️ [수입 원료 사용 제품의 'MADE IN KOREA' 표기 불가]: 대외무역관리규정에 따라 식품(1~24류)은 부가가치 51% 기준 적용을 받지 못하므로 수입 원료 포함 시 'MADE IN KOREA' 표기가 불가합니다. 해당 문구를 즉시 삭제하십시오."라고 지시하십시오.
"""
...
def main():
    ...
    # ⭐ 상단 고정 헤더 영역
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V335.00 - 미아 룰 패치 완료)")
    ...
    with tab1:
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("【정밀 법리 검수 매트릭스 연산 중...】"):
                ...
                # 🔥 [핵심 패치]: 미아 룰(Rule 46, 63, 57) 표 양식에 반영 완료
...
| ⭐ **190mL 전용 질소충전 확인** | [Rule 63] | (※ 내용량이 190mL일 경우 필수 확인. 아닐 경우 '해당 없음') | |
...
| ⭐ **'MADE IN KOREA' 숨김 텍스트 스캔** | [Rule 94] | (※ K-MILK 마크 하단 등 구석구석 스캔하여 발견 시 즉시 삭제 지시) | |
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                4. ⭐ [마스터 DB 출처 철통 격리]: 2-1 서류 기반 마스터표를 작성할 때는 오직 사용자가 업로드한 'PDF/엑셀 서류(한글라벨)'의 텍스트만을 100% 신뢰하여 작성하십시오. 제공되지 않은 서류를 시안에서 유추하여 마스터표를 창조(환각)해 내는 행위를 절대 금지합니다.
    ...
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【영양성분표 수치 환산 및 교차 검증 중...】"):
                ...
                # 🔥 [핵심 패치]: 미아 룰(Rule 83, 79) 표 양식에 반영 완료
    ...
    with tab4:
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("【의무표시 및 인증마크 해독 중...】"):
                ...
                # 🔥 [핵심 패치]: 미아 룰(Rule 63) 표 양식에 반영 완료
...
| ⭐ **'MADE IN KOREA' 숨김 텍스트 스캔** | [Rule 94] | (※ 주의문구, 바코드 주변 등 은폐 구역 스캔 후 발견 시 즉시 삭제 지시) | |
...
| ⭐ **190mL 전용 질소충전 확인** | [Rule 63] | (※ 내용량이 190mL일 경우 필수 확인. 주의사항 란 스캔) | |
