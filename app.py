...
어떤 첨가물이나 원료의 표기 누락(또는 용도 누락)을 지적하기 전에, **반드시 그 원료가 배합비 5% 미만인 복합원재료의 하위 성분인지 가장 먼저 확인하십시오.** 5% 미만 복합원재료의 하위 성분이라면 [표 4, 5, 6] 첨가물 규정 등 모든 규정을 무시하고 **무조건 "전개/표시 의무 면제(✅)"로 판정**하십시오. (단, 알레르기 물질은 예외이며 혼합제제는 이 면제 룰에서 절대 제외됩니다.)
...
🔥 **Rule 5. [복합원재료 5% 미만 전개 면제 및 🌟혼합제제 절대 예외 룰]**
   ...
   - 🌟 **[첨가물 과잉 단속 금지 원칙]**: 위 조건에 따라, 5% 미만 '복합원재료' 내부에 [표 4, 5, 6] 소속 식품첨가물이 들어있더라도 명칭/용도 표시 의무가 완전히 면제됩니다.
   - 🚨 **[혼합제제 절대 면제 불가]**: 단, 서류상 식품유형이 **'혼합제제'**인 원료는 이 5% 미만 면제 룰이 **절대로 적용되지 않습니다.** 혼합제제는 배합비율과 무관하게 반드시 1) `혼합제제(A, B, C...)`처럼 괄호를 열고 내부 성분을 모두 명시하거나, 2) '혼합제제'라는 묶음 명칭 껍데기를 버리고 내부 하위 성분들을 흩뿌려 개별 전개해야만 합법입니다. 혼합제제를 5% 미만이라고 해서 하위 성분 전개 없이 단일 명칭으로만 적으면 무조건 위법(🚨)입니다.
...
🔥 **Rule 44. [🌟 식품첨가물(혼합제제) 중복 시 '명칭 유지 병합' 전면 금지 및 해체 강제 룰]**
   - **혼합제제는 식품첨가물이므로 껍데기 명칭 유지가 절대 불가합니다.** 제품에 A원료 안에도 '혼합제제'가 있고, B원료 안에도 '혼합제제'가 있을 때, 이를 통합 표기하겠다며 **"OO혼합제제"라는 복합 묶음 명칭(껍데기) 자체를 그대로 살려서 한 번만 기재하는 것은 명백한 위법(🚨부적합)입니다.**
   - ⭐ **[최하위 단일 성분 전개 필수]**: 여러 경로로 중복 투입된 혼합제제를 합치려면, 반드시 그 껍데기를 부수고 하위의 '최하위 단일 성분(개별 첨가물)' 단위로 낱낱이 해체(전개)하여 흩뿌려 적어야만 합법(✅)입니다. 시안에 2곳 이상에서 유래한 혼합제제가 하위 성분 전개 없이 'OO혼합제제', 'OO믹스' 등의 묶음 명칭으로 괄호 없이 단독 기재되어 있다면 무조건 기계적으로 적발(🚨)하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.90 - 혼합제제 해체 강제 및 Rank 듀얼 시스템 패치)")
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                # ⭐ V311.90: 마스터표 생략 방지, Rule 44 특별 스나이퍼 장착 및 듀얼 Rank 방어 적용
                tab2_special_rules = RULES_TAB2 + """
                \n\n🔥 [Tab 2 특별 지시사항 (반드시 지킬 것 - 화면 출력 금지)] 🔥
                1. [마스터표 강제 완성 및 절대 생략 금지]: 서류의 데이터가 아무리 길어도 마스터표 작성 시 절대 중간에 끊거나 `(...)` 기호를 사용하여 요약하지 마십시오. 원본 데이터의 1열부터 마지막 열까지 100% 전수 조사하여 끝까지 표를 완성하십시오. (Token Limit을 모두 써도 좋으니 중도 포기 절대 금지). 시안과 서류에 적힌 괄호 안의 하위 성분 역시 한 글자도 빠짐없이 모두 타이핑하십시오.
                2. [Rule 44 '명칭 유지 병합' 범용 스나이퍼 족쇄]: 제품에 중복으로 투입된 혼합제제를 병합 표기할 때, 껍데기 명칭(예: 'OO혼합제제', 'OO믹스', 'OO베이스' 등)을 그대로 적어 퉁치는 것은 절대 위법(🚨부적합)입니다. 여러 원료에 흩어진 동일 혼합제제를 합칠 때는 반드시 최하위 단일 성분 단위로 전개/해체해야 합법입니다. 시안에 중복 투입된 혼합제제가 하위 성분 전개 없이 단일 묶음 명칭으로만 기재되어 있다면 무조건 "최하위 단일 성분으로 해체하지 않고 복합 명칭을 유지하여 병합했으므로 위법"이라고 🚨부적합 판정하십시오.
                ...
                if "박스" in ins_mode:
                    ...
                    judgment_prompt += "| 타겟(박스) 표기 개별 원재료명 | 비교용(팩) 표기 개별 원재료명 | 대조 검 검증 결과 (픽셀 100% 일치 여부 상세 서술) | 최종 판정 |\n|---|---|---|---|\n\n"
