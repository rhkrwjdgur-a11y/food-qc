...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V136.0 - 서류 누락 시 유연화 패치)")
...
"""
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【3-Pass】 분석 진행 중..."):
                # 🔥 V136.0 패치: 서류 누락 시 유연화 조항(확인 요망 처리) 추가
                base_tab2_warning = """
...
🚨 [혼합제제 하위 성분 100% 추적 및 맵핑 강제 명령] 🚨
환자식(특수의료용도식품) 등에서 '비타민프리믹스', '영양강화제' 같은 혼합제제가 사용된 경우, 기계적인 잣대로 "영양강화제와 개별 비타민이 섞여 있어 모순이다"라고 섣불리 지적하지 마십시오.
당신의 진짜 임무는 마스터 서류(왼쪽 표)에 기재된 혼합제제의 '모든 하위 성분(비타민, 미네랄 등 수십 개)'이 시안(오른쪽 표)의 원재료명 리스트 어딘가에 빠짐없이 전개되어 적혀 있는지 끝까지 추적하여 1:1로 맵핑(대조)하는 것입니다. 만약 서류에는 있는데 시안에서 단 하나라도 누락된 하위 성분이 있다면 그것을 정확히 찾아내어 🚨부적합 사유로 지적하십시오.
