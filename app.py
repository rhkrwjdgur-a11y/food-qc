...
# ==========================================
# 📚 3. 89대 마스터 룰북 원문 (V311.43 완결본 - 국내가공품 원산지 이중표기 룰 패치)
# ==========================================
RULE_BOOK_FULL = """
...
🔥 **Rule 5. [복합원재료 5가지 컷오프 및 첨가물 생략 합법성 룰 (유권해석 기반)]**
   ...
   - 🌟 **[조건 C: 5순위 밖 첨가물 생략 면제]**: 식약처 유권해석(2025.07)에 의거, 복합원재료 내 하위 순위(6순위 이하)에 해당하는 식품첨가물(착색료, 감미료 등)은 완제품에 기능적 영향을 발휘하더라도 표기를 완전히 생략하는 것이 합법(✅)입니다.
...
🔥 **Rule 89. [국내 제조 가공품 원료의 원산지 이중 표기 스나이퍼 (농관원 유권해석)]**
   ...
   - 반드시 `원료명(원료명: 국가명)` 형태로 괄호 안에 원료명을 한 번 더 명시한 뒤 원산지를 적어야 합법(✅)입니다.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.43 - 가공품 원산지 이중표기 픽스판)")
...
⭐ **[원산지 98% 컷오프 수학적 검증 족쇄 (Rule 1 절대 준수)]**: 
...
⭐ **[가공품 원산지 이중 표기 족쇄 (Rule 89 절대 준수)]**: 옥배유, 사과농축액 등 수입 원물로 국내에서 제조된 가공품을 원료로 쓴 경우 반드시 `원료명(원료명: 국가명)` 형태인지 확인하십시오. 시안에 단순히 `원료명(국가명)` 형태로 기재했다면 무조건 🚨부적합 처리하십시오.
