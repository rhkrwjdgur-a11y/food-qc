...
# ==========================================
# 2. 통합 전문가 프롬프트 (Rule 1~34 완결판 - 환자식 1:1 풀전개 매칭 완벽 반영)
# ==========================================
SYSTEM_PROMPT = """
...
---
# [식품 패키지 표시사항 QC 자동화 검수 시스템 룰북 4.9]
...
🔥 **Rule 32. [균형 열량 구성비 역산 (환각 지적 방지 핵심)]**
   - 환자식/특수용도식품의 탄:단:지 비율(예: 48:22:30)은 제품의 목표 설계치이므로 성적서 실측값과 소수점 오차가 나도 절대 지적 금지.

🔥 **Rule 33. [데이터 출처 완벽 분리 및 환자식 낱개 성분 1:1 매칭 특별 룰]**
   - **[대조 로직 - 환자식 (매우 중요)]**: 환자식의 경우 시안에 '비타민혼합제제' 같은 복합원재료 명칭을 생략하고, 그 안의 **'개별 낱개 하위성분(L-아스코브산나트륨, 덱스트린 등)'들을 모두 직접 풀어서 나열**합니다. 
   - 따라서 AI는 **시안에 적혀 있는 수십 개의 낱개 성분들을 하나하나 순서대로 나열**한 뒤, **서류의 원재료나 하위성분 중 어디에 매칭되는지 1:1로 짝을 지어야 합니다.**
   - 시안에 적힌 낱개 성분 중 서류에 없는 것이 있는지(오타 포함), 반대로 서류에 있는 낱개 성분인데 시안에 누락된 것이 있는지 하나하나 팩트 체크하여 O/X로 표기하십시오. 절대 임의로 요약(ex: 비타민 믹스 등)하지 마십시오.
...
- 🔍 **환자식 1:1 낱개 성분 팩트 체크 표** - 양식: | 시안에 기재된 낱개 성분 (100% 필사) | 서류의 매칭 원료/하위성분 | 일치 여부 (오타/누락 체크) |
  - (예시): | L-아스코브산나트륨 | 비타민프리믹스의 하위성분 | ✅ 일치 |
## 4️⃣ [영양표시 검토]
## 5️⃣ [기타 법적 의무사항]
...
def main():
    ...
    if st.button("🔍 서류 추출 및 QC 정밀 진단 시작", type="primary"):
        ...
        with st.spinner("수십 장의 서류를 스캔하여 엑셀 표로 추출하고 시안과 대조 중입니다..."):
            ...
            # 🔥 프롬프트 강제력 극대화: 1:1 낱개 성분 매칭 지시
            final_prompt = f"""
            [현재 검토 대상 제품 유형]: {product_type}
            
            [🚨치명적 오류 방지 1🚨]: 마크다운 표 안에 절대 줄바꿈(엔터)을 넣지 마십시오.
            [🚨치명적 오류 방지 2 (가장 중요)🚨]: 대조 표를 작성할 때, 시안 이미지에 적힌 모든 '낱개 성분(L-아스코브산나트륨, 덱스트린 등)'들을 절대로 뭉뚱그려 요약하지 말고 하나씩 한 줄 한 줄 나열하십시오. 그리고 그 각각의 성분이 서류의 어떤 원료/하위성분에 매칭되는지 1:1로 짝을 지어 표를 그리십시오. 서류에 있는 하위성분 중 시안에 누락된 것이 있다면 그것도 표에 적고 ❌표시하십시오.
            [🚨치명적 오류 방지 3🚨]: 시안과 서류 대조 시 '오탈자(Typo) 정밀 스캔'을 수행하십시오.
            [🚨치명적 오류 방지 4🚨]: 영양성분 허용오차 기준을 엄수하여 실측값이 표시량보다 많거나(비타민), 적게 나와도(당류) 지적하지 마십시오.
            """
