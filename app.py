...
🔥 **Rule 52. [영양성분 단순 명칭 강조 및 '함유' 컷오프 완벽 검증 룰]**
   - 패키지 전 구역(측면, 주표시면 등 위치 불문)에 '단백질 4g', '칼슘 함유' 등 영양소 명칭을 단독으로 기재하거나 아이콘/뱃지로 강조한 경우, 반드시 영양소별로 아래 **[함유/급원 4대 조건]** 중 **단 하나라도 충족**해야 합법(✅)입니다. (기계는 절대 1차원적으로 하나만 보고 부적합을 내리지 말고, 100kcal 환산 등 모든 조건을 대입해 볼 것!)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V208.0 - 표 바깥 텍스트 스나이퍼)")
...
4. ⭐ [데이터 증발 절대 금지]: 당신은 요약 봇이 아닙니다! Pass 1이 추출한 표(Table)나 긴 텍스트를 "생략", "동일함", "중략" 등의 말로 절대 줄이지 마십시오. 반드시 100% 전체 표와 내용을 그대로 복사해서 출력해야 합니다. 여기서 데이터가 날아가면 시스템이 붕괴됩니다.
...
## 2️⃣ [알레르기, 주의사항 교차 검증 (CS 클레임 방어 포함)]
...
⭐ [절대 원칙: 엑셀 VLOOKUP 하드 매핑]: [미션 A-1]과 [미션 A-2]에 추출된 시안의 영양성분 수치와 [미션 B]의 성적서 수치를 표에 매핑할 때, 귀찮아하거나 임의로 생략하지 마십시오! 반드시 추출된 텍스트의 모든 수치(나트륨, 탄수화물 등 전체)를 100% 그대로 기계적으로 복사해서 표의 각 빈칸을 채워 넣어야 합니다. 눈앞에 데이터가 있는데도 '데이터 없음'이라고 우기는 환각(Hallucination)과 근무 태만을 엄격히 금지합니다.
"""
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
...
- ⭐ [Rule 68] 다포장 낱팩 복붙 스나이퍼: (마찬가지로 표 바깥에 추출된 '총 내용량' 및 '1개당 칼로리' 문구를 샅샅이 스캔하여 낱팩 복붙 에러인지 3단계 강제 스캔할 것.)
- ⭐ [Rule 69] 비타민 아래첨자 스나이퍼:
