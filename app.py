...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V156.5 - 시안 배열고정 절대 무결점판)")
...
- ⭐ [배열(List) 강제 추출]: 시안의 원재료명 텍스트를 줄글로 뭉뚱그리지 말고, 쉼표(,)를 기준으로 완벽하게 쪼개서 [1. 원액두유, 2. 정제수, 3. 분리대두단백 ... 11. dl-α-토코페일아세티이트] 처럼 번호가 매겨진 세로 리스트 형태로 추출하십시오.
...
검증 규칙:
1. ⭐ [미션 A (시안) 검증]: Pass 1 텍스트가 번호가 매겨진 리스트 형태인지 확인하십시오. 시안 이미지에 오타가 있는데 AI가 마음대로 교정했다면 원본 이미지의 오타 그대로 훼손시켜 복구하십시오. 배열 순서가 시안의 실제 흐름과 일치하는지 강제 정렬하십시오.
