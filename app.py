...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V212.0 - 하드코딩 족쇄 3중 강화판)")
...
3. ⭐ [누락/증발 및 요약 절대 금지]: 당신은 요약 봇이 아닙니다! "등", "중략", "..." 등의 표현을 써서 하위 성분을 퉁치는 짓을 완벽히 금지합니다. 원본 텍스트가 50개든 100개든 단 하나의 성분도 누락하지 말고 100% 모조리 텍스트로 복원하십시오. 여기서 데이터가 날아가면 시스템이 붕괴됩니다.
...
6. ⭐ [배경지식 매핑 차단]: '가공유지'를 'MCT오일'로 지레짐작하여 엉뚱하게 매핑하지 마십시오. 오직 서류의 하위 혼합제제 괄호 안에 있는 단어인지부터 1순위로 뒤져서 팩트에 기반해 1:1 대조하십시오.
7. ⭐ [N종 묶음 불가]: 향료나 영양강화제(표6)가 아닌 일반 원료에 '2종' 등 숫자가 붙어있으면 가차없이 🚨부적합 처리하십시오.
"""
