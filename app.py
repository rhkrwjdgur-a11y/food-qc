...
🔥 **Rule 35. [서류 명칭 일치 및 식품공전 표준명칭 치환 허용 룰]**
   ...
   - **[🚨식품공전 표준명칭 치환 절대 허용]**: 서류상의 상업용 원료명이 식약처 식품공전상의 공식 식품유형(표준명칭)으로 치환된 경우 완벽한 합법(✅ 적합).
...
RULES_TAB1 = "[탭1 주표시면 관련 핵심 룰]\n" + get_sliced_rules([3, 9, 10, 15, 16, 17, 18, 19, 21, 24, 28, 40, 46, 47, 50, 51, 52, 53, 57] + COMMON_RULES)
RULES_TAB2 = "[탭2 정보표시면/원재료명 관련 핵심 룰]\n" + get_sliced_rules([1, 2, 5, 6, 8, 12, 13, 14, 25, 29, 30, 34, 35, 38, 39, 44, 48, 49, 52, 54, 57] + COMMON_RULES)
RULES_TAB3 = "[탭3 영양성분표 관련 핵심 룰]\n" + get_sliced_rules([3, 4, 6, 10, 11, 21, 23, 25, 26, 27, 31, 32, 33, 40, 41, 55] + COMMON_RULES)
RULES_TAB4 = "[탭4 기타면/측면 관련 핵심 룰]\n" + get_sliced_rules([7, 15, 17, 18, 20, 22, 24, 38, 56, 57] + COMMON_RULES)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V137.0 - 환자식 원재료 1:1 강제 분해 패치)")
...
"""
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【3-Pass】 분석 진행 중..."):
                # 🔥 V137.0 패치: 영양성분 분석 차단 및 원재료 1:1 강제 분해 맵핑
                base_tab2_warning = """
🚨 [영양성분표 분석 절대 금지 명령] 🚨
...
🚨 [원재료명 1:1 극강제 분해 및 맵핑 명령 (LLM 요약/생략/묶음 절대 금지)] 🚨
환자식 등 원료가 수십 개라도 절대 게으름을 피우지 마십시오.
시안의 '원재료명' 란에 적힌 모든 텍스트를 쉼표(,) 기준으로 완벽하게 분해하여 단 한 개의 원료도 빠짐없이 표의 개별 행(Row)으로 만드십시오.
절대로 "(전체 원료 리스트)", "(기타 비타민류)" 등으로 묶어서 퉁치거나 요약/생략하지 마십시오. 원료가 50개면 결과 표의 행(Row)도 반드시 50개여야 합니다.

각 원료 행마다 다음 3가지를 끈질기게 추적하여 판정 사유에 기재하십시오:
1. [순서 검증]: 해당 원료가 규격서 배합비 투입량에 맞게 내림차순으로 나열되었는가? (단, 2% 미만 원료는 Rule 34에 따라 순서 유연성 인정)
2. [원산지 검증]: 해당 원료가 농수산물인 경우, 원산지가 원재료명 '바로 뒤 괄호' 안에 합법적으로 표기되었는가? (예: 대두(외국산))
        ...
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【3-Pass】 분석 진행 중..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    judgment_prompt = """
🚨 [표 작성 필수 명령 절대 강제] 🚨
1. 🚨 [생략 절대 금지]: 표 안에 '...', '중략' 등으로 축약 금지. 모든 영양성분을 100% 전부 타이핑하십시오.
        ...
        if st.session_state["result_summary"]:
            ...
            st.markdown("""
                ...
                <div class='hide-on-print' style='text-align: right; margin-top: 20px; margin-bottom: 20px;'>
                    <button onclick='window.print()' style='background-color: #FF4B4B; color: white; border: none; padding: 12px 24px; border-radius: 6px; font-weight: bold; font-size: 16px; cursor: pointer; box-shadow: 0 4px 6px rgba(0,0,0,0.1);'>
                        🖨️ 종합 보고서 전용 인쇄
                    </button>
