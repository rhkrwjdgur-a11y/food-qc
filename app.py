...
🔥 **Rule 52. [논리적 모순 탐지 및 영양학적 팩트 기반 카운트 강제 룰 (🚨보수적 표기 허용)]**
   - 제품명이나 마케팅 문구에 특정 숫자(예: '비타민 15종')가 명시되어 있다면 대조 카운트하십시오.
   - **[🚨카운트 헛소리 방지 1 (비타민)]:** 이름에 비타민이 없어도 나이아신, 엽산, 판토텐산, 바이오틴을 무조건 합산하십시오.
   - **[🚨카운트 헛소리 방지 2 (미네랄)]:** 나트륨(Sodium)은 카운트에서 무조건 제외(합산 금지)하십시오!
   - **[🚨보수적 표기 및 그룹핑 인정 절대 명령]**: 마케팅 문구에 강조한 숫자가 실제 영양성분표나 원재료에 있는 숫자보다 **작거나 같은 경우**는 보수적 표기이므로 **무조건 적합(✅)** 처리하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V114.0 - 사유 설명 및 환각 방지 스나이퍼)")
            ...
            with st.spinner("서류 유형 분석 및 창의성 배제 100% 교차 검증 중..."):
                ...
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        4. 🚨 [사유 설명 강제]: 판정 열에 단순히 '불일치'라고 적지 말고, Rulebook에 의거하여 왜 틀렸는지 법적 사유나 수정 방향을 상세히 명시하십시오.
                    ...
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        ## 🔍 [전 구간 공통: 수량 모순 및 오탈자 검증]
                        - 🚨 포장 단위(수량) 2-Track 검증 (단품/세트 열량 반올림 오차 허용):
                        - 🚨 띄어쓰기 및 오탈자 적발: 한쪽 시안에만 존재하는 문구를 다른 쪽에도 있다고 상상하여 띄어쓰기를 비교하는 환각 금지.
                        """
