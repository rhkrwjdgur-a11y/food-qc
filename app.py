...
🔥 **Rule 44. [🌟 식품첨가물(혼합제제) 중복 시 '명칭 유지 병합' 전면 금지 및 1:N 해체 전개 합법성 룰]**
   - **혼합제제는 일반 복합원재료가 아니라 '식품첨가물'이므로 껍데기 명칭 유지가 절대 불가합니다.**
   - 1. 혼합제제가 단일 출처로 한 번만 쓰였다면 `혼합제제명(하위성분1, 하위성분2)` 형태로 껍데기를 유지하여 표기하는 것이 합법(✅)입니다.
   - 2. **[중복 투입 방어]**: 하지만 동일한 혼합제제가 여러 복합원재료를 통해 **중복으로 투입**되었거나, 추가로 투입된 경우, 이를 `OO혼합제제`라는 껍데기 명칭 하나로 통합하여 기재하는 것은 명백한 위법(🚨부적합)입니다.
   - 3. ⭐ **[1:N 전개 합법성 인정 (필수)]**: 위와 같이 중복 투입된 경우, 디자이너가 혼합제제의 껍데기를 부수고 그 하위 단일 성분들(비타민C, 덱스트린 등)을 원재료명 란에 낱낱이 흩뿌려 적은 것은 **가장 완벽한 합법(✅적합)**입니다. 서류상에는 하나로 묶여있더라도 시안에 여러 개의 개별 성분으로 분리 전개되어 있다면 "일부만 적었다"고 멍청하게 지적하지 마십시오. 무조건 100% 합법으로 통과시키십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V317.00 - 영양성분 %DV 검증 전용 패치)")
    ...
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【영양성분표 수치 환산 및 교차 검증 중...】"):
                ...
                5. ⭐ [% 기준치 역산 검증 강제]: 1일 영양성분 기준치가 있는 성분의 경우, 판정 사유란에 반드시 '(시안 표시량(B) / 1일 기준치) × 100 = O%' 수식을 명확히 기재하여 시안에 표시된 % 수치와 정확히 일치하는지 검증하십시오. (안전율 계산이나 괴리율 경고는 하지 마십시오.)
                """
