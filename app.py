...
🔥 **Rule 50. ['원액/100%' 명칭 기만광고 판별 룰 (단일 원료 vs 최종 제품 혼합 구분)]**
   - **[✅합법 조건]**: 투입되는 특정 원료 자체가 순수한 원액이라면, 최종 제품 공정에서 다른 첨가물과 섞이더라도 그 원료를 'OO원액'이라고 부르는 것은 완벽한 합법입니다.
   - **[🚨기만광고 적발 조건]**: 단, 납품받은 원료 그 자체에 이미 정제수나 덱스트린 등이 섞여 있음에도 묶어서 '100% 원액'이라고 부르는 경우에만 기만광고로 적발하십시오.
...
def main():
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        최종 7단계 리포트를 작성하기 전에, 반드시 `<thinking>` 태그를 열고 업로드된 시안과 서류를 51대 룰에 맞춰 어떻게 교차 검증했는지 당신의 '의사결정 논리(사고 과정)'를 먼저 꼼꼼하게 서술하십시오.
        (예시: "내포장에 190ml 기준 영양정보가 있네. 외포장은 1140ml니까 x6을 해야겠군...", "성적서에 식이섬유가 1g 잡혔지만 시안에는 아예 식이섬유 칸이 없네. 9대 필수영양소가 아니니 의도적 생략이므로 표에서 빼야겠다!" 와 같이 생각할 것)
        
        <thinking>
        (이곳에 51대 룰과 10대 긴급 차단 명령을 적용하여 시안과 서류를 대조하는 당신의 모든 계산식과 판단 논리를 먼저 출력할 것)
        </thinking>
        ...
        - 🚨 [긴급 차단 명령7 (선택 영양소 전략적 생략 인정/환각 금지)]: **시험성적서에는 '식이섬유'나 '기타 비타민' 등의 수치가 있더라도, 시안(패키지)에 해당 영양성분이 표기되어 있지 않다면 이는 '의도적 생략'이므로 영양성분 표에 임의로 추가하여 지적하지 마십시오. 오직 시안 이미지에 인쇄된 영양성분 항목들만 평가하십시오.**
    ...
    if st.button("🔍 전수 룰 QC 시작", type="primary"):
        ...
        with st.spinner(f"51대 룰북 원문 및 CoT 사고과정 실행 중... [{inspection_mode}]"):
