...
🔥 **Rule 35. [🌟 범용 간략명/동의어 허용 및 첨가물 임의 수식어 금지 룰 (QA 고도화 범용 패치)]**
   ...
   - **[🌟 수치 그대로 표시 허용 룰]**: 영양성분 표시 시, 소수점이 포함된 수치(예: 5.6g, 1.06g)는 식약처 규정의 "그 값을 그대로 표시하거나"에 해당하므로 억지로 반올림을 지적하지 말고 무조건 완벽한 합법(✅)으로 통과시키십시오.
...
# 💡 [V159.0 업데이트] 탭 2, 3에서는 CS/안전망 룰(59번 등)을 제거하고 본연의 임무에 집중합니다.
RULES_TAB2 = "[탭2 정보표시면/원재료명 관련 핵심 룰]\n" + get_sliced_rules([1, 2, 5, 6, 8, 12, 13, 14, 25, 28, 30, 34, 35, 38, 39, 44, 48, 52, 61, 65] + COMMON_RULES)
RULES_TAB3 = "[탭3 영양성분표 관련 핵심 룰]\n" + get_sliced_rules([3, 4, 6, 10, 11, 21, 23, 25, 26, 27, 31, 32, 33, 35, 40, 41] + COMMON_RULES)
# 💡 [V159.0 업데이트] 탭 4는 글로벌 스캐너로서 59, 38 등 공통/안전망 룰을 전담합니다.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V159.0 - 글로벌 통합 스캐너 및 % 연산 탑재)")
...
"""
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    judgment_prompt = """
⭐ [강제 연산 지시]: 기계적인 표 출력에 그치지 말고, '% 연산 검증' 컬럼에는 반드시 '시안 표시량 ÷ 1일기준치 × 100'을 직접 계산하여 그 결과값을 적어 넣으십시오!

## 4️⃣ [영양표시 오차 검증 및 팩/박스 교차 대조]
| 영양성분 | 성적서 실측값 | 비교용(팩) 시안 | 타겟(박스) 시안 | 팩 vs 박스 일치 | 🎯 % 연산 검증 (표시량÷1일기준치×100) | 최종 판정 (상세 사유 필수) |
|---|---|---|---|---|---|---|
...
## 4️⃣ [영양표시 및 % 기준치 검증]
| 영양성분 | 성적서 실측값 | 시안 표시량 | 오차 검증(실측vs표시) | 시안 표시 % | 🎯 % 연산 검증 (표시량÷1일기준치×100) | 판정 (상세 사유 필수) |
|---|---|---|---|---|---|---|
| (모든 영양성분을 축약 없이 100% 기재) | [내용 작성] | [내용 작성] | [내용 작성] | [내용 작성] | [내용 작성] | [내용 작성] |
"""
