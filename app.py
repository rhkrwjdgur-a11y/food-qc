...
🔥 **Rule 79. [열량 구성비(%) 4:4:9 수학적 역산 강제 검증]**
   - 시안에 탄수화물:단백질:지방 열량비율(예: 45:26:29)이 기재된 경우, 표기된 영양정보의 실제 g수를 끌어와 각각 4kcal, 4kcal, 9kcal를 곱하여 총 열량(환산값 합계) 대비 비율(%)을 직접 계산하여 시안의 숫자와 일치하는지 수학적으로 증명하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V310.8 - 궁극의 마스터 픽셀스나이퍼본)")
...
| **열량 구성비(4:4:9) 검증** | [Rule 79] | (시안에 비율 기재 시 반드시 4:4:9 역산 수식 증명 필수) | |
...
| 시안 표기 원재료명 (100% 나열) | 매칭된 서류 원료명 (없으면 '제출 안 됨') | 대조 검 검증 결과 (원산지 꼬리표 필수) | 최종 판정 |
|---|---|---|---|
...
⭐ [강제 지시 2: 원산지 꼬리표 부착]: 대조 검 검증 결과란 첫머리에는 반드시 굵은 글씨로 **[원산지 1순위]**, **[원산지 2순위]**, **[원산지 3순위]** 또는 **[원산지 산정 제외(Rule 28: 물/당류/첨가물 등)]** 꼬리표를 달아주십시오.
