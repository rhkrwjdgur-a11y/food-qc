...
# ==========================================
# 🛡️ [핵심 패치] AI 표 깨짐 강제 복구 함수
# ==========================================
def fix_markdown_table(text):
    """
    AI가 한 줄로 뱉어버린 표를 파이썬 정규식으로 강제 분리하여 마크다운 표를 복구합니다.
    """
    text = re.sub(r'([^\n])\s*(\|\s*No\s*\|)', r'\1\n\n\2', text)
    text = re.sub(r'([^\n])\s*(\|\s*영양성분명\s*\|)', r'\1\n\n\2', text)
    text = re.sub(r'\|\s+\|', '|\n|', text)
...
🔥 **Rule 13. [알레르기 정밀 추적 및 위치 오판 방지]**
   ...
   - **[🚨알레르기 위치 표기 절대 규칙]: 식약처 규정상 알레르기 정보는 바탕색과 구분되는 '별도 란(박스)'에 한 번만 기재되어 있으면 100% 합법입니다. '영양성분표 하단'이나 다른 곳에 알레르기 문구가 누락되었다고 지적하거나 중복 표기를 요구하는 환각(Hallucination)을 엄격히 금지합니다.**
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V44.0 - 54대 룰북 영구 보존판)")
    ...
    with tab2:
        ...
        if st.button("▶️ 정보표시면 원재료 100% 대조 시작", key="btn_info"):
            with st.spinner("서류의 품번 및 상세 내역을 표에 다이렉트로 옮겨 적는 중..."):
                prompt = """
                [지시]: 오직 아래의 원재료명 표와 알레르기 교차검증 양식만 출력하십시오. 절대 `<thinking>` 태그를 쓰지 말고 바로 표를 그리십시오.
                🚨 [경고]: Rule 53, 54 등에 대한 검토 내용은 **절대 표 안에 넣지 말고**, 반드시 표 바깥 아래쪽의 별도 항목으로 빼서 작성하십시오.
                ...
                - [Rule 53, 54 검토 사항]: (제품명 연동 원료, 복수 원산지 비율 등 표에 넣기 어려운 규정 위반 내용은 여기에 별도로 서술하십시오.)
