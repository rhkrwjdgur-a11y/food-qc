...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V174.0 - 궁극의 완성판)")
...
"""
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【3-Pass】 분석 진행 중..."):
                extract_mission = """
🎯 [미션 A: 정보표시면 배열 강제 추출 (내포장/외포장 분리 추출!!)]
- 만약 검토 모드가 '선물세트 박스(외포장) 교차 검토'라면, 반드시 1) '타겟(박스) 시안'의 원재료명 리스트와 2) '비교용(팩) 시안'의 원재료명 리스트를 [1. 원액두유, 2. 정제수...] 형태로 '각각 따로' 완벽히 쪼개서 추출하십시오. 둘 중 하나라도 누락하면 안 됩니다.
- 단품 검토 모드라면 해당 시안의 원재료명만 추출하십시오.
...
2. ⭐ [표 구조: 1열(Column 1) 절대 Lock]: 2번 대조 표를 그릴 때, 가장 맨 왼쪽 열(Column 1)은 반드시 **"[미션 A]에서 추출한 타겟 시안 원재료명 리스트"**를 1번부터 끝번까지 토시 하나 바꾸지 말고 순서대로 복붙해야 합니다. 서류(마스터표)에만 있는 묶음 단어(예: 영양강화제, 비타민D3혼합제제 등 시안에 없는 글자)를 1열에 창조해 내는 순간 이 시스템은 즉시 파괴됩니다. 시안에 적힌 낱개 원료들을 그대로 1열에 고정시키십시오!
...
## 🔍 [전 구간 공통: 수량 모순 및 오탈자 검증]
- ⭐ [Rule 70] 내포장(팩) vs 외포장(박스) 원재료명 100% 일치 스나이퍼: (동의어라도 팩과 박스 표기가 다르면 🚨부적합 처리)
- ⭐ 포장 단위(수량) 2-Track 검증:
- ⭐ [Rule 69] 비타민 아래첨자 스나이퍼:
...
- ⭐ [Rule 68] 다포장 낱팩 복붙 스나이퍼: (박스 시안일 경우 총내용량, 칼로 문구, 표 안쪽 열 제목이 '낱팩 복붙'인지 3단계 강제 스캔)
- ⭐ [Rule 69] 비타민 아래첨자 스나이퍼:
