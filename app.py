...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V47.0 - 중도 멈춤 완벽 치료판)")
    ...
    with tab1:
        ...
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("주표시면 텍스트 및 뱃지 정밀 대조 중..."):
                ...
                리뷰를 출력하기 전에 반드시 `<thinking>` 태그를 열어 분석하십시오. 분석이 끝나면 반드시 `</thinking>` 태그를 닫고 2단계로 넘어가십시오. 절대 중간에 멈추지 마십시오.
    ...
    with tab2:
        ...
        if st.button("▶️ 정보표시면 원재료 100% 대조 시작", key="btn_info"):
            with st.spinner("배합비 순위 정렬 및 표 렌더링 중..."):
                ...
                🚨 [1단계: 배합비 순위 정렬 전용 Thinking 강제] 🚨
                당신은 소수점이 포함된 배합비(%)를 한 번에 정렬하는 능력이 취약합니다. 표를 그리기 전에 반드시 `<thinking>` 태그를 열고, 배합비 서류에 적힌 모든 원료의 배합비(%) 숫자를 추출한 뒤, 가장 높은 숫자부터 내림차순으로 1위부터 끝까지 정렬하십시오. **정렬이 끝나면 반드시 `</thinking>` 태그를 닫고 2단계 표를 그리십시오.**
