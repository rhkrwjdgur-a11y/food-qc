...
🔥 **Rule 52. [논리적 모순 탐지 및 마케팅/원재료 N종 카운트 스나이퍼]**
   ...
   - **[🚨보수적 표기 인정]**: 마케팅 문구 숫자가 실제 서류 개수보다 작거나 같은 경우는 보수적 표기이므로 적합(✅) 처리하십시오.
...
🔥 **Rule 56. [HACCP 인증 마크 공식 텍스트 검증 (로고 환각 방지)]**
   - 패키지에 부착된 HACCP(해썹) 마크 안의 한글 텍스트를 픽셀 단위로 판독하십시오.
   - **[🚨공식 명칭 강제]**: 현재 식약처 기준 공식 HACCP 마크의 상단 텍스트는 무조건 **"안전관리인증"**이어야 합니다. 
   - 디자이너가 과거 로고나 잘못된 벡터 파일을 다운받아 **"식품안전관리인증"**, "위해요소관리우수" 등으로 기재했다면 즉시 명백한 로고 무단 변형 및 표시 위반(🚨부적합)으로 적발하고, 공식 마크로 교체할 것을 지시하십시오.
"""
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V119.0 - HACCP 로고 텍스트 스나이퍼)")
