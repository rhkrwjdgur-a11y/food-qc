...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V76.0 - 100% 전수 표 출력 및 셀 다이어트)")
    ...
    with tab2:
        st.info("비교 기준이 되는 '서류(PDF)'를 바탕으로 팩과 박스 시안에 모든 원료가 100% 일치하는지 표를 그립니다.")
        if st.button("▶️ 정보표시면 원재료 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("사용자의 요청에 따라 모든 원재료를 생략 없이 100% 표에 전개하는 중입니다... (최대 60초 소요)"):
                
                if inspection_mode == "선물세트 교차 검토 (팩 vs 박스 비교)":
                    prompt = """
                    [지시]: '선물세트 교차 검토' 모드입니다. 업로드된 **[비교용 서류(한글라벨 PDF)]**를 기준으로 **[팩(내포장) 시안]**과 **[박스(외포장) 시안]**이 일치하는지 교차 검증 표를 작성하십시오.
                    
                    🚨 [사용자 절대 명령: 행(Row) 생략 금지 & 100% 전수 출력] 🚨
                    어떠한 경우에도 원재료 항목을 요약하거나 "외 30건 일치" 처럼 묶어서 생략하지 마십시오. 1번 원재료부터 마지막 원재료까지 단 하나도 빠짐없이 무조건 개별 행(줄)으로 표를 꽉 채워 그리십시오.
                    
                    🚨 [출력 끊김(토큰 초과) 극복을 위한 셀(Cell) 압축 작성법] 🚨
                    원재료가 50개가 넘어가면 글자 수 제한 초과로 표가 중간에 끊깁니다. 이를 막기 위해, 팩이나 박스의 글자가 서류와 완벽히 똑같다면 긴 텍스트를 그대로 베껴 쓰지 말고 오직 **`✅ 서류와 동일`** 이라고만 적으십시오! 오직 글자가 틀리거나 누락되었을 때만 그 문제의 텍스트를 직접 적어 비교하십시오.
                    
                    ## 2️⃣ [서류 기준: 팩(내포장) vs 박스(외포장) 전수 대조 (생략 금지)]
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    
                    **(🚨 시안 원재료명 행 분리 절대 규칙 🚨): 괄호 '(', ')', '{', '}', '[', ']' 안에 있는 쉼표는 절대 분리 기준이 될 수 없습니다!! 복합원재료는 무조건 하나의 덩어리로 묶으십시오.**
                    
                    [🟢 스마트 압축 표 작성 예시]
                    | 서류 매칭 원료 (비교용 기준) | 팩(내포장) 시안 텍스트 | 박스(외포장) 시안 텍스트 | 대조 결과 | 판정 |
                    |---|---|---|---|---|
                    | 정제수 | ✅ 서류와 동일 | ✅ 서류와 동일 | 100% 일치 | ✅ |
                    | 분리유청단백(캐나다, 스페인 등) | ✅ 서류와 동일 | ✅ 서류와 동일 | 100% 일치 | ✅ |
                    | 식물성크림 | 식물성유지 | (누락됨) | 🚨 팩 오타, 박스 누락 | 🚨 |
                    | (모든 원료를 생략 없이 끝까지 하나씩 전부 다 나열할 것) | ... | ... | ... | ... |
                    ...
                    🚨 [사용자 절대 명령: 행(Row) 생략 금지 & 100% 전수 출력] 🚨
                    어떠한 경우에도 원재료 항목을 요약하거나 생략하지 마십시오. 1번 원재료부터 마지막 원재료까지 단 하나도 빠짐없이 무조건 개별 행(줄)으로 표를 꽉 채워 그리십시오.
                    
                    🚨 [출력 끊김(토큰 초과) 극복을 위한 셀(Cell) 압축 작성법] 🚨
                    원재료가 50개가 넘어가면 글자 수 초과로 출력이 끊깁니다. 이를 막기 위해, 시안의 글자가 서류와 완벽하게 똑같다면 긴 텍스트를 베껴 쓰지 말고 오직 **`✅ 서류와 동일`** 이라고만 적으십시오! 오직 글자가 틀리거나 누락되었을 때만 그 문제의 텍스트를 직접 적으십시오.
                    
                    ## 2️⃣ [원재료명 1:1 정밀 대조 및 원산지 검증]
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    
                    **(🚨 시안 원재료명 행 분리 절대 규칙 🚨): 괄호 '(', ')', '{', '}', '[', ']' 안에 있는 쉼표는 절대 분리 기준이 될 수 없습니다!!**
                    
                    [🟢 스마트 압축 표 작성 예시]
                    | 서류 매칭 원료 (서류 요약) | 시안 원재료명 (포장지) | 배합비 룰 (3순위/5%/2%) | 규정/오타/누락 검증 | 판정 |
                    |---|---|---|---|---|
                    | 정제수 | ✅ 서류와 동일 | 3순위 제외 | 100% 일치 | ✅ |
                    | 유함유가공품(프랑스산/버터밀크 등) | ✅ 서류와 동일 | 미상 | 100% 일치 | ✅ |
                    | 혼합제제(하위생략) | 유화제5종 | - | 🚨 Rule 44 위반 | 🚨 |
                    | (모든 원료를 생략 없이 끝까지 하나씩 전부 다 나열할 것) | ... | ... | ... | ... |
