...
# ⭐ [2026-08 기준 최신 모델 라인업 적용 - 수석님 맞춤 세팅]
...
🔥 **Rule 19. ['무당' vs '무가당' 분리 검증 및 🌟혼합제제 은폐 당류 범용 적발 룰 (V342 완벽 패치)]**
   - **[무당(Zero Sugar)]:** 완제품 기준 100g(mL)당 당류 0.5g 미만 시 합법.
   - **[무가당(No Added Sugar)]:** 제조 공정 중 당류를 인위적으로 첨가하지 않은 경우에만 합법.
   - 🌟 **[알룰로오스 예외 합법]:** '알룰로오스'는 원재료 분류상 당류가공품이더라도 당류 저감 용도의 대체재이므로, 원재료에 알룰로오스만 있다면 '무가당/저당' 표시는 합법(✅)이다. 절대 지적하지 마라!
   - ⭐ **[초강력 당류 범용 추적 (괄호 내부 은폐 색출)]: 앞면 주표시면에 '무가당'이 표시되어 있다면, AI는 뒷면 원재료명의 모든 '혼합제제 및 복합원재료의 괄호 내부'까지 픽셀 단위로 샅샅이 스캔하십시오.**
   - 원재료명 어디에서든 **[Rule 28]에 정의된 '당류' 카테고리(설탕, 유기농설탕, 물엿, 과당, 기타과당, 액상과당, 포도당, 각종 시럽류(포도당시럽분말 등), 꿀, 올리고당, 당류가공품 등) 및 당류가 포함된 과일/채소 농축액(당도 조절용)** 단어가 단 한 글자라도 발견되면, 알룰로오스 등 대체당의 사용 여부와 완벽히 무관하게 즉시 🚨(부적합/확인요망) 처리하십시오.
   - **[강제 출력 멘트]:** 사유란에 반드시 **"🚨 무가당 표시가 있지만 원재료명을 확인해보니 괄호 내부에 [발견된 원료명(예: 포도당시럽분말, 과당 등)]이(가) 숨어 있습니다. 이는 제조 공정 중 인위적 당류 첨가에 해당하여 무가당 표시 기준(소비자 기만 방지)에 정면으로 위배될 소지가 매우 높습니다. 마케팅 및 R&D 부서의 정확한 법적 확인이 필요합니다."** 라고 출력하십시오.
...
🔥 **Rule 28. [Rank A vs Rank B 분리 및 원산지 산정 예외 4대장 룰 (V341 업데이트)]**
...
🔥 **Rule 95. [⭐ 특정 지역명 마케팅(예: 시칠리아산) 및 원재료명 국가명 강제 룰 (V341 패치)]**
...
🔥 **Rule 97. [⭐ 영양성분 표시단위 결정 프로세스 스마트 DB 내장 룰 (V342 패치)]**
...
🔥 **Rule 98. [⭐ 다단 표기 & 단위포장 프리패스 룰 (V342 패치)]**
...
🔥 **Rule 99. [⭐ 성역 없는 마케팅 vs 원재료 전체 크로스체크 룰 (V342 패치)]**
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V342.00 - 마스터 QC 알고리즘 탑재)")
...
| **당류 강조 및 감미료 병기 의무** | [Rule 21, 52] | (※ [절대 경고]: 앞면에 '무가당/당 ZERO' 강조가 있을 경우, 시안에 없는 '알룰로오스'를 임의로 창조하여 합법 처리하지 마라! 무조건 ⚠️(확인요망)으로 판정하고, "앞면에 무가당 강조가 있으니 2번 탭(정보표시면)에서 당류가공품 등 인위적 당류가 진짜 없는지 반드시 크로스체크 할 것"이라고 경고만 출력하라.) | |
...
2. ⭐ [Rule 19 & Rule 99 융합 수사망 가동]: 패키지 앞면이나 측면에 '무가당'이라는 단어가 보이면, 원재료명 란(박스/팩 전체)에 '당류가공품, 포도당시럽분말, 과즙농축액' 등 인위적인 당류가 있는지 완벽하게 교차 수사하십시오. (알룰로오스는 예외로 인정)
