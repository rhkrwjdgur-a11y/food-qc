...
import os
import re
import tempfile  # 안전한 임시 파일 생성을 위한 모듈
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V103.0 - 기계적 1:1 대조 완결판)")
    ...
    with tab1:
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("주표시면 기계적 텍스트 추출 및 정밀 대조 중..."):
                ...
                🚨 [필수 출력 규칙 절대 명령] 🚨
                당신의 답변은 무조건, 반드시 <thinking> 이라는 정확한 태그 문자로 시작해야 합니다. 사전 분석이 끝나면 반드시 </thinking> 태그로 닫으십시오. [thinking], 사전 분석: 등 다른 단어로 변형하거나 태그를 생략하면 시스템 치명적 오류로 간주하고 답변을 거부하겠습니다.
                
                <thinking>
                1. 🚨 [시각 판독 및 필사 절대 명령 (창의성 0%)]: AI의 임의 추론이나 배경지식 개입을 엄격히 금지합니다. 업로드된 주표시면 이미지에 적힌 모든 큰 글씨, 마케팅 뱃지, 하단 설명 문구를 단 한 글자의 생략 없이 이미지에 보이는 그대로 100% 필사(Transcription)하여 먼저 나열하십시오.
                2. 키워드 스캔: 추출된 텍스트 안에 '원액', '100%', '제로', '무당', 숫자(N종, N곡 등)가 포함되어 있는지 확인하십시오.
                3. 🚨 [Rule 50 고형분 누락 스나이퍼]: 주표시면에 '원액', '추출물', '농축액' 등의 단어와 그 함량(%)이 강조되어 있다면, 이미지 상에서 '고형분 N%'라는 텍스트가 함께 기재되어 있는지 샅샅이 찾으십시오. 만약 고형분 수치가 안 보인다면 명백한 규정 누락입니다.
                4. 🚨 [기계적 수량/오탈자 교차 검증]: 주표시면의 포장 단위(예: N입, 총 용량)를 파악하고, 업로드된 다른 면의 수량 정보와 철자, 띄어쓰기까지 100% 일치하는지 비교하십시오.
                </thinking>
            ...
            with st.spinner("서류 유형 분석 및 창의성 배제 100% 교차 검증 중..."):
                ...
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        <thinking>
                        1. 🚨 [시각 판독 및 필사 절대 명령 (창의성 0%)]: [타겟_시안(박스)]과 [비교용_정답지_시안(팩)]의 '원재료명' 텍스트를 절대로 지어내지 말고 100% 필사하십시오. 당신의 배경지식으로 누락된 단어(예: 원산지)를 절대 채워 넣지 마십시오.
                        2. 🚨 [과부하 방지: 통합 문서 절대 기준화]: 업로드된 엑셀/PDF 서류를 '절대 정답지'로 간주하십시오. 
                        3. 🚨 [기계적 1:1 대조 절대 명령]: 업로드된 서류, 박스, 팩의 텍스트를 기계적으로 1:1 검증하십시오. 의미가 같더라도(예: 올레산 vs 올레인산, 알룰로스 vs 알룰로오스) 글자 형태나 철자가 하나라도 다르면 무조건 🚨부적합입니다. 박스에 있다고 팩에서 생략된 것을 합격 처리하는 임의의 융통성을 엄격히 금지합니다.
                        4. 🚨 [수량/오탈자 교차 검증]: 하단 구석의 '식품이력추적관리번호' 주변 수량을 추출하여 논리적 충돌을 적발하십시오.
                        </thinking>
                        
                        🚨 [표 작성 필수 명령 절대 강제 - 출력 단축(Lazy Output) 원천 봉쇄] 🚨
                        1. 표의 한 행에는 무조건 1개의 원료만 기재하십시오.
                        2. 🚨 [생략 절대 금지]: 대외 보고서 제출용입니다. 어떠한 경우에도 표 안에 '...', '중략', '이하 생략', '해당 없음' 등으로 축약하는 것을 엄격히 금지합니다. 원재료가 아무리 많아도 첫 번째부터 마지막까지 물리적으로 모든 행(Row)을 100% 전부 타이핑하여 완성하십시오.
                        ...
                        <thinking>
                        1. 🚨 [시각 판독 및 필사 절대 명령 (창의성 0%)]: [타겟_시안]의 '원재료명' 텍스트를 절대로 지어내지 말고 100% 필사하십시오. 배경지식 개입 금지.
                        2. 🚨 [과부하 방지: 통합 문서 절대 기준화]: 업로드된 엑셀/PDF 서류를 '절대 정답지'로 간주하십시오.
                        3. 🚨 [기계적 1:1 대조 절대 명령]: 엑셀/PDF 서류의 내용과 시안 텍스트를 1:1로 검증하십시오. 의미가 같아도 철자가 다르면 🚨부적합입니다.
                        4. 🚨 [수량/오탈자 교차 검증]: 이미지 하단 구석의 수량을 추출하고 비교하십시오.
                        </thinking>
                        
                        🚨 [표 작성 필수 명령 절대 강제 - 출력 단축 원천 봉쇄] 🚨
                        1. 🚨 [생략 절대 금지]: 대외 보고서 제출용입니다. 어떠한 경우에도 표 안에 '...', '중략', '이하 생략' 등으로 축약하는 것을 금지합니다. 모든 행(Row)을 100% 전부 타이핑하십시오.
                    ...
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        🚨 [표 작성 필수 명령 절대 강제 - 출력 단축 원천 봉쇄] 🚨
                        1. 🚨 [생략 절대 금지]: 대외 보고서 제출용입니다. 표 안에 '...', '중략' 등으로 축약하는 것을 엄격히 금지합니다. 모든 행을 100% 전부 타이핑하십시오. 
    ...
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("오차 계산 및 100% 전체 영양성분 3방향 대조 렌더링 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    ...
                    4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 총 내용량을 확인하여 다른 면의 수량과 충돌하는지 적발하십시오.
                    </thinking>
                    ...
                    3. 🚨 [수량/오탈자 교차 검증 절대 명령]: 영양성분표의 총 내용량을 확인하고 비교하십시오.
                    </thinking>
        ...
        if st.session_state["result_tab3"]:
            ...
            if thinking_match:
                thinking_log = thinking_match.group(1).strip()
                report_content = result.replace(thinking_match.group(0), "").strip()
                with st.expander("🧠 기계적 수치 오차 연산 로그 보기"):
                    st.markdown(f"*{thinking_log}*")
                st.markdown(report_content)
            else:
                st.markdown(result)
