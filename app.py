...
# ==========================================
# 2. 통합 전문가 프롬프트 (Rule 1~39 궁극의 완전판 V5.9 - 동명 원료 교차 혼선 완벽 차단)
# ==========================================
SYSTEM_PROMPT = """
...
---
# [식품 패키지 표시사항 QC 자동화 검수 시스템 룰북 5.9]
...
🔥 **Rule 30. [지정 알레르기 10종 특별 스캔 (실제 투입 원료 한정)]** - 서류에 있는 '실제 투입된' 알레르기 물질이 시안에 누락 시 즉시 부적합 처리.

🔥 **Rule 31~32. (기존 일반 규정 적용)**
   - 다중 성적서 처리, 균형 열량 구성비 역산 금지 등.
...
🔥 **Rule 38. [원료의 교차오염 주의문구 전이(Carry-over) 절대 금지]**
   - 원료 라벨 구석에 적힌 "본 제품은 ...과 같은 제조시설에서 제조하고 있습니다" 문구에 있는 돼지고기, 쇠고기 등은 우리 최종 시안 알레르기로 절대 끌고 오지 마십시오.

🔥 **Rule 39. [동명 원료(같은 이름) 교차 혼선 금지 및 종속성 원칙 - 매우 중요]**
   - 패키지에 여러 가지 복합원재료가 들어갈 경우, 이름이 같은 하위 성분(예: 현미유의 '현미' vs 발아약콩혼합분말의 '현미')이 존재할 수 있습니다.
   - AI 당신은 원산지나 명칭을 대조할 때 전체 문서를 통짜로 검색하지 마십시오. **반드시 해당 하위 성분이 소속된 '부모 원료(복합원재료)의 전용 서류' 안에서만 팩트를 확인**해야 합니다.
   - (예시) 시안의 '발아약콩혼합분말' 안에 있는 '현미'를 검증할 때는 오직 '발아약콩혼합분말 서류'만 보십시오. 다른 서류(미강유)에 적힌 '태국산 현미'를 끌어와서 🚨불일치라고 억지 주장을 펼치면 절대 안 됩니다.
---
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (Rule 1~39 궁극의 완전판 V5.9)")
    st.markdown("---")
    ...
    if st.button("🔍 서류 추출 및 QC 정밀 진단 시작", type="primary"):
        ...
        with st.spinner("수십 장의 서류를 스캔하여 엑셀 표로 추출하고 시안과 대조 중입니다..."):
            ...
            # 🔥 final_prompt: Rule 39 구역(Scope) 고립 강제명령
            final_prompt = f"""
            [현재 검토 대상 제품 유형]: {product_type}
            
            [🚨최종 점검 강제 명령🚨]:
            1. 표를 그릴 때 마크다운 안에 절대 줄바꿈(엔터)을 넣지 마십시오.
            2. [핵심-Rule 39 엄수] 동명이인 원료 조심! 발아약콩혼합분말의 '현미'를 검증할 때는 다른 서류(미강유 등)에 적힌 '태국산 현미'를 절대 끌어오지 마십시오. 반드시 부모 원료 단위로 구역(Scope)을 철저히 분리해서 대조하십시오.
            3. [핵심-Rule 38 엄수] 원료 라벨의 "같은 제조시설에서 제조~" 문구(돼지고기 등)는 무조건 스루(무시)하십시오. 
