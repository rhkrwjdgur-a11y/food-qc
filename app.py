...
# ==========================================
# 📚 3. 86대 마스터 룰북 원문 (V310.25 완결 집대성본)
# ==========================================
RULE_BOOK_FULL = """
...
🔥 **Rule 3. [주표시면 vs 영양성분표 수치 100% 일치 강제 룰]**
   ...
   - 세트 포장의 주표시면에는 '총 내용량'과 '총 열량(kcal)'이 모두 기재되어야 합니다[cite: 3].
...
🔥 **Rule 5. [복합원재료 5가지 컷오프 및 첨가물 이행(Carry-over) 절대 예외 룰]**
   ...
   - **[조건 B: 5가지 컷오프]**: 배합비가 5% 이상인 복합원재료라 하더라도, 시안의 괄호 안에 하위 성분이 **'물을 제외하고 많이 사용한 순서대로 5가지 이상'** 명시되어 있다면 나머지 일반 원료 생략은 합법(✅). AI는 무지성으로 누락을 지적하지 마십시오.
   - 🚨 **[조건 C: 첨가물 이행(Carry-over) 절대 예외]**: 단, 서류상 생략된 하위 성분 중에 최종 완제품에서 기능적 효과(유화, 안정, 보존 등)를 발휘하는 **'식품첨가물(예: 폴리소르베이트, 카라기난 등)'**이 단 하나라도 존재한다면, 앞의 5가지 컷오프 룰이 무력화되며 괄호 안에 끝까지 부활시켜 기재해야 합니다. 누락 시 무조건 부적합(🚨).
...
🔥 **Rule 23. [식약처 영양성분 '0' 표시 및 반올림 예외 절대 규정]**
   ...
   - [반올림의 유연성]: 열량, 나트륨 등은 실측값을 소수점 이하 버림 등으로 **'있는 그대로 표시'**하는 것과 **'법정 단위(열량 5kcal, 나트륨 10mg 등)로 반올림하여 표시'**하는 것 **둘 다 완벽한 합법(✅)**임. 반올림하지 않고 있는 그대로 적었다고 부적합 처리하지 마십시오.
...
🔥 **Rule 35. [🌟 범용 간략명/동의어 허용 및 N종 묶음 금지 (향료 완벽 예외)]**
   - 식약처 이명, 표 5 간략명, 내부 코드 생략은 완벽 합법.
   - 🚨 [원칙]: 혼합제제 괄호 내부를 '산도조절제 2종', '유화제 3종' 등 숫자로 묶어 용도명으로 은폐(블랙박스화)하는 것은 명백한 위법(🚨부적합).
   - ✅ **[향료 절대 예외]**: 단, 식약처 공식 FAQ 지침에 의거하여 **'향료(합성/천연)'의 경우에 한하여**, 서로 다른 품목을 사용했음을 알리기 위해 **'향료 2종', '합성향료 2종', '합성향료 1, 합성향료 2'** 등으로 숫자를 표기하여 구분하는 것은 완벽한 합법(✅)이며 절대 묶음 금지로 지적하지 마십시오.
...
🔥 **Rule 85. [식품첨가물 공전 명칭 사수 및 기호 창조 절대 금지 (범용 형식주의)]**
   - **[명칭 축약 엄격 금지]**: 식품첨가물은 어떠한 공간 부족을 이유로도 임의 축약을 금지함. 반드시 식약처 고시 원문의 [표 4, 5, 6]에 등재된 '정식 명칭' 또는 '법정 간략명'(예: 식용색소청색제1호)과 글자 단위로 100% 일치해야 합격. (예: '식용색소'를 빼고 '청색제1호'로 기재 시 즉시 부적합🚨).
   - **[임의 기호 창조 전면 금지]**: 원재료와 첨가물 용도를 병기하거나 하위 성분을 전개할 때, 대한민국의 식품 법정 표준 기호인 **'괄호()'** 외에 작성자의 편의를 위한 임의의 특수기호**(슬래시 `/`, 대시 `-`, 플러스 `+`, 콜론 `:` 등)**를 사용하여 구조화하는 행위는 전면 금지(🚨). (예: '적색제40호/착색료'와 같이 슬래시를 사용하는 행위는 즉시 부적합🚨).

🔥 **Rule 86. [국가 공인 인증 도안 기만 및 위조 변조 차단 룰 (형사 처벌 방어)]**
   - 시안 상에 HACCP, K-MILK, 친환경 등 '국가 공인 인증'에 대한 언급이나 문구, 엠블럼이 식별될 경우 반드시 공식 규격 도안과 기하학적 형태가 일치하는지 전수 대조할 것.
   - 🚨 [도안 미사용 텍스트 편법 규제]: 정식 인증 마크 도안을 인쇄하지 않고, 단순히 'HACCP 인증', 'K-MILK 인증' 등 **일반 텍스트(글자)만 주표시면이나 기타면에 적어 마케팅으로 부당 이득을 취하려는 시도는 무조건 부적합(🚨)** 처리함. 인증 표시는 관련 법령 및 기관이 지정한 '공식 엠블럼 도안'을 변형 없이 사용하는 것만 합법(✅)으로 인정함.
"""
...
COMMON_RULES = [36, 37, 42, 43, 45, 47, 70, 78, 79, 85, 86]
RULES_TAB1 = "[탭1 주표시면 관련 핵심 룰]\n" + get_sliced_rules([3, 9, 10, 15, 16, 17, 18, 19, 21, 24, 28, 40, 46, 50, 51, 52, 53, 57, 58, 59, 60, 62, 63, 64, 68, 71, 72, 84] + COMMON_RULES)
RULES_TAB2 = "[탭2 정보표시면/원재료명 관련 핵심 룰]\n" + get_sliced_rules([1, 2, 5, 6, 7, 8, 12, 13, 14, 20, 25, 28, 29, 30, 34, 35, 38, 39, 44, 48, 52, 54, 57, 58, 59, 60, 61, 65, 68, 73, 74, 75, 76, 77] + COMMON_RULES)
...
⭐ 추출 텍스트의 오탈자 복원만 수행하십시오. 글자를 유추하거나 변경하지 마십시오.
[분할 미션 통합 텍스트]
{extracted_text_combined}
...
🔥 [출력 형태 절대 강제 족쇄]: 당신은 친절한 챗봇이 아니라 오직 최종 마크다운 양식만 뽑아내는 기계입니다. "안녕하세요", "종합 검토", "총평" 같은 인사말이나 서론, 요약 설명글을 앞뒤로 단 한 글자도 쓰지 마십시오. 당신의 출력 첫 글자는 무조건 아래 지시된 마크다운 제목 뼈대(## 1️⃣)로 곧바로 시작해야 합니다. 로그나 생각 과정을 주저리주저리 노출 시 즉각 시스템 부적합 처리됩니다.
...
"""
        for attempt in range(3):
            try:
                ...
                return fix_markdown_table(pass2_response.text)
            except Exception as e:
                if "504" in str(e) or "Deadline" in str(e) or "503" in str(e):
                    if attempt < 2:
    ...
    def run_qc_model(prompt_text):
        ...
        full_prompt = f"""
        [제품유형]: {product_type}\n[검토모드]: {inspection_mode}\n[우리 공장 알레르기 마스터 목록]: {factory_allergens}
        {RULE_BOOK_FULL}\n========================================\n당신은 지금 선택된 탭의 임무만 완벽하게 수행해야 합니다. 서론이나 결론 설명 없이 지정된 제목 서식으로 직접 시작하십시오.\n{prompt_text}
        """
        try:
    ...
    def display_result(result, tab_name=""):
        ...
        # 🚨 무조건 모든 로그 백엔드 은폐 및 정해진 마크다운 서식 전면 출력 픽스!
        st.markdown(result)
...
| **국가 공인 인증 도안 마케팅** | [Rule 86] | (단순 텍스트만 적어 기만했는지 공식 엠블럼 도안을 바르게 인쇄했는지 엄격 심사) | |
        ...
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                missions = [
                    ...
                    "시안에 기재된 원재료명 중 '식품첨가물'로 의심되는 모든 단어(색소, 향료, 보존료 등)를 추출하고, 용도를 병기할 때 사용된 괄호 및 기호(예: 슬래시 /)를 있는 그대로 추출하십시오.",
...
"""
                if inspection_mode == "단품(팩/단일포장) 기본 검토":
                    common_tab2_prompts = """## 1️⃣ [마스터 서류 vs 시안 법적 대조 매트릭스]
⭐ [강제 지시 1: 시안 기준 Left Join]: 표의 뼈대는 무조건 '시안'에 적힌 원재료명 순서 그대로 100% 나열하십시오.
⭐ [강제 지시 2: Rule 5 (복합원재료 5가지 컷오프 및 Carry-over) 연동 적용]: 시안 괄호 안에 물 제외 5가지 이상 적혀있다면 합법(✅) 처리하되, 서류상 제외된 성분 중 최종 제품에서 기능하는 '식품첨가물'이 누락되었다면 5가지 룰을 무력화하고 가차 없이 부적합(🚨) 처리하십시오.
⭐ [강제 지시 3: 원산지 꼬리표 부착]: 대조 검증 결과란 첫머리에는 반드시 굵은 글씨로 **[원산지 1순위]**, **[원산지 2순위]**, **[원산지 3순위]** 또는 **[원산지 산정 제외]** 꼬리표를 달아주십시오.
| 시안 표기 원재료명 (100% 나열) | 매칭된 서류 원료명 (없으면 '제출 안 됨') | 대조 검증 결과 (원산지 및 Rule 5 충족여부 포함) | 최종 판정 |
|---|---|---|---|

### 🚨 [식품첨가물 범용 형식주의 스나이퍼 (Rule 85 강력 적용)]
⭐ [강제 지시]: 미션에서 추출한 첨가물 관련 단어들을 분석하여, **1) 명칭 축약(예: '식용색소' 누락), 2) 임의 기호 사용(예: 괄호 대신 슬래시 `/` 사용)** 여부를 엄격히 스캔하여 적발하십시오. (단, Rule 35에 따라 '향료 N종' 표기는 합법)
- **[명칭 축약 검사 결과]**: 
- **[임의 기호 창조 검사 결과]**: 
...
"""
                    ...
                    common_tab2_prompts = """## 1️⃣ [통합 마스터 대조 매트릭스]
⭐ [강제 지시 1: 박스 기준 Left Join]: 표의 뼈대는 무조건 '📦 타겟(박스) 시안'에 적힌 원재료명 순서 그대로 100% 나열하십시오.
⭐ [강제 지시 2: Rule 5 (복합원재료 5가지 컷오프 및 Carry-over) 연동 적용]: 시안 괄호 안에 물 제외 5가지 이상 적혀있다면 합법(✅) 처리하되, 서류상 제외된 성분 중 최종 제품에서 기능하는 '식품첨가물'이 누락되었다면 5가지 룰을 무력화하고 가차 없이 부적합(🚨) 처리하십시오.
⭐ [강제 지시 3: 원산지 꼬리표 부착]: 대조 검증 결과란 첫머리에는 반드시 굵은 글씨로 **[원산지 1순위]**, **[원산지 2순위]**, **[원산지 3순위]** 또는 **[원산지 산정 제외]** 꼬리표를 달아주십시오.
| 📦 타겟(박스) 시안 표기 (100% 나열) | 🧃 비교용(팩) 시안 표기 | 매칭된 증빙 서류 | 대조 검증 결과 및 사유 (원산지 및 Rule 5 충족여부 포함) | 최종 판정 |
|---|---|---|---|---|
    ...
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【허용오차 검증 매트릭스 가동 중...】"):
                ...
                if inspection_mode == "단품(팩/단일포장) 기본 검토":
                    judgment_prompt = """## 1️⃣ [영양표시 오차 검증 및 % 기준치 확인]
- 결론: (✅ 적합 또는 🚨 부적합)
...
- ⭐ [Rule 23 반올림의 유연성 검증]: 열량, 나트륨 등이 5단위/10단위 반올림 없이 '있는 그대로' 기재된 것도 전수 합법(✅) 판정했는지 검증.
"""
                else:
                    judgment_prompt = """## 1️⃣ [영양표시 오차 검증 및 팩/박스 교차 대조]
- 결론: (✅ 적합 또는 🚨 부적합)
...
- ⭐ [Rule 81] 하단 2000kcal 면책 문구 토씨 100% 대조: 
- ⭐ [Rule 82] 영양소 법정 특수 단위/아래첨자 정밀 검증: 
- ⭐ [Rule 83] 기준치 존재 성분 % 병기 룰 대조:
    ...
    with tab4:
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("【의무표시 및 인증마크 해독 중...】"):
                ...
                judgment_prompt = """## 1️⃣ [기타면/측면 표시사항 및 마케팅 뱃지 정밀 검증]
⭐ [강제 지시]: 아래의 표 양식을 단 1개의 행도 삭제하지 말고 그대로 출력하십시오. 검토 결과란에 합격(✅) 또는 부적합(🚨) 사유를 명확히 적으십시오.
...
| **특수의료용도식품 타겟 문구** | [Rule 78] | | |
| **열량 구성비 정밀 역산** | [Rule 79] | | |
| **'무가당' 및 당류 강조표시** | [Rule 19, 24] | | |
...
"""
    ...
    with tab5:
        ...
        if st.button("▶️ AI 법률 자문 자율 스캔 시작", key="btn_law"):
            with st.spinner("【법령 PDF 동기화 및 전수 대조 중...】"):
                missions = [
                    "1. 업로드된 시안의 모든 면을 종합적으로 스캔하여 마케팅 카피, 제품명, 강조 문구 추출.",
                    "2. 추출된 시안 문구/디자인과 관련된 제한 및 의무 사항을 업로드된 법령 PDF에서 검색하여 '법령명, 조항, 원문'을 그대로 추출하십시오."
                ]
                
                judgment_prompt = """## 1️⃣ [AI 법률 자문 자율 스캔 리포트]
⭐ [환각 원천 차단 및 다면 교차 검증(Cross-Check) 8대 강제 명령] ⭐
...
- [인증 마크 위변조 검수 필수]: 도안 없이 텍스트로만 퉁친 기만행위(Rule 86)가 적발되면 법적 근거와 함께 즉각 지적하십시오.

---

### 📋 [법률 스캔 결과 보고서]

#### 📌 [식별된 문구/디자인]: "추출된 내용 및 발견된 위치"
* **적용 법령 및 조항:** [문서명, 제O조 제O항]
* **법령 원문:** > ""
* **AI 법무팀 자문 의견 (위법 리스크):**
  * 🚨 **[리스크 총평]:** 
  * 🔍 **[다면(Double-Check) 교차 검증 결과]:** (HACCP/K-MILK 도안 규격 적합성 및 식품유형/첨가물 형식 전수 검사 결과 포함 작성)
---
"""
