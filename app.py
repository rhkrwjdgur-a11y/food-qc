...
---
# [식품 패키지 표시사항 QC 자동화 검수 시스템 룰북 3.3]

## ⚠️ 검토 대원칙: 32대 특수 지침 (절대 엄수)
...
🔥 **Rule 32. [열량 구성비 역산 및 탄수화물 칼로리 예외 인정 룰]**
   - 패키지에 표기된 **'탄수화물:단백질:지방 열량 구성비(예: 34:22:44)'**를 검증할 때, 단순히 영양성분표의 총 탄수화물 중량(g)에 4kcal를 곱하여 부적합 처리하는 1차원적인 계산을 절대 금지합니다.
   - 균형영양식이나 저당 제품의 경우, 탄수화물 항목에 열량이 없거나 낮은 대체감미료(알룰로스, 에리스리톨 0kcal) 및 식이섬유(2kcal)가 포함되어 있어 실제 발생 칼로리가 낮습니다.
   - **[검증 지침]**: 제품에 표기된 **'총 열량(kcal)'을 기준으로 비율을 역산**하십시오. (예: 총 170kcal * 단백질 22% = 37.4kcal -> ÷4 = 9.35g). 역산된 단백질과 지방의 중량이 영양성분표 수치와 거의 일치한다면, 탄수화물 칼로리 오차는 감미료/식이섬유에 의한 것으로 올바르게 인지하고 **✅적합(합법)**으로 판정하십시오.
---
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (Rule 1~32 완결판)")
    st.markdown("---")
    ...
    with col2:
        ...
        # 다중 파일 업로드를 위해 accept_multiple_files=True 옵션 유지
    ...
    if st.button("🔍 QC 정밀 진단 시작", type="primary"):
        ...
        with st.spinner("업로드된 자료를 분석하여 맞춤형 QC를 진행합니다..."):
            ...
            [🚨치명적 오류 방지 2🚨]: Rule 32에 따라 영양구성비(탄:단:지) 검증 시 총 탄수화물량에 무조건 4kcal를 곱하여 부적합 처리하지 말고, 알룰로스/식이섬유를 감안하여 총 열량 기준으로 역산 검증하십시오.
            """
