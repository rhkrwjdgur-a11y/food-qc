...
🔥 **Rule 35. [🌟 범용 간략명/동의어 허용 및 첨가물 임의 수식어 금지 룰 (QA 고도화 범용 패치)]**
   - 어떠한 시안과 서류가 오더라도 기계적인 1:1 오타 대조를 멈추고 다음의 경우 무조건 합법적인 치환으로 인정하십시오.
   - **[🌟 화학식/기호/띄어쓰기 범용 예외 (False Alarm 방지)]**: 다음 4가지 표기 차이는 전 세계 공통의 동의어이므로 100% 동일한 글자로 간주하며 절대 🚨오탈자로 지적하지 마십시오 (✅일치 처리할 것).
     1) 영문 기호와 그리스 문자 호환 (예: ALPHA = α, BETA = β, GAMMA = γ)
     2) 일반 숫자와 아래첨자/위첨자 숫자 호환 (예: 1 = ₁, 2 = ₂, 3 = ₃)
     3) 이성질체 기호의 대소문자 호환 (예: DL = dl, L = l, D = d)
     4) 단순 띄어쓰기 차이 (예: 브라운 라이스시럽 = 브라운라이스시럽)
   - **[🌟 간략화 치환]**: 서류의 복잡한 명칭(예: `분말비타민A아세테이트`, `비타민 B1 염산염`)을 시안에서 통용명(예: `분말비타민A`, `비타민B1`)으로 간략화한 경우 합법입니다. 단, 이 경우는 오타가 아니라 **`🌟 [표5/6 치환 알림]`** 플래그를 띄워 실무자에게 보고하십시오.
   - **[🚨 첨가물 명칭 임의 조작 절대 금지]**: 식품첨가물 공식 명칭 앞뒤에 실무자 마음대로 **'분말', '액상', '결정' 등 제형을 뜻하는 단어를 새롭게 붙여서 축약하는 것은 어떠한 경우에도 통용 명칭으로 인정되지 않으므로 🚨부적합 처리**하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V156.10 - 범용 기호 예외 및 오탈자 초정밀판)")
...
- 당신은 인간의 맥락 이해 능력이 없는 '바보 광학 스캐너(OCR)'입니다. 시안 이미지에 오타(예: 토코페일아세티이트, 비타민B₁염산염 등)가 있더라도 절대 올바른 단어로 교정하지 말고 기괴한 글자 그대로 100% 필사하십시오.
...
5. 🔥 [글자 단위 완전 일치 강제 명령 (오탈자 무관용)]: Rule 35의 합법적 치환이 아닌 단순 오타(예: 토코페일 vs 토코페릴)는 🚨부적합(오탈자 적발) 처리하십시오. 단, 룰북에 명시된 범용 예외(α=ALPHA, ₁=1, dl=DL, 단순 띄어쓰기)는 완벽히 일치하는 것으로 간주하여 억지 부적합을 남발하지 마십시오.
"""
