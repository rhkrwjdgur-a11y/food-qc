...
🔥 **Rule 24. [무당/무가당/설탕무첨가 2대 의무 표기 (감미료 & 열량 물리적 위치 강제)]**
   ...
   - 2) **[열량 병기 위치 강제]**: 저열량 미충족 시 총 열량을 병기해야 합니다.
...
🔥 **Rule 57. [세트포장 식품이력추적관리번호 수량 강제 룰]**
   - 박스 번호에 "수량(예: X 16입)"이 기재되어야 완벽한 합법(✅).

🔥 **Rule 59. [CS 및 1399 신고 의무표시 3종 강제 스캔 룰]**
   - 1) 소비자 상담 번호 2) 반품/교환 장소(구입처 등) 3) "부정·불량식품 신고는 국번없이 1399" 문구. 이 중 단 하나라도 누락 시 🚨부적합 처리하십시오. 
   - 💡[단, 고객상담실 번호에 '(수신자부담)'이라는 문구는 법적 강제 사항이 아니므로 없어도 합법입니다.]
...
# 💡 [V158.0 업데이트] 탭 2, 3에도 이중 안전망 룰(Rule 38 알레르기, Rule 59 1399문구 등) 강제 주입
RULES_TAB2 = "[탭2 정보표시면/원재료명 관련 핵심 룰]\n" + get_sliced_rules([1, 2, 5, 6, 8, 12, 13, 14, 25, 28, 30, 34, 35, 38, 39, 44, 48, 52, 59, 61, 65] + COMMON_RULES)
RULES_TAB3 = "[탭3 영양성분표 관련 핵심 룰]\n" + get_sliced_rules([3, 4, 6, 10, 11, 21, 23, 25, 26, 27, 31, 32, 33, 38, 40, 41, 59] + COMMON_RULES)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V158.0 - 이중 안전망 및 교차검증 봇 탑재)")
    ...
    # ==========================================
    # 🔥 3-Pass 파이프라인 (인자 변경: extract_mission 추가)
    # ==========================================
...
2. 🕵️‍♂️ [뒷면 핀셋 스캔 (교차 검증용)]: 전면 스캔이 끝났으면, 첨부된 다른 시안(영양성분표, 정보표시면)들을 빠르게 살펴보고 아래 2가지만 딱 뽑아오십시오.
   ...
   - 앞면에 "OOO 함유" 혹은 "비타민" 등 영양소가 강조되어 있다면, 영양성분표 사진에서 해당 영양소의 '% 기준치' 수치
"""
                judgment_prompt = """
## 1️⃣ [주표시면 마케팅 뱃지 및 교차 검증]
...
- ⭐ [Rule 47] 앞면 용량/열량 교차 검증 (스파이 스캔 데이터와 비교): (앞면에 적힌 내용량/열량이 뒷면 영양성분표에서 뽑아온 데이터와 100% 일치하는지 비교)
- ⭐ [Rule 50] 원액/추출물 고형분 병기 및 명칭 적합성:
- ⭐ [Rule 24] 무당/무가당/설탕무첨가 2대 의무 표기 적합성 검증:
   1) 포도당 등 당류 원천 차단 여부:
   2) 감미료 문구 위치:
   3) 열량 병기 물리적 위치:
- ⭐ [Rule 52] 영양강조 컷오프(7.5%/15%) 스파이 데이터 대조: (앞면에 강조된 비타민 등이 뒷면 영양표시에서 15% 또는 7.5%를 넘는지 비교하여 판정)
- 기타 특이사항:
...
"""
    ...
    # ── TAB 2: 정보표시면 (이중 안전망 패치 적용) ──
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【3-Pass】 원재료명 파싱 및 대조 진행 중..."):
                extract_mission = """
🎯 [미션 A: 정보표시면 배열 강제 추출 및 CS문구 이중 스캔]
1. 시안의 '원재료명' 텍스트를 줄글로 뭉뚱그리지 말고, 쉼표(,)를 기준으로 완벽하게 쪼개서 [1. 원액두유, 2. 정제수 ...] 처럼 번호가 매겨진 세로 리스트 형태로 추출하십시오. 
2. ⭐ [안전망 스캔]: 만약 원재료명 바로 주변이나 하단에 알레르기 유발물질 박스, 교차오염 주의문구, 1399 신고 문구, 반품교환처 등의 텍스트가 보인다면 무조건 함께 추출하십시오!
...
5. 🔥 [글자 단위 완전 일치 강제 명령 (오탈자 무관용)]: Rule 35의 합법적 치환이 아닌 단순 오타(예: 토코페일 vs 토코페릴)는 🚨부적합(오탈자 적발) 처리하십시오. 룰북에 명시된 범용 예외(α=ALPHA, ₁=1, dl=DL, 단순 띄어쓰기)는 완벽히 일치하는 것으로 간주하여 억지 부적합을 남발하지 마십시오.
"""
...
## 2️⃣ [안전망 스캐너 (CS 및 알레르기 주변 텍스트 검증)]
- ⭐ [Rule 38] 알레르기 유발물질 문구 적합성 (수학적 차집합): (추출된 경우에 한함)
- ⭐ [Rule 59] 필수 의무표시 (반품/교환, 1399 등): (추출된 경우에 한함)
"""
                    else:
                        judgment_prompt = base_tab2_warning + """
## 1️⃣ [원재료명 2-Way 정밀 교차 검증 (시안 기준 대조 + 누락 적발)]
...
## 3️⃣ [안전망 스캐너 (CS 및 알레르기 주변 텍스트 검증)]
- ⭐ [Rule 38] 알레르기 유발물질 문구 적합성 (수학적 차집합): (추출된 경우에 한함)
- ⭐ [Rule 59] 필수 의무표시 (반품/교환, 1399 등): (추출된 경우에 한함)
"""
                    else:
...
## 🔍 [안전망 스캐너 (표 주변 텍스트가 추출된 경우에 한함)]
- ⭐ [Rule 59] 필수 의무표시 (반품/교환, 1399) 적합성: (텍스트가 추출된 경우 판정)
- ⭐ [Rule 38] 알레르기 교차오염 문구 적합성: (텍스트가 추출된 경우 판정)
"""
                else:
                    judgment_prompt = """
## 4️⃣ [영양표시 및 % 기준치 검증]
...
"""
    ...
    with tab4:
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("【3-Pass】 뱃지 및 의무표시 스캔 중..."):
                extract_mission = """
🎯 [미션 A: 기타면/측면 필수 표시사항 추출]
- 시안 이미지들에서 1) 소비자 상담 번호(수신자부담 여부 등), 2) 반품/교환처, 3) 1399 부정불량식품 신고 문구, 4) HACCP 마크 텍스트, 5) 알레르기 주의사항(교차오염 박스) 텍스트를 정확하게 추출하십시오.
"""
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    judgment_prompt = """
