...
# ==========================================
# 📚 3. 70대 룰북 원문 (V193.0 N종 묶음 예외 완결판)
# ==========================================
RULE_BOOK_FULL = """
...
🔥 **Rule 35. [🌟 범용 간략명/동의어 허용 및 N종 묶음 예외 보장 (강력 예외 룰 V193.0)]**
   ...
   - **[🌟 괄호 상세명칭 생략]**: 서류에 `유성비타민A지방산에스테르(비타민A아세테이트)` 처럼 괄호로 이명이나 구체적 명칭이 부연 설명된 경우, 시안에서 괄호를 통째로 날리고 `유성비타민A지방산에스테르`라고만 적는 것은 완벽한 합법(✅)입니다. 누락이라고 억지 지적하지 마십시오.
   - **[🌟 화학식/기호/영문 약어 범용 예외]**: 식약처 공전에 등재된 공식 이명(예: 카복시메틸셀룰로스나트륨 = CMC-Na, CMC, 셀룰로스검)은 100% 합법입니다. 🚨부적합 처리 절대 금지. α = ALPHA 등도 모두 일치 처리.
   - **[🚨 'N종' 묶음 간략화 금지 및 예외 (V193.0 패치)]**: '비타민 3종', '야채농축액 5종'처럼 하위 성분을 '숨길 목적'으로 숫자로 묶는 것만 🚨부적합입니다. **단, 1) '향료 2종'처럼 향료에 숫자가 붙거나, 2) '영양강화제 2종'처럼 표 6(원래 일괄 표기 가능) 항목에 숫자를 붙인 경우, 또는 3) 숫자를 붙였으나 괄호나 하단에 개별 성분이 100% 전개되어 있는 경우에는 은폐가 아니므로 절대 부적합으로 잡지 말고 완벽한 합법(✅) 처리하십시오.**
...
🔥 **Rule 68. [다포장/세트포장 낱개 영양표시 복붙(Copy&Paste) 스나이퍼 룰]**
...
🔥 **Rule 70. [내/외포장 원재료명 100% 일치 강제 범용 스나이퍼 룰]**
   - 세트포장(박스/트레이) 검토 시, 내포장(낱팩)의 원재료명과 외포장(박스)의 원재료명은 토시 하나 틀리지 않고 100% 똑같아야 합니다. 
   - 서류와의 대조 시에는 Rule 35(동의어 허용)가 적용되어 합법일지라도, **내포장과 외포장 시안끼리의 표기가 서로 다르다면(단 한 글자, 띄어쓰기, 기호라도 다르면) 무조건 🚨부적합(내외포장 불일치)** 처리하십시오. 특정 성분에 국한되지 않고 모든 원료에 범용적으로 적용됩니다.
"""
...
RULES_TAB2 = "[탭2 정보표시면/원재료명 관련 핵심 룰]\n" + get_sliced_rules([1, 2, 5, 6, 7, 8, 12, 13, 14, 20, 25, 28, 29, 30, 34, 35, 38, 39, 44, 48, 52, 54, 57, 58, 59, 60, 61, 65, 69, 70] + COMMON_RULES)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V193.0 - N종 노이로제 패치)")
    ...
    with st.sidebar:
        ...
        st.info("💡 텍스트가 50개 이상으로 너무 빽빽해서 AI가 괄호 안의 글자(예: 가공유지)를 자꾸 빼먹는다면, 여기에 디자이너 원본 텍스트를 복붙해 주세요. 100% 완벽한 대조가 가능합니다.")
        manual_ingredients_target = st.text_area("📦 타겟(박스) 원재료명 직접 입력", height=100)
        manual_ingredients_compare = st.text_area("🧃 비교용(팩) 원재료명 직접 입력", height=100)
...
[사용자 수동 입력 원재료명 데이터 (우선 적용 대상)]
- 타겟(박스) 원재료명 수동 입력값: {manual_ingredients_target if 'manual_ingredients_target' in locals() and manual_ingredients_target else '없음'}
- 비교용(팩) 원재료명 수동 입력값: {manual_ingredients_compare if 'manual_ingredients_compare' in locals() and manual_ingredients_compare else '없음'}
(※ 만약 위 수동 입력값이 존재한다면, 시안 이미지에서 흐릿한 텍스트를 억지로 읽어내려 하지 말고, 무조건 이 수동 입력값을 100% 최우선 정답으로 사용하여 미션을 수행하십시오. 이것이 환각을 막는 절대 규칙입니다.)
...
검증 규칙:
1. ⭐ [오타 환각 복구]: Pass 1에서 추출한 텍스트에, 원본 이미지에 명백히 존재하는 오타가 임의로 정상 단어로 교정(환각)되었다면 즉시 원본 오타 그대로 훼손시켜 복구하십시오. 
2. ⭐ [누락/증발 텍스트 복구 강제 (가장 중요!)]: 원본 이미지에는 분명히 적혀 있는데, Pass 1 텍스트에서 누락된 단어(특히 괄호 안의 하위 성분, 예: 가공유지 등)가 없는지 글자 단위로 대조하십시오. 단 하나라도 빼먹은 단어가 있다면 무조건 텍스트를 수정하여 완벽하게 복구해 내십시오. 빼먹고 넘어가는 것은 치명적인 시스템 오류입니다.
3. ⭐ [미션 B 표 검증]: 미션 B의 결과물이 존재한다면 반드시 표(Table) 형태인지 확인하십시오. "등", "외 다수" 라는 표현이 표 안에 있다면 원래 성분명으로 복구하십시오.
...
1. [전면 스캔]: '주표시면(앞면)' 시안 사진을 집중적으로 보고 제품명, 내용량, 칼로리, 마케팅 강조문구(설탕 무첨가, 비타민 함유 등)를 빠짐없이 리스트로 작성하십시오. 원재료명 전체는 절대로 추출하지 마십시오.
...
- ⭐ [Rule 24] 당류 강조표시(무당/저당/무가당/설탕무첨가) 적합성 검증:
   ...
   2) 열량 병기 위치: (저당을 포함한 모든 당류 강조 시 적용. 저열량 기준 충족 여부를 확인하고 미충족 시 강조표시 주변 열량 표기 판정)
...
3. ⭐ [초정밀 분해 지시 - 괄호 안 누락/증발 절대 금지]: 텍스트를 문장 단위로 대충 읽어오지 마십시오. 반드시 '쉼표(,)'를 기준으로 모든 성분을 잘게 쪼개서 (1. 옥수수전분, 2. 가공유지, 3. 이산화규소...) 번호를 매겨 추출해야 합니다. 특히 괄호 '(...)'가 나오면 돋보기를 들이대고 그 안의 글자를 쉼표 단위로 낱낱이 해부하여 단 한 단어도 스킵(Drop)하거나 생략하지 마십시오. (가공유지가 증발하면 당신의 책임입니다.)
4. 알레르기(OO 함유) 텍스트는 원재료명 리스트에서 제외하고 따로 뽑으십시오.
5. ⭐ [교차오염 주의문구 추출]: 정보표시면에 "...같은 제조시설에서 제조하고 있습니다" 형태의 혼입(교차오염) 주의문구가 적혀 있다면 반드시 이것도 추출하십시오.
...
5. ⭐ [혼합제제 하위성분 환각 방지]: 괄호 안의 하위 성분(예: 가공유지 등) 누락을 지적하기 전에, 미션 A 추출 텍스트에 정말로 해당 성분이 없는지 3번 이상 반복해서 확인하십시오. 시안에 있는 성분을 없다고 오판(환각)하면 치명적인 에러입니다.
6. ⭐ [절대 생략 금지 (No Truncation) - 실무 QC 보고서 원칙]: 환자용 식품처럼 원재료가 40~50개에 달하더라도 표 작성 시 "중략", "...", "후략" 등의 요약/생략을 엄격히 금지합니다. 미션 A에서 추출한 [1번 원료부터 마지막 원료까지 100% 전부] 표의 행(Row)으로 생성하여 1:1 대조 판정을 작성해야 합니다. 단 하나라도 생략하면 치명적인 시스템 오류로 간주됩니다.
"""
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        judgment_prompt = base_tab2_warning + """
## 1️⃣ [Rule 70. 내포장(팩) vs 외포장(박스) 원재료명 100% 일치 스나이퍼]
- ⭐ [Rule 70 절대 강제 명령 - 의미 해석 금지!]: 팩과 박스 시안을 서로 비교할 때는 동의어 허용(Rule 35)을 절대 적용하지 마십시오! 당신은 글자의 '의미'를 모르는 픽셀 판독기입니다. 미션 A-1(박스)과 A-2(팩)의 텍스트를 글자 단위로 1:1 대조하여, 단 한 글자, 기호, 띄어쓰기라도 다르면 즉시 🚨부적합 처리하고, 정확히 어떤 원료가 다른지 명시하십시오. 완전히 동일할 때만 ✅적합입니다.
- 검증 결과: (상세 비교 결과 및 판정)

## 2️⃣ [원재료명 정밀 교차 검증 (시안 기준 대조 + 누락 적발)]
| 타겟(박스) 시안 표기 원재료명 (순서대로 100% 나열, 중략 절대 금지) | 매칭된 마스터 서류 원료명 | 원산지 산정 순위 | 대조 검증 (오탈자/순서/합법 간략화) | 판정 (상세 사유) |
|---|---|---|---|---|
...
## 3️⃣ [알레르기, 주의사항 교차 검증 (교차오염 크로스체크 포함)]
- 서류 기준, 박스와 팩의 '~함유' 알레르기 표시 완벽 일치 여부: (✅/🚨)
- ⭐ [Rule 38] 교차오염 문구 적합성 (정보표시면 크로스체크): (정보표시면 시안에 "...같은 제조시설에서 제조" 등의 교차오염 문구가 추출되었다면, [공장 마스터 - 미션 A-3에서 추출된 범용적인 직접 함유 알레르기 물질] 연산을 통해 적합하게 기재되었는지 교차 검증하십시오.)
...
"""
                        ...
                        judgment_prompt = base_tab2_warning + """
## 1️⃣ [원재료명 정밀 교차 검증 (시안 기준 대조 + 누락 적발)]
| 시안 표기 원재료명 (패키지 나열 순서 100% 나열, 중략 절대 금지) | 매칭된 마스터 서류 원료명 | 원산지 산정 순위 | 대조 검증 (오탈자/순서/합법 간략화) | 판정 (상세 사유) |
|---|---|---|---|---|
| (시안 기준 100% 나열) | (서류 원료명) | (도출 사유) | (검증 결과) | (✅/🚨 판정 및 사유) |
...
## 2️⃣ [Rule 70. 내포장(팩) vs 외포장(박스) 원재료명 100% 일치 스나이퍼]
- ⭐ [Rule 70 절대 강제 명령 - 의미 해석 금지!]: 팩과 박스 시안을 서로 비교할 때는 동의어 허용(Rule 35)을 절대 적용하지 마십시오! 당신은 글자의 '의미'를 모르는 픽셀 판독기입니다. 미션 A-1(박스)과 A-2(팩)의 텍스트를 글자 단위로 1:1 대조하여, 단 한 글자, 기호, 띄어쓰기라도 다르면 즉시 🚨부적합 처리하고, 정확히 어느 원료의 표기가 다른지 모두 찾아내어 명시하십시오. 완전히 동일할 때만 ✅적합입니다.
- 검증 결과: (상세 비교 결과 및 판정)

## 3️⃣ [원재료명 정밀 교차 검증 (시안 기준 대조 + 누락 적발)]
| 타겟(박스) 시안 표기 원재료명 (순서대로 100% 나열, 중략 절대 금지) | 매칭된 위 마스터 표 원료명 | 원산지 산정 순위 | 대조 검증 (오탈자/순서/합법 간략화) | 판정 (상세 사유) |
|---|---|---|---|---|
| (시안 기준 100% 나열) | (서류 원료명) | (도출 사유) | (검증 결과) | (✅/🚨 판정 및 사유) |
...
## 4️⃣ [알레르기, 주의사항 교차 검증 (교차오염 크로스체크 포함)]
- 서류 기준, 박스와 팩의 '~함유' 알레르기 표시 완벽 일치 여부: (✅/🚨)
- ⭐ [Rule 38] 교차오염 문구 적합성 (정보표시면 크로스체크): (정보표시면 시안에 "...같은 제조시설에서 제조" 등의 교차오염 문구가 추출되었다면, [공장 마스터 - 미션 A-3에서 추출된 범용적인 직접 함유 알레르기 물질] 연산을 통해 적합하게 기재되었는지 교차 검증하십시오.)
- ⭐ [Rule 63] 미드팩 190mL 주의사항 내 '질소충전' 표기 누락 점검:
- ⭐ [Rule 7] 당알코올 주의문구 적합성 검증 (10% 이상일 때만 표시):
...
## 🔍 [전 구간 공통: 수량 모순 및 오탈자 검증]
- ⭐ [Rule 20] 포장재질 표시 적합성: (종이 박스에 폴리에틸렌 등 내포장 재질만 적혀있어도 완벽히 합법(✅)이므로 오판 금지)
- ⭐ 포장 단위(수량) 2-Track 검증:
- ⭐ [Rule 69] 비타민 아래첨자 스나이퍼:
- ⭐ 띄어쓰기 및 오탈자 적발: (자음/모음 하나라도 다르면 무조건 🚨부적합 처리)
"""
...
## 2️⃣ [원재료명 정밀 교차 검증 (시안 기준 대조 + 누락 적발)]
| 시안 표기 원재료명 (패키지 나열 순서대로 100% 나열, 중략 절대 금지) | 매칭된 위 마스터 표 원료명 | 원산지 산정 순위 | 대조 검증 (오탈자/순서/합법 간략화) | 판정 (상세 사유) |
|---|---|---|---|---|
| (시안 기준 100% 나열) | (서류 원료명) | (도출 사유) | (검증 결과) | (✅/🚨 판정 및 사유) |
...
2. 영양정보표 바깥 하단에 적힌 "1일 영양성분 기준치에 대한..." 문구도 텍스트로 꼭 추출하십시오.
...
|---|---|---|---|---|---|---|---|
| (모든 영양성분을 축약 없이 100% 기재) | [내용 작성] | [내용 작성] | [내용 작성] | [내용 작성] | [내용 작성] | [내용 작성] | [내용 작성] |
...
- 당신은 패키지의 특정 면에 얽매이지 않는 '통합 스캐너'입니다. 업로드된 앞면, 뒷면, 영양정보, 기타면 시안 '전체'를 이 잡듯이 뒤져서 아래 항목들을 정확하게 추출하십시오.
  ...
  6) 알레르기 교차오염 주의문구 (예: "...제조시설에서 제조하고 있습니다")
  7) ⭐ [필수]: 직접 함유 알레르기 물질 표시 (예: "우유 함유", "대두, 밀 함유" 등 바탕색이 구분된 별도 박스 안의 텍스트). 특정 원료에 국한되지 않고 추출된 '모든' 직접 함유 알레르기를 차집합 연산에 범용적으로 사용합니다!
  
  - ⭐ 강력 지시: 검토 모드가 '선물세트 박스(외포장) 교차 검토'라면, 위 항목들을 **[타겟(박스) 시안]**과 **[비교용(팩) 시안]**에서 각각 별도로 분리하여 2세트를 완벽하게 추출하십시오! 둘 중 하나라도 누락하면 안 됩니다.
"""
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    judgment_prompt = """
...
- ⭐ [Rule 38] 알레르기 교차오염 문구 적합성 (수학적 차집합 검증 - 박스/팩 양쪽 확인):
   (※ 미션 A에서 추출한 '직접 함유 알레르기(7번 항목)'에 포함된 **모든** 물질을 범용적으로 활용하여 반드시 연산할 것. 우유 등 특정 물질에만 한정하지 마십시오!)
   1) [공장 마스터]: [내용 작성]
   2) [제품 함유 알레르기]: [내용 작성]
   3) [정답지 (1 - 2 차집합)]: [내용 작성]
