...
🔥 **Rule 11. [영양정보 허용오차 계산 절대 법칙 (하한선 오류 방지)]**
   - **[🚨가장 흔한 치명적 오류 경고]**: 반드시 '시안의 표시량'을 100% 기준으로 삼고 성적서 실측값을 평가하십시오.
   - **[80% 이상 합법 그룹 (좋은 영양소)]**: 비타민, 무기질, 단백질, 탄수화물, 식이섬유.
     * 수식: **(성적서 환산 실측값) $\ge$ (시안 표시량 $\times$ 0.8)** 이면 합격(✅). 상한선은 없습니다.
   - **[120% 미만 합법 그룹 (나쁜 영양소 - 하한선 없음!)]**: 열량, 당류, 지방, 포화지방, 트랜스지방, 콜레스테롤, 나트륨.
     * 수식: **(성적서 환산 실측값) $\le$ (시안 표시량 $\times$ 1.2)** 이면 무조건 합격(✅). **최소값 0부터 120% 이내면 합격**이며, 하한선(80%)은 존재하지 않으므로 임의로 하한선을 만들어 부적합 처리하지 마십시오.
...
🔥 **Rule 23. [식약처 영양성분 '0' 표시 완벽 규정 (단위 주의!)]**
   - **열량:** 5kcal 미만은 "0" 표시 가능.
   - **탄수화물, 당류, 지방, 포화지방:** 0.5g 미만은 "0" 표시 가능.
   - **트랜스지방:** **0.2g 미만**은 "0" 표시 가능. (0.5g 아님!)
   - **콜레스테롤:** **2mg 미만**은 "0" 표시 가능. (단위 mg, 0.5g 아님!)
   - **나트륨:** 5mg 미만은 "0" 표시 가능.
   - 이 기준에 해당하면 환산 실측값이 0이 아니더라도 시안에 "0"으로 표기한 것은 합법(✅)입니다. 단, 이 수치를 초과했는데 "0"으로 적었다면 명백한 불법(🚨)입니다.
...
🔥 **Rule 47. [선물용 포장(외포장/아웃박스) 100% 일치성 전수 검증]**
   - 선물세트 모드로 검토 시, 내포장(개별 팩)에 기재된 모든 정보가 외포장(선물박스)에 토씨 하나 틀리지 않고 동일하게 전이되었는지 스캔하십시오.
   - **[내용량 산식 검증]**: 외포장의 '총 내용량' 및 '총 열량'은 반드시 (단일 팩 내용량/열량 × 팩 갯수)로 한 치의 오차 없이 표기되어야 함을 계산하여 검증하십시오.

🔥 **Rule 48. [서류 역할의 완벽한 분리 (배합비 vs 한글라벨)]**
   - **배합비 서류의 역할**: 오직 원료의 '투입량 순서(내림차순)'와 '2% 미만 자유 배열'이 잘 지켜졌는지만 검증하십시오.
   - **한글라벨 서류의 역할**: 시안에 적힌 원재료 명칭, 원산지, 알레르기 문구가 정확한지 대조하는 최종 텍스트 기준입니다. 배합비와 한글라벨의 텍스트가 다르면 무조건 **한글라벨**을 기준으로 시안을 평가하십시오.

🔥 **Rule 49. [특수의료용도식품(균형영양식) 혼합제제 강제 전개 (Flattening) 합법성]**
   - 특수용도식품이나 균형영양식의 경우, 한글라벨이나 배합비 서류에 `혼합제제(A, B, C)` 또는 `비타민믹스(A, B, C)`로 묶여 있더라도, **시안에서는 이를 해체하여 `A, B, C`로 각각 흩어지게 전개(Flattening)하여 기재한 것이 완벽한 합법**입니다.

🔥 **Rule 50. [범용 '원액/100%' 명칭 기만광고 판별 룰 (효소/첨가물 스캔)]**
   - 원재료명이나 마케팅 문구에 **'원액'** 또는 **'100%'**라는 단어가 포함된 원료가 있다면, 배합비나 한글라벨을 스캔하여 해당 원료의 하위 성분에 정제수와 주원료 외의 **식품첨가물(효소제, 향료, 감미료 등)**이 혼합되어 있는지 확인하십시오.
   - 첨가물이 단 0.001%라도 섞여 있다면 '원액'이라는 표현은 부당한 표시광고(소비자 기만)입니다. 즉시 🚨부적합 처리하고, **"첨가물이 포함되었으므로 '원액'이라는 단어를 삭제하거나 '액'으로 수정하십시오."**라고 지시하십시오. 원료 납품 서류에 '원액'이라고 적혀 있어도 법적 리스크 방어를 위해 무조건 시안에서는 수정해야 합니다.
---
"""
...
def main():
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령1]: 정제수, 당류를 제외한 **'배합비 상위 3순위'가 아닌 미량 원료에 대해 원산지 누락을 지적하는 행위를 100% 엄격히 금지합니다.** (Rule 28, 29 위반 절대 금지)
        - 🚨 [긴급 차단 명령2]: Rule 50에 따라, 첨가물이 들어간 원료에 '원액' 명칭이 사용되었다면 즉시 부적합(🚨) 처리하고 '액'으로 수정 지시하십시오.
        - 🚨 [긴급 차단 명령3]: 표 작성 시 절대로 여러 성분을 뭉뚱그려 적지 마십시오. 시안에 적힌 모든 원료를 무조건 하나하나 개별 행(Row)으로 쪼개서 1:1 매칭하십시오.
        | No | 시안 원재료명 (개별 행으로 쪼갤 것) | 한글라벨 매칭 원료 (혼합제제 전개 매칭) | 배합비 순서 검증 (2% 룰) | 판정 |
        |---|---|---|---|---|
        ...
        ## 4️⃣ [영양표시 및 % 기준치 검증]
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령4]: 나쁜 영양소(지방, 포화지방, 나트륨 등)는 하한선(80%)이 없습니다! 상한선 120% 이내면 무조건 합격(✅)입니다.
        - 🚨 [긴급 차단 명령5]: 트랜스지방은 환산값이 0.2g 미만일 때 0g입니다. 콜레스테롤은 환산값이 2mg 미만일 때 0mg입니다. (Rule 23 단위 철저 준수)
        - 🚨 반드시 아래 표 양식을 유지하여 모든 영양성분을 대조하십시오.
        | 영양성분명 | 성적서 실측값 | 환산 실측값 | 시안 표시량 | 법적 허용오차 범위 (시안표시량 기준) | 1일 기준치 | 시안 % | % 검증 (계산식) | 판정 |
        |---|---|---|---|---|---|---|---|---|
        ...
        - 결론: (✅ 또는 🚨)
        - 내용: (단품 모드 시 "해당 없음". 선물세트 모드 시: 구역별로 정밀 대조하여 보고. 불일치 지적 시 증거 텍스트 필수 기재.)
