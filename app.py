...
🔥 **Rule 38. [교차오염 경고 상호 배타성 원칙]**
   - 원재료로 이미 투입된 성분을 '제조시설 공유(교차오염)' 주의사항에 중복 기재하면 부적합.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V95.0 - 수량 충돌 스나이퍼 로직 탑재)")
    ...
    with tab1:
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("주표시면 텍스트 추출 및 정밀 대조 중..."):
                ...
                3. 🚨 [수량/오탈자 교차 검증 절대 명령]: 주표시면에 기재된 포장 단위(예: N입, 총 용량) 숫자를 정확히 추출하고, 업로드된 다른 면(정보표시면 하단의 '식품이력추적관리번호' 주변 N입 등)의 수량 정보와 논리적으로 100% 일치하는지 비교하십시오. 띄어쓰기 오류도 샅샅이 스캔하십시오.
                </thinking>
                ...
                - 🚨 포장 단위(수량) 논리 충돌 여부: (예: 주표시면은 30입이나 정보표시면 식품이력추적관리번호 옆에는 16입으로 충돌함 등 명시)
            ...
            with st.spinner("서류 유형 분석 및 100% 교차 검증 중입니다..."):
                ...
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 이미지 하단 구석의 '식품이력추적관리번호' 주변을 돋보기처럼 스캔하여 기재된 포장 단위(예: 16입, 30입 등)를 무조건 텍스트로 추출(필사)하십시오. 추출한 이 단위가 주표시면이나 영양성분표의 총 내용량(예: 30팩) 숫자와 논리적으로 충돌하는지 100% 대조하여 적발하십시오.
                        </thinking>
                ...
                # 🛑 [모드 B]: 개별 라벨 무더기 제공 시
                    ...
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        🚨 [필수 출력 규칙] 🚨
                        답변을 시작할 때 반드시 <thinking> 태그를 열어 사전 분석을 수행하십시오.
                        ...
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 이미지 하단 구석의 '식품이력추적관리번호' 주변을 돋보기처럼 스캔하여 기재된 포장 단위(예: 16입 등)를 무조건 추출하여 숫자를 쓴 후, 다른 면(예: 30팩)과 비교해 충돌 여부를 확인하십시오.
                        </thinking>
                        ...
                        2. 🚨 [다중 라벨 병합 분석]: 시안을 기준으로, 업로드된 여러 장의 원료 라벨들을 매칭하여 마스터 리스트를 조립하십시오.
