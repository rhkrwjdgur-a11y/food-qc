...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V348.00 - 다중레이아웃 동적(Dynamic) 전개 패치)")
        ...
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【영양성분표 수치 환산 및 교차 검증 중...】"):
                ...
                if has_any_doc:
                    ...
                    judgment_prompt_tab3 += f"## {title_prefix} [영양표시 오차 검증 매트릭스 (★다중 레이아웃 동적 확장★)]\n"
                    judgment_prompt_tab3 += "🔥 [AI 게으름 방지 및 동적 전개 강제]: 시안의 영양정보표에 기재된 '단위(칼럼)'의 개수를 먼저 파악하라. 단품(1개)이면 표를 1개만, 1포당/총내용량당 2단 표기면 2개를, 2가지 맛 2단 표기면 4개를 생성해야 한다.\n"
                    judgment_prompt_tab3 += "아래 뼈대를 시안에 존재하는 단위(칼럼) 개수만큼 정확히 복제하여, 단 하나의 수치도 누락 없이 100% 개별 검증하라.\n\n"
                    judgment_prompt_tab3 += "### 🔹 Target 1: [맛 이름 및 단위 (예: 1팩(190ml)당, 플레인 1컵당 등 시안에 맞게 작성)]\n"
                    judgment_prompt_tab3 += "| 영양성분 | 🧪 성적서 환산값(A) | 📦 시안 표시량(B) | ⚖️ 허용오차 커트라인 | 📊 1일 기준치 % 검증 | 🎯 상세 사유 | 판정 |\n|---|---|---|---|---|---|---|\n\n"
                    judgment_prompt_tab3 += "(※ 시안에 검사해야 할 다른 단위(칼럼)가 더 있다면, 위 Target 1 형식을 Target 2, Target 3... 로 계속 복사해서 이어서 작성할 것. 더 없으면 1개로 끝낼 것.)\n\n"
