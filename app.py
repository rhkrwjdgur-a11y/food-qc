...
🔥 **Rule 21. ['고/풍부', '저', '무' 영양강조표시 엄격 컷오프 및 비중세탁 금지 룰]**
   ...
   - **['고', '풍부' 표시 기준]**: 아래 4가지 조건 중 **단 하나라도 충족**하면 합법(✅)입니다.
...
🔥 **Rule 23. [식약처 영양성분 '0' 표시 및 반올림 예외 절대 규정]**
   ...
   - 🥛 **[★냉장 흰우유 유통기한 예외 유예 규정]**: 대한민국 식품법상 소비기한 표시가 의무화되었으나, **'냉장 유통되는 살균 흰 우유(시유) 제품'에 한해서는 품질 수명 안정화를 위해 유통기한 표기가 2031년 12월 31일까지 법적으로 완벽 허용(✅합법)**됨.
...
🔥 **Rule 78. [특수의료용도식품 타겟 광고 문구 합법성 검증]**
   - 제품의 [식품유형]이 '특수의료용도식품'이거나 '환자식'으로 기재된 경우, 질환자를 타겟으로 한 영양공급 강조 문구는 무조건 합법(✅).
...
🔥 **Rule 87. [특정균(유산균/비피더스 등) 강조 표시 및 균수 분리 기재 합법성 룰]**
   ...
   - 단, **정보표시면(뒷면)**의 원재료명 란 또는 주의사항 근처에는 반드시 **구체적인 함유균수(예: 비피더스균 OOO CFU/g 이상)**를 명확히 특정하여 분리 기재해야 함.
"""
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V310.55 - 절대 서식 강제 패치)")
    ...
    # ==========================================
    # 🔥 3-Pass 파이프라인 (UI 붕괴 방지 절대 족쇄 패치)
    # ==========================================
...
🔥 [최종 출력 양식 및 절대 강제 족쇄] 🔥
아래 제시된 [출력 양식]을 단 한 줄도 삭제하거나 변형하지 말고, 100% 그대로 복사한 뒤 내용만 표의 빈칸에 채워 넣어 출력하십시오.
"안녕하세요", "PASS 1 추출 결과", "종합 검토 결과" 같은 불필요한 서론/요약/인사말을 단 한 글자라도 맨 앞이나 뒤에 출력하면 시스템 에러로 간주됩니다. 당신의 응답 첫 글자는 무조건 "## "으로 시작해야 합니다. 지정된 탭의 검토 항목 외에 다른 영역(예: 정보표시면 탭에서 영양정보나 바코드 검토)을 침범하는 월권행위를 절대 금지합니다.
