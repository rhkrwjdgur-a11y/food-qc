...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V182.0 - 범용 내/외포장 스나이퍼)")
...
- ⭐ [Rule 70 절대 강제 명령 - 의미 해석 금지!]: 팩과 박스 시안을 서로 비교할 때는 동의어 허용(Rule 35)을 절대 적용하지 마십시오! 당신은 글자의 '의미'를 모르는 픽셀 판독기입니다. 미션 A-1(박스)과 A-2(팩)의 텍스트를 글자 단위로 1:1 대조하여, **단 한 글자, 기호, 띄어쓰기라도 다르면 즉시 🚨부적합 처리**하고, 정확히 어느 원료의 표기가 다른지 모두 찾아내어 명시하십시오. 완전히 동일할 때만 ✅적합입니다.
- 검증 결과: (상세 비교 결과 및 판정)
