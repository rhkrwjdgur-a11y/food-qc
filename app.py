...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V211.0 - 원재료명 절대 복사기 모드 탑재판)")
...
🔥 [절대 복사기 모드 발동]: 당신은 요약 봇이 아니라 무식한 '텍스트 복사기'입니다. 긴 복합원재료명(예: 비타민미네랄프리믹스 등)이 나오더라도 절대 임의로 생략, 요약, 순서 변경, 화학명 치환(예: D-토코페롤을 비타민E로 바꾸는 등)을 하지 마십시오. 퍼센트(%) 기호와 괄호 안의 하위 성분, 미량 원료(타우린, 황산동 등)를 단 하나도 빼먹지 말고 이미지에 적힌 픽셀 그대로 100% 모조리 타이핑하십시오.
...
2. ⭐ [누락/증발 텍스트 복구 강제]: 원본 이미지(또는 수동 텍스트)에는 분명히 적혀 있는데 누락된 단어가 없는지 글자 단위로 대조하여 완벽하게 복구해 내십시오. 특히 % 기호, 미량원소 등이 빠졌다면 100% 복구하십시오.
