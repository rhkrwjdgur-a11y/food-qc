...
# ==========================================
# 2. 통합 전문가 프롬프트 (Rule 1~31 완결판 + 산식 오류 수정)
# ==========================================
SYSTEM_PROMPT = """
...
---
# [식품 패키지 표시사항 QC 자동화 검수 시스템 룰북 3.2]
...
🔥 **Rule 11. 영양정보 팩트 체크 및 허용오차 계산 절대 법칙**
   ...
   - **[🚨허용오차 계산 공식 엄수]**: 비타민, 무기질 등의 영양성분 허용오차 비율(%)을 계산할 때는 반드시 **(실측값 ÷ 표시량) × 100** 공식을 사용하십시오. 분모와 분자를 거꾸로 하여 (표시량 ÷ 실측값)으로 계산하는 오류를 절대 범하지 마십시오. (예: 인 표기량 117.65mg, 성적서 실측값 147.88mg인 경우 (147.88 ÷ 117.65) × 100 = 125.7% -> 80% 이상이므로 합격 처리)
...
def main():
    ...
    if st.button("🔍 QC 정밀 진단 시작", type="primary"):
        ...
        with st.spinner("업로드된 자료를 분석하여 맞춤형 QC를 진행합니다..."):
            ...
            # 🔥 산식 오류 방지를 위해 프롬프트 마지막에 한 번 더 강조(이중 장금 장치)
            final_prompt = """
            업로드된 자료만 가지고 평가해라.
            ...
            [🚨치명적 오류 방지🚨]: 영양성분(무기질, 비타민 등)의 허용오차 기준을 검증할 때 계산 공식은 무조건 `(성적서 실측값 ÷ 패키지 표시량) × 100` 입니다. 절대 분모/분자를 뒤집어서 계산하는 실수를 하지 마십시오.
            """
