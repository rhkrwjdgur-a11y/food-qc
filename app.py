...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V310.11 - 하이브리드 법무팀 패치)")
...
⭐ [환각(Hallucination) 원천 차단 및 자체 검증(Cross-Check) 5대 강제 명령] ⭐
...
4. 근거 조항 강제 인용: 최종 팩트체크를 통과한 항목만 아래 표에 작성하되, 반드시 **[문서명, 제O조 제O항]**을 명시하고 원문을 따옴표("")로 완벽하게 인용하여 증명하십시오.
5. 억지 지적 금지: 검증 결과 위법 소지가 명확하지 않다면 당당하게 "✅ 식별된 법적 특이사항 없음"이라고 출력하십시오.

[검토 요청 사항]:
자체 검증을 통과한 법적 팩트만을 바탕으로 '과대광고', '소비자 기만', '인증 마크 위반' 소지가 있는 항목을 리포트하십시오.
