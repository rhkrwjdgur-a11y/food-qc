...
---
[⚠️ 검토 대원칙: 19대 특수 지침 (Full Version)]
...
🆕 **Rule 19. ['무당(ZERO)' vs '무가당' vs '저당' 완벽 분리 및 엄격 적용]**
   - **저당(Low)**: 100g당 당류 5g 미만(액체는 2.5g 미만). (예: 100g당 7g인데 '저당/무당' 표기 시 🚨명백한 불법)
   - **무당(Zero/무)**: 100g(또는 100ml)당 당류 0.5g 미만. (제조 과정 무관, 최종 산출값만 기준)
   - **무가당(No Added Sugar)**: 설탕, 시럽 등을 인위적으로 첨가하지 않았을 때만 사용. 단, 원물 천연 당분으로 인해 영양표의 당류 수치가 높은데 패키지에 '무가당'만 강조할 경우, 소비자가 '제로(무당)'로 오인할 수 있으므로 🚨기만광고(소비자 오인) 리스크를 반드시 지적할 것.
   - 절대 '무당'과 '무가당'의 법적 기준을 혼용하여 평가하지 말 것.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (Rule 1~19 완결판)")
    st.markdown("---")
    ...
    if st.button("🔍 QC 정밀 진단 시작", type="primary"):
        ...
        with st.spinner("업로드된 자료를 분석하여 맞춤형 QC를 진행합니다..."):
            ...
            Rule 18(영유아 일반식품)과 Rule 19(무당/무가당/저당 구분)를 특히 주의해서 평가해라.
            """
