...
def main():
    ...
    st.title("식품 표시사항 정밀 검토 시스템 (V4.0 Ultimate)")
    ...
    with tab3:
        if st.button("영양성분표 수치 자동 환산 및 대조", key="btn_nutri"):
            with st.spinner("영양성분 수치 역산 및 오차 매트릭스 검증 중..."):
                ...
                judgment_prompt_tab3 += f"## 3-{prefix_num} [사전 연산 1: 다단 표(단위) 기둥 개수 강제 파악]\n"
...
"""
    ...
    with tab5:
        if st.button("AI 법무팀 자유 스캔 시작", key="btn_law"):
            with st.spinner("AI 법률 엔진 가동 중 (패키지 전면 순차 스캔 및 법령 PDF 심층 분석)..."):
                # 💡 [5번 탭: 오버라이드 및 순차 스캔 딥다이브 패치]
                free_style_prompt = """<pre_calc>
🚨 [시스템 룰 오버라이드 (Override) 발동]: 
이 순간부터 당신은 '기계적 1:1 대조 봇'이 아닙니다. 이전에 부여된 [식품 QC 마스터 룰북 99개]의 제약에서 완전히 벗어나십시오! 당신은 창의적이고 날카로운 통찰력을 지닌 대한민국 최고의 '식품 전문 변호사'이자 '수석 심사관'입니다.

[사전 연산: 패키지 면별 순차 스캔 및 논리 구축]
(AI는 반드시 아래 4단계 순서대로 시안을 모두 훑어보며 법적 쟁점을 메모할 것)
1. 주표시면 스캔: (강조 문구, 100%, 무첨가, 제품명 등 파악) -> 법적 근거 및 모순 여부 메모
2. 정보표시면 스캔: (원재료명, 함량 등 팩트 파악) -> 주표시면의 마케팅 문구와 교차 검증 메모
3. 영양성분표 스캔: (영양강조 팩트체크용 데이터 파악) -> 메모
4. 기타면/측면 스캔: (추가 마케팅 문구, 효능 표방, 타사 비방 등 파악) -> 메모
</pre_calc>

## 5. [AI 법무팀 특별 감사 리포트]
[절대 임무]: 당신이 판단할 유일한 기준은 사용자가 업로드한 「식품등의 표시·광고에 관한 법률」, 시행령, 시행규칙 등 **법령 PDF 원문 데이터**입니다. 패키지의 모든 면(주표시면 ➔ 정보표시면 ➔ 영양성분표 ➔ 기타면)을 순서대로 짚어가며 꼼꼼하게 종합 분석하십시오. 잘된 것은 칭찬(적합 사유 명시)하고, 모순이나 위법이 있으면 철퇴를 가하십시오.

### 챕터 1. 🔍 [패키지 구역별 마케팅·법률 심층 분석]
- 패키지를 4개 구역(주표시면, 정보표시면, 영양성분표, 기타면/측면)으로 나누어, 발견된 주요 문구 및 디자인에 대해 법률 PDF에 근거하여 상세히 브리핑하십시오.
- **[합법/적합 사례]**: "이 문구는 OOO법 제O조에 따라 이렇게 적용되어야 하는데, 현재 시안에 아주 잘 적용되어 있어 적합합니다." 형태로 실무자가 안심할 수 있게 설명하십시오.
- **[기만/위법 리스크]**: "이 문구는 앞면엔 이렇게 적혀있으나 뒷면 원재료를 보면 이러하므로, 소비자 기만(클린 라벨 워싱 등)에 해당할 위험이 있어 부적합(또는 확인 요망)합니다." 형태로 논리적으로 지적하십시오.

### 챕터 2. ⚖️ [리스크 적발 및 법률 위반 사항 총정리]
- 챕터 1에서 분석한 내용 중 **수정이나 확인이 필요한 법적 리스크(부적합 / 확인요망)**만 쏙 뽑아서 아래 마크다운 표 뼈대에 전수 조사하여 작성하십시오. 
- 🛑 [절대 명령]: 표의 행(Row)을 임의로 지우지 마십시오. 리스크가 전혀 없다면 표의 첫 줄에 '특이사항 없음'으로 당당하게 기재하십시오.
