...
# 시스템 지시어
# ==========================================
SYSTEM_PROMPT = f"""당신은 대한민국 최고의 '식품 표시사항 법규 및 품질관리(QC) 시스템'이자 '국립국어원 수준의 깐깐한 교정 전문가'입니다.
...
**Rule 24. [무당/무가당/무첨가 강조표시 연계 의무: 열량 및 감미료 병기]**
   ...
   - 🟢 **[저당 특례 및 강제 출력 멘트]**: 단, '저당' 강조표시는 법적으로 동일 시야각 내 열량 병기 의무가 없습니다. 따라서 시안에 '저당' 뱃지가 확인될 경우, 상세 사유란에 반드시 **"원래 무당, 무가당 등의 표현에는 주변에 열량 표시를 해야 하지만, 지금 같은 경우는 '저당'이므로 열량을 병기할 법적 의무는 없습니다."**라는 문구를 토시 하나 틀리지 않고 그대로 기재하여 합법(✅) 사유를 명확히 밝히십시오.
   - [감미료 함유 문구 필수 (Rule 52 연계)]: 당류를 줄인 대신 합성/천연 감미료를 사용했다면, 강조 문구 주변에 반드시 "감미료 함유" 문구를 병기해야 합니다.
...
def main():
    ...
    def run_qc_3pass(tab_rules: str, judgment_prompt: str, extract_missions_list: list = None):
        ...
        if extract_missions_list:
            ...
            # 🔥 [V4.9.4 핫픽스: 초정밀 오탈자/띄어쓰기 스캐너 탑재 유지]
            t_pass15_start = time.time()
            def run_pass15():
                p = f"[PASS 1.5 - OCR 무결점 정제]\n기계적인 OCR 노이즈(깨진 특수기호 등)만 제거하라.\n🛑[절대 금지]: 문맥을 파악하여 임의로 띄어쓰기를 교정하거나 오탈자를 고치지 마라. 이미지에 있는 오타와 잘못된 띄어쓰기를 그대로 100% 살려둬야 다음 단계에서 맞춤법 검사가 가능하다.\n[원본]\n{extracted_text_combined}"
                for _ in range(3):
            ...
            def run_pass18():
                p = f"[PASS 1.8 - 초정밀 맞춤법/띄어쓰기 스캐너]\n당신은 국립국어원 소속 깐깐한 교정 전문가다. 법적 판단은 하지 마라.\n다음 원본 텍스트를 픽셀 단위로 분석하여, 1) 명사 간 띄어쓰기 오류(예: '제품상단'->'제품 상단'), 2) 괄호 앞뒤 불필요한 공백(예: '등 )'->'등)'), 3) 단어 오탈자, 4) 어색한 주의문구(예: '냉장보관 하시거나' -> '냉장 보관하시거나')를 무자비하게 모두 찾아내어 지적하라.\n[원본]\n{extracted_text_combined}"
            ...
            st.info(f"[시스템 로그] Pass 1.5 & 1.8 (정제/초정밀 맞춤법 동시 실행) 소요시간: {t_pass15_elapsed:.1f}초")
...
🛑 [절대 명령]: Pass 1.8의 무자비한 맞춤법 스캔 결과를 바탕으로 명백한 글자 오류나 미세한 띄어쓰기 오류, 괄호 공백 등이 있다면 반드시 아래 표에 기재하십시오. 없다면 '특이사항 없음'으로 기재하십시오.
| 검토 항목 | 발견된 오류 문구 및 교정 제안 | 판정 |
|---|---|---|
| **오탈자 스캔 (명백한 글자 오류)** | | |
| **띄어쓰기 스캔 (간격 및 괄호 공백 오류)** | | |
"""
    ...
    with tab2:
        if st.button("정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("원재료 1:1 매칭 매트릭스 연산 중..."):
                ...
                7. 🛑 **[서류에 없는 고스트 원료(향료 등) 임의 합격 절대 금지]**: 시안에는 기재되어 있으나, 업로드된 서류(레시피/한글라벨)에서 찾을 수 없는 미상의 원료(예: 향료)가 발견된다면, 절대 "자체 투입" 등의 말을 지어내어 합법(✅) 처리하지 마십시오.
                   - '서류 매칭 원료' 칸에 반드시 **"⚠️ 서류 증빙 없음"**이라고 적으십시오.
                   - 판정은 무조건 **⚠️(확인 요망)**으로 처리하고, 상세 사유에 **"제출된 서류에 해당 원료 내역이 없습니다. 실제 레시피에 없는 원료가 오기재된 것인지, 혹은 서류상 '혼합제제'임에도 '향료' 등으로 임의 축약한 것인지(Rule 44 위반 위험) 실무자의 확인이 절대적으로 필요합니다."**라고 강력하게 경고하십시오.
                """
                ...
                judgment_prompt += "🛑 [절대 명령]: Pass 1.8의 무자비한 맞춤법 스캔 결과를 바탕으로 명백한 글자 오류나 미세한 띄어쓰기 오류, 괄호 공백 등이 있다면 반드시 아래 표에 기재하십시오. 없다면 '특이사항 없음'으로 기재하십시오.\n"
                judgment_prompt += "| 검토 항목 | 발견된 오류 문구 및 교정 제안 | 판정 |\n|---|---|---|\n"
                judgment_prompt += "| **오탈자 스캔 (명백한 글자 오류)** | | |\n"
                judgment_prompt += "| **띄어쓰기 스캔 (간격 및 괄호 공백 오류)** | | |\n\n"
    ...
    with tab3:
        if st.button("영양성분표 수치 자동 환산 및 대조", key="btn_nutri"):
            with st.spinner("영양성분 수치 역산 및 오차 매트릭스 검증 중..."):
                ...
                judgment_prompt_tab3 += "🛑 [절대 명령]: Pass 1.8의 무자비한 맞춤법 스캔 결과를 바탕으로 영양소 명칭 오타, 숫자/단위(g, mg, ㎍, kcal 등) 띄어쓰기 오류, 괄호 기호 누락 등이 있다면 반드시 아래 표에 기재하십시오. 없다면 '특이사항 없음'으로 기재하십시오.\n"
