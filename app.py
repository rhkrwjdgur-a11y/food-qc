...
🔥 **Rule 21. ['고/풍부', '저', '무' 영양강조표시 엄격 컷오프 검증 (4조건 완벽 방어 룰)]**
   시안에 '고', '풍부', '저', '무'가 사용된 경우 아래 명시된 9대 영양소 및 비타민/미네랄 기준을 엄격히 적용하십시오. 기계가 하나만 보고 부적합을 때리는 것을 영구 금지합니다.
   - **['고', '풍부' 표시 기준]**: 아래 4가지 조건(100g, 100mL, 100kcal, 1회섭취량) 중 **단 하나라도 충족**하면 합법(✅)입니다.
   ...
   - **['무(Zero)' 표시 기준]**:
      ...
      3) **트랜스지방**: 100g(또는 100mL)당 0.2g 미만.
      4) **포화지방/콜레스테롤**: 100g(또는 100mL)당 0.1g/2mg 미만.
   ⭐ **[부적합 시 수학적 증명 족쇄]**: 1회 섭취량이나 100mL 등 **하나의 기준만 대충 보고 🚨부적합을 내리지 마십시오.** 반드시 4가지 조건(100g, 100mL, 100kcal, 1회섭취량)의 수식을 모조리 나열하여, **4가지 모두 기준치에 미달함을 증명해야** 부적합입니다.
...
🔥 **Rule 28. [원산지 3순위 완벽 필터링 및 강제 재정렬 룰]**
   ...
   - 남은 '진짜 농수산물' 원료만으로 1, 2, 3순위를 재정렬하십시오. 4순위 이하 첨가물 원산지 누락 지적 금지.
...
🔥 **Rule 35. [🌟 범용 간략명/동의어 허용 및 N종 묶음 절대 금지]**
   - 식약처 이명, 표 5 간략명(비타민C), 내부 코드 생략은 완벽 합법.
   - 단, 일반 원료를 '비타민 3종' 등으로 숫자로 묶는 것은 🚨부적합 처리.
...
🔥 **Rule 38. [알레르기 교차오염 완벽 검증 (수학적 차집합 강제)]**
   - ⭐ **[강제 수식]**: `[교차오염 정답지] = [공장 취급 마스터] - [직접 투입 알레르기]` 수식을 도출하여 증명하십시오. 타사 납품업체 교차오염 억지 적용 금지.

🔥 **Rule 39. [동명 원료 및 식품유형 종속성 분리 룰]**
   - 명칭이 같아도 [식품유형]이 다르면(향료 vs 혼합제제) 분리 표기.
...
🔥 **Rule 52. [단순 명칭 강조 및 '함유/급원' 4조건 완벽 방어 룰]**
   시안에 '함유', '급원'이 사용되거나 특정 영양소의 단순 명칭(예: "단백질 4g")만 강조된 경우 적용합니다. 기계가 하나만 보고 부적합을 때리는 것을 영구 금지합니다.
   - **['함유', '급원', 단순 명칭 강조 표시 기준]**: 아래 4가지 조건 중 **단 하나라도 충족**하면 합법(✅)입니다.
...
🔥 **Rule 59. [CS 및 1399 신고 의무표시 3종 강제 스캔 룰 (Global Scan)]**
   - 전체 수집된 텍스트 중 패키지 어디에든 1399 등이 하나라도 존재하면 무조건 합법(✅).
...
🔥 **Rule 80. [세트포장(박스) 영양정보 레이아웃 강제]**
   - 박스 영양정보표 상단에 **`총 내용량 OOO mL (OOO mL X O개입)`** 및 **`1개(OOO mL)당`** 포맷 정확한지 확인.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V310.5 - 무결점 마스터본)")
...
- ⭐ [Rule 19, 24] '무가당' 및 당류 강조표시 정밀 검증: (시안에 '무가당' 텍스트 발견 시 절대 대충 넘어가지 마십시오! 반드시 원재료명에 인위적인 당류(설탕, 물엿, 시럽, 꿀, 과당 등)가 진짜로 없는지 팩트 체크하고, 주변에 열량(kcal)이 병기되어 있는지 확인한 후, "원재료 확인 결과 당류 첨가가 없으며 열량이 병기되어 무가당 표시가 적합함"이라고 명확히 서술하여 판정하십시오.)
- ⭐ [Rule 21, 52] 영양강조 컷오프(4대 조건) 깐깐한 교차 검증: ('고/풍부/무/저'인지 단순 '함유'인지 정확히 파악하여 Rule 21 또는 Rule 52를 적용하십시오. 부적합을 내리려면 반드시 100g, 100mL, 100kcal, 1회제공량 4가지 수식을 모조리 나열하여 전부 미달함을 증명하십시오!)
...
- ⭐ [Rule 19, 24] '무가당' 및 당류 강조표시 정밀 검증: ('무가당' 텍스트 발견 시 절대 대충 넘어가지 마십시오! 반드시 원재료명에 인위적인 당류가 없는지 확인하고, 열량 병기 여부를 체크한 뒤 "원재료 확인 결과 인위적 당류 첨가가 없어 무가당 표시가 적합함"이라고 명확히 판정하십시오.)
- ⭐ [Rule 21, 52] 영양강조 및 당류 컷오프 교차 확인: ('고/무/풍부/저'인지 단순 '함유'인지 파악하여 Rule 21 또는 Rule 52를 적용하십시오. 부적합 판정 시 반드시 100g, 100mL, 100kcal, 1회섭취량 4가지 수식을 모조리 증명하십시오!)
## 🔍 [오탈자 및 띄어쓰기 극단적 픽셀 대조]
- ⭐ 의미 추론 영구 정지: 기계적인 맞춤법 자동 교정 기능을 끄고, A 시안과 B 시안(또는 원본)의 스페이스바(공백) 개수와 위치까지 'Byte 대 Byte'로 대조하십시오. 단 하나의 공백 위치라도 다르면 즉시 🚨부적합 처리하십시오.
"""
