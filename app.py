...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V93.0 - 원료 스펙 마스터 표 생성 로직 도입)")
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 서류 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("서류 스펙 취합 및 100% 교차 검증 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    prompt = """
                    ...
                    2. 🚨 [서류 다중 라벨 병합 분석]: 시안에 기재된 원재료명을 기준으로, 사용자가 업로드한 여러 장의 원료 증빙 서류(한글라벨 등)를 매칭하십시오. 각 서류에 기재된 '식품유형', '실제 원료 제품명', '한글표시사항(하위성분 전개내용)', '원산지'를 발췌하십시오.
                    3. 하위 성분 전개 검증 로직: 특수의료용도식품이든 일반식품이든, 시안(박스 또는 팩)에서 혼합제제나 복합원재료를 '풀어쓴(전개한)' 경우, 서류(한글라벨)에 있는 하위 성분이 단 하나도 빠짐없이 정확히 기재되었는지 엄격히 대조하십시오. 박스와 팩의 표기 방식이 다르면 내/외포장 불일치(Rule 47)로 부적합 처리하십시오.
                    4. 🚨 전 구간 교차 검증 (수량/오탈자): 식품이력추적관리번호 등의 포장 단위(예: N입)를 파악하고, 다른 면(주표시면 등)의 수량 정보와 논리적으로 충돌하는지 대조하십시오. 쉼표 뒤 띄어쓰기 누락 등 문법 오류도 스캔하십시오.
                    </thinking>
                    
                    🚨 [표 작성 필수 명령] 🚨
                    1. 표의 한 행에는 무조건 1개의 원료만 기재하여 세로로 길게 전개되도록 작성하십시오. 여러 개의 원료를 쉼표(,)로 묶어서 기재하는 것을 엄격히 금지합니다.
                    
                    ## 1️⃣ [원료 스펙 마스터 취합표 (서류 기반)]
                    - 시안에 적힌 원재료명을 기준으로, 업로드된 개별 증빙 서류(한글라벨 등)를 매칭하여 엑셀 형식으로 취합한 마스터 스펙표입니다.
                    
                    | 시안 원재료명 | 증빙 서류 파일명 | 식품유형 | 원료 제품명 | 한글표시사항 (하위 전개 성분) | 원산지 |
                    |---|---|---|---|---|---|
                    | (예: 정제수) | 서류 없음 | - | - | - | - |
                    ...
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    - 위 1번 마스터 취합표의 '한글표시사항'과 '원산지' 정보를 기준으로, 박스와 팩 시안에 누락이나 불일치가 없는지 검증합니다.
                    
                    | 시안 원재료명 | 타겟(박스) 시안 표기 | 비교용(팩) 시안 표기 | 판정 |
                    |---|---|---|---|
                    | (1행 1원료 원칙 기재) | ... | ... | ... |
                    ...
                    4. 🚨 전 구간 교차 검증 (수량/오탈자): 포장 단위가 다른 면과 논리적으로 충돌하는지 100% 대조하십시오. 쉼표 뒤 띄어쓰기 누락 등도 스캔하십시오.
                    </thinking>
                    ...
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    - 위 1번 마스터 취합표의 '한글표시사항'과 '원산지' 정보를 기준으로, 시안에 누락이나 불일치가 없는지 검증합니다.
        ...
        if st.session_state["result_tab2"]:
            ...
            if thinking_match:
                thinking_log = thinking_match.group(1).strip()
                report_content = result.replace(thinking_match.group(0), "").strip()
                with st.expander("🧠 100% 필사 및 원료 스펙 취합 로그 보기"):
                    st.markdown(f"*{thinking_log}*")
                st.markdown(report_content)
            else:
                st.markdown(result)
