...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V213.0 - 시각/텍스트 환각 완전통제판)")
...
5. ⭐ [괄호 내부 축약 '(...)' 절대 금지]: 복합원재료나 혼합제제의 괄호() 안에 있는 수십 개의 하위 성분을 기계 임의로 '(...)' 기호로 축약해서 퉁치지 마십시오. 반드시 괄호 안의 모든 글자, 퍼센트(%), 화학기호를 100% 모조리 타이핑해서 복원해야 합니다.
6. ⭐ [복합 명칭 병합(환각) 및 스킵 절대 금지]: 쉼표(,)로 연결된 길고 복잡한 화학적 명칭들을 스캔할 때, 앞 단어와 뒤 단어를 제멋대로 섞어서 서류에 없는 유령 원료를 창조(병합)하거나, 중간에 있는 핵심 원료를 임의로 누락(스킵)하지 마십시오. 철저하게 쉼표(,) 단위로 끊어서 픽셀 그대로 100% 분리 추출하십시오.
...
- ⭐ 띄어쓰기 및 오탈자 적발: (글자 단위 1:1 대조. ⭐[AI 문맥 자동 보정 강제 비활성화]: '본제품은', '부유물이나침전물', '기준에의거' 처럼 픽셀 상에 띄어쓰기가 누락되어 뭉개져 있는 텍스트를 발견하면, 문맥을 파악해 알아서 띄어 읽는 환각을 엄격히 금지합니다. 띄어쓰기가 안 되어 있으면 즉시 🚨부적합(띄어쓰기 오류)으로 낱낱이 적발해 내십시오.)
"""
...
2. 영양정보표 바깥 하단이나 상단에 적힌 "총 내용량", "칼로리", "1일 영양성분 기준치에 대한..." 문구도 텍스트로 꼭 모두 추출하십시오.
   - ⭐ 강력 지시: 검토 모드가 '선물세트 박스(외포장) 교차 검토'라면, 반드시 **[타겟(박스) 영양성분표]**와 **[비교용(팩) 영양성분표]**의텍스트를 각각 별도로 분리하여 2세트를 추출하십시오! 둘 중 하나라도 누락하면 안 됩니다.
...
- ⭐ [Rule 69] 비타민 아래첨자 스나이퍼:
- 🔍 [전 구역 공통] 오탈자 및 띄어쓰기 정밀 스캔: (표 내 오타 여부. ⭐[AI 문맥 자동 보정 강제 비활성화]: 픽셀 상에 띄어쓰기가 누락되어 뭉개져 있는 텍스트를 발견하면, 문맥을 파악해 알아서 띄어 읽는 환각을 엄격히 금지합니다. 띄어쓰기가 안 되어 있으면 즉시 🚨부적합으로 적발하십시오.)
"""
                else:
                    judgment_prompt = base_tab3_warning + """
## 4️⃣ [영양표시 및 % 기준치 검증]
