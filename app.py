...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V206.0 - 영양성분 100% 매핑 강제판)")
...
⭐ [절대 원칙: 엑셀 VLOOKUP 하드 매핑]: [미션 A]에 추출된 팩/박스 시안의 영양성분 수치와 [미션 B]의 성적서 수치를 표에 작성할 때, 귀찮아하거나 임의로 생략하지 마십시오! 반드시 추출된 텍스트의 모든 수치(나트륨, 탄수화물 등 전체)를 100% 그대로 기계적으로 복사해서 표의 각 빈칸을 채워 넣어야 합니다. 눈앞에 데이터가 있는데도 '데이터 없음'이라고 우기는 환각(Hallucination)과 근무 태만을 엄격히 금지합니다.
"""
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
