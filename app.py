...
**Rule 18. [타겟 오인 명칭 금지]**
   - 일반식품임에도 제품명이나 패키지에 영유아 타겟 명칭('베베', '베이비', '아이', '키즈' 등)이나 젖병 등의 이미지를 사용하여 특수용도식품(영유아용)으로 오인·혼동하게 하는 기만행위 전면 금지 (🚨부적합).
...
def main():
    ...
    st.title("식품 표시사항 정밀 검토 시스템 (V4.3 Ultimate Master)")
    ...
    with st.sidebar:
        ...
        def get_uploaded_content():
            ...
            def upload_worker(idx, task):
                ...
                # Vision API로 이미지 사전 추출 (모든 글자를 꼼꼼하게)
    ...
    with tab3:
        if st.button("영양성분표 수치 자동 환산 및 대조", key="btn_nutri"):
            with st.spinner("영양성분 수치 역산 및 오차 매트릭스 검증 중..."):
                ...
                judgment_prompt_tab3 += f"## 3-{prefix_num} [사전 연산 1: 식약처 영양성분 표시단위 결정 프로세스 (품목별 완전 독립 적용)]\n"
                ...
                judgment_prompt_tab3 += "1. [1단계-참고량 확정] 이 맛(품목)의 식품유형과 성상(액상/호상)을 고려할 때, 1회 섭취참고량은 얼마인가? 👉 [ OO g/ml ]\n"
                judgment_prompt_tab3 += "2. [2단계-단위/참고량 충족 여부] 이 맛(품목)의 1개 단위 내용량(예: 85g)이 100g 이상이거나, 위 1단계에서 찾은 1회 섭취참고량 이상인가? 👉 [YES/NO]\n"
                judgment_prompt_tab3 += "   🚨 [절대 룰]: 만약 위 2단계가 'YES'라면, 표기 방법은 오직 '단위내용량당' 단 하나로 고정됩니다. 시안에 '총 내용량당' 등 다른 기둥이 병행 표기되어 있다면 무조건 삭제 지시(부적합)를 내리십시오.\n"
                judgment_prompt_tab3 += "3. [3단계-대용량 추적] 위 2단계가 'NO'일 경우, 총 내용량이 100g을 초과하고 1회 섭취참고량의 3배를 초과하는 대용량인가? 👉 [해당없음/YES/NO]\n"
                judgment_prompt_tab3 += "   🚨 [절대 룰]: 만약 위 3단계가 'YES'라면, 여러 기둥(100g당, 단위내용량당, 총내용량당 등)을 선택하거나 병행 표기하는 것이 합법입니다.\n"
                judgment_prompt_tab3 += "4. [최종 레이아웃 판정]: 위 품목별 흐름도 결과에 따라 본 시안의 표기 방식(1개당, 총 내용량당 등 다단 레이아웃)이 각각 완벽하게 부합하는지, 삭제해야 할 불법 기둥이 있는지 판정 사유를 상세히 적으시오.\n\n"
                ...
                judgment_prompt_tab3 += "▶ **[상황 A: 단일 기둥일 경우 (예: '1포당' 또는 '총 내용량당' 1개만 존재)]**\n"
                judgment_prompt_tab3 += "  - 타 기둥의 눈치를 보거나 총 내용량을 임의로 상상할 필요가 전혀 없습니다.\n"
                judgment_prompt_tab3 += "  - 오직 해당 단일 기둥의 환산치만 보고, 식약처 0표기 커트라인(예: 트랜스지방 0.2g 미만, 나트륨 5mg 미만 등) 미만이라면 '0' 표기는 완벽한 합법(✅)입니다.\n"
                judgment_prompt_tab3 += "▶ **[상황 B: 다단 기둥일 경우 (예: '1컵당' + '총 내용량당' 2개 이상 존재)]**\n"
                judgment_prompt_tab3 += "  - 1. 반드시 '총 내용량' 환산 실측치를 먼저 구하여 0표기 커트라인을 넘는지 확인합니다.\n"
                judgment_prompt_tab3 += "  - 2. 총 내용량당 수치가 0이 될 수 없는 상황이라면, 하위 단위(1컵당 등) 환산치가 아무리 0 커트라인 미만이더라도 절대 '0g'이나 '0mg'으로 표기할 수 없습니다. (반드시 'OO 미만' 또는 실제 수치 표기)\n"
                judgment_prompt_tab3 += "  - 3. 이 종속 법칙을 어기고 하위 기둥에 얌체처럼 '0'이라고 적었다면 🚨부적합 처리하십시오.\n\n"
                
                judgment_prompt_tab3 += "🛑 [사칙연산 강제 출력]: 각 영양소별 실측값 vs 표시량 대비 허용오차(80% 하한선, 120% 상한선) 계산식과 결과값을 이곳에 반드시 숫자로 풀어서 적으십시오.\n"
                if has_any_doc:
                ...
                judgment_prompt_tab3 += "🛑 [다단 0표기 종속 법칙 사유 강제 출력]: 다단 레이아웃에서 '단위당(예: 1컵당)' 기둥의 특정 성분이 '0' 또는 'OO 미만'으로 표기되어 있다면, 상세 사유 칸에 대충 숫자만 비교하지 마십시오. 반드시 **\"총 내용량당 수치가 0이 아니므로 다단 0표기 종속 법칙에 따라 '0'을 쓰지 못하고 'OO 미만'으로 표기한 것이 합법임\"** (또는 그 반대의 불법 사유) 형태로 법적 근거를 명확하게 소리 내어 작성하십시오.\n\n"
...
1. 검토 대상의 식품유형이 음료류인가, 우유류(축산물)인가?: [ ]
2. 시안에 1399 신고 문구가 존재하는가?: [YES / NO]
</pre_calc>
...
| **당알코올 및 식품유형별 의무 주의문구** | [Rule 7, 77] | | | |
...
1. 무가당/무첨가 기만 추적 (숨은 당류 적발): 주표시면에 '무가당', '설탕 무첨가'를 강조하면서 대체당(알룰로스 등)을 내세운 제품을 고강도로 수사하십시오. 정보표시면의 원재료명, 특히 '비타민 혼합제제' 등의 괄호 속에 부형제 용도로 몰래 들어간 '포도당시럽분말', '물엿', '덱스트린', '설탕' 등이 없는지 픽셀 단위로 스캔하십시오. 단 0.01%라도 발견된다면 "부형제라도 물리적인 당류가 포함되었으므로 무가당 표시는 명백한 부당 광고 및 기만행위"라며 법령에 근거해 강력히 철퇴를 가하십시오.
