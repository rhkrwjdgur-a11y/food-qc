...
# ==========================================
ADDITIVE_TABLE_4 = [
    # ⭐ [표 4 풀버전: 보존료, 감미료, 착색료, 산화방지제, 발색제, 표백제, 향미증진제 - 명칭+용도 병기 강제]
...
🔥 **Rule 56. [HACCP 인증 마크 제품유형별 교차 검증 스나이퍼]**
   - HACCP 마크 내부의 텍스트가 현재 검토 중인 [제품유형]과 일치하는지 반드시 대조하십시오.
   - **[공용 허용]**: "안전관리인증" 텍스트는 모든 유형에서 합법(✅).
   - **[일반 식품]**: "식품안전관리인증" 텍스트 합법(✅). (냉장 축산물 명칭 사용 시 🚨부적합)
   - ⭐ **[축산물 (우유/가공유 등)]**: "축산물안전관리인증" 텍스트 합법(✅). 만약 [식품유형]이 냉장 축산물인데 마크에 **"식품안전관리인증"**이라고 적혀 있다면 명백한 규정 위반이므로 🚨부적합 처리하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.51 - HACCP 및 질소충전 양방향 경고 패치)")
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                base_tab2_warning = "⭐ [1:1 대조 예외 절대 원칙 (Rule 35, Rule 90 범용 치환 맵핑 완벽 적용)] ⭐\n🔥 [초강력 테이블 분리 및 단답형 금지 족쇄]: 1. 원재료명 대조표를 출력할 때 **무조건 1줄(Row)에 딱 1개의 원료명만** 들어가게 쪼개십시오. 한 칸에 여러 원료를 뭉쳐서 적으면 치명적 오류입니다. 2. 사유를 '명칭 일치'처럼 성의 없게 단답형으로 적지 마십시오. 어떤 룰을 적용하여 어떻게 매칭했는지 **반드시 2~3문장 이상의 상세한 서술형**으로 답변하십시오.\n\n"
                common_tab2_prompts = ""
