...
# ==========================================
# 💡 마스터 룰북 100% 원문 완벽 보존 (Rule 21 함유 조건 및 Rule 63 위치 룰 패치 적용)
# ==========================================
RULE_BOOK_FULL = """
...
def main():
    ...
    st.markdown("<h1>Label Guard AI <span style='font-size:0.5em; color:#64748b;'>식품 패키지 무결점 검증 시스템 (V5.1 Ultimate)</span></h1>", unsafe_allow_html=True)
...
### 챕터 3. 📜 [최종 법령별 적합성 판정 (종합 결론)]
- 챕터 1, 2의 스캔 결과를 바탕으로, 제공된 주요 법령별로 최종 '적합/부적합/확인요망' 도장을 찍어 브리핑을 마무리하십시오.
- 위반 사항이 단 하나라도 있다면 '부적합' 또는 '조건부 적합(수정 요망)'으로 판정해야 합니다.

* **「식품등의 부당한 표시 또는 광고의 내용 기준」**: [✅ 적합 / 🚨 부적합 / ⚠️ 확인요망 중 택 1] (한 줄 요약 사유)
* **「식품등의 표시기준 고시」**: [✅ 적합 / 🚨 부적합 / ⚠️ 확인요망 중 택 1] (한 줄 요약 사유)
* **「농수산물의 원산지 표시 등에 관한 법률」**: [✅ 적합 / 🚨 부적합 / ⚠️ 확인요망 중 택 1] (한 줄 요약 사유)
(※ 이 외에도 스캔에 사용된 주요 법령이 있다면 추가하여 판정할 것)
"""
                cache_name = st.session_state.get("qc_cache_name")
                generation_config = genai.types.GenerationConfig(temperature=0.0, max_output_tokens=65536)
