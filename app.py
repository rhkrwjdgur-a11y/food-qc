...
🔥 **Rule 3. [주표시면 vs 영양성분표 수치(열량 및 영양성분 함량) 100% 일치 강제 룰]**
   ...
   - **[🚨함량 수치 보수적 표기 핑계 금지]**: 앞면은 200mg인데 뒷면 영양성분표는 220mg인 경우, 다르면 무조건 표시 불일치(🚨부적합)입니다. 보수적 표기(Rule 52)는 원료의 '개수(N종)'에만 적용되며 '함량/중량(mg, g)'에는 절대 적용하지 마십시오.
...
🔥 **Rule 11. [영양정보 단방향 허용오차 법칙 및 과다검출 이상치 경고 룰]**
   ...
   - **[🚨과다 검출 적합 및 ⚠️이상치 경고 (중요!)]**: 비타민 등의 실측값이 표시량보다 2배~10배 이상 높게 나오더라도 법적 상한선 제약이 없으므로 무조건 완벽한 합법(✅ 적합)입니다. 단, 실측값이 표시량의 3배(300%)를 초과할 정도로 비정상적으로 높다면 `⚠️ [실무 검토 권장]` 경고 코멘트를 덧붙이십시오.
...
🔥 **Rule 14. [첨가물 표 4, 표 5, 표 6 교차 검증 및 용도명 완벽 스나이퍼]**
   - **[🚨표 4 (명칭+용도명 병기 의무)]**: 감미료, 보존료, 산화방지제 등 표 4에 속하는 첨가물은 반드시 "명칭+용도명"으로 기재되어야 합니다. (예: 수크랄로스 ➔ 부적합 / 수크랄로스(감미료) ➔ 합법).
   - **[🚨표 5 (간략명 표기 허용 및 용도명 대체 금지)]**: 표 5에 속하는 첨가물은 반드시 정해진 명칭이나 허용된 '간략명'으로 적어야 합법입니다.
   - **[🚨표 6 (용도명 대체 100% 합법/선택)]**: 유화제, 산도조절제, 증점제, 팽창제, 영양강화제 등은 용도명만 표기해도 완벽한 합법(✅)입니다.
...
🔥 **Rule 21. [영양강조표시 4대 조건 교차 연산 및 절대 수치 강제]**
   - 영양성분 강조 시 식약처가 정한 4가지 기준 중 단 하나라도 충족하면 적합(✅)입니다.
...
🔥 **Rule 24. [무당/무가당 2대 의무 표기 조건 (감미료 & 열량 강제 병기)]**
   - 식약처 규정상 '무당(Zero)', '무가당'을 강조했을 때만 발동하는 2가지 의무입니다.
   - 1) **[🚨감미료 표기 강제 및 위치 검증]**: 감미료가 들어갔다면 "감미료 함유" 문구는 무조건 "가장 큰 폰트로 적힌 강조표시 주위"에 기재되어야 합법입니다.
   - 2) **[🚨저열량 미충족 시 열량 강제 병기]**: '무당', '무가당' 등을 강조했으나 제품 자체가 '저열량' 법적 기준을 충족하지 못하는 경우, 반드시 강조표시 주위에 '총 내용량에 해당하는 열량(예: 120 kcal)' 또는 '저열량 제품이 아님'을 표시해야 합니다. 누락 시 🚨부적합 처리하십시오.
...
✅ **Rule 32. [단순 역산에 의한 부적합 판정 금지 (열량 비율 등)]**
   - 영양성분표의 수치(탄수화물x4, 단백질x4, 지방x9)를 단순 계산하여 도출한 열량 비율과 마케팅 문구의 비율이 일치하지 않는다고 부적합 처리하지 마십시오. 반올림 규정 오차는 적합(✅)으로 인정.
...
🔥 **Rule 40. [열량 표기 및 반올림 원칙 (세트/단품 환산 모순 지적 금지)]**
   ...
   - **[🚨열량 환산 모순 지적 환각 금지]**: 단품의 열량이 5kcal 미만이어서 '0 kcal'로 적법하게 표기되었더라도, 세트의 총 열량은 실측 소수점을 합산하여 5kcal 단위로 반올림하므로 0이 아닌 숫자가 될 수 있습니다. 
...
🔥 **Rule 44. [혼합제제 3대 절대 원칙: 묶음 금지, 내부 순서 강제 검증, 해체 팁 제공]**
   ...
   - 2) **[🚨내부 순서 강제 검증]**: 혼합제제 괄호 안의 하위 성분들이 원료 규격서(배합비) 상의 **'투입량(%)이 높은 순서(내림차순)'대로 정확히 배열**되었는지 수학적으로 검증하십시오.
   - 3) **[💡공간 창출 해체 팁(Flattening) 제공]**: 합법(✅) 판정을 내리더라도 판정 사유란에 다음 멘트를 제안하십시오. ➔ *"💡QC 팁: 패키지 공간이 부족하다면 혼합제제 껍데기 명칭과 괄호를 삭제하고 알맹이 성분들만 꺼내어 2% 미만 원재료 구간에 순서 자유롭게 흩뿌리는 '해체 및 중복 병합 스킬' 적용이 가능합니다."*
...
🔥 **Rule 47. [디자인적/물리적 차이 예외 인정 룰]**
   ...
   - 단, 소비기한 표기 위치 등 용기 특성에 따른 물리적 문구 차이는 적합(✅)으로 인정합니다.
   - 박스(외포장)에 팩(내포장)에는 없는 '반품/교환처' 등이 더 추가되는 것이 정상이며 합법(✅)입니다.
...
🔥 **Rule 50. [원료사 명칭 꼼수 적발 및 원액/추출물 고형분 의무 표시 강제 룰]**
   - **[🚨고형분 누락 적발 절대 명령]**: 주표시면에 '원액', '추출물', '농축액'의 함량(%)을 강조하여 표시한 경우, **반드시 그 주변에 '고형분 함량(%)' 또는 '배합함량(%)'이 함께 기재되어 있어야 합법**입니다. (분말 제외)

🔥 **Rule 51. [데이터 1:1 매칭 및 고형분(Brix) 보수적 표기 예외]**
   - **[🚨고형분 함량 예외 인정]**: 과일 농축액 등의 **'고형분 함량(%)' 표기 시, 시안 수치가 서류 실제 스펙보다 같거나 낮게 표기된 경우 보수 표기이므로 무조건 적합(✅)** 판정하십시오.

🔥 **Rule 52. [논리적 모순 탐지 및 마케팅/원재료 N종 카운트 스나이퍼]**
   - 마케팅 문구 숫자(예: '15종')가 명시되어 있다면 배합비 서류에서 실제 항목 개수를 낱낱이 카운트하십시오.
   - **[🚨보수적 표기 인정 (N종 한정)]**: 마케팅 문구 숫자가 실제 서류 개수보다 작거나 같은 경우는 'N종 카운트'에 한하여 보수적 표기이므로 적합(✅) 처리하십시오.

🔥 **Rule 53. [제품명 연동 원료 함량 및 원산지 강제 추적 룰]**
   - 원료가 **'진짜 농수산물 원물(예: 아몬드, 우유, 쌀 등)'**인 경우, 가공품으로 투입되었더라도 반드시 정보표시면에 원물의 원산지가 기재되어야 합니다.

🔥 **Rule 54. [복수 원산지 혼합 비율 생략 합법성 검증 룰]**
   - 정보표시면 원재료명에 단일 원료에 대해 2개 이상의 국가가 병기되어 있고 **각 국가별 혼합 비율(%)이 기재되어 있지 않은 경우** 덮어놓고 적합으로 판정하지 마십시오. 반드시 🚨(확인 요망) 플래그를 띄우십시오.
...
🔥 **Rule 56. [HACCP 인증 마크 공식 텍스트 검증 (로고 환각 방지)]**
   - **[🚨합법 텍스트]**: "안전관리인증", "식품안전관리인증", "축산물안전관리인증" 모두 **완벽한 합법(✅)**입니다.
...
RULES_TAB4 = "[탭4 기타면/측면 관련 핵심 룰]\n" + get_sliced_rules([7, 15, 17, 18, 20, 22, 24, 38, 56] + COMMON_RULES)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V130.0 - 무가당 열량 표기 룰 패치)")
    ...
    # ==========================================
    # 🔥 핵심 개선: 2-Pass 파이프라인 함수 (V130.0 완성판)
    # ==========================================
...
- 🚨 [Rule 3] 주표시면 열량/수치 vs 영양성분표 수치 크로스 체크:
   1) 주표시면 추출 수치: [여기에 기재]
   2) 영양성분표 추출 수치: [여기에 기재]
   3) 판정: (위 두 수치가 단 1이라도 다르면 무조건 🚨부적합 처리하고 사유 기재)
