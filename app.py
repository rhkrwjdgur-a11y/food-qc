...
# 2. 통합 전문가 프롬프트 (Rule 1~49 무삭제 통합판)
...
## ⚠️ 검토 대원칙: 49대 특수 지침 (단 한 글자도 생략 없이 엄수할 것)
...
🔥 **Rule 11. [영양정보 허용오차 계산 절대 법칙 (기준점 오동작 방지)]**
   - **[🚨가장 흔한 치명적 오류 경고]**: 시험성적서(실측값)를 100% 기준으로 삼는 것은 불법입니다! **반드시 '시안의 표시량'을 100% 기준으로 삼고, 성적서의 실측값이 그 오차 범위 안에 들어오는지 판정하십시오.**
   - **[80% 이상 합법 그룹 (좋은 영양소)]**: 비타민, 무기질, 단백질, 탄수화물, 식이섬유.
     * 수식: **(성적서 실측값) $\ge$ (시안 표시량 $\times$ 0.8)** 이면 합격(✅). 상한선은 없습니다.
   - **[120% 미만 합법 그룹 (나쁜 영양소)]**: 열량, 당류, 지방, 포화지방, 트랜스지방, 콜레스테롤, 나트륨.
     * 수식: **(성적서 실측값) $\le$ (시안 표시량 $\times$ 1.2)** 이면 합격(✅).
   - **[100% 이상 합법 그룹]**: 타우린, 아미노산류, 콜라겐 등 임의표시 성분.
     * 수식: **(성적서 실측값) $\ge$ (시안 표시량 $\times$ 1.0)** 이면 합격(✅).
...
🔥 **Rule 49. [특수의료용도식품(균형영양식) 혼합제제 강제 전개 (Flattening) 합법성]**
   ...
   - 시안에 원료 `A`가 단독으로 기재되어 있을 때, 서류의 `혼합제제 하위성분`에 `A`가 존재한다면 무조건 "일치(✅)"로 판정하십시오. "왜 혼합제제로 묶지 않았냐"고 지적하는 행위를 엄격히 금지합니다.
---
"""
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (V6.62 - 영양정보 표 작성 강제판)")
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        # 🚨 [사용자님 요청 반영 핵심 패치] : 4번 목차 영양정보 표 강제 및 계산식 전개 명령 추가
        final_prompt = f"""
        [제품유형]: {ptype}
        [검토모드]: {imode}
        ...
        ## 2️⃣ [원재료명 및 원산지 대조 (Rule 48, 49 적용)]
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령1]: 정제수, 당류를 제외한 **'배합비 상위 3순위'가 아닌 미량 원료(MCT오일, 키토산, 각종 추출분말 등)에 대해 원산지가 누락되었다고 지적하는 행위를 100% 엄격히 금지합니다.** (Rule 28, 29 위반 절대 금지)
        - 🚨 [긴급 차단 명령2]: 시안에 개별 전개되어 있고 서류에 'BCAA' 등이 있다면 완벽한 합법(Flattening)이므로 ✅ 처리하십시오.
        ...
        ## 4️⃣ [영양표시 및 % 기준치 검증]
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령4]: 단순 텍스트 요약을 엄격히 금지합니다. 반드시 아래의 표 양식을 사용하여 환산된 성적서 실측값, 시안 표시량, 법적 허용오차 범위(Rule 11 기준), 1일 영양성분 기준치 대비 비율(%) 검증(Rule 41)을 모든 영양성분에 대해 빠짐없이 표로 전개하십시오.
        | 영양성분명 | 성적서 실측값 | 환산 실측값 | 시안 표시량 | 허용오차 범위 (Rule 11) | 1일 기준치 | 시안 % | % 검증 (계산식) | 판정 |
        |---|---|---|---|---|---|---|---|---|
    ...
    if st.button("🔍 전수 룰 QC 시작", type="primary"):
        ...
        with st.spinner(f"49대 룰북 원문 100% 적용 검증 중... [{inspection_mode}]"):
