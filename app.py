    ...
    with tab3:
        if st.button("영양성분표 수치 자동 환산 및 대조", key="btn_nutri"):
            with st.spinner("영양성분 수치 역산 및 오차 매트릭스 검증 중..."):
                ...
                1. 🛑 [영양소 표시 순서 절대 계층 룰]: 영양정보표의 나열 순서는 법적으로 엄격히 통제됩니다. 시안의 나열 순서가 아래 계층 구조와 단 한 칸이라도 다르면 🚨부적합 처리하십시오.
                   - [기본 9대]: 열량 ➔ 나트륨 ➔ 탄수화물 ➔ 당류 ➔ 지방 ➔ 트랜스지방 ➔ 포화지방 ➔ 콜레스테롤 ➔ 단백질
                   - 🌟 [추가 성분 종속 위치]: 
                     * '식이섬유', '당알코올', '알룰로스', '에리스리톨' 등은 반드시 [당류] 바로 밑이자 [지방] 바로 위에 위치해야 합니다. (단백질 밑으로 가면 🚨부적합)
                     * '비타민' 및 '무기질(칼슘, 철분 등)'은 반드시 [단백질] 아래 (표의 맨 밑)에 위치해야 합니다.
                ...
                # 🔥 [V5.3 핫픽스: 사전 연산 0번 (시각적 순서 강제 스캔) 추가]
                judgment_prompt_tab3 += f"## 3-{prefix_num} [사전 연산 0: 영양소 나열 순서 시각적 팩트체크]\n"
                judgment_prompt_tab3 += "1. 시안 이미지(영양성분표)가 **기본 표 형태이든, 줄글로 이어진 텍스트형이든, 다단 병행표기이든 상관없이** 영양소 명칭이 나열된 순서(시선 이동 순서)를 빠짐없이 적으시오: [ 예: 열량 -> 나트륨 -> 탄수화물 -> 당류 -> 식이섬유 -> 지방 -> ... -> 칼슘 ]\n"
                judgment_prompt_tab3 += "2. 위 순서 중 '식이섬유'나 '당알코올'이 '단백질' 밑에 있거나, 기본 9대 영양소의 순서가 뒤바뀌는 등 법정 서식 도안(계층 구조)을 위반한 항목이 있는가?: [ YES(위반 항목 명시) / NO ]\n\n"
