...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V197.0 - 투 트랙 완벽 분리판)")
...
(※ 만약 위 수동 입력값이 '없음'이 아니라면, 시안 이미지에서 텍스트를 읽지 말고, 무조건 이 수동 입력값을 절대 정답으로 사용하십시오.)
...
- ⭐ [Rule 62] 축산물 보관상태(냉동/냉장) 주표시면 명시 강제 점검: (증빙 서류가 없더라도 다른 시안 텍스트에서 '냉장보관' 단어가 스캔되면 제품유형을 '냉장'으로 간주하고 주표시면을 스캔할 시)
