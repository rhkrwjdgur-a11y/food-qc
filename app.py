...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V179.0 - AI 태업 방지 완결판)")
...
3. 알레르기(OO 함유) 텍스트는 원재료명 리스트에서 제외하고 따로 뽑으십시오.
...
- 업로드된 서류에서 원료 제품명, 하위 성분(100% 나열), 원산지를 표로 추출하십시오.
"""
                base_tab2_warning = """
...
🔥 [공식 보고서 작성 절대 규칙] 🔥
1. 표의 가장 맨 왼쪽 열(Column 1)은 반드시 "[미션 A-1] 타겟(박스) 시안 원재료명" 리스트를 그대로 복붙해야 합니다. 서류에만 있는 묶음 단어를 임의로 1열에 창조해 내지 마십시오.
...
- 검증 결과: (미션 A-1과 A-2를 비교하여 '구연산나트륨'과 '구연산삼나트륨'처럼 단 한 글자라도 다르면 🚨부적합 처리하고 정확한 차이점을 적발하십시오. 팩 시안 데이터가 없다는 핑계를 대지 마십시오. 추출본에 있습니다.)
