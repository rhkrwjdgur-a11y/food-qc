...
🔥 **Rule 24. [감미료 14pt 의무 표기 및 다중 강조표시 규칙]**
   ...
   - **[🚨다중 강조 시 위치 강제]: 단, 당류 강조표시(ZERO, 무당 등)가 시안에 2회 이상 반복되어 있는 경우에는, 반드시 "가장 큰 강조표시 주위"에 감미료 함유 문구를 기재해야 합니다.**
...
def main():
    ...
    with tab1:
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("주표시면 텍스트 추출 및 정밀 대조 중..."):
                ...
                <thinking>
                1. 🚨 [시각 판독 및 필사 절대 명령]: AI의 임의 추론이나 배경지식 개입을 엄격히 금지합니다. 업로드된 주표시면 이미지에 적힌 모든 큰 글씨, 마케팅 뱃지, 하단 설명 문구를 단 한 글자의 생략 없이 이미지에 보이는 그대로 100% 필사(Transcription)하여 먼저 나열하십시오.
                ...
                4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 주표시면의 포장 단위(예: N입, 총 용량)를 파악하고, 업로드된 다른 면(정보표시면 하단의 '식품이력추적관리번호' 주변 N입 등)의 수량 정보와 논리적으로 100% 일치하는지 비교하십시오.
                </thinking>
                
                ## 1️⃣ [주표시면 및 마케팅 뱃지]
                - 결론: (✅ 적합 또는 🚨 부적합/확인요망)
                - 🚨 [Rule 50] 원액/추출물 고형분 병기 및 명칭 적합성: (함량 강조 시 고형분 누락 여부 등 명시)
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 서류 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("서류 유형 분석 및 100% 교차 검증 중입니다..."):
                ...
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        <thinking>
                        1. 🚨 [시각 판독 및 필사 절대 명령]: [타겟_시안(박스)]과 [비교용_정답지_시안(팩)]의 '원재료명' 텍스트를 절대로 지어내지 말고, 이미지 픽셀에 보이는 글자 그대로 100% 필사하여 나열하십시오. 
                        2. 🚨 [과부하 방지: 통합 문서 절대 기준화]: 사용자가 업로드한 엑셀/PDF 서류를 '절대 정답지'로 간주하십시오. 
                        3. 3방향 교차 대조: 업로드된 엑셀/PDF 서류의 내용과 시안 텍스트를 1:1로 엄격하게 검증하십시오.
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 하단 구석의 '식품이력추적관리번호' 주변 수량을 추출(필사)하여 논리적 충돌을 적발하십시오.
                        </thinking>
                        
                        🚨 [표 작성 필수 명령 절대 강제 - 출력 단축(Lazy Output) 원천 봉쇄] 🚨
                        1. 표의 한 행에는 무조건 1개의 원료만 기재하여 세로로 길게 전개되도록 작성하십시오.
                        2. 🚨 [생략 절대 금지]: 대외 보고서 제출용입니다. 어떠한 경우에도 표 안에 '...', '중략', '이하 생략', '해당 없음', '이하 동문' 등의 기호나 단어로 퉁치거나 축약하는 것을 엄격히 금지합니다. 원재료가 50개든 100개든 1번부터 끝까지 물리적으로 모든 행(Row)을 100% 전부 타이핑하여 완성하십시오. 단 하나의 원료라도 건너뛰면 즉시 시스템 폐기 수준의 중대한 오류로 간주합니다.
                        ...
                        <thinking>
                        1. 🚨 [시각 판독 및 필사 절대 명령]: [타겟_시안]의 '원재료명' 텍스트를 절대로 지어내지 말고 100% 필사하십시오.
                        2. 🚨 [과부하 방지: 통합 문서 절대 기준화]: 업로드된 엑셀/PDF 서류를 '절대 정답지'로 간주하십시오.
                        3. 교차 대조: 엑셀/PDF 서류의 내용과 시안 텍스트를 1:1로 엄격하게 검증하십시오.
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 이미지 하단 구석의 수량을 추출하고 비교하십시오.
                        </thinking>
                        
                        🚨 [표 작성 필수 명령 절대 강제 - 출력 단축(Lazy Output) 원천 봉쇄] 🚨
                        1. 표의 한 행에는 무조건 1개의 원료만 기재하십시오.
                        2. 🚨 [생략 절대 금지]: 대외 보고서 제출용입니다. 어떠한 경우에도 표 안에 '...', '중략', '이하 생략', '해당 없음' 등으로 축약하는 것을 엄격히 금지합니다. 원재료가 아무리 많아도 첫 번째 원료부터 마지막 원료까지 물리적으로 모든 행(Row)을 100% 전부 타이핑하여 표를 완성하십시오. 단 하나의 항목이라도 건너뛰면 치명적 오류로 간주합니다.
                    ...
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        <thinking>
                        1. 🚨 [시각 판독 및 필사 절대 명령]: 박스와 팩 시안의 '원재료명' 텍스트를 절대로 지어내지 말고 100% 필사하십시오.
                        2. 🚨 [다중 라벨 병합 분석]: 원료 라벨들을 매칭하여 마스터 리스트를 조립하십시오.
                        3. 3방향 교차 대조: 도출된 정답 리스트를 바탕으로 박스/팩 시안과 1:1로 대조하십시오.
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 수량을 추출하고 비교해 충돌 여부를 확인하십시오.
                        </thinking>
                        
                        🚨 [표 작성 필수 명령 절대 강제 - 출력 단축(Lazy Output) 원천 봉쇄] 🚨
                        1. 표의 한 행에는 무조건 1개의 원료만 기재하십시오.
                        2. 🚨 [생략 절대 금지]: 대외 보고서 제출용입니다. 어떠한 경우에도 표 안에 '...', '중략', '이하 생략', '해당 없음' 등으로 축약하는 것을 엄격히 금지합니다. 원재료가 100개라도 1번부터 끝까지 물리적으로 모든 행(Row)을 100% 전부 타이핑하여 완성하십시오.
                        ...
                        <thinking>
                        1. 🚨 [시각 판독 및 필사 절대 명령]: 시안의 '원재료명' 텍스트를 절대로 지어내지 말고 100% 필사하십시오.
                        2. 🚨 [다중 라벨 병합 분석]: 여러 장의 원료 라벨들을 매칭하여 조립하십시오.
                        3. 교차 대조: 도출된 정답 리스트를 시안과 대조하십시오.
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 수량을 스캔하여 비교하십시오.
                        </thinking>
                        
                        🚨 [표 작성 필수 명령 절대 강제 - 출력 단축(Lazy Output) 원천 봉쇄] 🚨
                        1. 표의 한 행에는 무조건 1개의 원료만 기재하십시오.
                        2. 🚨 [생략 절대 금지]: 대외 보고서 제출용입니다. 어떠한 경우에도 표 안에 '...', '중략', '이하 생략', '해당 없음' 등으로 축약하는 것을 엄격히 금지합니다. 원재료가 아무리 많아도 첫 번째부터 마지막까지 물리적으로 모든 행(Row)을 100% 전부 타이핑하여 표를 완성하십시오. 
        ...
        if st.session_state["result_tab2"]:
            ...
            if thinking_match:
                thinking_log = thinking_match.group(1).strip()
                report_content = result.replace(thinking_match.group(0), "").strip()
                with st.expander("🧠 필사 및 서류 분기 대조 로그 보기"):
                    st.markdown(f"*{thinking_log}*")
                st.markdown(report_content)
            else:
                st.markdown(result)
        ...
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("오차 계산 및 100% 전체 영양성분 3방향 대조 렌더링 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    ...
                    🚨 [표 작성 필수 명령 절대 강제 - 출력 단축(Lazy Output) 원천 봉쇄] 🚨
                    1. 🚨 [생략 절대 금지]: 대외 보고서 제출용입니다. 어떠한 경우에도 표 안에 '...', '중략', '이하 생략', '해당 없음' 등으로 축약하는 것을 엄격히 금지합니다. 영양성분이 30개가 넘어도 첫 번째부터 마지막까지 물리적으로 모든 행(Row)을 100% 전부 타이핑하여 표를 완성하십시오.
    ...
    with tab4:
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("기타면/측면 텍스트 및 마케팅 적합성 검토 중..."):
                ...
                3. 🚨 [수량/오탈자 교차 검증 절대 명령]: 기재된 수량이 다른 모든 면과 충돌하는지 샅샅이 스캔하십시오.
                </thinking>
