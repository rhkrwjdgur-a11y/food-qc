...
# ==========================================
# 📚 2. 통합 전문가 프롬프트 (54대 룰 상세 버전 원상복구)
# ==========================================
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V18.2 - 룰북 상세 복구판)")
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        결과 도출 시, 54개 룰 전체를 검토하되 핵심 위반 사항과 주요 마크다운 표만 출력하여 시스템 과부하를 방지하십시오.
        
        <thinking>
        (이곳에 54대 룰 기반 의사결정 논리 서술)
        </thinking>
        ...
        - 🚨 [Rule 54 적용]: 복수 국가 원산지의 혼합 비율(%) 유무 검증. 비율 누락 시 확인 요망 처리.
        | No | 시안 원재료명 (개별 전개) | 한글라벨 매칭 원료 | 배합비 순서 검증 | 판정 및 수정안 |
        |---|---|---|---|---|
        
        ## 3️⃣ [서류 vs 시안 교차 검증 (알레르기 텍스트 추적)]
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령14]: '~함유' 기재 물질이 원재료에 없으면 즉시 부적합. 임의 합격 금지.
    ...
    if st.button("🔍 정밀 QC 검수 시작", type="primary"):
        ...
        with st.spinner(f"54대 상세 룰북 복구 및 정밀 교차 검증 중... [{inspection_mode}]"):
