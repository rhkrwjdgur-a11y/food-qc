...
⭐ **[내/외포장 1:1 대조 족쇄]**: 타겟(박스)의 원재료명과 비교용(팩)의 원재료명을 픽셀 단위로 대조하여 토씨 하나라도 다르면 무조건 🚨부적합 처리하십시오.
⭐ **[원산지 스나이퍼 족쇄]**: 배합비 상위 3순위(물, 당류, 첨가물 제외) 원료의 원산지 표기 여부를 검증하십시오. 단, 1순위가 98% 이상이면 1순위만, 1+2순위 합이 98% 이상이면 2순위까지만 표기해도 완벽한 합법입니다. 규정 위반 누락 시에만 🚨부적합 처리하십시오.
| 타겟(박스) 표기 원재료명 (100% 나열) | 비교용(팩) 표기 원재료명 (없으면 '제출 안 됨') | 대조 검증 결과 (픽셀 100% 일치 여부 및 ⭐상위 3순위 원산지 표기 여부⭐) | 최종 판정 |
|---|---|---|---|
...
### 🚨 [팩 시안 기준 최종 누락 스나이퍼 검증 (Anti-Join)]
...
⭐ **[원산지 스나이퍼 족쇄]**: 배합비 상위 3순위(물, 당류, 첨가물 제외) 원료의 원산지 표기 여부를 검증하십시오. 단, 1순위가 98% 이상이면 1순위만, 1+2순위 합이 98% 이상이면 2순위까지만 표기해도 완벽한 합법입니다. 규정 위반 누락 시에만 🚨부적합 처리하십시오.
| 시안 표기 원재료명 (100% 나열) | 매칭된 서류 원료명 (없으면 '제출 안 됨') | 대조 검증 결과 (명칭 대조 및 ⭐상위 3순위 원산지 표기 여부⭐) | 최종 판정 |
|---|---|---|---|
