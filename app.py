...
🔥 **Rule 3. [주표시면 vs 영양성분표 수치 100% 일치 및 총열량/식이섬유 열량 계산 강제 룰]**
   ...
   - **[⭐ 열량 계산 시 식이섬유 및 당알콜 분리 필수 적용]**: 영양성분표에 식이섬유나 당알콜이 표기되어 있다면 반드시 총 탄수화물에서 그 양을 분리하여 각 계수(식이섬유 2kcal, 일반 당알콜 2.4kcal, 에리스리톨 0kcal 등)를 적용해 계산하십시오.

✅ **Rule 4. 영양성분 실측값 허용**
   - 영양성분은 식약처 허용 오차 범위를 고려하여 시험성적서의 실측값을 시안에 그대로 반영한 경우 합법(적합)으로 인정하십시오.

🔥 **Rule 5. [복합원재료 5% 룰 및 첨가물 생략 허용]**
   - 배합비 5% 미만인 복합원재료는 법적으로 하위 성분을 전개할 의무가 없습니다. (예: 펙틴의 하위성분 생략 허용)
...
🔥 **Rule 24. [무당/무가당/설탕무첨가 2대 의무 표기 (감미료 & 열량 물리적 위치 강제)]**
   ...
   - 🚨 **[당류 원료 원천 차단]**: 포도당 등 단당류/이당류가 포함된 경우 설탕 무첨가/무가당 표시는 🚨부적합 처리하십시오.
...
🔥 **Rule 52. [영양성분 강조표시(명칭 단독 기재 및 N종) 컷오프 엄격 검증 룰]**
   - 패키지 주표시면이나 측면 등에 "비타민/미네랄 N종" 또는 "비타민C" 등 개별 영양소 명칭을 단독으로 기재/강조하는 것은 '영양성분 강조표시'입니다.
...
🔥 **Rule 59. [CS 및 1399 신고 의무표시 3종 강제 스캔 룰]**
   - 1) 소비자 상담 번호 2) 반품/교환처 3) 1399 신고 문구. 누락 시 🚨부적합. 수신자부담 문구는 법적 의무가 아닙니다.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V157.0 - 교차검증 스파이 봇 탑재)")
    ...
    def run_qc_3pass(tab_rules: str, judgment_prompt: str, extract_mission: str):
        ...
        # ⭐ [수정된 부분]: 하드코딩된 원재료명 추출을 삭제하고 extract_mission 인자를 주입
        pass1_prompt = f"""
