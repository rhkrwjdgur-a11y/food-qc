...
# ==========================================
# 📚 3. 55대 룰북 원문 (축소 금지! 풀텍스트 영구 보존판 + Rule 52 나트륨 패치)
# ==========================================
RULE_BOOK = """
...
🔥 **Rule 21. [비타민/무기질 영양강조 다중 조건 연산]**
   - 칼슘 등 영양성분 강조 시 식약처가 정한 4가지 기준 중 **단 하나라도 충족하면 적합(✅)**합니다.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V83.0 - 칼럼 다이어트 전수 표 복원)")
    ...
    with tab2:
        st.info("가시성을 극대화하기 위해, 1번부터 끝까지 단 하나의 생략 없이 '모든 항목'을 100% 3칸짜리 표로 출력합니다.")
        if st.button("▶️ 정보표시면 원재료 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("생략 없이 100% 전체 원재료 표를 그리는 중입니다... (최대 60초 소요)"):
                
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    prompt = """
                    [지시]: '선물세트 박스 교차 검토' 모드입니다. 업로드된 **[비교용 서류(PDF)]**를 기준으로 삼고, **[타겟: 박스 시안]**과 **[비교용 정답지: 팩 시안]**이 완벽히 일치하는지 교차 검증하십시오.
                    
                    🚨 [사용자 절대 명령: 전수 표 출력 & 칼럼(Column) 다이어트 적용] 🚨
                    줄글 리스트로 묶지 마십시오! 가시성을 위해 무조건 1번부터 끝까지 모든 원료를 표(Table)로 그리십시오. 단, 글자 수 초과로 뻗는 것을 막기 위해 표의 칸(칼럼) 수를 3칸으로 확 줄이고, 일치하는 항목은 글자를 반복해서 적지 마십시오.
                    
                    ## 2️⃣ [서류 기준: 박스(외포장) vs 팩(내포장) 전수 대조 결과]
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    
                    | 서류 매칭 원료 (기준) | 팩 vs 박스 교차 대조 결과 | 판정 |
                    |---|---|---|
                    | 정제수 | ✅ 양쪽 모두 100% 일치 | ✅ |
                    | A2단백원유(국산) | ✅ 양쪽 모두 100% 일치 | ✅ |
                    | 분리대두단백(외국산:미국, 싱가포르, 중국 등) | ✅ 양쪽 모두 100% 일치 | ✅ |
                    | 식물성크림 | 🚨 팩에는 있으나 박스에서 누락됨 | 🚨 |
                    | (모든 원료를 단 한 개도 생략 없이 끝까지 나열할 것) | ... | ... |
                    ...
                    |---|---|---|
                    | 정제수 | ✅ 100% 일치 | ✅ |
                    | 혼합제제(하위생략) | 🚨 시안에 유화제5종으로 묶음 표기 (Rule 44 위반) | 🚨 |
                    | (모든 원료를 단 한 개도 생략 없이 끝까지 나열할 것) | ... | ... |
    ...
    with tab3:
        st.info("영양성분이 25개가 넘어가도 뻗지 않도록, 초압축 암산과 슬림한 표(Table) 포맷으로 전 항목을 빠짐없이 렌더링합니다.")
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("초압축 수학 연산을 수행하고 100% 전체 표를 그리는 중입니다... (최대 40초 소요)"):
                
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    prompt = """
                    [지시]: '3번 탭: 영양성분표 검토' 모드입니다. 
                    
                    🚨 [시스템 셧다운 방지 초압축 암산 & 전체 표 렌더링] 🚨
                    `<thinking>` 태그 안에서는 절대 서술형 문장을 쓰지 말고 오직 `나트륨: 146.6<216(✅)` 처럼 기호로만 압축 암산하십시오. 그리고 표는 1번부터 끝까지 하나도 생략 없이 100% 전수 출력하되, 뻗지 않도록 칼럼 수를 줄이십시오.
                    
                    ## 4️⃣ [영양표시 오차 검증 및 교차 대조]
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    
                    **(🚨 Rule 11 단방향 허용오차 절대 명령 🚨): 당류, 지방 등 나쁜 영양소는 하한선 미달 시 무조건 합법! 탄수화물, 단백질 등 좋은 영양소는 상한선 초과 시 무조건 합법!**
                    **(🚨 Rule 41 % 표기 금지 룰): 열량(kcal)과 트랜스지방은 시안 % 칸에 무조건 '-' 기호를 넣어야 합법입니다.**
                    
                    | 영양성분명 | 성적서 기준 검증 | 팩 vs 박스 교차 대조 | 판정 |
                    |---|---|---|---|
                    | 나트륨 | ✅ 적합 | ✅ 100% 일치 | ✅ |
                    | 탄수화물 | ✅ 적합 | ✅ 100% 일치 | ✅ |
                    | 콜레스테롤 | 🚨 5mg 미만으로 수정 요망 | ✅ 100% 일치 | 🚨 |
                    | 열량 | ✅ 표기 대상 아님 | ✅ 100% 일치 | ✅ |
                    | (25개 넘는 모든 영양소를 생략 없이 끝까지 표로 나열할 것) | ... | ... | ... |
                    """
                else:
                    prompt = """
                    [지시]: '3번 탭: 영양성분표 검토' 모드입니다. 
                    ...
                    🚨 [시스템 셧다운 방지 초압축 암산 & 전체 표 렌더링] 🚨
                    `<thinking>` 태그 안에서는 절대 서술형 문장을 쓰지 말고 오직 기호로만 압축 암산하십시오. 그리고 표는 1번부터 끝까지 하나도 생략 없이 100% 전수 출력하십시오!
                    ...
                    |---|---|---|
                    | 나트륨 | ✅ 적합 | ✅ |
                    | 콜레스테롤 | 🚨 5mg 미만으로 수정 요망 | 🚨 |
                    | (모든 영양소를 생략 없이 끝까지 표로 나열할 것) | ... | ... |
                    """
        ...
        if st.session_state["result_tab3"]:
            ...
            if thinking_match:
                ...
                with st.expander("🧠 초압축 수학 연산 로그 보기 (Diet Thinking)"):
