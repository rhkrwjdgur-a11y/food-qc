...
# ==========================================
# 📚 3. 62대 룰북 원문 (V148.0 완결판 - Rule 60, 61 범용성 및 강제성 강화)
# ==========================================
RULE_BOOK_FULL = """
...
🔥 **Rule 60. [모든 복합원재료(페이스트/농축액 등) 원물 함량 강제 환산 및 서류 핑계 절대 금지 룰]**
   - 제품명에 특정 원물(예: 과일, 견과류, 곡물 등)을 강조했는데 해당 원료가 '복합원재료(페이스트, 추출액, 농축액, 분말 등)' 형태로 투입된 경우, 주표시면이나 원재료명에 복합원재료의 투입량(%)만 단독 표기하면 🚨부적합입니다. 반드시 시안 텍스트 자체에 괄호로 진짜 원물의 배합함량(%)을 기재하거나 환산된 최종 원물 함량(%)이 명시되어야 합법입니다.
...
🔥 **Rule 61. [모든 복합원재료 원산지 표기 범용 타겟팅 룰 (가공국 vs 원물국)]**
   - 국내에서 가공된 모든 복합원재료(페이스트, 추출액, 농축액, 유지류 등)의 원산지를 표기할 때, 타겟이 되는 원물 명칭 없이 국가명만 단독으로 괄호 안에 적으면(예: `딸기농축액(칠레산)`) 가공품 자체가 수입산이라는 의미가 되어 🚨부적합(원산지 허위표시)입니다.
   - **[✅합법 표기법]**: 반드시 `딸기농축액(딸기-칠레산)`, `아몬드페이스트(아몬드-미국산)`, `옥배유(옥수수배아-외국산)`처럼 타겟이 되는 원물 명칭을 국가명 앞에 명시해야 합니다. 특정 원료에 국한하지 말고 모든 복합원재료에 이 기준을 엄격히 적용하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V148.0 - 페이스트/원물국 범용성 강화 및 서류 핑계 방어 패치)")
...
- 🚨 [Rule 60 범용 적용] 모든 복합원재료(페이스트/농축액 등) 원물 함량 강제 환산 여부 검증 (서류 내용으로 시안 누락을 핑계 대는 것 절대 금지):
...
4. 🚨 [가공국 vs 원물국 범용 검증 (Rule 61)]: 모든 복합원재료(페이스트/농축액/추출액/유지류 등)에 대해 '원물명-국가명' 형태로 정확히 타겟팅되어 적혀있는가? (단순히 '(미국산)', '(칠레산)'이라고만 적혀있으면 무조건 🚨부적합 처리할 것)
