...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (V6.75 - 호밀 알레르기 환각 차단판)")
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령4 (호밀 알레르기 환각 금지)]: 원재료에 '호밀'이나 '호밀농축액'이 있다고 해서 알레르기 유발물질에 '밀'을 추가하라고 지적하는 행위를 엄격히 금지합니다.
        
        ## 4️⃣ [영양표시 및 % 기준치 검증]
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령5]: 나쁜 영양소는 하한선이 없고, 좋은 영양소는 상한선이 없습니다.
        - 🚨 [긴급 차단 명령6 (산수 붕괴 방지)]: **암산 금지!** 표의 '법적 허용오차 기준선' 칸에 반드시 "[시안 표시량] * 0.8 = [값] 이상" 또는 "[시안 표시량] * 1.2 = [값] 미만" 이라고 타이핑해서 계산식을 노출하십시오. 
        - 🚨 [긴급 차단 명령7 (0표시 예외구간 범용 방어)]: 콜레스테롤, 포화지방, 트랜스지방 등 영양소가 '0' 표시 기준을 초과했다면 Rule 23을 대입하여 범용적으로 정답을 제시하십시오.
