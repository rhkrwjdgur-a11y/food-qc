...
🔥 **Rule 21. [비타민/무기질 영양강조표시 다중 조건 완벽 계산 (고/풍부 vs 함유/급원)]**
   칼슘(무기질), 비타민 등을 강조할 때는 식약처의 4가지 복합 기준 중 **단 하나라도 만족하면 합법(✅)**입니다. 120mg 등의 가짜 고정 수치를 지어내지 말고 다음 4가지를 모두 계산하여 확인하십시오.
   - **[고(High)/풍부 강조 기준]**: 1) 고체 100g당 1일 기준치의 30% 이상, 2) 액체 100mL당 15% 이상, 3) 100kcal당 10% 이상, 4) 1회 섭취참고량당 30% 이상.
   - **[함유/급원 강조 기준]**: 1) 고체 100g당 15% 이상, 2) 액체 100mL당 7.5% 이상, 3) 100kcal당 5% 이상, 4) 1회 섭취참고량당 15% 이상.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (V6.77 - 영양강조 다중조건 범용판)")
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령1 (영양강조 다중조건 계산)]: 비타민/무기질(칼슘 등)의 '고/풍부' 강조를 판별할 때, 무식하게 1가지 기준만 보지 마십시오. (100g, 100mL, 100kcal, 1회 섭취참고량) 이 4가지 기준의 식약처 % 허용치를 모두 수식으로 계산해 본 뒤, 단 하나라도 만족한다면 무조건 완벽한 합법(✅) 처리하십시오.
