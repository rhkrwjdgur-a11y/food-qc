...
🔥 **Rule 11. [영양정보 허용오차 계산 절대 법칙 (하/상한선 및 역산 오류 완전 방지)]**
   ...
   - **[80% 이상 합법 그룹 (좋은 영양소)]**: 비타민, 무기질, 단백질, 탄수화물, 식이섬유. 
     * 수식: **(성적서 환산 실측값) $\ge$ (시안 표시량 $\times$ 0.8)** 이면 합법(✅)입니다. (예: 시안이 120mg이면 120*0.8=96mg. 실측값이 96mg 이상이면 180mg이든 1000mg이든 무조건 합법입니다. 상한선은 절대 없습니다.)
   - **[120% 미만 합법 그룹 (나쁜 영양소)]**: 열량, 당류, 지방, 포화지방, 트랜스지방, 콜레스테롤, 나트륨. 
     * 수식: **(성적서 환산 실측값) $\le$ (시안 표시량 $\times$ 1.2)** 이면 합법(✅)입니다. (하한선은 0입니다. 실측값이 120% 한계치 이하이기만 하면 합법입니다.)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (V6.69 - 영양오차 산수 붕괴 방지판)")
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        # 🚨 [신규 패치] 4번 표에 "법적 기준선 (계산식 표기)" 칸을 만들어 암산을 강제 금지함
        final_prompt = f"""
        [제품유형]: {ptype}
        [검토모드]: {imode}
        ...
        ## 4️⃣ [영양표시 및 % 기준치 검증]
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령3]: 나쁜 영양소는 하한선이 없고, 좋은 영양소는 상한선이 없습니다.
        - 🚨 [긴급 차단 명령4 (산수 붕괴 방지)]: **암산 금지!** 표의 '법적 허용오차 기준선' 칸에 반드시 **"[시안 표시량] * 0.8 = [값] 이상"** 또는 **"[시안 표시량] * 1.2 = [값] 미만"** 이라고 직접 타이핑해서 계산식을 노출하십시오. 성적서 실측값에 0.8을 곱하는 멍청한 짓을 절대 하지 마십시오.
        - 🚨 [긴급 차단 명령5]: 부적합(🚨) 판정 시 [판정 및 수정안] 란에 "0mg -> 5mg 미만으로 수정" 등 명확한 정답을 제시하십시오.
        | 영양성분명 | 성적서 실측값 | 환산 실측값 | 시안 표시량 | 법적 허용오차 기준선 (시안표시량 계산식 필수) | 1일 기준치 | 시안 % | % 검증 (계산식) | 판정 및 수정안 |
        |---|---|---|---|---|---|---|---|---|
