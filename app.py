...
어떠한 경우에도 텍스트를 임의로 요약하거나 `(...)`, `...`, `생략`, `이하 생략` 등의 기호나 단어를 사용하여 출력을 얼버무리지 마십시오. 
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V313.00 - 절대 생략 금지 및 무결점 융합 패치)")
        ...
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【영양성분표 수치 환산 및 교차 검증 중...】"):
                ...
                if has_any_doc:
                    ...
                    judgment_prompt_tab3 += "| 영양성분 | 성적서 환산값(A) | 시안 표시량(B) | ⚖️ 법적 커트라인 (표시량 기준 80% 또는 120%) | 판정 및 사유 (안전율 괴리율 경고 포함, 절대 생략 금지) |\n|---|---|---|---|---|\n\n"
