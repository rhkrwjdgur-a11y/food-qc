...
🔥 **Rule 11. [영양정보 허용오차 계산 절대 법칙 (하/상한선 오류 방지)]**
   - 반드시 '시안의 표시량'을 100% 기준으로 삼고 성적서 실측값을 평가하십시오.
   - **[80% 이상 합법 그룹 (좋은 영양소 - 상한선 절대 없음!)]**: 비타민, 무기질, 단백질, 탄수화물, 식이섬유. 
     * (성적서 환산 실측값) $\ge$ (시안 표시량 $\times$ 0.8) 이면 완벽한 합격(✅)입니다. 기준치를 수백 배 초과해도 합법이므로 절대 120% 상한선을 지어내어 부적합 처리하지 마십시오.
   - **[120% 미만 합법 그룹 (나쁜 영양소 - 하한선 절대 없음!)]**: 열량, 당류, 지방, 포화지방, 트랜스지방, 콜레스테롤, 나트륨. 
     * (성적서 환산 실측값) $\le$ (시안 표시량 $\times$ 1.2) 이면 완벽한 합격(✅)입니다. 최소값 0부터 120% 이내면 합격이므로 하한선을 지어내어 부적합 처리하지 마십시오.
...
🔥 **Rule 14. [표 4 의무 첨가물 용도명 병기]**
   - 「식품등의 표시기준」 [표 4]에 명시된 6가지 용도에 대해서만 용도명을 병기하라고 요구하십시오. '향료'는 '향미증진제'가 아닙니다.
...
🔥 **Rule 23. [식약처 영양성분 '0' 표시 완벽 규정 (단위 주의!)]**
   ...
   - **트랜스지방:** **0.2g 미만**은 "0" 표시 가능.
   - **콜레스테롤:** **2mg 미만**은 "0" 표시 가능.
   - **나트륨:** 5mg 미만은 "0" 표시 가능.
...
✅ **Rule 41. [% 영양소 기준치 정밀 검증]**
   - 1일 영양성분 기준치 비율(%)은 (시안 표시량 ÷ 식약처 기준치) × 100 으로 도출하여 검증하십시오.
...
🔥 **Rule 47. [선물용 포장(외포장/아웃박스) 100% 일치성 전수 검증]**
   - 선물세트 모드 시, 내포장과 외포장 정보가 100% 동일한지, '총 내용량' 산식이 정확한지 대조하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (V6.68 - 영양 상/하한선 완벽판)")
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        # 🚨 [신규 패치] 비타민/미네랄 오차범위 칸에 '~최대값'을 적지 못하도록 영구 차단
        final_prompt = f"""
        [제품유형]: {ptype}
        [검토모드]: {imode}
        ...
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령1]: 정제수, 당류를 제외한 **미량 원료에 대해 원산지 누락을 지적하는 행위를 100% 엄격히 금지합니다.** - 🚨 [긴급 차단 명령2]: 시안에 적힌 모든 원료를 무조건 하나하나 개별 행(Row)으로 쪼개서 표에 1:1 매칭하십시오. 뭉뚱그려 적지 마십시오.
        - 🚨 [긴급 차단 명령3]: 부적합(🚨) 지적 시 [판정 및 수정안] 란에 "아미노밸런스 -> 아미노엑기스로 수정" 등 명확한 정답을 제시하십시오.
        | No | 시안 원재료명 (개별 행으로 쪼갤 것) | 한글라벨 매칭 원료 (혼합제제 전개 매칭) | 배합비 순서 검증 (2% 룰) | 판정 및 수정안 |
        |---|---|---|---|---|
        ...
        ## 4️⃣ [영양표시 및 % 기준치 검증]
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령4]: 나쁜 영양소(지방, 나트륨 등)는 하한선이 없습니다. 상한선(120%)만 기재하십시오.
        - 🚨 [긴급 차단 명령5]: **비타민, 미네랄, 단백질 등 좋은 영양소의 '허용오차 범위' 칸에는 절대 물결(~)표시와 120% 상한선을 적지 마십시오. 오직 "[계산된 80% 값] 이상 (상한없음)" 이라고만 텍스트를 박아 넣고, 환산 실측값이 이를 넘으면 무조건 합격(✅) 처리하십시오.**
        - 🚨 [긴급 차단 명령6]: 부적합(🚨) 판정 시 "0mg -> 5mg 미만으로 수정" 등 계산된 올바른 표기법(수정안)을 제시하십시오. 콜레스테롤이 환산값 2mg 이상~5mg 미만이면 "5mg 미만"이 정답입니다.
