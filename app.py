...
🔥 **Rule 1. [원산지 상위 3순위 강제 표기 및 산정 제외 룰]**
   - 배합비 상위 3순위 이내의 원료는 반드시 원산지를 표기해야 합니다 (누락 시 🚨부적합).
   - 단, 정제수(물), 주정, 당류, 첨가물은 배합비율이 높아도 원산지 산정(1~3순위) 대상에서 100% 제외됩니다. 나한과추출분말 등을 임의로 첨가물로 오판하지 마십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.20 - 박스 vs 팩 영양성분 교차검증 완결판)")
...
⭐ **[원산지 스나이퍼 족쇄]**: 배합비 상위 3순위(물, 당류, 첨가물 제외) 원료의 원산지가 표기되었는지 반드시 확인하고, 누락 시 🚨부적합 처리하십시오.
...
⭐ **[원산지 스나이퍼 족쇄]**: 배합비 상위 3순위(물, 당류, 첨가물 제외) 원료의 원산지가 시안에 누락되었는지 반드시 확인하고, 누락 시 🚨부적합 처리하십시오.
...
"""
    ...
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【영양성분표 교차 검증 및 오차 연산 중...】"):
                missions = [
...
## 3️⃣-2. [영양표시 오차 검증 및 % 기준치 확인 (성적서 대조)]
(※ 성적서가 제출된 경우에만 작성, 없으면 '성적서 미제출로 실측 오차 검증 생략' 기재)
| 영양성분 | 성적서 환산값(A) | 시안 표시량(B) | 법적 기준선 (B의 80% 또는 120%) | 🎯 % 계산 검증 | 판정 및 상세 사유 (수식 증명) |
|---|---|---|---|---|---|
...
"""
                    ...
                    common_tab3_prompts = """## 3️⃣ [영양표시 오차 검증 및 % 기준치 확인 (성적서 대조)]
(※ 성적서가 제출된 경우에만 작성, 없으면 '성적서 미제출로 실측 오차 검증 생략' 기재)
