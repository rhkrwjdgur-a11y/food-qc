...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.71 - 무관용 100% 상세 출력 패치)")
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                3. [마스터 DB 100% 원본 복사 및 완전 분해]: 마스터표 작성 시 '코드 번호', '제조원', '수입원', '식품유형' 열은 지우고 4개 핵심 열만 작성하십시오. 서류의 행(Row) 개수와 형태를 100% 유지하되, '향료 5종', '영양강화제 3종', '혼합제제' 같은 묶음(그룹) 명칭은 표에서 완전히 폐기하고, 그 안에 들어있던 개별 단일 성분들을 1열(원료명)로 빼내어 독립된 항목으로 순위를 매기십시오.
                ...
                if has_any_doc:
                    ...
                    judgment_prompt += "| 서류상 단일 원료명 (그룹명 폐기 및 완전 해체) | 하위 전개 성분 | 원산지 | 배합비(%) / 순위 |\n|---|---|---|---|\n\n"
