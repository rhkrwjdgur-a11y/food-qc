...
🔥 **Rule 5. [복합원재료 5가지 컷오프 및 첨가물 생략 합법성 룰 (유권해석 기반)]**
   ...
   - 🌟 **[조건 C: 5순위 밖 첨가물 생략 면 면제]**: 식약처 유권해석(2025.07)에 의거, 복합원재료 내 하위 순위(6순위 이하)에 해당하는 식품첨가물(착색료, 감미료 등)은 완제품에 기능적 영향을 발휘하더라도 표기를 완전히 생략하는 것이 합법(✅)입니다.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.40 - 박스 모드 영양오차/수학역산 강제 픽스판)")
        ...
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("【의무표시 및 인증마크 해독 중...】"):
                missions = [
                    ...
                    "기타면/측면 이미지에서 '단백질', '비타민', '칼슘' 등 특정 영양소를 강조하는 마케팅 뱃지나 텍스트가 있는지 빠짐없이 스캔하여 추출하십시오. (단순 정보 제공이라 자의적으로 판단하여 추출을 누락하지 마십시오)."
                ]
