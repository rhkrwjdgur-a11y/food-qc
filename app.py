...
🔥 **Rule 55. [영양성분 소수점 및 반올림 강제 규정]**
   - 포화지방 5g 이상은 소수점 없이 정수로, 트랜스지방 0.2g 미만은 소수점 없이 0g으로 표시해야 합니다. 시안에 소수점이 기재되어 있다면 부적합(🚨) 처리하십시오.
"""
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V113.0 - 다중 강조표시 크기 비교 완결판)")
    ...
    with tab1:
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("주표시면 기계적 텍스트 추출 및 정밀 대조 중..."):
                ...
                - 🚨 포장 단위(수량) 2-Track 검증 (연산 오류 및 텍스트 불일치 여부): 
                - 🚨 띄어쓰기 및 오탈자 적발: 
                """
            ...
            with st.spinner("서류 유형 분석 및 창의성 배제 100% 교차 검증 중..."):
                ...
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        | 통합 서류 기준 원재료명 | 타겟(박스) 시안 표기 | 비교용(팩) 시안 표기 | 판정 |
                        |---|---|---|---|
                    ...
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        | 위 마스터 표 기준 원재료명 | 타겟(박스) 시안 표기 | 비교용(팩) 시안 표기 | 판정 |
                        |---|---|---|---|
                        | (무조건 1행 1원료 원칙, 순서대로 나열) | ... | ... | ... |
        ...
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("오차 계산 및 100% 전체 영양성분 3방향 대조 렌더링 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    ...
                    4. 오차 연산: 서류 실측값 대비 표시량이 오차 범위 내인지 계산하십시오.
                    ...
                    2. 단, 표 안에는 계산 과정 등 긴 설명을 쓰지 말고 '핵심 수치'와 '간결한 판정'만 적으십시오.
