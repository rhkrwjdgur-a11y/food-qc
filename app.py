...
# ==========================================
# 📚 3. 64대 룰북 원문 (V155.2 완결판 - 기만성 스나이퍼 & N종 CoT 패치)
# ==========================================
RULE_BOOK_FULL = """
...
🔥 **Rule 3. [주표시면 vs 영양성분표 수치 100% 일치 및 총열량/식이섬유 열량 계산 강제 룰]**
   ...
   - **[⭐ 열량 계산 시 식이섬유 및 당알콜 분리 필수 적용]**: 열량 구성비나 총 열량을 검증할 때 단순히 탄수화물 총량에 4kcal를 곱하는 구식 계산(Atwater 계수)을 금지합니다. 영양성분표에 **식이섬유**나 **당알콜**이 표기되어 있다면 반드시 총 탄수화물에서 그 양을 분리하여 계산하십시오. *(적용 공식 ➔ 식이섬유: 2kcal/g, 에리스리톨: 0kcal/g, 알룰로오스: 0kcal/g, 타가토스: 1.5kcal/g, 그 외 당알콜: 2.4kcal/g, 나머지 당질(탄수화물): 4kcal/g)*
...
🔥 **Rule 24. [무당/무가당/설탕무첨가 2대 의무 표기 (감미료 & 열량 물리적 위치 강제)]**
   ...
   - 2) **[열량 병기 위치 강제]**: 저열량 미충족 시 총 열량을 병기해야 하는데, 이 열량 텍스트가 패키지 맨 밑바닥이나 엉뚱한 곳에 떨어져 있다면 ⚠️**[실무 검토 권장]**을 띄우고, "법적 분쟁 소지를 없애기 위해 '설탕 무첨가' 뱃지 바로 주변으로 열량 텍스트를 옮기십시오"라고 지적하십시오.
...
🔥 **Rule 28. [원산지 과잉 지적 금지 및 상위 3순위 원칙]**
   - 가공식품의 원산지 표시는 배합비 상위 3순위 이내 원료에만 의무가 있습니다. 미미한 부원료는 원산지가 없어도 합법(✅)입니다.
...
🔥 **Rule 35. [범용 간략명/동의어 자동 매칭 및 통용명칭 100% 치환 절대 허용 룰]**
   - 기계적인 1:1 글자 대조를 멈추고 다음의 경우 무조건 합법(✅ 적합) 판정하십시오.
   - **[⭐ 첨가물 간략명 범용 예외 절대 원칙]**: 서류 텍스트와 다르더라도 시안 명칭이 식약처 「식품첨가물의 기준 및 규격」 고시 [표 5], [표 6]에 등재된 모든 공식 '간략명'인 경우 (예: 카복시메틸셀룰로스나트륨 ➔ 셀룰로스검, 결정셀룰로스 ➔ 결정섬유소 등 수백여 종 모두 포함) 절대 부적합 지적 금지.
   - **[⭐ 식품공전 및 업계 통용 명칭 치환]**: 서류상 원료명이 공전 명칭이나 통용되는 동의어로 적힌 경우 (예: 고과당➔기타과당, 백설탕➔설탕, 서목태➔약콩).
...
🔥 **Rule 44. [혼합제제 3대 절대 원칙]**
   - 혼합제제는 묶음 금지, 내부 내림차순 순서 강제 검증 수행.
...
🔥 **Rule 52. [논리적 모순 탐지 및 비타민/미네랄 N종 카운트 엄격 제한 룰]**
   - 패키지 측면이나 주표시면 등 '영양성분 표시서식 이외의 위치'에 "비타민/미네랄 N종"을 기재하는 것은 단순 나열이 아닌 식약처 법규상 **'영양성분 강조표시'**로 간주됩니다.
   - **[⭐ 영양강조 기준 미달 성분 카운트 제외 강제]**: N종 카운트에 포함되려면 해당 성분이 1일 영양성분 기준치의 **15% 이상 (액체의 경우 7.5% 이상)** 함유되어야 합니다.
   - 증빙 서류나 영양정보표에 해당 성분이 실재하더라도, 실측값이나 표시량이 이 기준(액체 7.5%, 고체 15%)에 미달한다면 카운트에서 반드시 제외하고 N값을 낮춰서 표기해야 합법(✅)입니다. (예: 칼륨이 7.5% 미만이면 제외). 기준치 미달 성분을 포함하여 카운트 숫자를 늘렸다면 🚨부적합(과대광고) 처리하십시오.
...
🔥 **Rule 60. [복합원재료 원물 함량 강제 환산 및 타겟 원물 스나이퍼 룰]**
   - 제품명에 강조된 원물이 '페이스트, 농축액' 등 복합원재료 형태면 반드시 괄호로 진짜 원물 배합함량(%)을 기재해야 합법. 조연 원물 함량 생략은 합법이므로 지적 금지.

🔥 **Rule 61. [모든 복합원재료 원산지 표기 범용 타겟팅 룰 (가공국 vs 원물국 분리)]**
   - 명칭이 '페이스트, 농축액, 추출액, 분말, 유지류(기름)' 등으로 끝나는 모든 가공 원료의 원산지 표기 시, 반드시 원물 명칭을 국가명 앞에 명시(예: `아몬드페이스트(아몬드-미국산)`).

🔥 **Rule 62. [축산물 주표시면 냉동/냉장 의무 표시 및 자체 추론 룰]**
   - 축산물 가공품이 냉장/냉동 제품인 경우 주표시면(앞면)에 상태를 명시해야 함.

🔥 **Rule 63. [미드팩 190mL 질소충전 표기 확인 룰]**
   - 내용량 190mL인 제품(콤비/SIG 팩 제외)은 반드시 "질소충전" 단어가 기재되어야 함. 누락 시 🚨확인 요망 플래그.

🔥 **Rule 64. [원물 기만표시(99.9% 등) 스나이퍼 룰]**
   - 패키지 주표시면(마케팅 문구)에 특정 원물명과 함께 높은 비율(예: 99.9%, 100%)이 짝지어 강조되어 있을 때, 그 비율이 실제 순수 '원물'의 투입량이 아니라 물이 섞인 '추출액, 원액, 농축액, 페이스트'의 비율이라면 🚨**무조건 부적합(기만표시)** 처리하십시오. 
   - 합법이 되려면 반드시 가장 큰 강조 텍스트 자체에 가공 형태를 명시해야 합니다.
   - *(판정 예시: "국산콩 99.9%" ➔ 🚨부적합 / "국산콩 원액두유 99.9%" ➔ ✅적합)*
"""
...
RULES_TAB1 = "[탭1 주표시면 관련 핵심 룰]\n" + get_sliced_rules([3, 9, 10, 15, 16, 17, 18, 19, 21, 24, 28, 40, 46, 47, 50, 51, 52, 53, 57, 58, 59, 60, 62, 64] + COMMON_RULES)
...
RULES_TAB4 = "[탭4 기타면/측면 관련 핵심 룰]\n" + get_sliced_rules([7, 15, 17, 18, 20, 22, 24, 38, 56, 57, 59, 64] + COMMON_RULES)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V155.2 - 기만성 스나이퍼 & N종 CoT 패치)")
...
- ⭐ [Rule 24] 무당/무가당/설탕무첨가 2대 의무 표기 적합성 (감미료 문구 위치 및 열량 강제 병기 물리적 위치 검증):
- ⭐ [Rule 52] N종 카운트 영양강조 컷오프(7.5%/15%) 정밀 타겟팅:
기계는 무지성으로 전체 개수를 세는 것을 엄격히 금지합니다. 반드시 아래 3단계를 거쳐 화면에 출력하십시오.
1) 영양정보표에 기재된 모든 비타민/미네랄 리스트를 나열하고, 괄호 안에 각각의 1일 기준치 대비 비율(%)을 적을 것.
2) 그중 액체 기준 7.5%(고체 15%)에 미달하는 항목을 찾아내어 "❌ 탈락(사유: 0.0%)" 표시를 하고 최종 카운트에서 가차 없이 제외할 것. (예: 칼륨 7.1% ➔ ❌ 탈락)
3) 최종 살아남은 항목의 개수만 합산하여 패키지의 'N종' 숫자와 대조 후 판정할 것.
- 기타 특이사항:
...
⭐ [범용 절대 원칙: 첨가물 간략명 100% 합법 인정 (Rule 35)] ⭐
배합비 서류의 명칭과 시안의 명칭이 텍스트상 다르더라도, 시안에 적힌 명칭이 식약처 고시(표5, 표6)에 등재된 '간략명'(예: 카복시메틸셀룰로스나트륨 ➔ 셀룰로스검, 결정셀룰로스 ➔ 결정섬유소 등)이라면 절대 1:1 불일치로 지적하지 말고 무조건 ✅적합(합법)으로 판정하십시오!

⭐ [순서 판정 시 절대 주의사항 (Rule 34)] ⭐
원재료 순서 오류를 지적하기 전에, 반드시 해당 원료의 배합비율(%)을 먼저 확인하십시오. 배합비율이 2% 미만인 원료들끼리는 기재 순서가 서로 뒤바뀌어 있어도 Rule 34에 의거하여 무조건 합법(✅ 적합)으로 판정해야 강제하며, 절대 부적합 사유로 지적하지 마십시오.
