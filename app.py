...
**Rule 21. ['고/풍부', '함유/급원', '저', '무' 영양강조표시 4대 조건 OR 법칙]**
...
**Rule 89. [국내 제조 가공품 원료의 원산지 이중 표기 규정 (농관원 유권해석)]**
   ...
   - 🛑 [유연성 룰 압살의 법칙]: 이 Rule 89가 발동되는 원료에 대해서는 Rule 35(관용명/단축 표기) 등 어떠한 유연성 룰도 절대 적용할 수 일 수 없습니다.
...
**Rule 91. ['혼합제제' 명칭 단축 표기 절대 합법성]**
   - 서류상에 '식품첨가물혼합제제'나 고유명칭(예: 비타민미네랄혼합제제)으로 기재되어 있더라도, 패키지 시안에 단순히 '혼합제제'라고만 줄여서 표기하는 실무상 완벽 합법(✅)입니다. 이를 두고 규정 위반이나 명칭 축약 오류라며 🚨부적합 처리하지 마십시오.
...
# 탭별 맞춤형 룰 타겟팅
# ==========================================
TAB1_RULES = [3, 9, 17, 18, 19, 21, 24, 46, 50, 52, 53, 57, 62, 63, 66, 68, 71, 72, 84, 86, 87, 88, 94, 95, 96, 99]
...
def main():
    ...
    st.markdown("<h1>Label Guard AI <span style='font-size:0.5em; color:#64748b;'>식품 패키지 무결점 검증 시스템 (V5.9 Ultimate)</span></h1>", unsafe_allow_html=True)
    ...
    with tab3:
        if st.button("영양성분표 수치 자동 환산 및 대조", key="btn_nutri"):
            with st.spinner("영양성분 수치 역산 및 오차 매트릭스 검증 중..."):
                ...
                # 🔥 [V5.5 핵심 패치: 상세 사유 작성 시 용량 오차와 % 비율 검증 멘트 무조건 동시 출력 강제]
