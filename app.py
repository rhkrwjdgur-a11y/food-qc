...
**Rule 12. [증빙 대조 강제 및 임의 추론 금지 (하단문구 심층분석)]**
   - 주표시면에 원료 함량(%)이나 영양성분 수치(mg, g)가 기재되어 있을 경우, 면제 룰(Rule 3 등) 적용 여부와 무관하게 **반드시 업로드된 배합비나 시험성적서와 1:1로 수치가 일치하는지 팩트체크** 하십시오.
   - 만약 증빙 서류가 없다면 절대 임의로 적합(✅) 처리하지 말고, 무조건 ⚠️(확인 요망)으로 판정하여 실무자에게 서류 대조를 지시하십시오.
   - 서류 없이 레시피를 상상하여 합격 처리하는 것을 전면 금지합니다.
...
4-1. 🛑 [하단 함량 표시 증빙 대조]: 주표시면에 구체적인 함량(%)이나 수치(mg)가 명시되어 있다면, 반드시 업로드된 배합비나 성적서 서류에서 실측값을 스캔했는지 확인하십시오. 서류가 없다면 무조건 ⚠️(확인 요망) 처리할 준비를 하십시오.
...
| **주표시면 하단 함량 표시 팩트체크 (증빙 대조 강제)** | [Rule 12, 21, 52] | (※ 앞면에 원료(%)나 영양성분 수치 기재 시 반드시 배합비/시험성적서와 대조할 것. 증빙이 없으면 절대 임의 합격 처리 금지, ⚠️ 확인 요망 판정) | |
