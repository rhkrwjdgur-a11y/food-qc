...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V94.0 - 서류 형태 분기 & 과부하 완벽 차단)")
    ...
    with st.sidebar:
        ...
        # ✅ 핵심 옵션 추가: 증빙 서류 형태에 따라 AI의 행동(마스터표 생성 유무)을 제어
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 서류 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("서류 유형 분석 및 100% 교차 검증 중입니다..."):
                ...
                # 🛑 [모드 A]: 통합 엑셀 제공 시 (마스터표 생략, 과부하 방지)
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        4. 🚨 전 구간 교차 검증: 수량 모순 및 띄어쓰기 오탈자를 스캔하십시오.
                        </thinking>
                ...
                # 🛑 [모드 B]: 개별 라벨 무더기 제공 시 (마스터표 강제 생성)
                else:
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        prompt = """
