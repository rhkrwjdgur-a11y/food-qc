...
# ==========================================
# 📚 2. 시스템 지시어 (V155.4 헌법 수정 - 우선순위 부여)
# ==========================================
...
# ==========================================
# 📚 3. 65대 룰북 원문 (V155.4 완결판 - 영양강조 단독명칭 스나이퍼 패치)
# ==========================================
RULE_BOOK_FULL = """
...
🔥 **Rule 52. [영양성분 강조표시(명칭 단독 기재 및 N종) 컷오프 엄격 검증 룰]**
   - 패키지 주표시면이나 측면 등 '영양성분 표시서식 이외의 위치'에 "비타민/미네랄 N종"이라는 그룹 명칭 또는 **"비타민C, 아연, 나이아신" 등 개별 영양소 명칭을 단독으로 기재/강조**하는 것은 모두 식약처 법규상 **'영양성분 강조표시(함유/급원)'**로 간주됩니다.
   - **[⭐ 영양강조 기준 미달 성분 적발 강제]**: 앞면에 이름이 적힌 영양성분(또는 N종 카운트에 포함된 성분)은 1일 영양성분 기준치의 **15% 이상 (액체의 경우 7.5% 이상)** 함유되어야 합니다.
   - 증빙 서류나 영양정보표를 확인하여, 기준치(액체 7.5%, 고체 15%)에 미달함에도 앞면에 명칭을 적었거나 N종 카운트에 포함했다면 무조건 🚨부적합(허위/과대광고) 처리하십시오. 서류가 없다면 🚨확인 요망을 띄워 실무자에게 7.5%/15% 컷오프를 반드시 확인하라고 지시하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V155.4 - 영양강조 컷오프 스나이퍼 패치)")
...
3. [혼합제제 전개 검증]: 영양강화제 등 혼합제제의 경우, 마스터 서류에 있는 수많은 하위 성분이 시안에 단 단 하나도 빠짐없이 전개되었는가?
