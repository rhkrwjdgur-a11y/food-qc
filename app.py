...
🔥 **Rule 35. [🌟 범용 간략명/동의어 허용 및 고도 정제/부형제 생략 보장 (강력 예외 룰)]**
   ...
   - **[🚨 'N종' 묶음 간략화 금지 및 향료 예외 (V177.0 패치)]**: 복합원재료나 혼합제제 하위 성분을 '비타민 3종', '야채농축액 5종'처럼 임의로 숫자로 묶어 기재하는 것은 명백한 원료 은폐이므로 🚨부적합입니다. **단, '향료'의 경우는 절대 예외입니다.** '향료(2종)', '합성향료(밀크향 2종)'처럼 향료에 숫자가 붙는 것은 실무적으로 통용되는 합법 표기이므로 절대 부적합으로 잡지 말고 ✅적합 처리하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V177.0 - 향료 예외 패치)")
...
1. [원재료명 추출]: 시안의 '원재료명' 텍스트를 [1. 원액두유, 2. 정제수 ...] 처럼 번호가 매겨진 세로 리스트 형태로 추출하십시오. 
   ...
   - ⭐ 강력 지시: 만약 검토 모드가 '선물세트 박스(외포장) 교차 검토'라면, **반드시 [박스 시안 원재료명 리스트]와 [팩 시안 원재료명 리스트] 2개를 각각 따로 작성**하십시오! 둘 중 하나라도 누락하면 안 됩니다.
