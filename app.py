...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V99.0 - 그룹핑 및 보수적 산정 완벽 반영)")
            ...
            with st.spinner("서류 유형 분석 및 100% 교차 검증 중입니다..."):
                ...
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        <thinking>
                        1. 🚨 [시각 판독 및 필사 절대 명령]: [타겟_시안(박스)]과 [비교용_정답지_시안(팩)]의 '원재료명' 텍스트를 절대로 지어내지 말고, 이미지 픽셀에 보이는 글자(고형분 %, 괄호 내용 등) 그대로 100% 필사하여 나열하십시오. 
                        2. 🚨 [과부하 방지: 통합 문서 절대 기준화]: 사용자가 업로드한 엑셀/PDF 서류를 '절대 정답지'로 간주하십시오. 따로 마스터 표를 생성하지 말고, 바로 대조 단계로 진입하십시오.
                        3. 3방향 교차 대조: 업로드된 엑셀/PDF 서류의 내용(전개 방식, 함량, 원산지)과 위 1번에서 필사한 박스/팩 시안 텍스트를 1:1로 엄격하게 검증하십시오.
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 이미지 하단 구석의 '식품이력추적관리번호' 주변을 돋보기처럼 스캔하여 기재된 포장 단위(예: 16입, 30입 등)를 무조건 추출(필사)하십시오. 이 단위가 주표시면/영양성분표 수량과 논리적으로 충돌하는지 100% 대조하여 적발하십시오.
                        </thinking>
                        
                        🚨 [표 작성 필수 명령 절대 강제] 🚨
                        1. 표의 한 행에는 무조건 1개의 원료만 기재하여 세로로 길게 전개되도록 작성하십시오. 여러 개의 원료를 쉼표(,)로 묶어서 기재하는 것을 엄격히 금지합니다.
                        2. 🚨 [생략 절대 금지]: 대외 보고서 제출용이므로 원료가 50개가 넘더라도 절대 중간에 "... (이하 생략)", "... (내용 동일)", "... (이하 다수 원료)" 등으로 축약하거나 생략하지 마십시오. 첫 번째 원료부터 마지막 원료까지 100% 빠짐없이 전부 표에 기재해야 합니다. 생략 시 시스템 치명적 오류로 간주합니다.
                        ...
                        2. 🚨 [과부하 방지: 통합 문서 절대 기준화]: 업로드된 엑셀/PDF 서류를 '절대 정답지'로 간주하십시오. 따로 마스터 표를 생성하지 말고 바로 대조하십시오.
                        3. 교차 대조: 엑셀/PDF 서류의 내용과 시안 텍스트를 1:1로 엄격하게 검증하십시오.
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 이미지 하단 구석의 '식품이력추적관리번호' 주변을 스캔하여 기재된 포장 단위(예: 16입 등)를 추출하고 다른 면과 비교하십시오.
                        </thinking>
                    ...
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        <thinking>
                        1. 🚨 [시각 판독 및 필사 절대 명령]: 박스와 팩 시안의 '원재료명' 텍스트를 절대로 지어내지 말고 이미지에 보이는 대로 100% 필사하십시오.
                        2. 🚨 [다중 라벨 병합 분석]: 시안에 기재된 원재료명을 기준으로, 업로드된 여러 장의 원료 라벨들을 매칭하십시오. 각 서류의 '식품유형', '원료 제품명', '하위성분 전개내용', '원산지'를 샅샅이 발췌하여 마스터 리스트를 조립하십시오.
                        3. 3방향 교차 대조: 도출된 정답 리스트를 바탕으로 박스/팩 시안과 1:1로 대조하십시오.
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 이미지 하단 구석의 '식품이력추적관리번호' 주변을 돋보기처럼 스캔하여 기재된 포장 단위(예: 16입 등)를 무조건 추출하여 숫자를 쓴 후, 다른 면과 비교해 충돌 여부를 확인하십시오.
                        </thinking>
                        ...
                        - 사용자가 업로드한 개별 라벨 무더기를 분석하여 AI가 직접 완성한 기준 스펙표입니다.
                        ...
                        ## 3️⃣ [알레르기, 주의사항 교차 검증]
                        - 결론: (✅ 적합 또는 🚨 부적합)
                        - 서류 기준, 박스와 팩의 '~함유' 알레르기 표시 일치 여부:
                        
                        ## 🔍 [전 구간 공통: 수량 모순 및 오탈자 검증]
                        - 🚨 포장 단위(수량) 논리 충돌 여부: (식품이력추적관리번호 옆 수량과 타 면의 수량 충돌 지적)
                        - 🚨 띄어쓰기 및 오탈자 적발:
                        """
                    else:
                        prompt = """
                        ...
                        <thinking>
                        1. 🚨 [시각 판독 및 필사 절대 명령]: 시안의 '원재료명' 텍스트를 절대로 지어내지 말고 이미지에 보이는 대로 100% 필사하십시오.
                        2. 🚨 [다중 라벨 병합 분석]: 시안을 기준으로, 업로드된 여러 장의 원료 라벨들을 매칭하여 '식품유형', '원료 제품명', '하위성분 전개내용', '원산지'를 발췌해 마스터 리스트를 조립하십시오.
                        3. 교차 대조: 도출된 정답 리스트를 시안과 대조하십시오.
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 이미지 하단 구석의 '식품이력추적관리번호' 주변을 스캔하여 기재된 포장 단위(예: 16입 등)를 추출하고 비교하십시오.
                        </thinking>
        ...
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("오차 계산 및 100% 전체 영양성분 3방향 대조 렌더링 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    ...
                    4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 영양성분표 상단의 '총 내용량(예: 200mL x 30팩)'을 확인하여 숫자를 기록하고, 다른 면(정보표시면 하단의 '식품이력추적관리번호' 주변 N입 등)의 수량 정보와 논리적으로 충돌(예: 30 vs 16)하는지 무조건 대조하여 적발하십시오.
                    </thinking>
                    
                    🚨 [표 작성 필수 명령 절대 강제] 🚨
                    1. 🚨 [생략 절대 금지]: 대외 보고서 제출용이므로 영양성분이 25개가 넘더라도 절대 중간에 "... (이하 생략)", "... (내용 동일)", "... (이하 다수 성분)" 등으로 축약하거나 생략하지 마십시오. 첫 번째 성분부터 마지막 성분까지 100% 빠짐없이 전부 표에 기재해야 합니다.
                    ...
                    ## 🔍 [전 구간 공통: 수량 모순 및 오탈자 검증]
                    - 🚨 포장 단위(수량) 논리 충돌 여부: (예: 영양성분표는 30팩이나 정보표시면 식품이력추적번호 옆은 16입으로 충돌함 등 명시)
                    - 🚨 띄어쓰기 및 오탈자 적발: 
                    """
                else:
                    prompt = """
                    ...
                    3. 🚨 [수량/오탈자 교차 검증 절대 명령]: 영양성분표의 총 내용량을 확인하고 정보표시면 식품이력추적관리번호 옆 수량 등과 일치하는지 비교하십시오.
                    </thinking>
                    
                    🚨 [표 작성 필수 명령 절대 강제] 🚨
                    1. 🚨 [생략 절대 금지]: 대외 보고서 제출용이므로 영양성분이 많더라도 절대 중간에 "... (이하 생략)" 등으로 축약하지 마십시오. 100% 빠짐없이 전부 표에 기재해야 합니다.
    ...
    with tab4:
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("기타면/측면 텍스트 및 마케팅 적합성 검토 중..."):
                ...
                <thinking>
                1. 🚨 [시각 판독 및 필사 절대 명령]: 기타면/측면에 적힌 설명, 주의사항, 마케팅 문구를 단 한 글자도 생략하지 말고 100% 필사하여 나열하십시오.
                2. 키워드 스캔: '무첨가', '질병 예방' 등 규정 위반 여부를 검색하십시오.
                3. 🚨 [수량/오탈자 교차 검증 절대 명령]: 기타면에 기재된 수량이나 스펙 정보가 다른 모든 면(주표시면 30입, 정보표시면 16입 등)과 비교했을 때 어느 쪽과 충돌하는지 적발하십시오. 띄어쓰기 등 오탈자를 샅샅이 스캔하십시오.
                </thinking>
