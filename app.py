...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V74.0 - 생략 금지 100% 전체 표 출력)")
    ...
    with tab2:
        ...
        if st.button("▶️ 정보표시면 원재료 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("사용자의 요청에 따라 모든 원재료를 생략 없이 100% 3단 대조 표로 그리는 중입니다... (시간 소요됨)"):
                
                if inspection_mode == "선물세트 교차 검토 (팩 vs 박스 비교)":
                    prompt = """
                    [지시]: '선물세트 교차 검토' 모드입니다. 업로드된 **[비교용 서류(한글라벨 PDF)]**를 기준으로 **[팩(내포장) 시안]**과 **[박스(외포장) 시안]**이 100% 일치하는지 3단 교차 검증 표를 작성하십시오.
                    
                    🚨 [사용자 절대 명령: 전체 표 출력 (생략 금지)] 🚨
                    **시스템 부하를 걱정하지 말고, 사용자가 직접 눈으로 확인할 수 있도록 서류에 있는 1번 원료부터 마지막 원료까지 단 한 개도 생략하거나 요약하지 말고 모두 개별 행(줄)으로 표를 꽉 채워서 그리십시오! "일치하는 항목 요약" 같은 짓을 하면 절대 안 됩니다.**
                    
                    ## 2️⃣ [서류 기준: 팩(내포장) vs 박스(외포장) 3단 정밀 대조]
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    
                    **(🚨 시안 원재료명 행 분리 절대 규칙 🚨): 표를 작성할 때 쉼표(,)를 기준으로 행을 나누되, 괄호 '(', ')', '{', '}', '[', ']' 안에 있는 쉼표는 절대 분리 기준이 될 수 없습니다!! 복합원재료는 쪼개지 마십시오.**
                    ...
                    🚨 [사용자 절대 명령: 전체 표 출력 (생략 금지)] 🚨
                    **시스템 부하를 걱정하지 말고, 사용자가 직접 눈으로 확인할 수 있도록 서류에 있는 1번 원료부터 마지막 원료까지 단 한 개도 생략하거나 요약하지 말고 모두 개별 행(줄)으로 표를 꽉 채워서 그리십시오!**
                    
                    ## 2️⃣ [원재료명 1:1 정밀 대조 및 원산지 검증]
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    
                    **(🚨 시안 원재료명 행 분리 절대 규칙 🚨): 표의 '시안 원재료명' 열을 작성할 때 쉼표(,)를 기준으로 행을 나누되, 괄호 '(', ')', '{', '}', '[', ']' 안에 있는 쉼표는 절대 분리 기준이 될 수 없습니다!! 쪼개면 시스템이 붕괴됩니다.**
