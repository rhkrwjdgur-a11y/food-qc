...
🔥 **Rule 14. [표 4 및 표 6 의무/예외 표기 마스터 룰 (숫자 묶음 표기 허용)]**
   ...
   - **[🚨향료 괄호 환각 절대 금지]**: '향료'는 6대 용도에 속하지 않습니다. '향료' 뒤에 '(착향료)'라고 괄호를 쳐서 용도명을 병기하라고 지적하는 행위를 100% 엄격히 금지합니다.
   - **[표 6 예외 표기 및 숫자 묶음 합법성]**: 구연산나트륨, 제이인산나트륨 등은 개별 명칭 대신 **'영양강화제' 또는 '산도조절제'**라는 주용도명으로 묶어서 표기하는 것이 합법입니다. 시안에 **"영양강화제 2종"**처럼 뒤에 숫자를 합성하여 묶음 표기하는 것도 실무적으로 완벽한 합법(✅)입니다.
...
🔥 **Rule 21. [비타민/무기질 영양강조표시 다중 조건 완벽 계산 (고/풍부 vs 함유/급원)]**
   칼슘(무기질), 비타민 등을 강조할 때는 식약처의 4가지 복합 기준 중 **단 하나라도 만족하면 합법(✅)**입니다. 
   - **[고(High)/풍부 강조 기준]**: 1) 고체 100g당 1일 기준치의 30% 이상, 2) 액체 100mL당 15% 이상, 3) 100kcal당 10% 이상, 4) 1회 섭취참고량당 30% 이상.
...
🔥 **Rule 30. [한국 내수용 알레르기 22종 절대 준수 및 호밀/보리 환각 차단 룰]**
   - **[🚨환각 엄격 금지]**: 식약처 규정상 **호밀, 귀리, 보리는 '밀' 알레르기 대상이 절대 아닙니다.** 시안이나 서류에 '호밀' 등이 있다고 해서 "밀 함유 문구를 추가하라"고 지적하는 오작동(환각)을 100% 엄격히 금지합니다.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (V6.78 - 내/외포장 재질 강제 대조판)")
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        # 🚨 6번 목차에 '내/외포장 재질 대조' 강제 기입 명령 추가
        final_prompt = f"""
        [제품유형]: {ptype}
        [검토모드]: {imode}
        ...
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령1 (영양강조 다중조건 계산)]: 비타민/무기질(칼슘 등)의 '고/풍부' 강조를 판별할 때 1가지 기준만 보지 마십시오. (100g, 100mL, 100kcal, 1회 섭취참고량) 이 4가지 기준의 식약처 % 허용치를 모두 수식으로 계산해 본 뒤, 단 하나라도 만족한다면 무조건 완벽한 합법(✅) 처리하십시오.
        ...
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령9 (포장재질 강제 대조)]: 선물세트 모드일 경우, **'내포장의 용기/포장재질(예: 폴리에틸렌 등)'과 '외포장의 용기/포장재질(예: 종이 등)'**이 서로 맞게 잘 적혀 있는지 반드시 스캔하여 그 결과를 이곳에 텍스트로 명시하십시오. 절대 누락하지 마십시오.
