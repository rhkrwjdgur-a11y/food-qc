...
🔥 [오탈자 무관용 및 환각 차단 원칙]: 단어의 의미가 통하더라도 글자(자음/모음)가 단 하나라도 다르면 무조건 부적합 처리하십시오. 기계의 배경지식으로 글자를 유추하여 소설을 쓰는 행위를 엄격히 금지합니다.
...
🔥 **Rule 1. [원산지 3순위 산정 제외 및 임의 분류 금지]**
   - 정제수, 주정, 당류, 그리고 모든 식품첨가물은 배합비율이 높아도 원산지 산정에서 제외됩니다.
   - 임의로 추출물 등을 첨가물로 오판하지 마십시오.

✅ **Rule 2. 향료 및 첨가물 명칭 유연화 (통합 표기 합법성)**
   - 배합비 서류에 구체적인 개별 향료명이 명시되어 있더라도, 시안 원재료명에 단순히 '향료'라고 묶어서 표기한 것은 적합입니다.

🔥 **Rule 3. [주표시면 vs 영양성분표 수치 100% 일치 강제 룰]**
   - 주표시면(앞면)에 특정 영양소 함량이 강조되어 있다면, 뒷면 영양성분표의 수치와 단 1의 오차도 없이 100% 일치해야 합니다.

✅ **Rule 4. 영양성분 실측값 허용**
   - 식약처 허용 오차 범위를 고려하여 성적서 실측값을 그대로 반영한 경우 합법(적합)입니다.

🔥 **Rule 5. [복합원재료 5% 룰 및 첨가물 생략 허용]**
   - 배합비 5% 미만인 복합원재료는 하위 성분을 전개할 의무가 없습니다.

✅ **Rule 6. 당류/시럽 필터링**
   - 원재료에 당류가 있음에도 당류가 0g 표기 시 0.5g 미만인지 검증하십시오.

🔥 **Rule 7. [당알코올 10% 컷오프 룰]**
   - 당알코올류가 최종 제품에 **10% 미만** 사용 시 주의문구 생략은 합법(✅)입니다.

✅ **Rule 8. 수입 원료 원산지 유연성 보호**
   - 특정 국가명 대신 '외국산' 표기는 적합합니다.

✅ **Rule 9. 식품유형 vs 제품명 구분**
   - 혼동되지 않도록 명확히 구분.

✅ **Rule 10. 영양성분 강조표시 (액체/고체 분리)**
   - 100g 또는 100mL 당 기준을 분리 적용.

🔥 **Rule 11. [영양정보 단방향 허용오차 법칙 (수학적 역산 절대 금지!)]**
   - **[하한선 그룹]**: `(용량 환산 실측값) >= (시안 표시량 × 0.8)` 이면 무조건 합법(✅).
   - **[상한선 그룹]**: `(용량 환산 실측값) <= (시안 표시량 × 1.2)` 이면 합법(✅).

✅ **Rule 12. [원재료명 교차 검증 및 임의 추론 금지]**
   - 배합비 서류 없이 레시피 상상 금지.

🔥 **Rule 13. [알레르기 정밀 추적 및 위치 표기 절대 규칙]**
   - 바탕색과 구분되는 '별도 란'에 기재되어 있으면 합법.

🔥 **Rule 14. [첨가물 교차 검증]**
   - 표 4: 명칭+용도명 병기. 표 5: 간략명 허용. 표 6: 용도명만 적어도 합법.
...
🔥 **Rule 20. [포장재질 표시]**
   - 종이나 유리는 텍스트 재질 표시 의무가 없으므로 생략해도 합법.

🔥 **Rule 21. ['고/풍부', '저', '무' 영양강조표시 엄격 컷오프 검증 룰 (수학 증명 강제)]**
   시안에 '고', '풍부', '저', '무' 단어가 사용된 경우 아래 명시된 9대 영양소 및 비타민/미네랄 기준만 엄격히 적용하십시오. (기계적 환각을 막기 위해 '등' 표현을 완전 배제함)
   - **['고', '풍부' 표시 기준]**: 아래 4가지 조건(100g, 100mL, 100kcal, 1회섭취량) 중 **하나라도 충족**해야 합법(✅).
      1) **단백질, 식이섬유**: 1일 기준치의 20%(100g당) / 10%(100mL당) / 10%(100kcal당) / 20%(1회섭취량당) 이상.
      2) **비타민 및 무기질**: 1일 기준치의 30%(100g당) / 15%(100mL당) / 10%(100kcal당) / 30%(1회섭취량당) 이상.
   - **['저' 표시 기준]**:
      ...
      5) **콜레스테롤**: 100g당 20mg 미만 또는 100mL당 10mg 미만.
      6) **포화지방**: 100g당 1.5g 미만 또는 100mL당 0.75g 미만.
   - **['무(Zero)' 표시 기준]**:
      1) **열량**: 100mL당 4kcal 미만.
      2) **나트륨**: 100g(또는 100mL)당 5mg 미만.
      3) **당류**: 100g(또는 100mL)당 0.5g 미만.
      4) **지방**: 100g(또는 100mL)당 0.5g 미만.
      5) **트랜스지방**: 100g(또는 100mL)당 0.2g 미만.
      6) **콜레스테롤**: 100g(또는 100mL)당 2mg 미만.
      7) **포화지방**: 100g(또는 100mL)당 0.1g 미만.
   ⭐ **[부적합 시 수학적 증명 족쇄]**: 부적합(🚨) 판정 시 반드시 4가지 조건의 수식을 나열하고 미달함을 증명하십시오.
...
🔥 **Rule 24. [당류 강조표시 연계 의무 표기 룰]**
   - 무당/저당 등 강조 시 열량 병기 의무, 감미료 함유 문구 기재 확인.
...
🔥 **Rule 28. [원산지 3순위 완벽 필터링 및 강제 재정렬 룰]**
   - 정제수, 당류, 첨가물, **유산균, 효모, 미생물류는 원산지 산정에서 100% 삭제**하십시오.
   - 남은 원료만으로 1, 2, 3순위를 재정렬하십시오. 4순위 이하 누락은 부적합 아님.

🔥 **Rule 29. [국내 가공 복합원재료 원산지 역추적 합법성]**
   - 하위 원물 원산지를 역추적해 전개 표기했다면 합법(✅).
...
🔥 **Rule 35. [🌟 범용 간략명/동의어 허용 및 N종 묶음 절대 금지]**
   - 식약처 이명(구연산삼나트륨)이나 표 5 간략명(비타민C) 표기는 100% 합법.
   - 단, 일반 원료를 '비타민 3종' 등으로 묶는 것은 🚨부적합.
...
🔥 **Rule 44. [혼합제제 전개 및 해체 병합 완벽 허용 룰]**
   - 혼합제제는 괄호를 깨고 내림차순으로 흩어지게 적어도 완벽 합법(✅).
...
🔥 **Rule 50. [원액/추출물 고형분 의무 표시 강제 룰]**
   - 앞면에 함량(%) 강조 시 반드시 주변에 '고형분 함량(%)' 병기 강제!

🔥 **Rule 51. [고형분(Brix) 보수적 표기 예외]**
   - 시안 수치가 서류 스펙보다 낮으면 합법(✅).

🔥 **Rule 52. [단순 명칭 강조 및 '함유/급원' 표시 컷오프 검증 룰 (수학 증명 강제)]**
   시안에 '함유', '급원' 단어가 사용되거나 특정 영양소의 명칭/함량을 단순히 강조(예: "단백질 4g", "칼슘 함유")한 경우 아래 1단계 기준만 엄격히 적용하십시오.
   - **['함유', '급원', 단순 명칭 강조 표시 기준]**: 아래 4가지 조건(100g, 100mL, 100kcal, 1회섭취량) 중 **하나라도 충족**해야 합법(✅).
      1) **단백질, 식이섬유**: 1일 영양성분 기준치의 10%(100g당) / 5%(100mL당) / 5%(100kcal당) / 10%(1회섭취량당) 이상.
      2) **비타민 및 무기질**: 1일 영양성분 기준치의 15%(100g당) / 7.5%(100mL당) / 5%(100kcal당) / 15%(1회섭취량당) 이상.
   ⭐ **[부적합 시 수학적 증명 족쇄]**: 1회 제공량 하나만 계산하고 부적합을 때리지 마십시오. 4가지 조건의 수식을 모조리 나열하여, 모두 미달함을 증명해야 부적합입니다.
...
🔥 **Rule 59. [CS 및 1399 신고 의무표시 3종 강제 스캔 룰 (Global Scan)]**
   - 1399 문구 등이 패키지 어디에든 하나라도 존재하면 무조건 합법(✅).

🔥 **Rule 60. [복합원재료 원물 함량 기재 면제 룰]**
   - 괄호 안에 '고형분(%)' 명시 시 배합함량 기재 강요 금지(✅).
...
🔥 **Rule 77. [범용 식품유형 필수 주의문구 강제 스캔]**
   - 냉동, 고카페인, 고체 젤리, 아스파탐 등 필수 문구 스캔 (액체류 목걸림 적용 불가).

🔥 **Rule 80. [세트포장(박스) 영양정보 레이아웃 강제]**
   - 박스 영양정보표 상단에 **`총 내용량 OOO mL (OOO mL X O개입)`** 및 **`1개(OOO mL)당`** 포맷 강제 확인(✅).

🔥 **Rule 81. [영양표시 하단 면책 문구 토시 대조]**
   - **`"1일 영양성분 기준치에 대한 비율(%)은 2,000 kcal 기준이므로 개인의 필요 열량에 따라 다를 수 있습니다."`** 토씨 100% 일치 확인(✅).
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V310.1 - 강조표시 초정밀 컷오프 마스터)")
...
- ⭐ [Rule 21, 52] 영양강조 컷오프(4대 조건) 깐깐한 교차 검증: ('고/풍부/무/저'인지 단순 '함유'인지 정확히 파악하여 Rule 21 또는 Rule 52를 적용하십시오. 부적합 시 반드시 4가지 수식을 모조리 텍스트로 써서 증명하십시오!)
## 🔍 [주표시면 전용: 오탈자 검증]
...
⭐ [강제 지시 2: 누락 역추적]: 서류(한글라벨, 배합비)에는 명백히 존재하지만, 디자이너가 시안에 아예 빼먹어서 위 표(2번 매트릭스)에 등장조차 하지 않은 원료가 있는지 샅샅이 뒤져 적발하십시오. 
...
"""
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    judgment_prompt = base_tab2_warning + common_tab2_prompts.replace("## 1️⃣ [원료 스펙 마스터 취합표]", "<!-- 마스터표 생략 -->")
                else:
...
- ⭐ [Rule 21, 52] 영양강조 및 당류 컷오프 교차 확인: ('고/무/풍부/저'인지 단순 '함유'인지 파악하여 Rule 21 또는 Rule 52를 적용하십시오. 반드시 4가지 조건의 수학적 증명 나열 필수!)
## 🔍 [기타면/측면 전용: 오탈자 검증]
