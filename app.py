...
---
# [식품 패키지 표시사항 QC 자동화 검수 시스템 룰북 6.22]
...
🔥 **Rule 24. [감미료 14pt 의무 표기]**
   - 주표시면에 "무당, 당류 무첨가, 무가당 (ZERO 포함)" 강조 시에만 14pt 주의문구 지적하십시오.
...
🔥 **Rule 28. [원산지 과잉 지적(오지랖) 절대 금지]**
   - 상위 3순위 밖 미량 원료의 원산지 표시 누락은 합법이므로 지적하지 마십시오.

🔥 **Rule 29. [국내 제조 가공품 원산지 정밀 표기]**
   - 수입산 원료를 국내에서 1차 가공한 원료는 반드시 최종제품명(기원원료명:원산지) 형태가 병기되어야 합니다.

🔥 **Rule 30. [한국 내수용 알레르기 22종 절대 준수 룰]**
   - 해외 기준(생선 전체, 나무견과류 등)을 적용하지 마십시오. 오직 한국 법정 22종(고등어, 게, 새우, 조개류 등)에 해당하는 경우에만 부적합을 때리십시오. '피쉬젤라틴'은 지적 대상에서 제외하십시오.
...
🔥 **Rule 40. [열량 5kcal 단위 반올림 절대 우선의 원칙 (Rule 11 무력화 강제)]**
   - 열량 검토 시 절대 120% 오차율을 먼저 계산하지 마십시오. 무조건 실측값을 '가장 가까운 5kcal 단위'로 반올림한 값을 먼저 구하십시오 (예: 6.954kcal ➔ 5kcal). 
   - 반올림한 실측값이 시안 표시량과 일치한다면, 계산상 120%가 넘더라도 오차 지적을 절대 금지하며 무조건 ✅적합 판정하십시오.

🔥 **Rule 41. [% 영양소 기준치 정밀 검증 (실측값 계산 절대 금지)]**
   - 오직 (시안 표시량 ÷ 기준치) × 100 으로만 계산하십시오. 성적서의 '실측값'을 사용하여 %를 역산하여 부적합 지적하는 행위 절대 금지.
...
🔥 **Rule 45. [전략적 누락 허용 및 유령 성분 검토 금지 (총내용량 환산 의무)]**
   - 성적서에는 있으나 시안 영양정보란에 아예 없는 항목(예: 비타민 B2)은 전략적 제외로 간주하여 부적합 지적 금지. 시안 수치 공란 처리.
---
"""

def main():
    st.set_page_config(page_title="식품 QC 마스터", page_icon="🏭", layout="wide")
    st.title("🏭 식품 표시사항 정밀 검토 (V6.22 - 무삭제/최강화판)")
    st.markdown("---")
    ...
    if st.button("🔍 구역별 데이터 매칭 QC 시작", type="primary"):
        ...
        with st.spinner("단 한 조각의 룰도 생략하지 않고 정밀 검수 중입니다..."):
