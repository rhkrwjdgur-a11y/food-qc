...
🔥 **Rule 28. [원산지 과잉 지적 금지]**
   - 하위 성분까지 과도하게 전개하여 원산지를 추가 요구하지 마십시오. 단, Rule 53에 해당하는 경우는 예외입니다.
...
🔥 **Rule 53. [제품명 연동 원료 함량 및 원산지 강제 추적 룰]**
   ...
   - 포함되어 있다면 다음 2가지를 강제 대조하십시오.
     1) **함량 검증:** 주표시면에 해당 원료의 함량(%)이 명시되어 있는지 확인. (누락 시 🚨 부적합)
     2) **원산지 검증:** 해당 원료가 배합비 상위 3순위 밖이더라도, 정보표시면의 원재료명 리스트에 **해당 원료의 '원산지'가 반드시 기재**되어 있어야 합니다. (누락 시 🚨 부적합)

🔥 **Rule 54. [복수 원산지 혼합 비율 생략 합법성 검증 룰 (전설의 54번 룰)]**
   - 원재료명에 단일 원료에 대해 2개 이상의 국가가 쉼표(,)로 병기되어 있고(예: 가나산, 에콰도르산), **각 국가별 혼합 비율(%)이 기재되어 있지 않은 경우** 무조건 적합(✅)으로 판정하지 마십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V18.0 - 원산지 비율 검증판)")
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        최종 7단계 리포트를 작성하기 전에, 반드시 `<thinking>` 태그를 열고 업로드된 시안과 서류를 54대 룰에 맞춰 어떻게 교차 검증했는지 당신의 '의사결정 논리'를 먼저 상세히 서술하십시오.
        
        <thinking>
        (이곳에 54대 룰과 긴급 차단 명령을 적용하여 시안과 서류를 대조하는 당신의 산술 연산 및 논리적 판단 과정을 먼저 출력할 것)
        </thinking>
        ...
        ## 2️⃣ [원재료명 및 원산지 대조 (Rule 48, 49, 50, 51, 52, 53, 54 적용)]
        - 결론: (✅ 또는 🚨)
    ...
    if st.button("🔍 정밀 QC 검수 시작", type="primary"):
        ...
        with st.spinner(f"54대 품질관리 룰셋 및 데이터 정합성 검증 중... [{inspection_mode}]"):
