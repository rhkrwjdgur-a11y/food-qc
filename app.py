...
# ==========================================
# 📚 2. 시스템 지시어 (V155.3 헌법 수정 - 우선순위 부여)
# ==========================================
...
기본적으로 철자, 띄어쓰기, 기호가 다르면 '불일치(부적합)'로 판정하되, **제공된 룰북(Rule)에 명시된 예외 조항(예: 향료 통합, 간략명/동의어 허용, 내부 코드 생략, 2% 미만 순서 유연성 등)은 이 1:1 기계적 대조 원칙보다 무조건 최우선으로 적용하여 합법(✅) 처리하십시오.**
...
# ==========================================
# 📚 3. 65대 룰북 원문 (V155.3 완결판 - 내부코드 생략 룰 추가)
# ==========================================
RULE_BOOK_FULL = """
...
🔥 **Rule 65. [원료명 내부 식별 코드(버전) 생략 합법성]**
   - 서류상 원료명 뒤에 붙은 내부 식별용 숫자, 알파벳, 기호(예: `메이플시럽혼합페이스트-2`, `딸기향(A)`)는 법적 명칭이 아니므로, 시안에서 이를 생략하고 `메이플시럽혼합페이스트`, `딸기향`으로 표기해도 100% 합법(✅)입니다. 절대 지적하지 마십시오.
"""
...
RULES_TAB2 = "[탭2 정보표시면/원재료명 관련 핵심 룰]\n" + get_sliced_rules([1, 2, 5, 6, 8, 12, 13, 14, 25, 28, 29, 30, 34, 35, 38, 39, 44, 48, 49, 52, 54, 57, 58, 59, 60, 61, 65] + COMMON_RULES)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V155.3 - 1:1 대조 예외 & 내부코드 허용 패치)")
