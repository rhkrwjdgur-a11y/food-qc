...
🔥 **Rule 44. [🌟 식품첨가물(혼합제제) 표기 방식의 자율성 보장 (1:1 묶음 및 1:N 해체 모두 합법)]**
   - 1. **[1:1 묶음 표기 합법]**: `혼합제제명(하위성분1, 하위성분2)` 묶어서 쓰면 합법(✅).
   - 2. **[1:N 해체 전개 합법]**: `하위성분1`, `하위성분2`로 낱낱이 흩뿌려 적었더라도 이 역시 완벽한 합법(✅).

🔥 **Rule 52. [당 ZERO 강조 및 감미료 병기 의무 룰]**
...
🔥 **Rule 70. [내/외포장 100% 일치 강제 및 내용량/타이포그래피 예외 룰]**
   - 내포장(팩)과 외포장(박스)을 1:1 대조 시 원재료명, 영양성분 등은 픽셀 단위로 일치해야 합니다. 단, 마침표나 띄어쓰기 등 단순 기호 유무는 합법(✅)으로 인정.

🔥 **Rule 88. [100% 강조표시 기만 검증 룰]**
   - **[원재료 100% 금지]**: 서류에 정제수나 첨가물이 단 0.01%라도 있는데 "OO 100%"라고 쓰면 소비자 기만(🚨).
   - **[원산지 100% 합법]**: "국산 OO 100%" 처럼 원산지 수식은 첨가물이 섞여 있어도 합법(✅).

🔥 **Rule 89. [국내 제조 가공품 원료의 원산지 이중 표기 규정]**
   - 수입 원물을 국내에서 가공한 원료(Rank B 1~3위 타겟)는 `원료명(원료명: 국가명)` 형태로 괄호 안에 원료명을 한 번 더 명시한 뒤 원산지를 적어야 합법(✅). `원료명(국가명)`은 부적합(🚨).
"""
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V325.00 - 마스터피스 완성본)")
...
함께 업로드된 [마스터 서류(배합비/한글라벨)]의 완벽한 문맥을 바탕으로, 시안 텍스트에 낀 명백한 OCR 노이즈를 스스로 찾아내어 원래의 깨끗한 단어로 자동 교정(Auto-Correction)하십시오.
...
🔥 [탭 역할 수행 및 스마트 스캔(Local to Global) 원칙 - 환각 방지] 🔥
당신은 해당 탭의 [검토 항목] 표를 채우는 역할을 합니다. 정보의 과부하로 인한 환각(Hallucination)을 방지하기 위해 다음 순서로 스캔하십시오.
1단계 (집중 스캔): 먼저 사용자가 업로드한 '현재 탭에 해당하는 면(예: 2번 탭이면 정보표시면)'의 텍스트와 이미지에서 타겟(원재료명, 마크 등)을 최우선으로 집중해서 찾으십시오.
2단계 (전면 스캔): 만약 1단계에서 타겟을 찾지 못했다면, 그때 시야를 넓혀 '제공된 전체 이미지(다른 면 전체)'를 샅샅이 뒤져서 찾아내십시오. 
결론적으로 "이 면에는 없으므로 해당 없음"이라고 무시하고 패스하는 성급한 판단(터널 시야)은 금지하되, 처음부터 수천 자의 전체 텍스트를 무작정 뒤섞어 읽어 문맥을 혼동하는 일은 없어야 합니다.
또한, 지시받지 않은 다른 탭의 표를 임의로 창조하여 덧붙이지 마십시오. 오직 아래에 제시된 [출력 양식] 표 딱 하나만 채워 넣고 출력을 강제 종료(Stop)하십시오.
        ...
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("【정밀 법리 검수 매트릭스 연산 중...】"):
                missions = [
                    ...
                    "⭐ [스마트 스캔 명령 - 시야 확장]: 당신은 1번 탭(주표시면) 담당이므로 앞면을 우선적으로 검토하되, 찾고자 하는 필수 마크나 정보가 앞면에 보이지 않는다면 전체 이미지(다른 면)를 뒤져서 찾아내십시오."
                ]
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                if "박스" in ins_mode:
                    missions.append("⭐ [내외포장 1:1 분할 매칭 강제명령]: 타겟(박스) 시안과 비교용(팩) 시안의 원재료명 전체 텍스트를 표의 한 칸(행)에 통째로 때려 넣는 행위를 절대 금지합니다. 반드시 쉼표(,)를 기준으로 각각의 개별 원재료를 쪼개어, 표의 1개 행(Row) 당 딱 1개의 원재료만 1:1로 나란히 매칭되도록 길게 표 작성하십시오.")
        ...
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("【의무표시 및 인증마크 해독 중...】"):
                missions = [
                    ...
                    "⭐ [스마트 스캔 명령 - 시야 확장]: 패키지 디자인의 유동성 때문에 필수 마크나 문구가 기타면에 없을 수 있습니다. 4번 탭 우선 탐색 후 없으면 전체 이미지를 뒤져서 찾아내 판정하십시오."
                ]
