...
✅ **Rule 2. 향료 및 첨가물 명칭 유연화 (통합 표기 합법성)**
   - 배합비 서류에 구체적인 개별 향료명이 명시되어 있더라도, 시안 원재료명에 단순히 '향료'라고 묶어서 표기한 것은 적합.

✅ **Rule 3. 영양정보 vs 강조표시 (이원화 대조)**
   - 영양성분표의 수치와 주표시면의 마케팅 강조 문구가 서로 충돌하지 않는지 대조하십시오.

✅ **Rule 4. 영양성분 실측값 허용**
   - 영양성분은 식약처 허용 오차 범위를 고려하여 시험성적서의 실측값을 시안에 그대로 반영한 경우 적합.

🔥 **Rule 5. [복합원재료 5% 룰 및 첨가물 생략 허용]**
   - 배합비 5% 미만인 복합원재료는 하위 성분을 전개할 의무가 없습니다. 생략은 합법입니다.

✅ **Rule 6. 당류/시럽 필터링**
   - 원재료에 당류가 있음에도 영양표시 당류가 0g이면, 실제 0.5g 미만인지 검증.

✅ **Rule 7. 감미료 주의문구 (조건부 발동)**
   - 당알콜류 사용 시 설사 관련 주의 문구 누락 확인.
...
✅ **Rule 10. 영양성분 강조표시 (액체/고체 기준 강제 분리)**
   - 100g 또는 100mL 당 기준 엄격히 분리.

🔥 **Rule 11. [영양정보 허용오차 산술 연산 법칙 (역산 오류 방지)]**
   - **[🚨역산 금지]**: 무조건 **'시안 표시량'에 0.8 또는 1.2를 곱하여** 법적 기준선을 도출하십시오.
   - [80% 이상 합법]: (성적서 환산 실측값) >= (시안 표시량 * 0.8) 이면 적합.
   - [120% 미만 합법]: (성적서 환산 실측값) <= (시안 표시량 * 1.2) 이면 적합.

✅ **Rule 12. [원재료명 임의 추론 금지]**
   - 배합비 데이터 없이 레시피 임의 지적 금지.

🔥 **Rule 13. [알레르기 정밀 추적 및 위치 오판 방지]**
   - 시안의 "~함유" 박스에 적힌 알레르기 물질은 '원재료명' 리스트에 존재해야 함. 
   - 알레르기 정보는 바탕색과 구분되는 '별도 란'에 한 번만 기재되어 있으면 적합. 중복 표기 요구 금지.

🔥 **Rule 14. [표 4 및 표 6 의무/예외 표기 마스터 룰]**
   - 향료 뒤에 '(착향료)' 병기 요구 금지.
   - 일반 첨가물 표기 시 구연산나트륨 등을 묶어서 시안에 "영양강화제 2종" 표기 적합. (단, Rule 44의 혼합제제 내부는 예외)
...
🔥 **Rule 23. [식약처 영양성분 '0' 표시 예외 규정]**
   ...
   - 포화지방 등: 0.5g 미만은 "0g" 표시.

🔥 **Rule 24. [감미료 14pt 의무 표기 및 다중 강조표시 규칙]**
   - 무당/제로 강조 시 14포인트 이상 "감미료 함유". 
   - **다중 강조 시 "가장 큰 강조표시 주위"에 기재.**
...
🔥 **Rule 28. [원산지 과잉 지적 금지]**
   - 배합비 하위 성분까지 전개하여 원산지 추가 요구 금지. (Rule 53 예외)
...
✅ **Rule 34. [2% 미만 원재료 순서 유연성]**
   - 배합비 기준 2% 미만 원료는 기재 순서가 달라도 적합.

✅ **Rule 35. [서류 명칭 일치(간략명) 허용]**
   - 의미상 동일한 간략 명칭은 적합. (예: 정제소금, 천일염 -> '소금' 표기 허용)

✅ **Rule 36. [주의사항 오탈자 스캔]**
✅ **Rule 37. [법적 서류 우선 원칙]**

🔥 **Rule 38. [교차오염 경고 상호 배타성 원칙]**
   - 원재료에 투입된 성분을 교차오염에 중복 기재 시 부적합. 원재료에 없는 물질을 알레르기 함유 칸에 기재해도 부적합.
...
🔥 **Rule 41. [% 영양소 기준치 정밀 검증 및 예외 항목]**
   - 비율(%) 계산 시 외부 데이터 절대 사용 금지. 식약처 수치 대입.
   - **[🚨 열량 및 트랜스지방 % 표기 금지]:** 열량(kcal)과 트랜스지방은 % 비율을 표기하는 항목이 아닙니다. 절대 %를 계산하지 마십시오. (빈칸 처리)
   - **[🚨 연동 재계산 룰]:** Rule 23에 의해 표시량을 수정해야 한다면(예: 2mg ➔ 5mg 미만 수정), % 비율 역시 수정될 올바른 기준값(5mg)을 바탕으로 재계산해야 합니다.

✅ **Rule 42. [완제품 서류 혼동 방지]**
✅ **Rule 43. [육안 재확인 요망 처리]**

🔥 **Rule 44. [혼합제제 하위성분 '주용도 묶음표기' 절대 금지]**
   - 혼합제제의 괄호 안 하위 성분은 용도가 같아도 '산도조절제', '유화제' 등 주용도명으로 묶어서 표기할 수 없습니다. '구연산' 등 개별 명칭을 전개해야 합니다. 시안에 혼합제제(유화제5종) 처럼 묶어 썼다면 즉시 🚨부적합.

✅ **Rule 45. [선택적 누락 허용]**
🔥 **Rule 46. [제품명 숫자 강조 시 전개 확인]**
🔥 **Rule 47. [내/외포장 물리적 차이 예외 인정]**
🔥 **Rule 48. [배합비와 한글라벨 역할 분리]**
🔥 **Rule 49. [균형영양식 강제 전개 적합성]**
🔥 **Rule 50. ['원액/100%' 명칭 표시 합법성]**

🔥 **Rule 51. [고형분(Brix) 보수적 표기 예외]**
   - 과일 농축액 등의 고형분 함량이 시안에 보수적(낮게) 표기된 경우 합법.

🔥 **Rule 52. [논리적 모순 탐지]**
   - 숫자(예: 15종, 5無)와 원재료명 개수 일치 대조.

🔥 **Rule 53. [제품명 연동 원료 강제 추적 (🚨농산물 원물 한정)]**
   1) 함량 검증(공통): 주표시면에 해당 원료 함량(%) 표기. (누락 시 🚨 부적합)
   2) 원산지 검증(농산물 원물 한정): 원료가 '진짜 농수산물 원물(우유, 딸기, 돼지고기 등)'인 경우에만 원산지 필수. 
   - [가공품 면제]: '가공품(코코아분말, 페이스트 등)'이나 첨가물은 제품명에 쓰였어도 원산지 누락 합법(✅).

🔥 **Rule 54. [복수 원산지 혼합 비율 생략 룰]**
   - 2개국 이상 병기 후 비율 미기재 시 🚨확인 요망.

🔥 **Rule 55. [영양성분 소수점 및 반올림 강제 규정]**
   - 포화지방 5g 이상은 정수로, 트랜스지방 0.2g 미만은 0g 표시. 시안에 8.0g 처럼 소수점 있으면 🚨부적합.
"""
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V62.0 - 2번 탭 사전 생각 원천 금지)")
    ...
    # ==========================================================
    # 탭 2: 정보표시면 검토 (V62 뇌정지 원천 차단: <thinking> 태그 폐지)
    # ==========================================================
    with tab2:
        st.info("시안(왼쪽)과 서류(오른쪽)를 1:1로 맵핑하여 누락, 오타, 비율 룰(3순위/5%/2%), 혼합제제 묶음 오류를 총괄 검증합니다.")
        if st.button("▶️ 정보표시면 원재료 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("머릿속 암산을 멈추고, 즉시 원재료 1:1 대조 표를 그립니다... (최대 30초 소요)"):
                prompt = """
                [지시]: 오직 아래의 원재료명 1:1 맵핑 표와 검증 리포트만 출력하십시오.
                
                🚨 [뇌정지 완벽 차단 특별 명령: <thinking> 태그 사용 절대 금지!] 🚨
                미리 머릿속으로 수십 개의 에러를 계산하려다 시스템이 뻗는 현상을 막기 위해, 사전 분석(<thinking>) 과정을 완전히 생략하십시오. 즉시 본론인 **`## 2️⃣ [원재료명 1:1 정밀 대조 및 원산지 검증]`** 제목을 적고, 1번 원료부터 표를 그리면서 눈으로 보면서 하나씩 채워나가십시오. 절대 멈추지 마십시오.
                
                ## 2️⃣ [원재료명 1:1 정밀 대조 및 원산지 검증]
                - 결론: (✅ 적합 또는 🚨 부적합)
                
                **(🚨 텍스트 다이어트 필수 경고 🚨): 서류 매칭 원료 칸에 복합원재료의 기나긴 하위 성분(괄호)을 절대로 통째로 복사해서 붙여넣지 마십시오! 핵심 원료명, 원산지, 비율만 극단적으로 짧게 요약하십시오.**
                
                [🟢 올바른 표 작성 예시]
                | 시안 원재료명 (포장지) | 서류 매칭 원료 (서류 요약) | 배합비 룰 (3순위/5%/2%) | 규정/오타/누락/Rule 44 검증 | 판정 |
                |---|---|---|---|---|
                | 유함유가공품(프랑스산) | 유함유가공품 (프랑스 / 괄호생략) | 미상 | ✅ 적합 | ✅ |
                | 유화제5종 | 혼합제제 (하위성분생략) | - | 🚨 Rule 44 위반 (개별명칭 전개 필수) | 🚨 |
                | 추정 | 주정 | - | 🚨 오타 적발 (추정 -> 주정) | 🚨 |
                
                (위 양식에 맞춰 시안에 있는 모든 원재료를 끝까지 1:1로 대조하는 표를 즉시 그리십시오.)
                
                - 🚨 [추가 지적 사항 (누락 검증)]: (서류에는 명시되어 있으나 시안에서 아예 '누락'된 원료를 지적하십시오. 단, Rule 5 합법 생략은 패스.)
                - [Rule 53 관련]: (제품명 연동 원료 함량/원산지 위반 사항 별도 서술. 가공품은 원산지 면제 합법.)
    ...
    # ==========================================================
    with tab3:
        st.info("영양성분표 이미지와 성적서를 대조합니다. 열량/트랜스지방 %는 계산하지 않으며, 수치 오류 시 수정된 값을 기준으로 %를 재계산합니다.")
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("영양성분 오차 수식 계산 및 % 재계산 표 렌더링 중..."):
                prompt = """
                ...
                <thinking>
                (9개 성분의 허용오차 기준선 및 1일 기준치 비율 계산식 기록. 단, 열량과 트랜스지방은 % 계산 생략)
                </thinking>
                ...
                **(🚨 Rule 41 핵심 명령): 열량(kcal)과 트랜스지방은 시안 % 칸에 무조건 '-' 기호를 넣고 표기 대상이 아님을 명시하십시오. % 누락을 지적해서는 절대 안 됩니다.**
                **(🚨 Rule 41 재계산 명령): 실측값이 허용오차를 벗어나거나 Rule 23에 의해 시안 수치를 수정해야 한다면(예: 2mg -> 5mg 미만으로 수정 필요), 1일 영양성분 기준치 비율(%)도 '틀린 시안 수치'가 아니라 '수정될 올바른 수치(예: 5mg)'를 기준으로 다시 계산하여 지시하십시오.**
                
                [🟢 올바른 표 작성 예시]
                | 영양성분명 | 성적서 실측값 | 시안 표시량 | 법적 허용오차 기준선 | 1일 기준치 | 시안 % | % 검증 및 재계산 | 판정 |
                |---|---|---|---|---|---|---|---|
                | 열량 | 253.98kcal | 255kcal | 304.8kcal 이하 | - | - | 표기 대상 아님 | ✅ 적합 |
