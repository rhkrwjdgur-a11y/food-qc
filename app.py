...
def main():
    ...
    st.markdown("<h1>Label Guard AI <span style='font-size:0.5em; color:#64748b;'>식품 패키지 무결점 검증 시스템 (V5.9.4 Ultimate)</span></h1>", unsafe_allow_html=True)
...
| **주표시면 강조 원료 구조적 무결성 (1:1 픽셀 대조)** | [Rule 100] | (※ 쉼표 등 기호 삽입이나 띄어쓰기로 인해 뒷면 원재료명의 형태소가 분리/훼손되었는지 적발. 단, "스캔 오류일 수 있으니 육안 확인 부탁드립니다" 라는 멘트를 사유에 반드시 포함하여 ⚠️ 확인 요망으로 유도할 것) | |
