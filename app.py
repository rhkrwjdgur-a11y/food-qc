...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.70 - UI 프롬프트 다이어트 & 100% 족쇄 완성)")
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                # V311.70: UI에서 룰 설명 문자열 제거 및 백그라운드 지시어로 이동
                ...
                2. [마스터 DB 100% 해체]: 마스터표 작성 시 '코드 번호', '제조원', '수입원', '식품유형' 열은 지우고 4개 핵심 열만 작성하십시오. '향료 5종', '영양강화제 3종', '혼합제제' 같은 묶음(그룹) 명칭은 표에서 완전히 폐기하고, 그 안에 들어있던 개별 단일 성분들을 1열(원료명)로 빼내어 독립된 항목으로 순위를 매기십시오.
                3. [명칭 유지 병합 금지]: 복합원재료(예: 비타민E혼합제제)를 중복 합산 시 껍데기 명칭을 그대로 살려 적으면 🚨부적합입니다. 반드시 최하위 단일 성분으로 해체해야 합법입니다.
                4. [원산지 순위 명시]: 대조 매트릭스의 '원산지 룰 검증' 란에 `✅ [1순위] 표기됨`, `✅ [4순위 이하] 추가 정보로 표기됨`, `✅ [산정제외] 의무 없음` 등 산정 순위를 대괄호로 명시하여 직관적으로 판단하십시오.
                5. [순서 역전 스나이퍼]: 표를 채우며 위아래 원료의 2% 이상 배합비 순위를 대조하고, 서류 순서와 시안 나열 순서가 다를 경우(역전 현상) 즉시 🚨부적합 처리하십시오.
                6. [출력 형태 통제]: '⭐ [Rule 90 범용 치환...]' 같은 부연 설명 텍스트를 출력 양식에 적지 마십시오. 오직 결과 표만 깔끔하게 출력하십시오.
                """
                ...
                if has_any_doc:
                    ...
                    judgment_prompt += "| 서류상 단일 원료명 (그룹명 폐기) | 하위 전개 성분 | 원산지 | 배합비(%) / 순위 |\n|---|---|---|---|\n\n"
                ...
                if "박스" in ins_mode:
                    ...
                    judgment_prompt += "| 타겟(박스) 표기 개별 원재료명 | 비교용(팩) 표기 개별 원재료명 | 대조 검증 결과 (픽셀 100% 일치 여부) | 최종 판정 |\n|---|---|---|---|\n\n"
                    judgment_prompt += f"### 🚨 [팩 시안 기준 최종 누락 스나이퍼 검증]\n- (누락 원료 기재 또는 '✅ 팩 시안 대비 통째로 누락된 원료 없음')\n\n"

                if has_any_doc:
                    target_name = "박스 시안" if "박스" in ins_mode else "시안"
                    step_offset += 1
                    judgment_prompt += f"## 2️⃣-{step_offset}. [마스터 서류 vs {target_name} 법적 대조 매트릭스]\n"
                    judgment_prompt += "| 시안 표기 개별 원재료명 (1줄에 1개씩) | 매칭된 서류 원료명 | ⚖️ 배합비(%) 및 순위 | 🌍 원산지 룰 검증 | 최종 대조 결과 및 사유 (상세 서술, 생략 금지) | 판정 |\n|---|---|---|---|---|---|\n\n"
                    judgment_prompt += f"### 🚨 [서류 기준 최종 누락 스나이퍼 검증]\n- (누락 원료 기재 또는 '✅ 서류상 누락된 원료 없음')\n\n"
