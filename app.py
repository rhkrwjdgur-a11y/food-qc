...
# ==========================================
# 📚 3. 62대 룰북 원문 (V149.0 완결판 - Rule 60, 61 초강력 범용화)
# ==========================================
RULE_BOOK_FULL = """
...
🔥 **Rule 60. [모든 복합원재료(페이스트/농축액/추출액/분말/엑기스/시럽 등) 원물 함량 강제 환산 및 서류 핑계 절대 금지 룰 (초강력 범용)]**
   - 제품명이나 주표시면에 특정 원물을 강조했는데 해당 원료가 명칭 끝이 **'~페이스트, ~농축액, ~추출액, ~분말, ~엑기스, ~시럽'** 등으로 끝나는 **'모든 복합원재료'** 형태로 투입된 경우, 복합원재료 투입량(%)만 단독 표기하면 🚨부적합입니다. 반드시 시안 텍스트 자체에 괄호로 진짜 원물의 배합함량(%)을 기재하거나 환산된 최종 원물 함량(%)이 명시되어야 합법입니다.
   - **[🚨절대 금지: 서류 의존 환각]**: 증빙 서류에 해당 복합원재료가 '원물 100%'라고 적혀 있더라도, 시안(패키지) 텍스트에 원물 함량 표기가 누락되었다면 서류를 핑계로 ✅적합 처리하는 것을 엄격히 금지합니다. 소비자에게 보이지 않는 서류는 면죄부가 될 수 없습니다. 무조건 🚨부적합 처리하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V149.0 - 초강력 범용 복합원재료 스캔 패치)")
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        product_type = st.radio("📌 1. 식품유형", ("일반식품", "특수의료용도식품 / 환자식", "축산물(우유/가공유 등)"))
        ...
        st.info("💡 팁: 실행 파일 옆에 `default_docs` 폴더를 만들고 PDF를 넣어두면 🚀버튼 클릭 시 자동으로 읽어옵니다.")
...
- 🚨 [Rule 60 범용 적용] 제품명에 강조된 원료뿐만 아니라, 명칭이 '페이스트, 농축액, 추출액, 분말, 엑기스, 시럽' 등으로 끝나는 **모든** 원료를 스캔하여 괄호 안에 원물 함량(%)이 적혀 있는지 강제 검증 (서류 내용으로 시안 누락 핑계 절대 금지):
