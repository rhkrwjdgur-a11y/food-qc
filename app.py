...
# ==========================================
# 🧬 [첨가물 표 4, 5, 6 하드코딩 DB (V311.73 고시 원문 100% 동기화 완결판)]
# ==========================================
ADDITIVE_TABLE_4 = [
    # 보존료
...
ADDITIVE_TABLE_6 = [
    ...
    # ⭐ [V311.73 보강] 향미증진제/영양강화제 (표 6 소속 - 용도 생략 가능, 주용도 단독 기재 가능)
...
🔥 [생략 및 축약 절대 금지 (무관용 원칙 - 토큰 최적화 금지)]:
...
🔥 [0순위 절대 방어막: 5% 미만 복합원재료 하극상 금지 (Rule 5 적용)]:
...
🔥 **Rule 5. [복합원재료 5% 미만 전개 면제 및 5가지 컷오프 룰 (최우선 방어막)]**
   ...
   - 🌟 **[첨가물 과잉 단속 금지 족쇄]**: 위 조건 A에 따라, 5% 미만 복합원재료 내부에 [표 4, 5, 6] 소속 식품첨가물이 들어있더라도 명칭/용도 표시 의무가 완전히 면제됩니다. (예: 아미노밸런스 0.3% 내의 첨가물 지적 불가)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.73 - DB 고시 원문 완벽 동기화)")
...
(주의: 뼈대에 없는 부연 설명이나, 룰에 대한 텍스트(예: ⭐ [Rule 90...])는 출력 화면에 보이지 않도록 절대 출력하지 마십시오. 오직 표와 결과만 깔끔하게 출력하십시오.)
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                # V311.73: 고시 DB 100% 일치 적용 & 5% 룰 하극상 절대 방어
                ...
                2. [100% 상세 서술]: 표 작성 시 판정 이유를 절대로 생략하거나 짧게 쓰지 마십시오. 적합이든 부적합이든 무조건 구체적인 룰(Rule) 번호와 근거를 100% 빠짐없이 상세하게 서술하십시오.
                3. [마스터 DB 원본 복사]: 서류에서 데이터를 추출할 때 AI 임의로 분석/해체/병합을 하지 마십시오. 오직 서류 원문에 있는 [식품유형, 제품명, 한글표시사항, 원산지, 알러지유발물질] 딱 5개의 정보만 100% 그대로 복사하여 2-1 마스터표를 작성하십시오.
                4. [명칭 유지 병합 금지]: 복합원재료(예: 비타민E혼합제제)를 중복 합산 시 껍데기 명칭을 그대로 살려 적으면 🚨부적합입니다. 반드시 최하위 단일 성분으로 해체해야 합법입니다.
                5. [원산지 순위 명시]: 대조 매트릭스의 '원산지 룰 검증' 란에 `✅ [1순위] 표기됨`, `✅ [4순위 이하] 추가 정보로 표기됨`, `✅ [산정제외] 의무 없음` 등 산정 순위를 대괄호로 명시하여 직관적으로 판단하십시오.
                6. [순서 역전 스나이퍼]: 표를 채우며 윗줄과 아랫줄 원료의 2% 이상 배합비 순위를 대조하고, 서류 순서와 시안 나열 순서가 다를 경우(역전 현상) 즉시 🚨부적합 처리하십시오.
                7. [출력 형태 통제]: '⭐ [Rule 90 범용 치환...]' 같은 부연 설명 텍스트를 출력 양식에 적지 마십시오. 오직 결과 표만 깔끔하게 출력하십시오.
                """
                ...
                if "박스" in ins_mode:
                    ...
                    judgment_prompt += f"### 🚨 [팩 시안 기준 최종 누락 스나이퍼 검증]\n- (누락 원료 상세 기재 또는 '✅ 팩 시안 대비 통째로 누락된 원료 없음')\n\n"
                ...
                if has_any_doc:
                    ...
                    judgment_prompt += f"### 🚨 [서류 기준 최종 누락 스나이퍼 검증]\n- (누락 원료 상세 기재 또는 '✅ 서류상 누락된 원료 없음')\n\n"
                ...
                judgment_prompt += f"### 🚨 2️⃣-{num_add}. [식품첨가물 범용 형식주의 스나이퍼]\n- **[명칭 축약 및 용도 표시 검사 결과]**:\n- **[임의 기호 창조 검사 결과]**:\n\n"
    ...
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【영양성분표 수치 환산 및 교차 검증 중...】"):
                ...
                judgment_prompt_tab3 += """## 🔍 [영양성분표 치명적 레이아웃 및 뼈대 스나이퍼]
