...
🔥 **Rule 47. [디자인적/물리적 차이 예외 인정 룰]**
   ...
   - **[🚨물리적 차이 및 주의사항 추가 예외 인정]**: 박스(외포장)는 면적이 넓기 때문에 팩(내포장)에는 없는 '반품/교환처', '고객상담실', '추가 보관방법(예: 개봉 후 냉장보관)' 등의 행정/취급 문구가 더 추가되는 것이 지극히 정상이며 합법(✅)입니다. 글자가 완전히 똑같지 않다고 주의사항을 부적합 처리하지 마십시오. 핵심 경고문구(교차오염 등)가 포함되어 있는지만 확인하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V78.0 - UI 동기화 및 팩/박스 비교 패치)")
    ...
    with st.sidebar:
        ...
        # 🚨 [V78.0 패치] 실무 작업 순서에 맞춘 UI 텍스트 변경
        ...
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("영양성분 단방향 오차 계산 및 교차 검증 렌더링 중..."):
                ...
                **(🚨 Rule 41 % 표기 금지 및 재계산 룰): 열량(kcal)과 트랜스지방은 시안 % 칸에 무조건 '-' 기호를 넣으십시오. 시안 수치를 수정해야 한다면 1일 영양성분 기준치 비율(%)도 '수정될 올바른 수치'를 기준으로 재계산하여 지시하십시오.**
                ...
                - (비교용 팩 영양성분표 이미지가 업로드된 경우에만, 박스의 수치와 팩의 수치가 100% 일치하는지 대조하십시오. 없으면 "대조용 팩 이미지 미업로드"라고 적으십시오.)
                """
