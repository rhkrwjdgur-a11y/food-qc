...
**Rule 20. [포장재질 및 종이팩(카톤팩) 분리 표시 룰]**
- 환경부 규정에 따른 분리배출 도안에는 '종이팩'으로 표기하되, 식약처 용기·포장재질(텍스트) 표기 의무는 식품과 직접 접촉하는 면(합성수지제 등)에만 발생합니다.
- 따라서 우유팩의 경우 뼈대인 '종이'는 생략하고 내면 코팅 재질인 '폴리에틸렌(내면)' 등 식품 직접 접촉면만 기재하는 것이 100% 완벽한 합법(✅)입니다. (종이나 유리는 텍스트 재질 표시 의무 없음)
...
# 탭별 맞춤형 룰 타겟팅
# ==========================================
TAB1_RULES = [3, 9, 17, 18, 19, 21, 24, 46, 50, 52, 53, 57, 62, 63, 66, 68, 71, 72, 84, 86, 87, 88, 94, 95, 96, 99, 100, 103]
...
TAB4_RULES = [7, 15, 20, 22, 36, 58, 59, 73, 74, 75, 76, 77, 78, 81, 92, 93, 103]
        ...
        if cache_name:
            try:
                ...
                if "403" in str(e) or "404" in str(e):
                    return "**[캐시 만료 안내]**<br>파일 연동 후 60분이 초과되어 데이터 지워졌습니다. 좌측 사이드바에서 캐싱 버튼을 다시 눌러주세요."
