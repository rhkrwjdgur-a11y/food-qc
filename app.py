...
🔥 **Rule 35. [🌟 범용 간략명/관용명 허용 및 혼합제제 괄호 내부 N종 은폐 금지 범용 룰]**
   ...
   - ⭐ **[향료 N종 무조건 합법]**: **'향료 3종', '합성향료 2종'**처럼 향료에 숫자를 붙여 묶는 것은 식약처 규정상 **완벽한 합법**입니다. 
   - ⭐ **[혼합제제 괄호 내부 은폐 절대 불가]**: 패키지 시안에 `혼합제제(산도조절제 2종)` 또는 복수의 첨가물을 `영양강화제 3종`처럼 **임의로 숫자로 묶어 은폐(블랙박스화)한 경우 명백한 위법(🚨부적합)**으로 처리하십시오. (증빙 서류에 묶여있더라도 라벨에는 반드시 개별 명칭을 전개해야 합니다.)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.95 - 추출 강제 및 복합 논리 완성판)")
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                # ⭐ V311.95: 마스터표 생략 완벽 방지, 환각 차단 및 Rule 44 특별 스나이퍼 장착
