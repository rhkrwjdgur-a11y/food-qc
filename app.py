...
# ==========================================
# 📚 3. 85대 마스터 룰북 원문 (V310.20 완벽 집대성본)
# ==========================================
RULE_BOOK_FULL = """
...
🔥 **Rule 21. ['고/풍부', '저', '무' 영양강조표시 엄격 컷오프 및 비중세탁 금지 룰]**
   ...
   - 🚨 **[액체 비중 세탁 영구 봉쇄]**: 내용량을 'mL' 단위로 유통하는 액체 식품의 경우, 영양강조 세부기준은 무조건 **'100mL당'** 기준선(예: 저당의 경우 100mL당 2.5g 미만)만 적용해야 함. 마케팅팀이나 R&D에서 비중을 곱해 고체 기준인 '100g당' 수치로 가공하여 강조 요건을 우회하는 행위는 심각한 소비자 기만이자 고시 위반(🚨부적합)임. 영양성분 표시단위는 무조건 내용량 단위와 동일하게 일치해야 함.
...
🔥 **Rule 85. [식품첨가물 공전 명칭 사수 및 기호 창조 절대 금지 (범용 형식주의)]**
   ...
   - **[임의 기호 창조 전면 금지]**: 원재료와 첨가물의 용도를 병기하거나 하위 성분을 전개할 때, 대한민국의 식품 법정 표준 기호인 **'괄호()'** 외에 작성자의 편의를 위한 임의의 특수기호**(슬래시 `/`, 대시 `-`, 플러스 `+`, 콜론 `:` 등)**를 사용하여 구조화하는 행위는 전면 금지(🚨). (예: '적색제40호/착색료'와 같이 슬래시를 사용하는 행위는 즉시 부적합🚨).
"""
...
COMMON_RULES = [36, 37, 42, 43, 45, 47, 70, 78, 79, 85]
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V310.20 - 마스터 범용 아키텍처 완결본)")
...
"""
    ...
    # ── TAB 2: 정보표시면 (범용 형식주의 스나이퍼 모드 완결판) ──
    with tab2:
...
⭐ [Rule 23 반올림의 유연성 적용]: 열량, 나트륨 등은 실측값을 그대로 적는 것과 법정 단위(5kcal, 10mg 등)로 반올림하여 적는 것 둘 다 완벽한 합법(✅)이므로 단순 버림/반올림 차이로 부적합을 내리지 마십시오. 단, 콜레스테롤 2mg 이상인데 0mg으로 둔갑시킨 것은 즉각 부적합(🚨)입니다.
...
"""
    ...
    # ── TAB 5: AI 법률 자문 스캔 (Track 2 - 다면 교차 및 범용 형식주의 탑재) ──
    with tab5:
...
3. 환각 완전 삭제: 만약 단어 하나라도 다르게 조작되었거나 존재하지 않는 조항이라면, 그 지적 사항은 삭제(Drop)하십시오.
...
6. 🎯 [다면(多面) 교차 검증 및 조기 종료 금지]: 법령에서 특정 표현이나 의무 사항을 규정할 때, 시안의 한 면만 보고 판단을 조기 종료하지 마십시오. 전체 구역을 입체적으로 교차 검증하십시오.
7. 🚨 [Rule 23 반올림 예외 및 '그대로 표시' 전수 인정]: 열량, 나트륨 등은 실측값을 '있는 그대로' 적거나 '법정 단위로 반올림'하는 두 가지 방식을 모두 고시에서 허용하므로, 단순 반올림 오차를 이유로 부적합 판정을 내리는 행위를 엄격히 금지합니다.
8. 🎯 **[식품첨가물 표기 및 액체 영양강조 범용 검수]**: 시안을 교차 스캔하여 다음 두 가지를 완벽히 감시하십시오.
    - **[첨가물 형식 검수]**: 첨가물 단어 발견 시 [명칭 축약 금지](PDF [표 4, 5, 6] 공식 명칭과 단 한 글자라도 다르면 즉각 적발) 및 [근본 없는 기호 금지](용도 병기 시 슬래시 `/` 등 임의 기호 사용 시 즉각 적발).
    - **[액체 영양강조 검수]**: 제품의 유통 성상이 액체(mL)일 경우, '저당' 등 영양강조 세부기준 판정 시 고체 기준(100g당) 대입을 완벽히 차단하고 오직 '100mL당' 기준선만 적용하여 위법 여부를 리포트할 것.
...
#### 📌 [식별된 문구/디자인]: "(여기에 추출된 내용 및 발견된 위치 작성)"
* **적용 법령 및 조항:** [문서명, 제O조 제O항]
* **법령 원문:** > "(여기에 원문을 그대로 인용)"
* **AI 법무팀 자문 의견 (위법 리스크):**
  * 🚨 **[리스크 총평]:** (종합적인 위법 사유 요약)
  * 🔍 **[다면(Double-Check) 교차 검증 결과]:** (시안 전체를 스캔하여 법령의 요구조건을 충족했는지, 혹은 특정 구역에서 무엇이 누락되거나 위반되었는지 다각도로 분석한 내용 기재)
---
"""
