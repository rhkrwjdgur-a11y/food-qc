...
# ==========================================
# 📚 3. 94대 마스터 룰북 원문 (100% 무손실 보존본 + Rule 94 추가)
# ==========================================
RULE_BOOK_FULL = """
...
def main():
    ...
    # ⭐ 상단 고정 헤더 영역
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V334.00 - 대외무역법 원천 방어 패치)")
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                3. ⭐ [1:N 해체 전개 합법성 인정 방패]: 표를 작성할 때 서류의 '혼합제제' 1개가 시안의 여러 개별 하위 성분(비타민C, 덱스트린 등)으로 쪼개져 매칭되는 것은 Rule 44에 따른 완벽한 합법입니다. 개별 전개된 모든 성분에 대해 "✅ 적합 (Rule 44에 따른 합법적인 1:N 해체 전개)"라고 기재하십시오.
                ...
                5. ⭐ [투트랙 스마트 1:1 대조 및 억지 맵핑 금지]:
                   - [Track 1. 유연한 맵핑 허용]: 당류/시럽류, 올리고당류, 식품첨가물, 명확한 동의어는 문맥상 일치하면 `✅ 적합`으로 정상 매칭하십시오.
                   - [Track 2. 억지 치환 절대 금지]: 과일류, 유가공품 등 핵심 원물은 종류가 다르면 절대 억지로 매칭하지 마십시오. (예: 사과를 포도에 매칭 불가)
                ...
                7. ⭐ [표(Table) 레이아웃 붕괴 방어 및 가독성 강제]: 원재료명이나 사유 텍스트 내부에 파이프 기호(`|`)가 포함되어 있으면 표가 깨집니다. 파이프는 슬래시(`/`)로 대체하고, 사유는 반드시 `<br>` 태그를 사용하여 줄바꿈 하십시오.
                ...
                # 🔥 [수석님 지시사항: Rule 76 제품유형 종속성 스캔 프롬프트 적용]
