...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V117.0 - 고시 원문 대조 스나이퍼)")
    ...
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("오차 계산 및 100% 전체 영양성분 3방향 대조 렌더링 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    ...
                    6. 🚨 [수량 2-Track 검증 (연산 & 텍스트)]: 총 내용량 정보를 추출하여 1) 수학적 곱하기 연산이 정확한지 검증하고, 2) 텍스트 형태(팩/개입, 대소문자 X)가 완벽히 일치하는지 둘 다 검증하십시오.
                    </thinking>
                    ...
                    | 영양성분명 | 기준 서류(성적서/라벨) | 비교용(팩) 시안 | 타겟(박스) 시안 | 팩 vs 박스 일치 여부 | 최종 판정 (상세 사유 필수) |
                    |---|---|---|---|---|---|
                    ...
                    5. 🚨 [수량 2-Track 검증 (연산 & 텍스트)]: 총 내용량 정보를 추출하여 수학적 연산 및 텍스트 형태가 완벽히 일치하는지 검증하십시오.
                    </thinking>
                    ...
                    | 영양성분명 | 성적서 실측값 | 시안 표시량 및 % | 법적 허용오차 기준 | 오차 검증 결과 | 판정 (상세 사유 필수) |
                    |---|---|---|---|---|---|
                    | (모든 영양성분을 생략 없이 기재할 것) | ... | ... | ... | ... | ... |
        ...
        if st.session_state["result_tab3"]:
            ...
            if thinking_match:
                thinking_log = thinking_match.group(1).strip()
                report_content = result.replace(thinking_match.group(0), "").strip()
                with st.expander("🧠 기계적 수치 오차 연산 및 소수점 검증 로그 보기"):
                    st.markdown(f"*{thinking_log}*")
                st.markdown(report_content)
            else:
                st.markdown(result)
    ...
    with tab4:
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("기타면/측면 교차오염 멘트 차집합 연산 중..."):
                prompt = """
        ...
        if st.session_state["result_tab4"]:
            ...
            if thinking_match:
                thinking_log = thinking_match.group(1).strip()
                report_content = result.replace(thinking_match.group(0), "").strip()
                with st.expander("🧠 차집합 연산 및 문구 스캔 로그 보기"):
                    st.markdown(f"*{thinking_log}*")
                st.markdown(report_content)
            else:
                st.markdown(result)
