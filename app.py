...
# ✅ 복잡한 연산, 텍스트 100% 필사, 하위 전개 분기 처리, 대용량 출력을 위해 Pro 모델 적용
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V91.0 - 이미지 100% 필사 OCR 강제 명령 추가)")
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 서류 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("서류 기준 하위성분 전개 및 100% 맵핑 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    ...
                    <thinking>
                    1. 🚨 [시각 판독 및 필사 절대 명령]: AI의 임의 추론이나 배경지식 개입을 엄격히 금지합니다. [타겟_시안(박스)] 이미지와 [비교용_정답지_시안(팩)] 이미지의 '원재료명' 란에 적힌 텍스트를 단 한 글자의 요약이나 생략 없이, 이미지 픽셀에 보이는 글자 그대로 100% 필사(Transcription)하여 먼저 나열하십시오. 이미지에 없는 글자나 숫자를 임의로 지어내면 치명적 오류로 간주합니다.
                    2. 서류 분석: 업로드된 [근거_서류(배합비/한글라벨 자료)]를 바탕으로 정답 원재료명 리스트를 도출하십시오.
                    3. 하위 성분 전개 검증 로직: 특수의료용도식품이든 일반식품이든, 무조건 '풀어써야만' 합법인 것은 아닙니다. 
                    ...
                    5. 🚨 전 구간 교차 검증 (수량/오탈자): 식품이력추적관리번호 등의 포장 단위(예: N입)를 파악하고, 다른 면(주표시면 등)의 수량 정보와 논리적으로 충돌하는지 100% 대조하십시오. 원재료명 내 쉼표 뒤 띄어쓰기 누락 등 문법 오류도 스캔하십시오.
                    </thinking>
                    ...
                    <thinking>
                    1. 🚨 [시각 판독 및 필사 절대 명령]: AI의 임의 추론이나 배경지식 개입을 엄격히 금지합니다. [타겟_시안] 이미지의 '원재료명' 란에 적힌 텍스트를 단 한 글자의 요약이나 생략 없이, 이미지 픽셀에 보이는 글자 그대로 100% 필사(Transcription)하여 먼저 나열하십시오. 이미지에 없는 글자를 지어내면 치명적 오류로 간주합니다.
                    2. 서류 분석: 업로드된 [근거_서류(배합비/한글라벨 자료)]를 바탕으로 정답 원재료명 리스트를 도출하십시오.
                    ...
                    5. 🚨 전 구간 교차 검증 (수량/오탈자): 식품이력추적관리번호 등의 포장 단위(예: N입)를 파악하고, 다른 면의 수량 정보와 논리적으로 충돌하는지 100% 대조하십시오. 쉼표 뒤 띄어쓰기 누락 등도 스캔하십시오.
                    </thinking>
        ...
        if st.session_state["result_tab2"]:
            ...
            if thinking_match:
                thinking_log = thinking_match.group(1).strip()
                report_content = result.replace(thinking_match.group(0), "").strip()
                with st.expander("🧠 100% 필사 및 전개 검증 로그 보기"):
                    st.markdown(f"*{thinking_log}*")
                st.markdown(report_content)
            else:
                st.markdown(result)
