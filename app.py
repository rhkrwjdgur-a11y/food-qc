...
🔥 **Rule 11. [영양정보 허용오차 계산 절대 법칙 (하/상한선 및 역산 오류 완전 방지)]**
   ...
   - **[80% 이상 합법 그룹 (좋은 영양소)]**: 비타민, 무기질, 단백질, 탄수화물, 식이섬유. 
     * 수식: **(성적서 환산 실측값) $\ge$ (시안 표시량 $\times$ 0.8)** 이면 합법(✅)입니다. (예: 시안이 120mg이면 120*0.8=96mg. 실측값이 96mg 이상이면 무조건 합법입니다. 상한선은 절대 없습니다.)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (V6.71 - 영양 0표시/단위 범용판)")
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        - 🚨 [긴급 차단 명령4 (산수 붕괴 방지)]: **암산 금지!** 표의 '법적 허용오차 기준선' 칸에 반드시 "[시안 표시량] * 0.8 = [값] 이상" 또는 "[시안 표시량] * 1.2 = [값] 미만" 이라고 타이핑해서 계산식을 노출하십시오. 
        - 🚨 [긴급 차단 명령5 (0표시 예외구간 범용 방어)]: 콜레스테롤, 포화지방, 트랜스지방 등 영양소가 '0' 표시 기준을 초과했다면 기계적 1단위 반올림을 하지 마십시오. Rule 23에 명시된 해당 성분의 법적 최소 표기 단위(예: '5mg 미만', '0.5g 미만', 소수점 첫째 자리 등)를 대입하여 범용적으로 정답을 제시하십시오.
