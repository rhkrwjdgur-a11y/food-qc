...
🔥 **Rule 3. [주표시면 vs 영양성분표 수치(열량 및 영양성분 함량) 100% 일치 강제 룰]**
   ...
   - **[🚨세트포장(선물용) 열량 스나이퍼]**: 단품 영양성분표에 '1팩당 0 kcal'로 적혀 있더라도, 세트(박스) 주표시면의 총 열량은 '단품 실측값 × 수량' 후 5kcal 단위로 반올림한 수치여야 합니다.
...
🔥 **Rule 13. [알레르기 정밀 추적 및 파생 원료 예외 허용]**
   ...
   - **[🚨기계적 대조 예외 (오판 방지)]**: 파생 원료(예: 카제인칼슘 ➔ 우유, 분리대두단백 ➔ 대두)가 존재한다면 절대 불일치나 부적합으로 오판하지 말고 무조건 적합(✅) 처리하십시오.

🔥 **Rule 14. [첨가물 표 4, 표 5, 표 6 교차 검증 및 용도명 완벽 스나이퍼]**
   - **[🚨표 4 (명칭+용도명 병기 의무)]**: 감미료, 보존료, 산화방지제 등 표 4에 속하는 첨가물은 반드시 "명칭+용도명"으로 기재되어야 합니다. 특히 **'수크랄로스', '아스파탐', '아세설팜칼륨', '아질산나트륨'** 등의 핵심 첨가물이 시안에 용도명 없이 단독으로 적혀 있다면 즉시 🚨부적합 처리하십시오.
   - **[🚨표 5 (간략명 표기 허용 및 용도명 대체 금지)]**: 표 5에 속하는 첨가물은 용도명으로 뭉뚱그려 쓸 수 없으며 반드시 정해진 명칭이나 허용된 '간략명'으로 적어야 합법입니다.
   - **[🚨표 6 (용도명 대체 100% 합법/선택)]**: 유화제, 산도조절제, 증점제, 팽창제, 영양강화제 등 표 6에 해당하는 첨가물은 개별 화학적 성분명을 생략하고 오직 용도명만 표기해도 완벽한 합법(✅)입니다.
...
🔥 **Rule 23. [식약처 영양성분 '0' 표시 예외 규정 (법적 사유 지적 강제)]**
   ...
   - **[🚨위반 지적 시 설명 의무]**: 단순히 "불일치합니다"라고 하지 말고 "Rule 23에 따라 트랜스지방 0.2g 미만은 반드시 '0g'으로 표시해야 하므로 '0g 미만' 표기는 법적으로 부적합합니다"라고 명확히 사유를 설명하십시오.
...
🔥 **Rule 24. [감미료 14pt 의무 표기 조건 엄격 분리 및 다중 강조표시 크기 비교 규칙]**
   ...
   - **[🚨다중 강조 시 위치 강제 및 시각적 크기 비교]**: "감미료 함유" 문구는 무조건 "가장 큰 폰트로 적힌 강조표시 주위"에 기재되어 있어야 합법입니다. 작은 글자 옆에만 기재되어 있다면 즉시 🚨부적합 처리하십시오.
...
✅ **Rule 32. [단순 역산에 의한 부적합 판정 금지 (열량 비율 등)]**
   - 영양성분표의 수치(탄수화물x4, 단백질x4, 지방x9)를 단순 계산하여 도출한 열량 비율과 마케팅 문구의 비율이 일치하지 않는다고 부적합 처리하지 마십시오. 반올림 규정으로 인해 단순 역산은 오차가 발생하므로 비율은 적합(✅)으로 인정하십시오.
...
🔥 **Rule 35. [서류 명칭 일치 및 식품공전 표준명칭 치환 허용 룰]**
   ...
   - **[🚨식품공전 표준명칭 치환 절대 허용]**: 서류상의 상업용 원료명이 시안에서 **식약처 식품공전상의 공식 식품유형(표준명칭)**으로 치환된 경우 완벽한 합법(✅ 적합)입니다. 
...
🔥 **Rule 38. [알레르기 교차오염 완벽 검증 룰 (수학적 차집합 연산)]**
   - 알레르기 교차오염 멘트를 검증할 때는 사용자가 사이드바에 입력한 **[우리 공장 알레르기 마스터 목록]**을 절대 기준으로 삼습니다.
   - **[🚨수학적 차집합 검증 로직]**: 
...
🔥 **Rule 40. [열량 표기 및 반올림 원칙 (세트/단품 환산 모순 지적 금지)]**
   ...
   - **[🚨열량 환산 모순 지적 환각 금지]**: 단품의 열량이 5kcal 미만이어서 '0 kcal'로 적법하게 표기되었더라도, 세트의 총 열량은 실측 소수점을 합산하여 5kcal 단위로 반올림하므로 '85 kcal' 등 0이 아닌 숫자가 될 수 있습니다. 이는 합법적이며 수학적으로 정상입니다. 단품의 0 kcal와 세트의 총 열량 값을 단순 비교하여 "열량 계산이 모순된다"라고 지적하는 환각을 엄격히 금지합니다.
...
🔥 **Rule 41. [% 영양소 기준치 정밀 검증 및 수식 노출 강제]**
   ...
   - **[🚨 % 계산 수식 노출 강제]:** 반드시 `(시안 표시량 ÷ 1일 영양성분 기준치) × 100` 의 계산 과정과 반올림 결과를 명시하여 사용자가 검증할 수 있도록 하십시오.
...
🔥 **Rule 44. [혼합제제 3대 절대 원칙: 묶음 금지, 내부 순서 강제 검증, 해체 팁 제공]**
   - 1) **묶음 표기 금지**: 혼합제제의 괄호 안 하위 성분들은 용도가 겹치더라도 묶을 수 없고 개별 명칭을 전개해야 합니다.
...
🔥 **Rule 51. [데이터 1:1 매칭 및 고형분(Brix) 보수적 표기 예외]**
   ...
   - **[🚨고형분 함량 예외 인정]**: 과일 농축액 등의 **'고형분 함량(%)' 표기 시, 시안 수치가 서류 실제 스펙보다 같거나 낮게 표기된 경우(예: 서류 72.74% ➔ 시안 70%)는 보수 표기이므로 무조건 적합(✅)** 판정하십시오.

🔥 **Rule 52. [논리적 모순 탐지 및 마케팅/원재료 N종 카운트 스나이퍼]**
   - 특정 숫자(예: '향료 3종', '비타민 15종')가 명시되어 있다면, 반드시 배합비 서류에서 실제 항목 개수를 낱낱이 카운트하여 대조하십시오.
...
🔥 **Rule 53. [제품명 연동 원료 함량 및 원산지 강제 추적 룰 (🚨마카다미아 등 농산물 원물 한정)]**
   ...
   2) 원산지 검증(농산물 원물 강제 추적): 원료가 **'진짜 농수산물 원물(예: 아몬드, 고구마, 우유, 쌀 등)'**인 경우, 가공품으로 투입되었더라도 반드시 정보표시면에 원물의 원산지가 기재되어야 합니다.
...
🔥 **Rule 55. [영양성분 소수점 및 반올림 강제 규정]**
   - 포화지방 5g 이상은 소수점 없이 정수로, 트랜스지방 0.2g 미만은 소수점 없이 0g으로 표시해야 합니다. 시안에 소수점이 기재되어 있다면(예: 8.0g ➔ 8g 수정 필요) 부적합(🚨) 처리하십시오.
...
# ==========================================
# 📚 4. 탭별 핵심 룰 슬라이싱 (관련 룰 풀텍스트 동적 주입)
# ==========================================
...
RULES_TAB1 = "[탭1 주표시면 관련 핵심 룰]\n" + get_sliced_rules([3, 9, 10, 15, 16, 17, 18, 19, 21, 24, 28, 40, 46, 50, 51, 52, 53] + COMMON_RULES)
RULES_TAB2 = "[탭2 정보표시면/원재료명 관련 핵심 룰]\n" + get_sliced_rules([1, 2, 5, 6, 8, 12, 13, 14, 25, 29, 30, 34, 35, 38, 39, 44, 48, 49, 52, 54] + COMMON_RULES)
RULES_TAB3 = "[탭3 영양성분표 관련 핵심 룰]\n" + get_sliced_rules([3, 4, 6, 10, 11, 21, 23, 25, 26, 27, 31, 32, 33, 40, 41, 55] + COMMON_RULES)
RULES_TAB4 = "[탭4 기타면/측면 관련 핵심 룰]\n" + get_sliced_rules([7, 15, 17, 18, 20, 22, 38, 56] + COMMON_RULES)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V129.0 - 완전 무결점 마스터 버전)")
    ...
    with st.sidebar:
        ...
        st.info("💡 팁: 실행 파일 옆에 `default_docs` 폴더를 만들고 PDF(식약처 고시 등)를 넣어두면 🚀버튼 클릭 시 자동으로 읽어옵니다.")
    ...
    # ==========================================
    # 🔥 핵심 개선: 2-Pass 파이프라인 함수 (V129.0 완성판)
    # ==========================================
    def run_qc_2pass(tab_rules: str, judgment_prompt: str):
        """
        Pass 1: 이미지에서 텍스트 추출만 수행 (판정 완전 금지)
        Pass 2: 추출된 텍스트 + 서류(content)를 함께 제공하여 룰 판정 수행 (식약처 PDF 검색 가능)
        """
...
각 이미지에 라벨이 붙어 있습니다 (예: [타겟_시안_주표시면]). 
...
"""
        ...
        # ── PASS 2: 추출된 텍스트 + 원본 서류(content)를 함께 제공하여 룰 판정 ──
        pass2_prompt = f"""
[PASS 2 - 룰 판정 전용 명령]

아래 [추출된 텍스트 데이터]는 Pass 1에서 이미지와 서류로부터 기계적으로 필사된 원문입니다.
이 텍스트 데이터와 함께 제공된 원본 PDF(식약처 고시 등)를 사실(FACT)로 교차 검증하여 룰북과 대조 판정하십시오.
...
========================================
[추출된 텍스트 데이터 - Pass 1 결과]
{extracted_text}
========================================

🚨 [자기검증 필수 명령]
판정을 시작하기 전, 위 추출 텍스트에서 핵심 수치(소수점, 단위, 괄호 안 성분)를 한 번 더 눈으로 확인하십시오.
추출 과정에서 오독된 수치가 있다면 지금 수정하고 판정을 진행하십시오.

{judgment_prompt}
"""
        try:
            pass2_response = model.generate_content(
                content + [pass2_prompt],  # ✅ 원본 서류(content) 포함 전송!
                generation_config=generation_config,
                safety_settings=safety_settings
            )
            final_output = f"<extracted_log>{extracted_text}</extracted_log>\n{pass2_response.text}"
            return fix_markdown_table(final_output)
        except Exception as e:
            return f"🚨 Pass 2 (룰 판정) 오류 발생: {e}"

    # ==========================================
    # 종합 보고서 전용 단일 호출 (Legacy)
    # ==========================================
    ...
    # 결과 출력 헬퍼 함수
    # ==========================================
    def display_result(result, log_title="🧠 Pass 1 텍스트 추출 로그 보기"):
        if not result:
            return
        extracted_match = re.search(r'<extracted_log>(.*?)</extracted_log>', result, re.DOTALL)
        if extracted_match:
            extracted_log = extracted_match.group(1).strip()
            report_content = result.replace(extracted_match.group(0), "").strip()
            with st.expander(log_title):
                st.markdown(f"*{extracted_log}*")
            thinking_match = re.search(r'<thinking>(.*?)</thinking>', report_content, re.DOTALL)
            if thinking_match:
                thinking_log = thinking_match.group(1).strip()
                report_content = report_content.replace(thinking_match.group(0), "").strip()
                with st.expander("🧠 Pass 2 사전 판독 로그 보기"):
    ...
    with tab1:
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("【2-Pass】 Pass1 텍스트 추출 → Pass2 룰 판정 순으로 진행 중..."):
                judgment_prompt = """
## 1️⃣ [주표시면 및 마케팅 뱃지]
...
- 🚨 띄어쓰기 및 오탈자 적발:
"""
                st.session_state["result_tab1"] = run_qc_2pass(RULES_TAB1, judgment_prompt)
        display_result(st.session_state["result_tab1"], "🧠 Pass 1 주표시면 텍스트 추출 로그")

    # ── TAB 2: 정보표시면 ──
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【2-Pass】 Pass1 텍스트 추출 → Pass2 원재료 대조 판정 중..."):
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        judgment_prompt = """
🚨 [표 작성 필수 명령 절대 강제 - 출력 단축 원천 봉쇄] 🚨
1. 표의 한 행에는 무조건 1개의 원료만 기재하십시오.
2. 🚨 [생략 절대 금지]: '...', '중략' 등으로 축약하는 것을 엄격히 금지합니다.
3. 🚨 [오류 유형별 그룹화 절대 금지]: 원재료 순서 그대로 작성하십시오.
4. 🚨 [사유 설명 강제]: 판정 열에 단순히 '불일치'라고 적지 말고 법적 사유 및 수정 방향을 명시하십시오.
...
3. 🚨 [사유 설명 강제]: 판정 열에 법적 사유와 수정 방향을 상세히 명시하십시오.
...
- 🚨 띄어쓰기 및 오탈자 적발:
"""
                st.session_state["result_tab2"] = run_qc_2pass(RULES_TAB2, judgment_prompt)
        display_result(st.session_state["result_tab2"], "🧠 Pass 1 원재료명 텍스트 추출 로그")

    # ── TAB 3: 영양성분표 ──
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【2-Pass】 Pass1 수치 추출 → Pass2 오차 계산 및 % 검증 중..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    judgment_prompt = """
🚨 [표 작성 필수 명령 절대 강제 - 출력 단축(Lazy Output) 원천 봉쇄] 🚨
1. 🚨 [생략 절대 금지]: 표 안에 '...', '중략' 등으로 축약하는 것을 엄격히 금지합니다. 1번부터 끝까지 100% 전부 타이핑하여 표를 완성하십시오.
2. 🚨 [사유 설명 강제]: 판정 열에 단순히 '불일치'라고 적지 말고, Rulebook에 의거하여 상세한 사유를 반드시 명시하십시오.
...
"""
                    ...
                    judgment_prompt = """
🚨 [표 작성 필수 명령 절대 강제 - 출력 단축(Lazy Output) 원천 봉쇄] 🚨
1. 🚨 [생략 절대 금지]: 모든 행(Row)을 100% 전부 타이핑하여 표를 완성하십시오.
2. 🚨 [사유 설명 강제]: 판정 열에 단순히 '불일치'라고 적지 말고, Rulebook에 의거하여 상세한 사유를 반드시 명시하십시오.
...
- 🚨 띄어쓰기 및 오탈자 적발:
"""
                st.session_state["result_tab3"] = run_qc_2pass(RULES_TAB3, judgment_prompt)
        display_result(st.session_state["result_tab3"], "🧠 Pass 1 영양성분 수치 추출 로그")

    # ── TAB 4: 기타면/측면 ──
    with tab4:
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("【2-Pass】 Pass1 텍스트 추출 → Pass2 교차오염 차집합 연산 및 로고 검증 중..."):
                judgment_prompt = """
...
- 🚨 띄어쓰기 및 오탈자 적발:
"""
                st.session_state["result_tab4"] = run_qc_2pass(RULES_TAB4, judgment_prompt)
        display_result(st.session_state["result_tab4"], "🧠 Pass 1 기타면 텍스트 추출 로그")
            ...
            if not any([st.session_state["result_tab1"], st.session_state["result_tab2"],
                ...
                with st.spinner("모든 분석 데이터를 병합하여 최종 수정 지시서를 작성 중입니다..."):
                    def strip_logs(result):
                        if not result:
                            return "분석 안 함"
                        result = re.sub(r'<extracted_log>.*?</extracted_log>', '', result, flags=re.DOTALL)
