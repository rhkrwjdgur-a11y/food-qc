...
🔥 **Rule 3. [주표시면 vs 영양성분표 수치 100% 일치 및 총열량/식이섬유 열량 계산 강제 룰]**
   ...
   - **[⭐ 열량 계산 시 식이섬유 및 당알콜 분리 필수 적용]**: 영양성분표에 **식이섬유**나 **당알콜**이 표기되어 있다면 반드시 총 탄수화물에서 그 양을 분리하여 계산하십시오. *(적용 공식 ➔ 식이섬유: 2kcal/g, 에리스리톨: 0kcal/g, 자일리톨 등: 2.4kcal/g, 나머지 당질: 4kcal/g)*
...
🔥 **Rule 44. [혼합제제 전개 및 해체 병합(Merge) 완벽 허용 룰]**
   - 혼합제제 하위 성분들을 괄호 없이 개별 원료처럼 분산하여 적는 것은 완벽한 합법(✅)입니다. "혼합제제 괄호가 없다"는 식의 오판을 절대 금지합니다.
...
🔥 **Rule 68. [다포장/세트포장 낱개 분할 표기 스나이퍼 룰]**
   - 검토 모드가 '선물세트 박스(외포장)'일 경우, 단위 내용량이 100g(mL) 이상인 제품은 주표시면과 영양성분표 상단의 총 내용량을 반드시 **식약처 법정 규격인 [총량(단위량 x 개수)] 형태**로 기재해야 합니다. (예: `3,000mL(125mL x 24개)`)
   - 낱개 분할 표기 없이 단순히 `3,000mL`처럼 총량만 덜렁 적혀 있다면 즉시 🚨부적합 처리.
"""
...
RULES_TAB1 = "[탭1 주표시면 관련 핵심 룰]\n" + get_sliced_rules([3, 9, 10, 15, 16, 17, 18, 19, 21, 24, 28, 40, 46, 47, 50, 51, 52, 53, 57, 58, 59, 60, 62, 64, 68] + COMMON_RULES)
RULES_TAB2 = "[탭2 정보표시면/원재료명 관련 핵심 룰]\n" + get_sliced_rules([1, 2, 5, 6, 8, 12, 13, 14, 25, 28, 29, 30, 34, 35, 38, 39, 44, 48, 52, 54, 57, 58, 59, 60, 61, 65] + COMMON_RULES)
RULES_TAB3 = "[탭3 영양성분표 관련 핵심 룰]\n" + get_sliced_rules([3, 4, 6, 10, 11, 21, 23, 25, 26, 27, 31, 32, 33, 40, 41, 55, 59, 66, 67, 68] + COMMON_RULES)
RULES_TAB4 = "[탭4 기타면/측면 관련 핵심 룰]\n" + get_sliced_rules([7, 15, 17, 18, 20, 22, 24, 38, 56, 57, 59, 63, 64] + COMMON_RULES)
...
- ⭐ 포장 단위(수량) 2-Track 검증:
- ⭐ 띄어쓰기 및 오탈자 정밀 스캔: (원재료명 내 자음/모음 오타 여부)
"""
                    else:
                        judgment_prompt = base_tab2_warning + """
## 1️⃣ [원재료명 2-Way 정밀 교차 검증 (시안 기준 대조 + 누락 적발)]
...
- ⭐ [Rule 68] 다포장 낱개 분할 표기 스나이퍼: (박스 시안일 경우 총내용량 표기가 [총량(단위량 x 개수)] 형태인지 스캔)
