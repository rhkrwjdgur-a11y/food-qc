...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V328.00 - 최종 궁극의 마스터본)")
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                4. ⭐ [마스터 DB 출처 철통 격리]: 2-1 서류 기반 마스터표를 작성할 때는 오직 업로드된 'PDF/엑셀 서류(한글라벨, 배합비)'의 텍스트만 100% 신뢰하여 작성하십시오. 시안(포장지)에 서류에 없는 원료(예: 복숭아농축액)가 적혀 있더라도 절대 시안의 텍스트를 서류 DB 표에 섞어 넣지(Merge) 마십시오! 공통 원료가 같다고 해서 다른 맛 원료를 임의 병합하면 치명적 환각 오류입니다.
                5. ⭐ [투트랙 스마트 1:1 대조 및 억지 맵핑 금지]:
                   ...
                   - [Track 2. 핵심 원물 억지 치환 절대 금지]: 단, 과일류(과채농축액), 유가공품 및 균류(유산균, 원유 등), 곡류, 육류 등 제품의 본질을 결정하는 핵심 원물은 종류가 다르면 절대 Rule 90을 핑계로 억지 매칭하지 마십시오! (예: 서류의 `복숭아농축액`을 시안의 `유산균배양액`과 매칭하는 것은 최악의 오류입니다.) 핵심 원물이 불일치하거나 매칭할 상대가 없다면 해당 칸에 `❌ 매칭 없음`을 기재하고 즉시 `🚨 부적합 (원료 완전 불일치)` 판정을 내리십시오.
                6. [순서 역전 정밀 검증 (Rank A 적용)]: 모든 원료의 절대 배합비 순위(Rank A) 중 2% 이상인 원료의 순위를 대조하고, 서류와 시안의 순서가 역전되었다면 🚨부적합 처리하십시오. (단, 배합비 없으면 동적 추론 허용)
