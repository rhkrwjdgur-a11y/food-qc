...
# ==========================================
# 📚 3. 54대 룰북 원문 (단 한 글자도 생략/축소 금지 100% 풀버전)
# ==========================================
RULE_BOOK = """
...
🔥 **Rule 24. [감미료 14pt 의무 표기 및 다중 강조표시 규칙]**
   ...
   - **[🚨다중 강조 시 위치 강제]: 단, 당류 강조표시(ZERO, 무당 등)가 시안에 2회 이상 반복되어 있는 경우에는, 반드시 "가장 큰 강조표시 주위"에 감미료 함유 문구를 기재해야 합니다.** 만약 1회만 표시되었거나, 뱃지가 가장 큰 강조표시 주위에 배치되어 있다면 뱃지 형태라도 적합(✅)합니다. 뱃지라고 무조건 부적합 처리하지 마십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V46.0 - 전(全) 탭 Thinking 부활판)")
    ...
    def run_qc_model(prompt_text):
        ...
        표(Table)를 그릴 때 무조건 마크다운 파이프(|) 기호를 지키고 행(Row)마다 **엔터(Enter, 줄바꿈)**를 치십시오.
    ...
    with tab1:
        ...
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("주표시면 텍스트 및 뱃지 정밀 대조 중..."):
                ...
                🚨 [사전 판단(Thinking) 강제]: 리뷰를 출력하기 전에 반드시 `<thinking>` 태그를 열어 시안의 가장 큰 글씨(강조표시)가 무엇인지, 그 옆에 뱃지가 적절히 붙어있는지, 고형분 함량이 서류보다 낮은지 등을 먼저 논리적으로 분석하십시오.
                ...
                ## 1️⃣ [주표시면 및 마케팅 뱃지]
                - 결론: (✅ 적합 또는 🚨 부적합/확인요망)
                - 🚨 [Rule 24] 감미료 함유 문구 위치 적합성: (가장 큰 강조표시 주위에 감미료 함유가 있는지 확인)
                - 100% 원액 및 고형분 강조 적합성: (고형분 함량이 서류 스펙보다 낮게 보수적으로 적힌 것은 합법)
                - 마케팅 숫자(N종, N곡 등) 정합성: 
                - 제품명 연동 함량(%) 표기 여부: 
                - 기타 특이사항 (과장광고 등): 
                """
    ...
    # ==========================================================
    # 탭 2: 정보표시면 검토 (Thinking 유지 - 정렬 전용)
    # ==========================================================
    with tab2:
        st.info("정보표시면 이미지와 한글라벨, 배합비를 대조하여 원재료명, 원산지, 알레르기를 전수 검토합니다.")
        if st.button("▶️ 정보표시면 원재료 100% 대조 시작", key="btn_info"):
            with st.spinner("배합비 순위 정렬 및 표 렌더링 중..."):
                prompt = """
                [지시]: 오직 아래의 원재료명 표와 알레르기 교차검증 양식만 출력하십시오. 
                
                🚨 [배합비 순위 정렬 전용 Thinking 강제] 🚨
                당신은 소수점이 포함된 배합비(%)를 한 번에 정렬하는 능력이 취약합니다. 표를 그리기 전에 반드시 `<thinking>` 태그를 열고, 배합비 서류에 적힌 모든 원료의 배합비(%) 숫자를 추출한 뒤, 가장 높은 숫자부터 내림차순으로 1위부터 끝까지 정렬하여 메모하십시오. 
                
                <thinking>
                (여기에 원료명과 배합비(%)를 내림차순으로 정렬한 리스트 작성)
                </thinking>
                
                ## 2️⃣ [원재료명 및 원산지 대조]
                - 결론: (✅ 적합 또는 🚨 부적합)
                
                (🚨 경고: 아래 표를 작성할 때 무조건 줄바꿈을 적용하고 파이프(|) 기호를 누락하지 마십시오. Rule 53, 54 내용은 표에 넣지 말고 바깥으로 빼십시오.)
                | No | 시안 원재료명 | 서류 매칭 원료 (품번, 스펙 100% 기재) | 배합비 검증 (투입 순위 필수) | 판정 및 사유 |
                |---|---|---|---|---|
                
                - [Rule 53, 54 검토 사항]: (제품명 연동 원료, 복수 원산지 비율 위반 사항이 있다면 여기에 별도로 서술하십시오.)
                ...
                if result:
                    ...
                    if thinking_match:
                        thinking_log = thinking_match.group(1).strip()
                        report_content = result.replace(thinking_match.group(0), "").strip()
                        with st.expander("🧠 배합비 내림차순 정렬 및 알레르기 교차 검증 로그 보기"):
                            st.markdown(f"*{thinking_log}*")
                        st.markdown(report_content)
                    else:
                        st.markdown(result)
    ...
    with tab4:
        ...
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("기타면/측면 텍스트 및 마케팅 적합성 검토 중..."):
                ...
                🚨 [사전 판단(Thinking) 강제]: 리뷰를 출력하기 전에 반드시 `<thinking>` 태그를 열어 기타면 이미지에 있는 마케팅 문구를 추출하고 룰 위반 여부를 먼저 판단하십시오.
