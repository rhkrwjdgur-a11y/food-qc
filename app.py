...
🔥 **Rule 44. [혼합제제 하위성분 '주용도 묶음표기' 절대 금지]**
   - 혼합제제의 괄호 안 하위 성분으로 들어가는 첨가물들은, 용도가 겹치더라도 '산도조절제', '유화제' 등의 주용도명으로 묶어서 퉁칠 수 없습니다. 반드시 '구연산', '사과산' 등 각각의 개별 첨가물 명칭을 낱낱이 전개해야 합법입니다. 시안에 혼합제제(산도조절제) 처럼 묶어 썼다면 즉시 부적합(🚨) 처리하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V56.0 - 하드코어 1:1 매칭 최적화)")
    ...
    # ==========================================================
    # 탭 2: 정보표시면 검토 (V56 띵킹 과부하 완벽 차단 패치)
    # ==========================================================
    with tab2:
        st.info("정보표시면 이미지와 한글라벨, 배합비를 대조하여 원재료명, 원산지, 알레르기를 전수 검토합니다.")
        if st.button("▶️ 정보표시면 원재료 100% 대조 시작", key="btn_info"):
            with st.spinner("서류 분석 및 원재료 1:1 정밀 대조 표 생성 중... (최대 40초 소요)"):
                prompt = """
                [지시]: 오직 아래의 원재료명 표와 알레르기 교차검증 양식만 출력하십시오.
                
                🚨 [1단계: 초단기 전략 수립 (Thinking 강제)] 🚨
                `<thinking>` 태그 안에서는 원재료 1:1 매칭을 절대 하지 마십시오! 성분이 너무 많아 시스템이 멈춥니다. 매칭은 2단계 표를 그리면서 직접 하십시오.
                여기는 오직 아래 3가지만 단답형으로 적고 즉시 닫으십시오.
                <thinking>
                1. 전체 배합비(%) 유무: (O/X)
                2. Rule 53 타겟 원료 파악: (제품명에 쓰인 원료만 단답형 기재)
                3. 알레르기 특이사항: (없음/있음)
                </thinking>
                ...
                ## 2️⃣ [원재료명 및 원산지 대조]
                - 결론: (✅ 적합 또는 🚨 부적합)
                (🚨 절대 경고: 아래 표를 작성할 때 무조건 파이프(|) 기호를 사용하십시오. 원재료가 50개가 넘더라도 절대 멈추지 말고 끝까지 표를 그리며 대조를 수행하십시오.)
                [🟢 올바른 표 작성 예시]
                | No | 시안 원재료명 | 서류 매칭 원료 (품번, 스펙 등) | 배합비 검증 (모르면 생략) | 판정 및 사유 |
                |---|---|---|---|---|
                | 1 | 코코아분말 | 코코아 100% (네덜란드) | 서류 내 % 미상 | ✅ 적합 (Rule 53: 가공품이므로 원산지 표기 면제) |
                
                (여기에 위 예시를 참고하여 시안에 적힌 모든 원료를 대조하는 표를 그리십시오.)
                
                - [Rule 53, Rule 44 검토 사항]: (제품명 연동 원료, 혼합제제 묶음표기 등 위반 사항 별도 서술)
        ...
        if st.session_state["result_tab2"]:
            ...
            if thinking_match:
                thinking_log = thinking_match.group(1).strip()
                report_content = result.replace(thinking_match.group(0), "").strip()
                with st.expander("🧠 서류 분석 초단기 전략 로그 보기"):
                    st.markdown(f"*{thinking_log}*")
                st.markdown(report_content)
            else:
                st.markdown(result)
