...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (V6.60 - 혼합제제 전개 완벽 지원판)")
    ...
    def process_qc(ptype, imode, content_hashes):
        ...
        # 🚨 [사용자님 요청 반영 패치 구역] : 표 2번 양식을 완벽하게 수정했습니다.
        final_prompt = f"""
        [제품유형]: {ptype}
        [검토모드]: {imode}
        ...
        - 결론: (✅ 또는 🚨)
        - 🚨 [긴급 차단 명령1]: 향료는 향미증진제가 아닙니다. 오지랖 지적 금지.
        - 🚨 [긴급 차단 명령2]: 한글라벨의 '혼합제제'가 시안에서 하위성분으로 전개(Flattening)되어 흩어져 기재된 경우, 이를 완벽한 합법(✅)으로 처리하고 지적하지 마십시오. 배합비는 순서 검증에만 쓰십시오.
        | No | 시안 원재료명 | 한글라벨 매칭 원료 (혼합제제 전개 포함) | 배합비 순서 검증 (2% 룰) | 판정 |
        |---|---|---|---|---|
        ...
        - 결론: (✅ 또는 🚨)
        - 내용: (단품 모드 시 "해당 없음". 선물세트 모드 시: 구역별로 정밀 대조하여 보고. 텍스트 불일치 지적 시 반드시 증거 텍스트를 나란히 제시할 것. 환각 금지.)
