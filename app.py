...
# ==========================================
# 📚 3. 62대 룰북 원문 (V147.0 완결판 - 축산물 냉장/냉동 자체 추론 룰 추가)
# ==========================================
RULE_BOOK_FULL = """
...
🔥 **Rule 54. [복수 원산지 혼합 비율 생략 합법성 검증 룰]**
   - 정보표시면 원재료명에 단일 원료에 대해 2개 단 이상의 국가가 병기되어 있고 각 국가별 혼합 비율(%)이 기재되어 있지 않은 경우 반드시 🚨(확인 요망) 플래그를 띄우십시오.
...
🔥 **Rule 60. [복합원재료(페이스트/농축액 등) 원물 함량 강제 환산 룰]**
   - 제품명에 특정 원료(예: 호두, 아몬드)를 강조했는데 그 원료가 페이스트, 추출액, 농축액 등 '복합원재료' 형태로 투입된 경우, 복합원재료 투입량(%)만 단독으로 표기하면 식약처 단속 대상(🚨부적합)입니다.
   - **[✅합법 표기법]**: 반드시 괄호 안에 진짜 원물의 배합함량(%)을 기재하거나(예: 아몬드페이스트 0.15%(아몬드 100%)), 환산된 최종 원물 함량(%)을 주표시면 또는 원재료명에 명시해야 합니다.

🔥 **Rule 61. [원산지 표기 타겟팅 절대 룰 (가공국 vs 원물국 혼동 방지)]**
   - 국내에서 가공된 페이스트, 추출액, 유지류(기름) 등의 복합원재료 원산지를 표기할 때, 원물 명칭 없이 국가명만 단독으로 괄호 안에 적으면 🚨부적합(원산지 허위표시)입니다.
   - **[🚨위반 예시]**: `아몬드페이스트(미국산)` ➔ 페이스트 자체(가공품)가 수입산이라는 의미가 되므로 불법.
   - **[✅합법 표기법]**: `아몬드페이스트(아몬드-미국산)` 또는 `옥배유(옥수수배아-외국산)`처럼 원료의 타겟이 되는 원물 명칭을 반드시 함께 기재해야 합니다.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V147.0 - 축산물 보관상태 자체 추론 패치)")
...
- 🚨 [Rule 60] 복합원재료(페이스트/농축액 등) 원물 함량 강제 환산 여부 검증:
...
4. 🚨 [가공국 vs 원물국 검증 (Rule 61)]: 페이스트/유지류 등에 '원물명-국가명' 형태로 정확히 타겟팅되어 적혀있는가? (단순히 '(미국산)'이라고 적혀있으면 🚨부적합)
