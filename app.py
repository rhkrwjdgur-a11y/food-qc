...
✅ **Rule 10. 영양성분 강조표시 (액체/고체 분리)**
   - 제형에 따라 100g/100mL 당 기준을 분리하여 심 심사.
...
🔥 **Rule 56. [HACCP 인증 마크 공식 정식 텍스트 검증]**
   - "안전관리인증", "식품안전관리인증" 확인.
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                doc_mode = st.session_state.get("doc_type_state", doc_type)
                ins_mode = st.session_state.get("inspection_mode_state", inspection_mode)
...
| 시안 표기 원재료명 (100% 나열) | 매칭된 서류 원료명 (없으면 '제출 안 됨') | 대조 검증 결과 (명칭 대조 및 ⭐상위 3순위 원산지 표기 누락 여부⭐) | 최종 판정 |
|---|---|---|---|
...
"""
                # 🚀 [분기 2] 증빙 서류 없이 '박스 vs 팩' 1:1 비교만 하는 경우
                    ...
                    if "박스" in ins_mode:
                        missions.extend([
                            "오직 '타겟(박스) 시안'의 원재료명 리스트만 100% 나열하십시오.",
                            "오직 '비교용(팩) 시안'의 원재료명 리스트만 100% 나열하십시오. 증빙 서류는 없으므로 원산지 산정 검증은 생략합니다."
                        ])
                        common_tab2_prompts = """## 2️⃣-1. [내포장(팩) 정답지 원재료 DB] (비교용 팩 시안 기준)
| 비교용 팩 시안 원재료명 | 비고 (서류 미제출로 배합비 알 수 없음) |
|---|---|
...
| 타겟(박스) 표기 원재료명 | 비교용(팩) 표기 원재료명 | 대조 검증 결과 (픽셀 100% 일치 여부) | 최종 판정 |
|---|---|---|---|
...
"""
    ...
    with tab5:
        st.info("💡 [AI 자율 스캔 모드] 기계적 검증(1~4번 탭)이 잡아내지 못하는 문맥상의 위법성, 과대광고, 소비자 기만 행위를 법령 PDF를 기반 심층 스캔합니다.")
