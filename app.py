...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V106.0 - 수량 텍스트 1:1 스나이퍼 완결판)")
    ...
    with tab1:
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("주표시면 기계적 텍스트 추출 및 정밀 대조 중..."):
                ...
                4. 🚨 [기계적 수량/오탈자 교차 검증]: 주표시면의 포장 단위(예: N입, 총 용량)를 파악하고, 업로드된 다른 면의 수량 정보와 수학적 계산이 아닌 '텍스트 형태(개입/팩, x/X 등)'가 100% 일치하는지 기계적으로 비교하십시오.
                </thinking>
                
                ## 1️⃣ [주표시면 및 마케팅 뱃지]
                - 결론: (✅ 적합 또는 🚨 부적합/확인요망)
                - 🚨 [Rule 50] 원액/추출물 고형분 병기 및 명칭 적합성: 
                - 추가 마케팅 문구(추출분말 등) 적합성: (분말의 고형분 면제 및 명칭 일치 여부 명시)
                ...
                - 🚨 포장 단위 텍스트 1:1 완벽 일치 여부: 수학적 총량(계산)이 맞더라도, 기재된 단어(예: 개입 vs 팩)나 기호(대소문자 X)가 패키지 면마다 단 하나라도 다르게 혼용되었다면 무조건 🚨부적합(불일치)으로 적발하십시오.
                - 🚨 띄어쓰기 및 오탈자 적발: 
                """
            ...
            with st.spinner("서류 유형 분석 및 창의성 배제 100% 교차 검증 중..."):
                ...
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        2. 🚨 [과부하 방지: 통합 문서 절대 기준화]: 사용자가 업로드한 엑셀/PDF 서류를 '절대 정답지'로 간주하십시오. 
                        3. 🚨 [기계적 1:1 대조 절대 명령]: 업로드된 서류, 박스, 팩의 텍스트를 기계적으로 1:1 검증하십시오.
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 하단 구석의 '식품이력추적관리번호' 주변 수량을 추출하여 타 면과 수학적 계산이 아닌 '텍스트 형태(대소문자, 단어)'가 100% 일치하는지 적발하십시오.
                        </thinking>
                        
                        🚨 [표 작성 필수 명령 절대 강제 - 출력 단축(Lazy Output) 원천 봉쇄] 🚨
                        1. 표의 한 행에는 무조건 1개의 원료만 기재하십시오.
                        2. 🚨 [생략 절대 금지]: 대외 보고서 제출용입니다. 표 안에 '...', '중략', '이하 생략', '해당 없음', '이하 동문' 등의 기호나 단어로 퉁치거나 축약하는 것을 엄격히 금지합니다. 원재료가 50개든 100개든 1번부터 끝까지 물리적으로 모든 행(Row)을 100% 전부 타이핑하여 완성하십시오. 단 하나의 원료라도 건너뛰면 즉시 시스템 폐기 수준의 중대한 오류로 간주합니다.
                        3. 🚨 [오류 유형별 그룹화(Categorizing) 절대 금지]: '[원산지 표기 불일치]', '[원료 누락]' 등과 같이 에러 종류별로 묶어서 표를 임의로 쪼개거나 변형하지 마십시오. 반드시 시안에 적힌 원재료 순서 그대로 1번부터 끝까지 순차적으로 1:1 매칭 표를 작성하십시오.
                        ...
                        3. 🚨 [기계적 1:1 대조 절대 명령]: 엑셀/PDF 서류의 내용과 시안 텍스트를 1:1로 검증하십시오.
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 이미지 하단 구석의 수량을 추출하여 타 면과 수학적 계산이 아닌 '텍스트 형태(대소문자, 단어)'가 100% 일치하는지 적발하십시오.
                        </thinking>
                    ...
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        3. 🚨 [기계적 3방향 교차 대조]: 도출된 정답 리스트를 바탕으로 박스/팩 시안과 대조하십시오. 의미가 같아도 철자나 괄호 유무가 다르면 무조건 🚨부적합입니다.
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 수량을 추출하여 타 면과 수학적 계산이 아닌 '텍스트 형태(대소문자, 단어)'가 100% 일치하는지 적발하십시오.
                        </thinking>
                        
                        🚨 [표 작성 필수 명령 절대 강제 - 출력 단축 원천 봉쇄] 🚨
                        1. 🚨 [생략 절대 금지]: 대외 보고서 제출용입니다. 어떠한 경우에도 표 안에 '...', '중략', '이하 생략' 등으로 축약하는 것을 엄격히 금지합니다. 모든 행을 100% 전부 타이핑하십시오.
                        2. 🚨 [오류 유형별 그룹화(Categorizing) 절대 금지]: '[원산지 표기 불일치]', '[원료 누락]' 등과 같이 에러 종류별로 묶어서 표를 임의로 변형하지 마십시오. 반드시 시안에 적힌 원재료 순서 그대로 1번부터 끝까지 순차적으로 1:1 매칭 표를 작성하십시오.
                        ...
                        3. 🚨 [기계적 교차 대조]: 도출된 정답 리스트를 시안과 대조하십시오. 의미가 같아도 철자가 다르면 🚨부적합입니다.
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 수량을 추출하여 타 면과 수학적 계산이 아닌 '텍스트 형태(대소문자, 단어)'가 100% 일치하는지 적발하십시오.
                        </thinking>
        ...
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("오차 계산 및 100% 전체 영양성분 3방향 대조 렌더링 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    ...
                    2. 🚨 [소수점 및 행열 스나이퍼 절대 명령]: 영양성분표의 숫자를 추출할 때, **소수점(.)**을 절대 놓치지 마십시오. (예: 2.5mg을 25mg이나 250mg으로 오독하는 시각적 환각 엄격히 금지). 또한 좁은 표 안에서 특정 성분(예: 칼륨, 아연)을 건너뛰거나 다른 줄의 숫자를 끌어오는 행열 이탈을 절대 금지합니다. 돋보기를 들이대듯 픽셀 하나하나를 검증하십시오.
                    3. 오차 연산: 서류 실측값 대비 표시량이 오차 범위 내인지 계산하십시오.
                    4. 일치 여부 확인: 팩과 박스의 숫자가 100% 동일한지 대조하십시오.
                    5. 🚨 [수량/오탈자 교차 검증 절대 명령]: 총 내용량을 확인하여 다른 면의 수량과 텍스트 형태(팩/개입, x/X 등)가 완벽히 1:1 일치하는지 적발하십시오. (수학적 계산 합격 처리 금지)
                    </thinking>
                    
                    🚨 [표 작성 필수 명령 절대 강제 - 출력 단축(Lazy Output) 원천 봉쇄] 🚨
                    1. 🚨 [생략 절대 금지]: 대외 보고서 제출용입니다. 어떠한 경우에도 표 안에 '...', '중략', '이하 생략', '해당 없음' 등으로 축약하는 것을 엄격히 금지합니다. 영양성분이 30개가 넘어도 1번부터 끝까지 물리적으로 모든 행(Row)을 100% 전부 타이핑하여 표를 완성하십시오. 단 하나의 성분이라도 건너뛰면 치명적 오류로 간주합니다.
                    ...
                    2. 🚨 [소수점 및 행열 스나이퍼 절대 명령]: 영양성분표의 숫자를 추출할 때, **소수점(.)**을 절대 놓치지 마십시오. (예: 2.5mg을 250mg으로 오독하는 시각적 환각 엄격히 금지). 표 안의 성분을 건너뛰거나 줄을 헷갈리는 행열 이탈을 절대 금지합니다. 돋보기를 들이대듯 픽셀 하나하나를 검증하십시오.
                    3. 오차 연산: 서류 대비 표시량이 오차 범위 내인지 계산하십시오.
                    4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 영양성분표의 총 내용량을 확인하여 텍스트 형태(팩/개입, x/X 등)가 완벽히 일치하는지 비교하십시오.
                    </thinking>
                    
                    🚨 [표 작성 필수 명령 절대 강제 - 출력 단축(Lazy Output) 원천 봉쇄] 🚨
                    1. 🚨 [생략 절대 금지]: 대외 보고서 제출용입니다. 어떠한 경우에도 표 안에 '...', '중략', '이하 생략' 등으로 축약하는 것을 엄격히 금지합니다. 모든 행(Row)을 100% 전부 타이핑하여 표를 완성하십시오.
    ...
    with tab4:
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("기타면/측면 기계적 텍스트 검토 중..."):
                prompt = """
                ...
                3. 🚨 [수량/오탈자 교차 검증 절대 명령]: 기재된 수량이 다른 모든 면과 텍스트 형태(글자, 기호)가 100% 일치하는지 기계적으로 스캔하십시오. 계산이 맞다고 합격시키지 마십시오.
                </thinking>
        ...
        if st.button("▶️ 최종 종합 리포트 생성", key="btn_summary"):
            if not any([st.session_state["result_tab1"], st.session_state["result_tab2"], st.session_state["result_tab3"], st.session_state["result_tab4"]]):
                ...
                with st.spinner("모든 분석 데이터를 병합하여 최종 수정 지시서를 작성 중입니다..."):
                    ...
                    ### 📌 [핵심 지적 사항 및 수정 지시]
                    (위 분석 데이터에서 '부적합(🚨)' 또는 '확인요망'이 나온 내용들만 뽑아서 1, 2, 3번 불릿 포인트로 요약하십시오. 특히 '포장 단위 텍스트 충돌', '띄어쓰기 오탈자', '고형분 누락'은 가장 중요하게 다루십시오.)
