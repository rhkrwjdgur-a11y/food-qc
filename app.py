...
✅ **Rule 2. 향료 및 첨가물 명칭 유연화 (통합 표기 합법성)**
   ...
   - **[🚨 통합 예외 족쇄]**: 단, '향료'로 묶어 표기할 수 있는 대상은 서류상 식품유형이 명백히 '향료'이거나 '천연/합성향료'인 경우에만 한정됩니다. 이름에 '향'이 들어간다고 해서 식품유형이 '혼합제제'인 원료까지 '향료'라는 단어 속에 임의로 흡수시키지 마십시오.

🔥 **Rule 3. [주표시면 vs 영양성분표 수치 100% 일치 및 총열량/식이섬유 열량 계산 강제 룰]**
   - 주표시면(앞면)에 특정 영양소 함량(예: 칼슘 200mg)이 강조되어 있다면, 뒷면 영양성분표의 수치와 단 1의 오차도 없이 100% 일치해야 합니다. (수치 보수적 표기 핑계 금지)
   - 박스/세트 포장의 주표시면에는 반드시 '총 내용량'과 '총 열량(kcal)'이 모두 기재되어야 합니다.
   - 식이섬유나 당알콜 표기 시 총 탄수화물에서 그 양을 분리하여 열량을 계산하십시오.
...
🔥 **Rule 7. [당알코올 주의문구 개정 (10% 컷오프 룰)]**
   - 자일리톨, 에리스리톨 등 당알코올류가 최종 제품에 **10% 미만**으로 사용된 경우 "과량 섭취 시 설사..." 주의문구 생략은 100% 합법(✅)입니다.
...
🔥 **Rule 11. [영양정보 단방향 허용오차 법칙 (수학적 역산 절대 금지!)]**
   - **[하한선 그룹(비타민,미네랄,단백질 등)]**: `(성적서 실측값) >= (시안 표시량 × 0.8)` 이면 무조건 합법(✅). 거꾸로 나누어 부적합 내는 환각 금지.
   - **[상한선 그룹(열량,나트륨,당류 등)]**: `(실측값) <= (시안 표시량 × 1.2)` 이면 합법(✅).

✅ **Rule 12. [원재료명 3단 교차 검증 및 임의 추론 금지]**
   - 배합비 서류 없이 레시피를 상상하지 마십시오.

🔥 **Rule 13. [알레르기 정밀 추적 및 위치 표기 절대 규칙]**
   - 알레르기 정보는 바탕색과 구분되는 '별도 란(박스)'에 한 번만 기재되어 있으면 100% 합법입니다. 파생 원료가 있다면 일치로 인정하십시오.
...
🔥 **Rule 20. [포장재질 표시 (식약처 vs 환경부 분리 스나이퍼)]**
   - 종이나 유리는 텍스트 재질 표시 의무가 없으므로 생략해도 합법입니다. 박스 시안에 종이 분리배출 마크가 누락되어도 합법(✅)입니다.
...
🔥 **Rule 24. [당류 강조표시 연계 의무 표기 룰]**
   - 무당/저당 등 강조 시 열량 병기 의무, 당류 컷오프 확인 강제, 감미료 함유 문구 기재 여부 스캔.
...
🔥 **Rule 28. [원산지 3순위 완벽 필터링 및 강제 재정렬 룰 (숫자 세기 환각 절대 금지!)]**
   - 정제수, 당류, 첨가물은 물론이고 **유산균, 효모, 사균체 분말, 포스트바이오틱스 등 '미생물류' 역시 농수산물이 아니므로 원산지 산정에서 100% 강제 삭제**하십시오.
   - 단순 텍스트 나열 숫자를 세는 환각을 버리고, 남은 '진짜 농수산물' 원료만 모아 배합량(순서)대로 새롭게 법적 1, 2, 3순위를 재정렬하십시오.

🔥 **Rule 29. [국내 가공 복합원재료 원산지 역추적 합법성]**
   - 국내 가공 복합원재료라도 하위 원물의 원산지를 역추적해 전개 표기했다면 합법(✅)입니다.
...
🔥 **Rule 35. [🌟 범용 간략명/동의어 허용 및 N종 묶음 절대 금지]**
   - 식약처 공식 이명(구연산나트륨=구연산삼나트륨)이나 화학식(CMC) 표기는 100% 합법입니다.
   - 고도 정제 원료 기원 생략, 첨가물 부형제 생략, 괄호 생략 모두 완벽한 합법(✅).
...
🔥 **Rule 38. [알레르기 교차오염 완벽 검증 (수학적 차집합)]**
   - [교차오염 정답지] = [공장 취급 마스터] - [직접 투입 알레르기 물질]. 누락 시 부적합(🚨).

🔥 **Rule 39. [동명 원료 및 식품유형 종속성 분리 룰 (병합 절대 금지)]**
   - 서류상 원료 명칭이 같거나 비슷하더라도(예: '요구르트향'과 '요구르트향 베이스'), **해당 원료들의 [식품유형]이 서로 다르면(예: 향료 vs 혼합제제) 절대 하나의 항목으로 병합하거나 중복으로 취급하지 마십시오.** 각각 분리 표기되어야 합법입니다.

🔥 **Rule 40. [열량 표기 및 반올림 원칙]**
   - 세트 총 열량은 실측 소수점을 합산하여 5kcal 단위로 반올림하므로 모순 지적 금지.
...
🔥 **Rule 44. [혼합제제 전개 및 해체 병합 완벽 허용 룰]**
   - 혼합제제는 괄호로 묶든, 괄호를 깨고 내림차순으로 뿔뿔이 흩어지게 적든 모두 완벽한 합법(✅)입니다.
...
🔥 **Rule 50. [원액/추출물 고형분 의무 표시 강제 룰]**
   - 앞면에 '농축액', '추출물' 함량(%) 강조 시 반드시 주변에 '고형분 함량(%)' 병기 강제!

🔥 **Rule 51. [고형분(Brix) 보수적 표기 예외]**
   - 시안 수치가 서류 스펙보다 낮으면 보수 표기이므로 합법(✅).

🔥 **Rule 52. [영양성분 단순 명칭 강조 컷오프 완벽 검증 룰 (수학 증명 강제)]**
   - '단백질 4g', '칼슘 함유' 등 강조 시 아래 조건 중 **단 하나라도 충족**해야 합법(✅).
   - [단백질]: 100g 10% / 100mL 5% / 100kcal 5% / 1회섭취량 10%
   - [비타민/무기질]: 100g 15% / 100mL 7.5% / 100kcal 5% / 1회섭취량 15%
   - ⭐ **[부적합 시 수학적 증명 족쇄]**: 기계가 🚨부적합 판정을 내리려면, 반드시 위 4가지(100g, 100mL, 100kcal, 1회섭취량)에 대한 모든 수식을 나열하고 4가지 모두 기준치에 미달함을 수학적으로 증명해야만 합니다. 하나라도 통과하면 부적합 판정을 즉각 철회하십시오!
...
🔥 **Rule 59. [CS 및 1399 신고 의무표시 3종 강제 스캔 룰]**
   - 상담번호, 반품처, "국번없이 1399" 문구가 전 구역 중 하나라도 있으면 합법.

🔥 **Rule 60. [복합원재료 원물 함량 기재 면제 룰]**
   - 농축액 괄호 안에 '고형분(%)'이 명시되어 있다면 추가 배합함량 기재를 강요하지 마십시오. 합법(✅).
...
🔥 **Rule 66. [영양성분 법정 단위 하드코딩 매칭 룰 (단위 스나이퍼)]**
   - 영양소 단위(g, mg, µg)가 법정 단위와 다르면 🚨부적합.

🔥 **Rule 67. [영양정보표 하단 법정 안내문구 토시 검증 룰]**
   - 하단 2000kcal 안내 문구가 법적 용어 그대로 기재되어야 함. 축약 금지.

🔥 **Rule 68. [다포장/세트포장 낱개 영양표시 복붙 스나이퍼]**
   - 박스 시안 내용량이 낱팩 용량만 적혀있거나 영양표시가 '1개당' 기준이 아니면 복붙 에러(🚨).

🔥 **Rule 69. [비타민 아래첨자(Subscript) 타이포 스나이퍼]**
   - 비타민 B1, B2 등이 일반 숫자면 ⚠️아래첨자 통일 교정 권고.

🔥 **Rule 70. [내/외포장 원재료명 100% 일치 강제 범용 스나이퍼]**
   - 내포장(팩)과 외포장(박스) 텍스트 대조 시에는 의미 해석과 Rule 35를 완전히 끄고 픽셀 단위로 대조하여 다르면 🚨부적합 처리.
...
🔥 **Rule 76. [OEM 업소명 타이틀 강제 스캔 (유통전문판매원)]**
   - 패키지에 '제조원(타사)'이 명시된 위탁생산 제품일 경우, 자사의 상호명 앞에는 반드시 **'유통전문판매원:'** 또는 **'판매원:'** 등의 법적 영업소 타이틀이 기재되어야 합니다. 직함 없이 상호명만 적혀있다면 🚨부적합(타이틀 누락) 처리하십시오.

🔥 **Rule 77. [범용 식품유형 필수 주의문구 강제 스캔 (질식/해동 등)]**
   - 식품유형에 따라 아래 필수 문구를 스캔하십시오.
   - 1) **냉동식품:** "이미 냉동되었으니 해동 후 다시 냉동하지 마십시오."
   - 2) **고카페인:** "어린이, 임산부 및 카페인 민감자는 섭취에 주의..."
   - 3) **고체 젤리/과채류:** "섭취 시 목에 걸릴 우려가 있으니 주의..." (🚨 주의: 과채음료, 주스 등 액체류는 목걸림 문구 해당 없음! 억지 지적 절대 금지)
   - 4) **아스파탐 첨가:** "페닐알라닌 함유"
"""
...
RULES_TAB2 = "[탭2 정보표시면/원재료명 관련 핵심 룰]\n" + get_sliced_rules([1, 2, 5, 6, 7, 8, 12, 13, 14, 20, 25, 28, 29, 30, 34, 35, 38, 39, 44, 48, 52, 54, 57, 58, 59, 60, 61, 65, 68, 69, 70, 73, 74, 75, 76, 77] + COMMON_RULES)
RULES_TAB3 = "[탭3 영양성분표 관련 핵심 룰]\n" + get_sliced_rules([3, 4, 6, 10, 11, 21, 23, 25, 26, 27, 31, 32, 33, 40, 41, 52, 55, 59, 66, 67, 68, 69] + COMMON_RULES)
RULES_TAB4 = "[탭4 기타면/측면 관련 핵심 룰]\n" + get_sliced_rules([7, 15, 17, 18, 20, 22, 24, 38, 52, 56, 57, 59, 63, 64, 69, 73, 74, 75, 77] + COMMON_RULES)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V309.0 - 통합 마스터 매트릭스 & 환각 완전 차단)")
    st.markdown("<hr class='hide-on-print'>", unsafe_allow_html=True)

    with st.sidebar:
        st.header("📄 검토 설정 및 파일 업로드")
        
        with st.expander("⚙️ 고급 설정 (OCR & 수동 입력)", expanded=False):
            use_vision_api = st.checkbox("🔠 Vision API 병행 사용 (강력한 환각 억제)", value=False, help="GCP 서비스 계정 키가 설정되어 있어야 작동합니다.")
...
2. ⭐ [누락 및 요약 절대 금지]: "등", "등 다수", "..."을 써서 퉁치는 행위를 영구히 금지합니다. 단 하나도 빠짐없이 100% 모조리 복원하십시오. 만약 '등 다수'라는 단어를 사용하면 시스템이 파괴됩니다.
3. ⭐ [XML 괄호 보존]: 만약 태그나 표로 추출되었다면 그 형태를 훼손하지 마십시오.
"""
            try:
...
- ⭐ [Rule 52] 영양강조 컷오프(4대 조건) 검증 (반드시 4가지 수식 모두 증명!):
## 🔍 [주표시면 전용: 오탈자 검증]
...
🔥 [시스템 절대 족쇄: 순서 검증 시 환각 및 얼렁뚱땅 금지 (2% 기준선 분리)] 🔥
"""
...
⭐ [강제 지시]: 1:1 대조 결과를 반드시 아래 표 양식으로 출력하십시오.
⭐ [원산지 순위 도출 절대 원칙]: Rule 28에 따라 물, 당류, 첨가물, '미생물(포스트바이오틱스 등)'을 완전히 소거한 후 남은 진짜 원료만으로 1, 2, 3순위를 재정렬하여 표기하십시오. 단순 순서 카운팅 절대 금지!
...
⭐ [강제 지시]: 기존의 1:1 대조표들을 하나로 합쳐서 아래의 강력한 3-Way 대조표를 생성하십시오.
⭐ [원산지 순위 도출 절대 원칙]: Rule 28에 따라 물, 당류, 첨가물, '미생물'을 완전히 소거한 후 남은 진짜 원료만으로 1, 2, 3순위를 재정렬하여 표기하십시오.
⭐ [판정 로직]: 
  1. 서류 == 박스 == 팩 ➔ ✅ 적합
  2. 서류 == 박스 BUT 박스 != 팩 ➔ 🚨 확인요망 (단순 오타면 부적합)
  3. 서류 != 박스 ➔ 🚨 부적합
...
### 🚨 [서류 기준 최종 누락 스나이퍼 검증]
## ⚖️ 3️⃣ [배합비 기반 2% 룰 및 전개 순서 정밀 검증 (Rule 34)]
## 4️⃣ [알레르기, 주의사항 교차 검증] (팩/박스 내용 100% 일치 여부 포함)
## 🏛️ 5️⃣ [행정 정보 교차 검증]
- ⭐ [Rule 76] 유통전문판매원/판매원 타이틀 강제 확인:
## 🔍 [정보표시면 전용: 오탈자 검증]
- ⭐ 띄어쓰기 및 오탈자 적발: (오직 정보표시면 텍스트에 한정하여 무관용 전수 검사.)
        ...
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【분할 미션 스캔 중... (잠시만 기다려주세요)】"):
                missions = [
                    ...
                    "시험성적서 서류에서 각 영양성분의 실측값 데이터를 추출."
                ]
...
- 결론: (✅ 적합 또는 🚨 부적합)
⭐ [계산 검증 원칙]: 하한선 그룹(비타민/단백질 등)은 무조건 (성적서 실측값) >= (법적 기준선) 이면 ✅적합입니다. 절대 역산하지 마십시오.
⭐ [강제 지시]: 사용자가 성적서 서류를 업로드하지 않았더라도 핑계 대지 말고 아래 표는 100% 그려야 합니다! 
⭐ [부적합 사유 증명 필수]: 부적합(🚨) 판정 시 반드시 "왜 부적합인지" 수학적 부등호 수식(예: 9.74 < 14.4)을 기재하여 증명하십시오.
...
- 결론: (✅ 적합 또는 🚨 부적합)
⭐ [계산 검증 원칙]: 하한선 그룹은 무조건 (성적서 실측값) >= (법적 기준선) 이면 ✅적합입니다. 절대 역산하지 마십시오.
⭐ [강제 지시]: 사용자가 성적서 서류를 업로드하지 않았더라도 핑계 대지 말고 아래 표는 100% 그려야 합니다!
...
"""
    ...
    with tab4:
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("【분할 미션 스캔 중... (잠시만 기다려주세요)】"):
                missions = [
                    "전 구역 이미지를 스캔하여 필수 의무표시 3종(상담번호, 교환처, 1399 문구)과 HACCP 인증 마크 텍스트 추출.",
                    "알레르기 직접 함유 표시(바탕색 별도 박스) 및 분리배출 마크 텍스트 추출.",
                    "포장재질 표기(뚜껑, 본체 등 세부 재질 포함) 및 CS 방어 주의문구/기타 주의문구 100% 추출."
                ]
                judgment_prompt = """
...
- ⭐ [Rule 24, 52] 영양강조 및 당류 컷오프 교차 확인 (반드시 4가지 조건의 수학적 증명을 나열할 것!):
## 🔍 [기타면/측면 전용: 오탈자 검증]
