...
🔥 **Rule 21. ['고/풍부', '저', '무' 영양강조표시 4대 조건 OR 법칙 및 수학적 증명 룰]**
   ...
   ⭐ **[부적합 시 절대 족쇄]**: 부적합 판정을 내리려면 4가지 조건의 수식을 모조리 나열하여 전부 미달임을 증명해야 합니다. 하나라도 충족하면 합법 처리하십시오.
...
🔥 **Rule 52. [단순 명칭 강조 및 '함유/급원' 4조건 완벽 방어 룰]**
   - 제품명이나 패키지 여백(앞면, 측면 등)에 단순히 영양소 명칭이나 함량(예: 단백질 4g, 아연 함유 등)을 뱃지나 리스트 형태로 나열한 경우에도 **"단순 정보 제공"이라며 룰 적용을 회피하지 마십시오. 영양정보표 바깥에 기재된 모든 영양소 수치는 예외 없이 영양강조표시로 간주**하여 컷오프(%)를 검증하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V310.90 - 영양강조 4대 OR법칙 복구 완결본)")
