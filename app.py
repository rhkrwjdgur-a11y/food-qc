...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V92.0 - 다중 라벨 병합 및 사전 정답지 출력)")
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown("#### 📑 증빙 서류 업로드 (개별 라벨 무더기 업로드 가능)")
        report_docs = st.file_uploader("📑 시험성적서", type=["pdf", "jpg", "png"], accept_multiple_files=True)
        recipe_docs = st.file_uploader("📑 배합비/한글라벨 서류 (여러 장 가능)", type=["pdf", "jpg", "png"], accept_multiple_files=True)
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 서류 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("서류 취합/병합 및 하위성분 전개 100% 맵핑 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    prompt = """
                    ...
                    <thinking>
                    1. 🚨 [시각 판독 및 필사 절대 명령]: AI의 임의 추론이나 배경지식 개입을 엄격히 금지합니다. [타겟_시안(박스)] 이미지와 [비교용_정답지_시안(팩)] 이미지의 '원재료명' 란에 적힌 텍스트를 단 한 글자의 요약이나 생략 없이, 이미지 픽셀에 보이는 글자 그대로 100% 필사(Transcription)하여 먼저 나열하십시오. 
                    2. 🚨 [서류 다중 라벨 병합 분석]: 개별 원료의 '한글라벨' 사진이나 문서가 엑셀표처럼 한 장이 아니라, 무더기로 여러 장 업로드된 경우, 해당 라벨들에 적힌 '원료명 및 하위 전개 성분' 데이터를 단 하나도 누락하지 말고 모두 취합하십시오.
                    3. 하위 성분 전개 검증 로직: 특수의료용도식품이든 일반식품이든, 시안(박스 또는 팩)에서 혼합제제나 복합원재료를 '풀어쓴(전개한)' 경우, 서류(한글라벨)에 있는 하위 성분이 단 하나도 빠짐없이 정확히 기재되었는지 엄격히 대조하십시오. 박스와 팩의 표기 방식이 다르면(한쪽은 전개, 한쪽은 묶음) 내/외포장 불일치(Rule 47)로 부적합 처리하십시오.
                    4. 교차 대조: 도출된 정답 리스트를 바탕으로 위 1번에서 필사한 박스/팩 시안 텍스트와 1:1로 검증하십시오.
                    5. 🚨 전 구간 교차 검증 (수량/오탈자): 식품이력추적관리번호 등의 포장 단위(예: N입)를 파악하고, 다른 면(주표시면 등)의 수량 정보와 논리적으로 충돌하는지 대조하십시오. 쉼표 뒤 띄어쓰기 누락 등 문법 오류도 스캔하십시오.
                    </thinking>
                    
                    🚨 [표 작성 필수 명령] 🚨
                    1. 표의 첫 번째 열(AI가 추출한 서류 바탕 정답 원재료명)에는 절대 여러 개의 원료를 쉼표(,)로 묶어서 기재하지 마십시오. 무조건 1개의 열(행)에는 1개의 원료만 기재하여 세로로 길게 전개되도록 작성하십시오.
                    2. 도출된 정답 리스트의 모든 원료를 생략 없이 기재하십시오.
                    
                    ## 1️⃣ [AI 도출 서류 기반 정답 원재료명 리스트]
                    - 사용자님이 업로드하신 다중 서류/한글라벨 데이터를 병합하여 아래와 같은 최종 정답 원료 리스트를 도출했습니다. 
                    
                    | 도출된 원료명 (하위성분 전개 포함) | 근거 서류(또는 파일명) |
                    |---|---|
                    | (예: 난소화성말토덱스트린) | 3. 아미노엑기스.pdf |
                    | (예: 솔비탄지방산에스텔) | 6. almax 3800.pdf |
                    | ... | ... |
                    
                    ## 2️⃣ [원재료명 정밀 교차 검증 (위 정답 리스트 vs 박스 시안 vs 팩 시안)]
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    
                    | AI 도출 서류 바탕 정답 원재료명 | 타겟(박스) 및 비교용(팩) 시안 대조 결과 | 판정 |
                    |---|---|---|
                    | (1행 1원료 원칙 기재) | ... | ... |
                    ...
                    <thinking>
                    1. 🚨 [시각 판독 및 필사 절대 명령]: AI의 임의 추론이나 배경지식 개입을 엄격히 금지합니다. [타겟_시안] 이미지의 '원재료명' 란에 적힌 텍스트를 단 한 글자의 요약이나 생략 없이, 이미지 픽셀에 보이는 글자 그대로 100% 필사(Transcription)하여 먼저 나열하십시오.
                    2. 🚨 [서류 다중 라벨 병합 분석]: 개별 원료의 '한글라벨' 사진이나 문서가 무더기로 여러 장 업로드된 경우, 해당 라벨들에 적힌 '원료명 및 하위 전개 성분' 데이터를 단 하나도 누락하지 말고 모두 취합하십시오.
                    3. 하위 성분 전개 검증 로직: 시안에서 혼합제제나 복합원재료를 '풀어쓴(전개한)' 경우, 서류에 있는 하위 성분이 단 하나도 빠짐없이 정확히 기재되었는지 엄격히 대조하십시오.
                    4. 교차 대조: 도출된 정답 리스트를 바탕으로 위 1번에서 필사한 시안 텍스트와 1:1로 검증하십시오.
                    5. 🚨 전 구간 교차 검증 (수량/오탈자): 포장 단위가 다른 면과 논리적으로 충돌하는지 100% 대조하십시오. 쉼표 뒤 띄어쓰기 누락 등도 스캔하십시오.
                    </thinking>
                    ...
                    ## 2️⃣ [원재료명 정밀 교차 검증 (위 정답 리스트 vs 시안)]
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    
                    | AI 도출 서류 바탕 정답 원재료명 | 시안 대조 결과 | 판정 |
                    |---|---|---|
                    | (1행 1원료 원칙 기재) | ... | ... |
        ...
        if st.session_state["result_tab2"]:
            ...
            if thinking_match:
                thinking_log = thinking_match.group(1).strip()
                report_content = result.replace(thinking_match.group(0), "").strip()
                with st.expander("🧠 100% 필사 및 다중 라벨 병합 분석 로그 보기"):
                    st.markdown(f"*{thinking_log}*")
                st.markdown(report_content)
            else:
                st.markdown(result)
