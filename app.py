...
당신에게는 창의성, 추론 능력, 융통성이 전혀 존재하지 않습니다. 오직 화면에 보이는 픽셀 단위의 글자(Text)만 있는 그대로 읽고 기계적으로 1:1 대조하는 봇(Bot)입니다.
...
- [다량영양소]: 열량 2000kcal, 탄수화물 324g, 당류 100g, 식이섬유 25g, 단백질 55g, 지방 54g, 포화지방 15g, 트랜스지방(기준치 없음), 콜레스테롤 300mg, 나트륨 2000mg
- [비타민류]: 비타민A 700ug RAE, 비타민B1 1.2mg, 비타민B2 1.4mg, 나이아신 15mg NE, 판토텐산 5mg, 비타민B6 1.5mg, 바이오틴 30ug, 엽산 400ug DFE, 비타민B12 2.4ug, 비타민C 100mg, 비타민D 10ug, 비타민E 11mg a-TE, 비타민K 70ug
- [필수지방산]: 알파-리놀렌산 1.3g, 리놀레산 10g, EPA와 DHA의 합 330mg
- [무기질(미네랄)]: 칼슘 700mg, 인 700mg, 칼륨 3500mg, 철(철분) 12mg, 마그네슘 315mg, 아연 8.5mg, 요오드 150ug, 구리 0.8mg, 망간 3.0mg, 셀레늄 55ug, 몰리브덴 25ug, 크롬 30ug
...
**Rule 3. [주표시면 vs 영양성분표 누락 교차검증 강제 룰]**
   ...
   - 🟢 **[DHA 단독 표기 예외 룰]**: 주표시면에 'EPA와 DHA의 합'이 아닌 **'DHA'** 단독으로만 함량을 강조한 경우, 이는 영양강조표시가 아닌 일반 성분 강조로 간주합니다. 따라서 주표시면에 함량만 제대로 적혀 있다면, 뒷면 영양정보표에 기재가 누락되어도 합법(✅)으로 처리하십시오. (단, 11개월 이하 영아용 식품은 제외)
...
**Rule 5. [복합원재료 5% 미만 전개 면제 및 혼합제제/유효 첨가물 예외 룰]**
   - [대원칙]: 배합비 5% 미만인 '복합원재료(일반 가공식품)'는 괄호를 열고 하위 성분을 전개할 의무가 아예 없습니다. 생략 합법(✅).
   - 🛑 [첨가물 과잉 단속 예외 (캐리오버 원칙)]: 단, 5% 미만 원료에 포함된 첨가물이라도 완제품에 물리적/화학적 효과를 발휘하는 보존료, 감미료, 타르색소 등이 스캔될 경우 면제를 취소하고 ⚠️(확인 요망) 처리하여 실무자의 판단을 요구하십시오.
   - [혼합제제 절대 면제 불가 - Rule 44와 연계]: 서류상 식품유형이 '혼합제제'인 원료는 이 5% 미만 면제 룰이 절대로 적용되지 않습니다. 혼합제제의 하위 성분을 검사할 때는 이 Rule 5를 완전히 머릿속에서 지우고, 무조건 Rule 44로 넘어가서 [표 4, 5, 6] 기준에 따라 첨가물 용도 표시 여부를 깐깐하게 따지십시오.

**Rule 6. 당류/시럽 필터링**
   - 당류 0g 표기 시 0.5g 미만인지 검증.

**Rule 7. [당알코올 과량 섭취 주의문구 보수적 판정 룰]**
   - 원재료에 당알코올류(에리스리톨, 말티톨, 소비톨, 자일리톨 등)가 단 0.01%라도 감지되면, 무조건 ⚠️(확인 요망)으로 판정하십시오. (10% 미만 합법 룰 적용 금지)
   - 사유에는 "당알코올이 감지됨. 해당 원료가 주요 원재료에 해당하는지, 과량 섭취 시 설사를 유발할 수 있습니다 주의문구 기재가 필요한지 실무 부서의 확인 요망"이라고 출력하십시오.
...
**Rule 12. [증빙 대조 강제 및 영양성분 임의 추론 절대 금지]**
   - 주표시면에 원료 함량(%)이나 영양성분 수치(mg, g)가 기재되어 있을 경우, 면제 룰(Rule 3 등) 적용 여부와 무관하게 반드시 업로드된 배합비나 시험성적서와 1:1로 수치가 일치하는지 팩트체크 하십시오.
   - 🛑 [영양성분(mg/g) 검증의 절대 한계]: 영양성분 함량(예: DHA 1mg, 칼슘 110mg 등)을 검증할 때, 단순히 '배합비(%)'에 해당 원료가 들어갔다는 사실만으로 합격(✅) 처리하는 것을 엄격히 금지합니다. 배합비는 '투입 비율'일 뿐 '영양성분 순수 함량'의 증거가 아닙니다.
   - 🛑 [산출 불가 시 보류 강제]: 영양성분 수치를 합법으로 판정하려면 반드시 1) 완제품 시험성적서의 실측치 또는 2) 원료 규격서(COA)의 순도(%)를 바탕으로 한 명확한 산출 근거가 제출되어야 합니다. 이 두 가지 중 하나라도 부족하여 정확한 역산이 불가하다면, 임의로 합격(✅) 처리하지 마십시오.
   - ⚠️ 위 경우 무조건 ⚠️(확인 요망)으로 판정하고, 사유에 **"현재 제출된 서류(배합비 등)만으로는 OOmg이라는 수치를 정확히 산출 및 보증할 수 없습니다. 해당 수치를 증명할 수 있는 추가 서류(완제품 성적서 또는 원료 COA)의 확인이 필요합니다."**라고 명확히 지적하십시오.
...
**Rule 18. [타겟 오인 명칭 및 영유아 기준 확인 룰]**
   - [영유아 명칭 보수적 접근]: 일반식품에 '아기(아가)', '베이비(베베)', '앙팡' 등의 명칭이 스캔되면 무조건 🚨부적합 처리하지 말고 ⚠️(확인 요망)으로 판정하십시오. 사유에 "영유아 타겟 명칭 스캔됨. 해당 제품이 '영유아 섭취대상 식품' 규격을 충족한 성적서가 있는지 실무 부서 확인 요망"이라고 기재하십시오.
   - 🟢 [어린이/키즈 명칭 허용]: 단, '어린이', '키즈(Kids)', '주니어' 등의 단어는 영유아 범주에 속하지 않으므로 일반식품 마케팅에 사용하는 것을 완벽한 합법(✅적합)으로 허용하라.

**Rule 19. 🌟 ['무가당/설탕 무첨가' 5대 무관용 원칙 및 알룰로스 화이트리스트 특별 예외 (식약처 고시 완벽 동기화)]**
   - [대원칙]: 패키지에 '무가당', '설탕 무첨가', '당 ZERO' 등의 마케팅 강조 문구가 있을 경우, 영양성분표의 당류가 0g이더라도 속지 말고 아래 식약처 5대 금지 원료가 0.01%라도 있는지 전수 스캔하십시오.
   - 1) [기능적 대체 원료 금지]: 덱스트린, 말토덱스트린, 물엿, 시럽, 올리고당, 꿀 등 다당류/당류가공품이 존재하면 🚨부적합 (혼합제제 부형제 핑계 절대 불가).
   - 2) [당 첨가 원료 금지]: 잼, 젤리, 가당 과일 등이 포함되면 🚨부적합.
   - 3) [농축/건조 원료 금지]: 농축과일주스, 농축액, 말린과일페이스트 등 수분 증발로 당 함량이 높아진 원료를 사용했다면 🚨부적합.
   - 4) [효소분해 원료 금지]: 효소분해 등으로 식품의 당 함량이 높아진 원료(예: 곡물당화액 등)가 확인되면 🚨부적합.
   - 🌟 **[비당류 감미료 및 알룰로스 화이트리스트 절대 예외 허용]**: 단, 식약처 고시에 따라 당류에 해당하지 않는 식품첨가물 감미료(에리스리톨, 스테비아, 수크랄로스, 나한과 추출물 등) 단독 사용은 예외로 두어 무가당 표시를 합법(✅) 처리하십시오. 또한, 화학 구조상 식품 원료(단당류)임에도 불구하고 생리적 특성을 인정받아 **유일하게 식약처가 무가당 표기에 허용한 원재료인 '알룰로스(알룰로오스)' 역시 단독 예외 물질(Safelist)로 필터링하여 완벽한 합법(✅)** 처리하십시오. (단, 이런 허용 물질을 썼더라도 부형제로 덱스트린 등이 섞여 있다면 불법입니다.)

**Rule 20. [포장재질 및 분리배출 표시 도안 명칭 강제 룰 ('종이팩' 사용 절대 금지)]**
- 🛑 **[분리배출 마크 내부 명칭 규제]**: 환경부 「분리배출 표시에 관한 지침」에 따라 도안 내부에는 오직 **'일반팩'** 또는 **'멸균팩'**으로만 기재해야 합법입니다.
- 🛑 **['종이팩' 텍스트 적발 룰]**: 시안의 분리배출 마크 내부에 '종이팩'이라고 적혀있다면 즉시 🚨부적합 처리하고, 상세 사유에 반드시 **"종이팩은 분리배출표시도안에 없는 명칭입니다. 제품 스펙에 맞게 '일반팩' 또는 '멸균팩'으로 수정해야 합니다."**라고 출력하십시오. (사선 기호 대상인 경우 하단 '종량제 배출' 병기 여부도 점검할 것)
- [식약처 텍스트 표기 제한]: 식약처 규정에 따른 용기·포장재질(텍스트) 표기 의무는 식품과 직접 접촉하는 '합성수지제 및 고무제'에만 발생합니다. 텍스트로 '종이'라고 적는 순간 과잉/오기재 위반(🚨부적합)입니다. (예: '포장재질: 종이, 폴리에틸렌' -> 🚨부적합 / '포장재질: 폴리에틸렌(내면)' -> ✅적합)

**Rule 21. ['고/풍부', '함유/급원', '저', '무' 영양강조표시 4대 조건 OR 법칙 및 증빙 기반 검증]**
   ...
   - 🛑 **[적합/부적합 판정 시 팩트 기반 수학적 증명 강제]**: 합법(✅) 판정을 내릴 때도 단순히 "기준을 충족함"이라고 얼버무리지 마십시오. **반드시 4가지 기준(100g당, 100mL당, 100kcal당, 1회섭취량당) 중 어느 기준을 적용하여 통과했는지, 그리고 그 기준의 정확한 법적 커트라인(예: 100mL당 기준치의 7.5% 이상) 수식과 도출된 결과값을 상세 사유에 명확히 적어** 실무자가 쉽게 이해하고 검증할 수 있도록 하십시오. 부적합 시에는 4가지 조건을 모두 나열하여 전부 미달임을 증명하십시오.
   - 🛑 **[검증 데이터 우선순위 및 차선책(Fallback) 룰]**:
     1) **[1순위: 시험성적서]**: 영양강조표시의 합법성(컷오프)을 검증할 때는 반드시 **시험성적서의 100g/100mL당 실측값을 최우선 기준**으로 삼아 정확하게 계산하십시오.
     2) **[2순위: 시안 역산 (차선책)]**: 만약 업로드된 서류 중 **시험성적서가 없다면**, 포기하지 말고 **시안(패키지) 영양정보표에 적힌 수치를 역산**하여 컷오프 충족 여부를 확인하십시오. 단, 이 경우 상세 사유에 반드시 **"⚠️ 시험성적서 미제출로 시안 표기값 기준 역산함 (반올림 오차 존재 가능)"**이라고 명시하여 검증의 한계를 밝히십시오.

**Rule 22. [다국어 폰트 크기 및 영문 BI 단독 표기 규제 룰]**
- 법적 의무표시사항(제품명, 원재료명, 내용량, 주의문구 등)을 외국어로 병기할 때는 반드시 한글보다 작거나 같아야 한다 (위반 시 🚨부적합).
- 🟢 [마케팅/디자인 예외]: 법적 의무가 아닌 단순 마케팅 추가 문구는 한글보다 커도 합법(✅).
- 🟢 [상표법 등록 영문 BI 예외]: 「상표법」에 등록된 영문 상표(BI, 예: OURHOME)라면 예외적으로 한글 제품명보다 영문을 훨씬 크게 표기하는 것이 합법입니다. 단, 주표시면 어딘가에 작게라도 반드시 한글 제품명(예: 아워홈)이 병기되어야만 합법(✅)이며, 한글을 100% 삭제하고 영문 단독 표기 시 미표시 위반(🚨부적합)입니다.

**Rule 23. [식약처 영양성분별 '0' 표시 절대 규정 및 다단 표기 종속 법칙]**
   - 영양성분별로 '0'으로 표기할 수 있는 절대 커트라인이 명확히 다릅니다. AI는 0표기 합법성을 검증할 때 반드시 아래의 해당 성분별 기준을 엄격히 적용하십시오.
     * [열량]: 5kcal 미만
     * [나트륨]: 5mg 미만
     * [탄수화물, 당류, 단백질, 지방, 포화지방]: 0.5g 미만
     * [콜레스테롤]: 2mg 미만
     * 🌟 **[트랜스지방 세부 판정 룰]**: 실측치 < 0.2g 이면 '0g' 표기 합법(✅). 0.2g ≤ 실측치 < 0.5g 이면 '0.5g 미만' 표기 합법(✅). 시안에 '0.5g 미만'으로 표기되어 있다면 기계적으로 부적합 처리하지 말고 성적서를 대조할 것.
...
**Rule 24. [무당/무가당 강조표시 연계 의무: 열량 및 감미료 병기 (식약처 고시 완벽 반영)]**
   - [감미료/당알코올 표기]: '무당', '무가당', '당 ZERO' 강조 시, 원재료에 감미료가 쓰였다면 해당 강조표시 주위(인접한 둘레)에 **'감미료 함유'**를 표기해야 단, 당알코올이 쓰였다면 **'당알코올 함유'**로 표기해도 합법(✅)입니다. 
   - [열량 표기 및 대체 문구]: 제품이 '저열량' 기준에 적합하지 않은 경우, 강조표시 주변에 반드시 **'총 내용량 열량(OOO kcal)'** 또는 **'저열량 제품이 아님(열량을 낮춘 제품이 아님)'**을 기재해야 합니다. (누락 시 🚨부적합).
   - 🟢 [저열량 프리패스]: 단, 제품 자체가 식약처 '저열량'이나 '무열량' 기준(예: 100mL당 4kcal 미만 등)을 충족하는 경우, 무가당 강조표시 주변에 열량을 적지 않아도 완벽한 합법(✅)입니다. 
   - [반복 표시 예외]: 시안에 무가당 강조문구가 여러 번 쓰였더라도, 가장 큰 강조표시 주위에 한 번만 (14pt 이상으로) 적혀 있다면 전체 합법(✅) 처리하십시오.
...
**Rule 28. [Rank A vs Rank B 분리 및 원산지 산정 예외 4대장 엄격 증명 룰]**
   - AI는 배합비 순위를 검토할 때 반드시 두 가지 랭크를 분리해서 계산하십시오.
   - [Rank A: 배합비 절대 순위]: 모든 원료의 원래 % 비율대로 세운 순위. (Rule 34 나열 순서 검증에만 사용)
   - [Rank B: 원산지 산정용 순위]: 전체 배합비 목록에서 [정제수, 당류, 주정, 식품첨가물] 이 4가지 카테고리의 원료를 100% 삭제(제외)하고 남은 순위.
   - 🛑 **[Rank B 면제 사유 엄격 증명 및 임의 추론(환각) 금지]**: AI는 특정 원료를 Rank B에서 제외할 때, 반드시 [정제수, 당류, 주정, 식품첨가물] 중 어디에 속하는지 팩트 근거를 명시해야 합니다.
     1) [첨가물 판정 권한 박탈]: 시스템 하드코딩 DB(표 4,5,6)에 존재하거나, 서류상 식품유형이 '식품첨가물'인 경우에만 첨가물로 인정하여 제외합니다.
     2) [자연물/조미료 환각 방어]: 천일염, 정제소금 등 식염류나 후추, 천연 향신료 등은 '일반 원료(수산물/농산물 등)'입니다. 배합비가 낮거나 조미료 역할을 한다고 해서 절대 첨가물이나 4대 면제 대상으로 착각(환각)하여 임의 삭제하지 마십시오. 증빙이 없으면 무조건 Rank B에 남겨두십시오.
   - 🛑 **[4순위 승급 절대 금지 룰]**: Rank B의 3순위 원료가 '국내 가공 복합원재료'일 때, 그 하위 성분들이 모두 [면제 4대장]에 해당하여 원산지를 표기할 대상이 하나도 없더라도, **절대로 4순위 원료를 3순위로 끌어올려 원산지 표기를 강요하지 마십시오.** (3순위 표기 생략으로 합법 종료)
   - 🛑 **[식품유형 절대 우선의 법칙]**: 특정 원료명(예: 맥아엑기스, 시럽 등)이 당류처럼 보이더라도, 서류상 식품유형이 '기타가공품' 등 일반 가공식품이라면 면제 특권을 즉시 박탈하고 원산지 표시 대상에 포함시키십시오.
...
**Rule 34. [배합비 전개 순서 100% 일치 강제 및 2% 미만 예외 룰]**
   - [절대 원칙]: 원재료명은 반드시 서류상의 배합비율(%)이 높은 중량 순서대로 기재되어야 합니다. 배합비 순위가 시안의 나열 순서와 단 한 칸라도 다르면 명백한 표시기준 위반(🚨부적합)입니다. (Rule 28의 Rank A 기준 적용)
...
**Rule 35. [범용 간략명/관용명 허용 및 혼합제제 괄호 내부 N종 은폐 금지 범용 룰]**
   - [관용명/동의어 합법 처리]: 실무적으로 호환되는 동의어나 관용명 표기는 100% 합법(✅)입니다. (예: 옥배유=옥수수기름, 액상과당=기타과당=고과당, 백설탕=설탕 등 화학적/법적으로 완전히 동일한 물질만 허용)
   - 🛑 [과잉 치환 방지]: DE 수치나 화학적 공정이 다른 물질(예: 말토덱스트린 vs 포도당시럽분말)을 기능이 비슷하다는 이유로 임의 치환하면 명백한 위법(🚨부적합)입니다. 확신이 없다면 무조건 🚨부적합 또는 ⚠️확인요망 처리하십시오.
...
**Rule 38. [알레르기 22종 하드코딩 및 투트랙 교차오염 완벽 검증 룰]**
   - [알레르기 22종 절대 족쇄]: 알레르기 판정 시 오직 [한국 식약처 지정 22종: 난류, 우유, 메밀, 땅콩, 대두, 밀, 고등어, 게, 새우, 돼지고기, 복숭아, 토마토, 아황산류, 호두, 닭고기, 쇠고기, 오징어, 조개류(굴, 전복, 홍합 포함), 잣]에 대해서만 검증하십시오. 
   - 🛑 [조개류 괄호 강제]: 조개류는 반드시 괄호를 포함하여 `조개류(굴, 전복, 홍합 포함)`으로 기재해야 합법입니다. 괄호 누락 시 🚨부적합 처리하십시오.
   - 🟢 **[알레르기 유의어 매핑 허용]**: '알류' 대신 '계란', '달걀', '메추리알', '돼지고기' 대신 '돈육', '우유' 대신 '산양유' 등 소비자가 직관적으로 위해를 인지할 수 있는 일상 명칭 및 하위 동의어 표기는 융통성 있는 완벽한 합법(✅)입니다. 기계적으로 틀렸다고 경고하지 마십시오.
   - 🌟 **[투트랙(Two-Track) 알레르기 교차 검증 룰]**
     - **[Track 1. 자사 공장 (제조원이 자사인 경우)]**: `[직접 투입(함유) 물질]` + `[같은 제조시설 교차오염 물질]` 의 합집합을 스캔하여, 사용자가 설정한 '공장 알레르기 마스터 11종' 등과 개수 및 종류가 100% 일치해야 합법입니다. (부족하거나 과기재 시 오류 🚨)
     - **[Track 2. OEM/외주 공장 (제조원이 타사인 경우)]**: 자사 마스터 룰을 강요하지 마십시오. 단, 기재된 모든 물질이 법정 22종 바운더리(유의어 포함) 내에 있는지 확인하고, 비법정 물질(예: 사과, 참깨 등)이 있다면 🚨삭제 경고를 내십시오. 추가로 사유란에 **"⚠️ 해당 시안은 OEM 제품이므로, 기재된 알레르기 물질이 해당 외주 제조사의 실제 교차오염 환경과 100% 일치하는지 현업 부서와 공장 간 반드시 교차 검증 요망"**이라는 알림을 강제 출력하십시오.
...
**Rule 44. [🌟 혼합제제 하위 성분 전개 절대 원칙 및 범용성 룰]**
   - [절대 명령]: 서류상 **식품유형이 '혼합제제'**로 명시되어 있다면, 향료 등 어떤 사용 목적을 불문하고 예외 없이 하위 성분(용매제, 유화제 등 포함) 전체를 시안에 전개하여 기재해야 합니다.
...
**Rule 58. [함량 생략 합법성 및 관할 부처 충돌(농관원 우선) 검증 룰]**
   - [일반 법률]: 앞면(주표시면)에 일반 원물 함량(%)을 명시했다면, 뒷면 정보표시면(원재료명)에서는 생략하는 것이 합법(✅)입니다.
   - 🌟 **[농관원 유기가공식품 특별법 우선 적용]**: 단, 함량을 강조한 원료 앞에 **'유기농'** 또는 **'유기'**라는 단어가 붙어 있다면, 식약처 일반법의 예외 조항(생략 가능)을 적용하지 마십시오. 이 경우 특별법(친환경농어업법 시행규칙)이 발동되어, 앞면 표기 여부와 상관없이 뒷면 '원재료명' 란에 백분율(%)을 의무적으로 강제 기재해야 합니다. (기재 누락 시 🚨부적합 처리)
   - *주의: 단, 해당 제품이 아래 Rule 84의 '비유기 원료 0% 완전 열외 룰'을 충족하여 의무가 면제된 상태라면 합법(✅) 처리합니다.*
...
**Rule 62. [보관상태 전면 표기 의무 및 상온 면제 룰]**
   - [냉장/냉동 제품 (특히 축산물)]: 부패/변질 우려가 크므로 주표시면(앞면)에 반드시 '냉장' 또는 '냉동' 상태가 명시되어야 합니다. (예: `냉장보관 (0~10℃)` 표기 완벽 합법). 앞면에 누락 시 🚨부적합 처리하십시오.
   - [상온/실온 제품 (멸균팩 등)]: 주표시면(앞면)에 '상온' 또는 '실온'을 표기할 법적 의무가 전혀 없습니다. 앞면에 문구가 없어도 과잉 지적하지 말고 무조건 면제 및 ✅합법 처리하십시오. (오직 정보표시면 보관방법 란에서만 기재 여부 스캔)

**Rule 63. [190mL 전용 질소충전 동적(Dynamic) 스캔 룰]**
   - 190mL 용량의 제품은 패키지 형태에 따라 질소충전 문구 기재 여부가 다릅니다.
   - 🛑 [위치 무관 절대 원칙]: AI는 패키지의 앞면, 뒷면, 측면을 모두 스캔하여 **어딘가 한 곳(주로 정보표시면 포장재질 란)에라도 '질소충전' 문구가 존재한다면 100% 합법(✅)**으로 판정하십시오. 주표시면(앞면)에 없다는 이유로 지적하거나 ⚠️(확인 요망) 처리하는 것을 절대 금지합니다.
...
**Rule 66. [다중/분할 주표시면 교차 검증 및 일치 강제 룰]**
   - 패키지 형태(예: 미드팩 등)로 인해 주표시면이 여러 장의 이미지(앞면 1장, 뒷면 반쪽 2장 등)로 분할 업로드된 경우, AI는 조각난 이미지들의 텍스트를 논리적으로 결합하여 하나의 면으로 취급하십시오.
   - [교차 검증]: 앞면(Front)과 뒷면(Back) 양쪽 모두가 주표시면의 역할을 할 경우, 한쪽 면에만 법적 의무 표시사항(내용량, 열량 등)이 있고 다른 쪽 면에는 누락되었거나 수치가 다르다면 즉시 🚨부적합 처리하십시오. (마케팅 문구는 다를 수 있으나, 법정 의무 수치는 양면 모두 완벽히 100% 기재되어야 합법임)
...
**Rule 74. [액상 음료 및 우유류 주의문구 강제 스캔 룰]**
- 식약처 「식품등의 표시기준」에 따라, '과일·채소류 음료'뿐만 아니라 **'우유류(가공유 등 유가공품 축산물 포함)'** 등 개봉 후 부패·변질 우려가 높은 제품에는 반드시 **"개봉 후 냉장보관하거나 빨리 드시기 바랍니다"** (또는 이와 유사한 의미의 문구)를 기재해야 합니다. (우유류 면제 아님, 누락 시 🚨부적합)
...
**Rule 77. [식품유형별 법정 의무 주의사항 동적 스캔 및 특수용도식품 강제 룰]**
   - [특수의료용도식품 절대 규정]: 왼쪽 사이드바에 설정된 [제품유형]이 '특수의료용도식품'일 경우, 시안(기타면/측면)에 반드시 1) "의사, 임상영양사 등 전문가와 상담 후 섭취하여야 합니다" 및 2) "정맥주사용으로 사용하지 마십시오"(해당 시) 라는 필수 문구가 토시 하나 틀리지 않고 존재하는지 강제 스캔하십시오. 누락 시 🚨부적합 처리하십시오.
   - [일반 규정]: 그 외 두유류의 전자레인지 가열 금지 등 식품유형별 전용 주의문구도 누락 없이 검증하십시오.
...
**Rule 82. [영양소 법정 현행 단위 엄격 검증 및 구형 단위 적발 룰]**
   - 비타민 단위 등 특수기호가 룰북의 정답지(1일 영양성분 기준치)와 100% 일치하는지 대조하십시오.
   - 🛑 [구형 단위 철퇴]: 식약처 고시 개정에 따라 비타민A는 `µg RAE`, 나이아신은 `mg NE`, 엽산은 `µg DFE`를 사용해야 합니다. 시안에 과거 단위인 `µg RE`, `µgRE` 등으로 적혀있다면 절대 합법으로 넘기지 말고 기계적으로 🚨부적합 처리하십시오.

**Rule 83. [영양성분 % 병기 강제 원칙]**
   - 기준치 존재 성분 옆에 비율(%) 병기 필수.

**Rule 84. [유기가공식품 3단계 정밀 판정 및 비유기 원료 0% 완전 열외 룰]**
   ...
   - [70% 미만]: 인증 마크 불가(🚨). 제품명 불가(🚨). 패키지 전면/측면 등 마케팅 목적의 '유기농' 강조 문구 전면 금지(🚨부적합). 주표시면 하단 함량 표시란을 포함하여 앞면에 '유기농' 단어가 단 한 글자라도 노출되면 위법(🚨부적합)입니다. 오직 뒷면 '원재료명' 리스트 내부에만 '유기농OOO' 표기 허용(✅적합).
   - 🌟 **[Rule 84-1. 비유기 원료 0% 시 백분율 표기 완전 면제 특례 (농관원 유권해석)]**: 유기농 제품의 함량을 계산할 때, '인위적으로 첨가한 물(정제수)과 소금'은 모수(분모)에서 법적으로 완전 삭제(열외) 취급합니다. 따라서, 원재료명 리스트에서 물과 소금을 제외한 후 남은 원료 중에 일반 농산물, 식품첨가물 등 **'비유기 원료'가 단 0개일 경우, [별표 6]에 따른 원재료명 란 내부 유기농 백분율(%) 기재 의무가 완전히 면제(✅합법)**됩니다. (예: 오직 정제수 + 유기농 과채농축액으로만 구성된 제품은 뒷면에 % 안 적어도 무조건 합법)
...
**Rule 90. [범용 명칭 치환(유연한 맵핑) 엄격 룰]**
   - 서류상에 기재된 여러 종류 시럽류, 페이스트류 등의 복합원재료가 패키지 시안에서는 식품유형 대분류에 따라 `당류가공품`, `기타가공품` 등의 범용 명칭으로 치환되어 묶음 표기되는 것은 식품업계의 합법 관행입니다.
   - 🛑 단, 이 룰은 A원료를 B원료로 마음대로 바꾸는 것(예: 포도당시럽분말 -> 말토덱스트린)을 허용하는 것이 아닙니다. 오직 '당류가공품' 등 법적 식품유형 대분류로 묶을 때만 합법(✅) 처리하십시오.
...
**Rule 100. [주표시면 마케팅 텍스트 구조적 무결성 및 형태소 분리 금지 룰]**
   - 주표시면(앞면)에 특정 원재료명을 강조할 때, 띄어쓰기나 단순 줄바꿈은 디자인적 허용 범위이므로 합법(✅) 처리하십시오.
   - 🛑 **[형태소 분리 및 기호 삽입 엄격 금지]**: 단, 뒷면 원재료명에 하나의 단어로 결합된 복합 명칭(예: `비타민D31.0혼합제제`)을 앞면에서 임의의 쉼표(,)나 마침표(.)를 사용해 두 개의 단어로 쪼개거나(예: `비타민D3 1.0, 혼합제제`), 법정 단위(mg, µg 등)를 누락하는 것은 명백한 정보 왜곡이므로 🚨부적합 처리하십시오. AI는 절대 스스로 문맥을 선해(善解)하여 교정하지 마십시오.

**Rule 101. [식품유형 vs 원재료 배합 논리 모순 적발 룰]**
   - AI는 시안에 기재된 **[식품유형]** 명칭과 제품의 **[원재료명 및 강조 문구]** 간의 논리적 모순을 반드시 교차 검증하십시오.
   - 🛑 **[원물 100% 모순 적발]**: 제품 앞면에 'OO 100%'라고 원물을 강조했거나, 뒷면 원재료명에 타 첨가물(설탕, 식염, 향료 등)이 일절 없는 단일 원료 제품임에도 불구하고, **식품유형에 '가공', '혼합' 등의 단어가 포함되어 있다면 명백한 식품유형 기재 오류이므로 🚨부적합 처리**하십시오. (예: 원액두유 100%인데 '가공두유'로 표기 시 🚨부적합)

**Rule 103. [수치+단위+상태 지표 결합 어순 룰 (세균수 표기 등)]**
- 측정 수치와 단위는 하나의 세트로 먼저 묶이고 그 뒤에 미만/이하 등의 상태 지표가 붙어야 합니다.
- 단, 원유 위생등급 표기(예: 3만 미만 개/mL)는 [Rule 108]에 따라 이 어순 룰 적용을 면제합니다. 일반적인 수치(예: 중량 등)에 한해서만 어순(수치+단위+상태)을 점검하고 어법 오류 시 지적(⚠️)하십시오.

**Rule 105. [고카페인 함유 액상음료 의무 표시 신설 룰]**
- 제품이 액상음료(커피, 차류, 혼합음료 등)이고 카페인이 1mL당 0.15mg 이상 함유된 경우(또는 시안에 고카페인 관련 텍스트가 감지될 경우), 다음 3가지를 강제 스캔하십시오.
  1) 주표시면의 `고카페인 함유` 문구
  2) 주표시면의 `총 카페인 함량 OOO mg` 문구
  3) 기타면/정보표시면의 `어린이, 임산부, 카페인 민감자는 섭취에 주의하여 주시기 바랍니다` 문구
- 위 3가지 중 하나라도 누락 시 즉시 🚨부적합 처리하십시오.

**Rule 106. [수치 강조 문구 논리적 동기화 강제 룰 (스탠다디제이션 등)]**
- 패키지 전면이나 측면에 특정 영양성분의 수치(예: '유지방 3.6% 표준화')를 마케팅 포인트로 강조한 경우, 영양정보표 내 해당 성분의 기재값은 식약처의 일반적인 반올림 허용 규정(예: 3.6을 4로 표기)과 무관하게, 반드시 강조된 수치와 소수점까지 1:1로 완벽하게 일치하게 적어야 합니다. (예: '지방 3.6g' 또는 '3.6g').
- 강조 수치와 영양정보표 수치가 논리적으로 어긋난다면 소비자 기만 및 오인 리스크가 매우 크므로, 기계적인 오차(Rule 11) 범위 내에 있더라도 무조건 🚨부적합 또는 ⚠️확인 요망 처리하십시오.

**Rule 107. [우유류 '유통기한' 표기 유예 특례 룰]**
- 원칙적으로 모든 식품은 '소비기한'을 표기해야 합니다.
- 🌟 **[냉장 우유류 절대 예외]**: 단, 검토 대상의 [식품유형]이 '우유류'이고 [보관방법]이 '냉장보관'인 제품은 2030년 12월 31일(2031년 1월 1일 전)까지 '유통기한' 표기가 법적으로 완벽하게 허용됩니다.
- AI는 냉장 우유 제품 패키지(정보표시면, 앞면 등)에서 '유통기한' 텍스트를 발견하더라도 절대 부적합 처리하지 말고 무조건 합법(✅) 처리하십시오. (일반 가공식품에 유통기한을 썼을 때만 🚨부적합 처리)

**Rule 108. [원유위생등급 수치 동기화 및 법정 용어 특례 룰]**
- 1) **[법정 텍스트 면제 특례]**: 패키지에 '3만 미만 개/mL', '3만 미만/mL' 등 원유 위생등급(세균수/체세포수)과 관련된 문구가 스캔될 경우, 이는 식약처 고시 원문을 따른 것이므로 Rule 103(어순 룰)을 절대 적용하지 말고 무조건 합법(✅) 처리하십시오. 
- 2) **[등급 팩트체크 강제]**: 시안에 '1급A' 또는 '1A등급' 마케팅 문구가 있다면, AI는 반드시 그 주변이나 정보표시면에 '3만', '미만', 'mL' 키워드가 정확히 매칭되는지 대조하십시오. 만약 '1급A'라고 적어두고 세균수 기준을 '10만'으로 적었거나 단위가 틀렸다면 명백한 소비자 기만(🚨부적합)으로 판정하십시오. 체세포수 '1등급' 강조 시 '20만 미만/mL' 인지 교차 검증하십시오.
"""
...
# 탭별 맞춤형 룰 타겟팅
# ==========================================
TAB1_RULES = [3, 9, 12, 17, 18, 19, 21, 24, 46, 50, 52, 53, 57, 62, 63, 66, 68, 71, 72, 84, 86, 87, 88, 94, 95, 96, 99, 100, 103, 105, 106, 107, 108]
TAB2_RULES = [1, 2, 4, 5, 8, 9, 12, 13, 14, 16, 28, 29, 30, 34, 35, 38, 39, 44, 45, 47, 48, 51, 53, 54, 60, 61, 64, 65, 70, 85, 89, 90, 91, 95, 101, 106, 107, 108]
TAB3_RULES = [3, 6, 10, 11, 23, 24, 25, 26, 27, 31, 32, 33, 40, 41, 55, 68, 79, 80, 82, 83, 97, 98, 106]
TAB4_RULES = [7, 15, 20, 22, 36, 58, 59, 73, 74, 75, 76, 77, 78, 81, 92, 93, 103, 105, 106, 107, 108]
...
def main():
    ...
    st.markdown("<h1>Label Guard AI <span style='font-size:0.5em; color:#64748b;'>식품 패키지 무결점 검증 시스템 (V5.9.9 Ultimate)</span></h1>", unsafe_allow_html=True)
    ...
    def run_qc_3pass(tab_rules: str, judgment_prompt: str, extract_missions_list: list = None):
        ...
        if extract_missions_list:
            ...
            def run_pass18():
                p = f"[PASS 1.8 - 초정밀 맞춤법/띄어쓰기 스캐너]\n당신은 국립국어원 소속 깐깐한 교정 전문가다. 법적 판단은 하지 마라.\n다음 원본 텍스트 단위를 분석하여, 1) 명사 간 띄어쓰기 오류(예: '제품상단'->'제품 상단'), 2) 괄호 앞뒤 불필요한 공백(예: '등 )'->'등)'), 3) 단어 오탈자, 4) 어색한 주의문구(예: '냉장보관 하시거나' -> '냉장 보관하시거나')를 무자비하게 모두 찾아내어 지적하라.\n[원본]\n{extracted_text_combined}"
    ...
    with tab1:
        if st.button("주표시면 분석 시작", key="btn_main"):
            with st.spinner("데이터 스캔 및 판정 중 (최적의 3분할 미션 스캔)..."):
                missions = [
                    "[미션 1. 마케팅 텍스트 전수 스캔]: '주표시면'으로 업로드된 **모든 이미지(주표시면 1, 2, 3 등)**의 텍스트를 각각 추출하십시오. 🛑 [초정밀 스캔 경고]: 쉼표(,), 마침표(.), 띄어쓰기 등 아주 작은 문장부호나 기호도 절대 임의로 삭제하거나 문맥에 맞게 교정하지 마십시오. 이미지에 찍힌 픽셀의 형태소를 100% 원문 그대로 추출해야 합니다. 팩 모양에 의해 반쪽씩 나뉘어 찍힌 이미지라면 문맥을 합쳐서 하나의 면으로 인식하십시오. 양쪽 면(앞/뒤)의 의무 표기 내용이 일치하는지 비교할 준비를 하고, 마케팅 클레임 진위 여부(Rule 99)를 위해 뒷면의 '원재료명' 란도 전수 스캔하십시오.",
...
2. 원재료명 텍스트 스캔 시 '포도당, 시럽, 물엿, 덱스트린류, 과당, 올리고당, 꿀' 같은 당류 대체재나 **'농축액(주스), 과일페이스트, 잼' 등 농축/건조 원료**가 단 한 글자라도 존재하는가? [YES / NO]
...
4. 🛑 [영양강조 팩트체크 보류 원칙]: 주표시면에 '고칼슘', '저당' 등의 영양강조 문구가 있으나, 해당 성분의 구체적인 함량(mg, g) 수치가 앞면에 명시되어 있지 않다면 절대 임의의 숫자를 지어내서 계산하지 마십시오. 반드시 "앞면 수치 미표기로 시험성적서 실측값(미제출 시 뒷면 영양성분표 수치 역산)을 통한 교차 검증 요망"이라고 기재하십시오.
4-1. 🛑 [하단 함량 표시 증빙 대조 (Rule 12)]: 주표시면에 구체적인 함량(%)이나 수치(mg)가 명시되어 있다면, 반드시 업로드된 배합비나 성적서 서류에서 실측값을 스캔했는지 확인하십시오. 서류가 없다면 무조건 ⚠️(확인 요망) 처리할 준비를 하십시오.
5. 🛑 [무가당 연계 표시 의무 시각적 스캔 (Rule 24)]: 위 1번에서 '무가당', '당 ZERO' 등이 YES라면, 해당 마케팅 글자 바로 주변(동일 시야각)에 '감미료 함유(또는 당알코올 함유)' 및 '총 열량(또는 저열량 제품이 아님)' 문구가 명확하게 병기되어 있는가? (단, 저열량/무열량 제품인 경우 열량 병기는 생략 가능) 👉 [해당없음 / YES / NO(누락 항목 명시)]
6. 🛑 [앞뒷면 글자 1:1 교차 검증 (Rule 100)]: 앞면에 강조된 원료명이 뒷면 원재료명과 비교했을 때, 단순 줄바꿈이나 띄어쓰기 외에 쉼표(,) 같은 기호로 인해 하나의 원료가 두 개로 쪼개지는 등 의미론적 훼손이 발생했는가? 👉 [ 해당없음 / YES(부적합) / NO(적합) ]
7. 🛑 [주표시면 총 열량 교차 환산 정밀 검증 (Rule 3 연계)]:
   - A. 주표시면 하단 텍스트 스캔: 총 용량 = [ OO mL/g ], 괄호 안 열량 = [ OO kcal ]
   - B. 정보표시면(영양정보표) 스캔: 기준 용량 = [ OO mL/g당 ], 열량 = [ OO kcal ]
   - C. 환산 공식: (주표시면 총 용량 ÷ 영양정보표 기준 용량) × 영양정보표 열량 = [ 환산값 도출 ]
   - D. 단방향 오차 허용 판정: 도출된 환산값이 주표시면에 적힌 열량과 정확히 일치하거나, 성적서 실측 오차를 고려해 주표시면에 약간 낮게 적힌 '보수적 표기'라면 합법(✅). 하지만 주표시면에 무턱대고 훨씬 높게 적혀있거나 단순 곱셈 오차를 벗어나면 명백한 디자이너 오기재이므로 🚨부적합 처리하고 정답(환산값)을 제시할 것.
</pre_calc>

## 2. [주표시면 및 마케팅 뱃지 정밀 검증]
🛑 [절대 명령]: 위 사전 연산 결과를 바탕으로 반드시 아래 마크다운 표 뼈대를 100% 그대로 복사하여 내용을 채우십시오. 줄글 형태로 풀어서 쓰는 것을 절대 금지합니다.

| 검토 항목 | 검토 룰(Rule) | 상세 사유 (오탈자 무관용, <br> 태그로 줄바꿈 필수) | 판정 (반드시 '✅ 적합', '🚨 부적합', '⚠️ 확인 요망' 텍스트 포함) |
|---|---|---|---|
| **다중 주표시면(양면) 내용 불일치 검증** | [Rule 66] | (※ 주표시면 이미지가 1장뿐이라면 '해당없음' 처리. 2장 이상일 경우 내용량, 열량 등 의무표시사항 누락/불일치 여부 교차 검증) | |
| **주표시면 강조 원료 구조적 무결성 (1:1 픽셀 대조)** | [Rule 100] | (※ 쉼표 등 기호 삽입이나 띄어쓰기로 인해 뒷면 원재료명의 형태소가 분리/훼손되었는지 적발. 디지털 시안이므로 기호가 있다면 디자이너의 명백한 오타/기재 오류임) | |
| **주표시면 하단 함량 표시 팩트체크 (증빙 대조 강제)** | [Rule 12, 21, 52] | (※ 앞면에 원료(%)나 영양성분 수치 기재 시 반드시 배합비/시험성적서와 대조할 것. 증빙이 없거나 산출이 불가하면 절대 임 임의 합격 처리 금지, 무조건 ⚠️ 확인 요망 판정) | |
| **주표시면 총 열량 환산 무결성 점검** | [Rule 3] | (※ 사전 연산 7번의 수학적 곱셈/오차 환산 결과를 명시하고, 오류 시 수정 제안) | |
...
| **보관상태(상온/냉동/냉장) 명시** | [Rule 62] | (※ 냉장/냉동은 앞면 표기 필수, 상온은 앞면 생략 완벽 합법) | |
...
| **영양강조 컷오프(4대 조건)** | [Rule 21, 52] | (※ 어떤 기준(100mL, 100g 등)으로 통과했는지 수식과 법적 컷오프 수치를 상세 사유에 반드시 명시할 것. 앞면에 실측 숫자가 없다면 환각 계산 금지) | |
...
| **무당/무가당 주변 열량 및 감미료 병기 의무** | [Rule 24] | (※ '무가당/ZERO' 강조 시 동일 시야각 내 열량 및 감미료 함유 문구 누락 시 🚨부적합. '저당' 또는 저열량 충족 시 예외) | |
...
| **고카페인 액상음료 의무 표시** | [Rule 105] | (※ 액상음료 중 카페인 0.15mg/mL 이상 시 앞면 '고카페인 함유' 및 총 함량 스캔) | |
| **수치/단위/상태 결합 어순** | [Rule 103] | (※ '3만 미만 개' 등 단위 단절 스캔) | |
...
| 검토 항목 | 발견된 오류 문구 및 교정 제안 | 판정 (반드시 '✅ 적합', '🚨 부적합', '⚠️ 확인 요망' 텍스트 포함) |
|---|---|---|
| **오탈자 스캔 (명백한 글자 오류)** | | |
    ...
    with tab2:
        if st.button("정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("원재료 1:1 매칭 매트릭스 연산 중..."):
                ...
                # 🔥 [V5.7 핫픽스: 서류 역할 분리 및 유연한 맵핑의 명시적 증명 강제]
                ...
                4. 🛑 [기타가공품/당류가공품 명시적 맵핑 강제 (Rule 90 적용)]: 시안에 '기타가공품', '당류가공품' 등의 범용 명칭이 쓰였을 경우, '서류 매칭 원료' 칸에 절대로 '당류가공품'이라고 똑같이 따라 적지 마십시오. **반드시 마스터표나 배합비에서 실제로 매칭되는 진짜 원료명(예: 번트카라멜시럽, 저분자유청단백가수분해물, 진득찰추출분말 등)을 찾아내어 정확히 기재**하고, 상세 사유에 "OOO 원료가 기타가공품으로 치환됨"을 증명하십시오.
                5. 🛑 [특례/예외 룰 적용 시 사고과정 강제 노출 및 혼합제제 절대 법칙]: 
                    - 서류의 텍스트와 시안의 텍스트가 물리적으로 다름에도 특정 Rule을 적용하여 **합법(✅)** 처리를 할 때는 "표기 규정 준수함"이라고 결과만 적지 말고, 반드시 **"서류상으로는 [A, B]이나, [Rule X]에 따라 시안에 [C]로 치환 표기한 것은 합법임"**이라고 인과관계를 상세 사유에 소리 내어 증명하십시오.
                    - 원료 서류의 공식 **[식품유형]**이 **'혼합제제'**라면 향료 특례를 임의 적용하지 마십시오. 시안에 단일 향료 명칭으로만 적혀있다면 **"서류상 식품유형이 혼합제제이므로 하위 성분 누락"**으로 판단하고 🚨부적합 처리하십시오.
                6. 🛑 [혼합제제 내부 성분 묶음(그룹화) 절대 불가]: 혼합제제 하위 성분을 전개할 때, 여러 성분을 [표 6] 용도명(예: 유화제, 산도조절제 2종 등)으로 묶어서 표기하는 것은 절대 불가(은폐 기만행위)하다. 개별 명칭을 100% 명시하라. 단, 혼합제제가 아닌 단일 원료들이 개별적으로 투입된 경우에 한해서만 'N종' 묶음 표기를 허용하라.
                7. 🛑 **[스펙 서류(한글라벨) 부재 시 엄격한 경고 표기 (정제수, 설탕 등 포함)]**: 정제수, 설탕, 소금처럼 흔한 원료이거나 배합비(레시피)에는 적혀 있더라도, **업로드된 '원료 한글라벨(스펙 서류)' 목록에 해당 원료가 존재하지 않는다면, '서류 매칭 원료' 칸에 무조건 "⚠️ 스펙 서류 없음 (배합비로만 확인)" 이라고 명확히 기재하십시오.** 그냥 '정제수', '설탕'이라고 기재하며 퉁치는 행위를 절대 금지합니다.
                8. 🛑 [최고 수준 경고: 증빙 서류 역할 강제 분리 (Rule 48 적용)]: 
                    - [배합비/레시피 서류]: 오직 원재료의 **'표기 순서(중량순)'**를 검증하고, 5% 미만(Rule 5) 등 **'비율(%) 계산'**을 할 때만 사용하십시오.
                ...
                judgment_prompt += "(AI는 아래 5단계를 단답형으로 100% 명확히 작성하여 논리를 확정할 요것)\n"
                ...
                judgment_prompt += "\n## [사전 연산 2: 식품유형 vs 원재료 배합 정합성 자체 검증 (Rule 101)]\n"
                judgment_prompt += "1. 시안에 기재된 [식품유형] 추출: [ ]\n"
                judgment_prompt += "2. 해당 식품유형의 '식품공전상 핵심 정의/조건' 스스로 회상: [ ]\n"
                judgment_prompt += "3. 시안의 [원재료명] 리스트 확인 시 위 2번 조건을 충족하는가? (원물 100%인데 '가공/혼합'이 붙었는지 등 모순 교차 검증): [ ]\n"
                judgment_prompt += "4. 최종 판정: [ 정합성 일치(합법) / 정합성 모순(🚨부적합) ]\n"
                judgment_prompt += "</pre_calc>\n\n"
                ...
                if doc_mode == "개별 한글라벨":
                    if has_any_doc:
                        ...
                        judgment_prompt += "🛑 [절대 명령]: 위 사전 연산에서 파악한 원료명 리스트와 개수가 100% 일치하도록, 단 하나의 원료(향료 등)도 묶어서 병합하거나 누락하지 말고 아래 표에 각각 1줄(행)씩 독립적으로 작성하십시오.\n"
                ...
                if "선물세트" in ins_mode:
                    ...
                    judgment_prompt += "| No | 타겟(박스) 시안 원재료명 | 비교용 팩(내포장) 시안 원재료명 | 상세 사유 (타이포그래피 차이는 합법 처리) | 판정 (반드시 '✅ 적합', '🚨 부적합', '⚠️ 확인 요망' 텍스트 포함) |\n|---|---|---|---|---|\n\n"

                table_title = "## 2-3. [원재료명 1:1 정밀 대조 및 법규 검증]\n" if "선물세트" in ins_mode else "## 2-2. [원재료명 정밀 대조 및 법규 검증]\n"
                judgment_prompt += table_title
                # 🔥 [표 헤더 컬럼명 강제 지시어로 변경 및 판정 텍스트 누락 방지]
                judgment_prompt += "| No | 시안 원재료명 (표시 순서대로) | 서류 매칭 원료 (실제 명칭 기재, 스펙 없으면 '⚠️ 서류 없음') | 상세 사유 (치환 시 원본 매칭 관계 상세 설명) | 판정 (반드시 '✅ 적합', '🚨 부적합', '⚠️ 확인 요망' 텍스트 포함) |\n|---|---|---|---|---|\n\n"
                ...
                judgment_prompt += "※ 발견된 누락 원료가 Rule 5에 따른 '일반 복합원재료'라서 합법적으로 생략된 정당한 것인지, 아니면 디자이너의 치명적 실수(🚨부적합)인지 명확히 판정하십시오. (누락된 것이 전혀 없다면 표 첫 줄에 '누락된 원료 없음'으로 기재할 것)\n"
                judgment_prompt += "| 서류상 누락된 원재료명 | 추정 배합비율(%) | 누락 사유 판정 (Rule 5 합법 생략 정당 vs 단순 기재 누락 위법) | 판정 (반드시 '✅ 적합', '🚨 부적합', '⚠️ 확인 요망' 텍스트 포함) |\n|---|---|---|---|\n\n"
                
                judgment_prompt += "## 2-4. [알레르기 및 교차오염 완벽 검증]\n"
                judgment_prompt += "| 검토 항목 | 타겟(시안) 텍스트 | 상세 사유 및 교차오염 뺄셈 공식 증명 | 판정 (반드시 '✅ 적합', '🚨 부적합', '⚠️ 확인 요망' 텍스트 포함) |\n|---|---|---|---|\n"
                judgment_prompt += "| **알레르기 유발물질 (직접 투입)** | | | |\n"
                judgment_prompt += "| **교차오염 주의문구** | | | |\n\n"
                
                judgment_prompt += "## 2-5. [행정 및 기타 의무 표시사항]\n"
                judgment_prompt += "| 검토 항목 | 시안 텍스트 | 상세 사유 | 판정 (반드시 '✅ 적합', '🚨 부적합', '⚠️ 확인 요망' 텍스트 포함) |\n|---|---|---|---|\n"
                judgment_prompt += "| **품목보고번호/식품유형** | | (※ 사전연산 2의 정합성 검증 결과를 반드시 반영. 100% 원액인데 '가공'으로 적혀있는 등 모순 적발 시 🚨부적합 처리 및 올바른 유형 제안) | |\n"
                judgment_prompt += "| **영업소 명칭 및 소재지** | | | |\n"
                judgment_prompt += "| **유통기한 / 소비기한 명칭** | | (※ Rule 107에 따라 우유류(냉장)는 유통기한 합법, 기타는 소비기한 필수) | |\n"
                ...
                judgment_prompt += "| 검토 항목 | 발견된 오류 문구 및 교정 제안 | 판정 (반드시 '✅ 적합', '🚨 부적합', '⚠️ 확인 요망' 텍스트 포함) |\n|---|---|---|\n"
    ...
    with tab3:
        if st.button("영양성분표 수치 자동 환산 및 대조", key="btn_nutri"):
            with st.spinner("영양성분 수치 역산 및 오차 매트릭스 검증 중..."):
                ...
                # 🔥 [V5.5 핫픽스: Rule 1 계층 구조 위배 범용 룰 적용]
                tab3_special_rules = RULES_TAB3 + """
                \n\n[Tab 3 특별 지시사항 (반드시 지킬 것 - 화면 출력 금지)]
                1. 🛑 [영양소 표시 순서 범용 계층 룰]: 영양정보표의 나열 순서는 법적으로 엄격히 통제되며, 어떠한 경우에도 아래의 '법정 마스터 순서' 흐름을 거스르면 🚨부적합 처리하십시오.
                    - [법정 마스터 순서]: 열량 ➔ 나트륨 ➔ 탄수화물 ➔ 당류 ➔ (식이섬유, 당알코올, 알룰로스 등 탄수화물 하위) ➔ 지방 ➔ 트랜스지방 ➔ 포화지방 ➔ 콜레스테롤 ➔ 단백질 ➔ (비타민 및 무기질류)
                    - 시안에 기재된 영양소 배열이 위 마스터 순서의 위계를 100% 준수해야 합니다. 단 하나의 영양소라도 순서를 역행하거나 잘못된 그룹에 끼어있으면 즉시 적발하십시오.
                2. [단방향 허용오차 완벽 계산 (Rule 11)]: 성적서 실측값을 기준 삼아 1.2를 곱하지 마십시오. 반드시 시안의 '표시량'을 기준으로 상한선(120%)/하한선(80%) 커트라인을 구하십시오.
                3. [산수 오류 원천 봉쇄]: 열량 계산 시 애트워터 계수(탄4, 단4, 지9)를 적용하여 시안 열량의 합법성(보수적 표기 포함)을 교차 검증하십시오.
                4. [0 표기 및 불검출 룰 (Rule 23 적용)]: 실측값이 식약처 규정 미만일 경우, 시안에 0g/0mg으로 적혀 있는 무조건 완벽한 합법입니다.
                5. [다단 표기 & 단위포장 프리패스 (Rule 97, 98)]: '1포당', '1개당' 등 개별 단위포장 되어있다면 참고량 기준 초과를 따지지 말고 즉시 합법(✅) 처리하십시오.
                6. 🛑 [총 내용량 꼼수 계산 절대 금지]: '총 내용량당' 수치 검증 시, 시안의 '1회 섭취량'에 임의로 배수를 곱하지 마십시오. 반드시 [성적서 100g/mL 실측치]를 총 내용량 비율에 맞게 처음부터 다시 곱하여 비교하십시오.
                """
                ...
                if "선물세트" in ins_mode:
                    ...
                    judgment_prompt_tab3 += "| 영양성분명 | 타겟(박스) 1개당 시안 | 비교용(팩) 1개당 시안 | 상세 사유 (타이포그래피 차이는 합법 처리) | 판정 (반드시 '✅ 적합', '🚨 부적합', '⚠️ 확인 요망' 텍스트 포함) |\n|---|---|---|---|---|\n\n"
                    prefix_num += 1
                
                judgment_prompt_tab3 += "<pre_calc>\n"
                
                # 🔥 [V5.5 핵심 패치: 마스터 템플릿 기반 범용 순서 스캔]
                judgment_prompt_tab3 += f"## 3-{prefix_num} [사전 연산 0: 영양소 나열 순서 범용 팩트체크]\n"
                judgment_prompt_tab3 += "1. [추출]: 시안 이미지(영양성분표)에 나열된 영양소 명칭을 시선 이동 순서대로 모두 적으시오: [ 예: 열량 -> 나트륨 -> 탄수화물 -> 당류 -> ... ]\n"
                judgment_prompt_tab3 += "2. [마스터 템플릿 대조]: 위 1번에서 추출한 배열이 룰북 1번 항목의 [법정 마스터 순서] 흐름을 완벽히 따르고 있습니까? 특정 영양소가 위계를 벗어나 역행하거나 엉뚱한 위치에 있다면 해당 항목을 명시하십시오. 👉 [ 예: 위반 발견 (식이섬유가 단백질 밑에 있음) / 위반 발견 (나트륨이 지방 밑에 있음) / 위반 없음 ]\n"
                judgment_prompt_tab3 += "3. [최종 판정]: 표시 순서 위반 사항이 있습니까? 👉 [ YES / NO ]\n\n"
                ...
                judgment_prompt_tab3 += "🛑 [0표기 절대 룰 및 마케팅 수치 동기화 점검 (Rule 23, 106 적용)]\n"
                ...
                judgment_prompt_tab3 += "▶ **[상황 C: 마케팅 수치 동기화 (Rule 106 강제 발동)]**\n"
                judgment_prompt_tab3 += "  - 1. 마케팅 강조 수치 스캔: 측면 등에 '유지방 3.6%' 등 특정 성분의 수치가 강조되었는가? 👉 [ 추출된 텍스트 및 수치 기재 ]\n"
                judgment_prompt_tab3 += "  - 2. 영양정보표 표기 스캔: 위 성분의 영양정보표 기재 숫자가 무엇인가? 👉 [ OO g ]\n"
                judgment_prompt_tab3 += "  - 3. 🚨 강제 판정 룰: 만약 1번의 수치(예: 3.6)와 2번의 수치(예: 4)가 소수점까지 100% 동일하지 않다면, AI는 **어떠한 오차범위 계산(Rule 11)도 절대 적용하지 마십시오!** 즉시 🚨부적합 처리하고, 상세 사유에 '패키지에는 유지방 3.6%로 표준화를 강조했으나, 영양정보표에는 반올림된 4g으로 기재되어 소비자 오인 소지가 큼. 영양정보표를 3.6g으로 일치시킬 것'이라고 강제 출력하십시오.\n\n"
                
                # 🔥 [V5.9.2 핵심 패치: 성적서 유무에 따른 상세 사유 분기(IF-ELSE) 강제 처리]
                judgment_prompt_tab3 += "🛑 [상세 사유 작성 절대 규칙]: '상세 사유' 칸에는 성적서 유무에 따라 아래 지침대로 작성하십시오.\n"
                judgment_prompt_tab3 += "  ▶ [시험성적서가 있는 경우]: 1) 용량 오차 검증 (실측값 vs 표시량 80%/120% 충족 여부) 및 2) 1일 기준치(%) 검증 (사칙연산 포함)을 모두 기재하십시오.\n"
                judgment_prompt_tab3 += "  ▶ [시험성적서가 없는 경우]: 1) 용량 오차 검증은 **'⚠️ 성적서 부재로 오차 검증 불가'**로 기재하고 절대 스스로 합격이라고 지어내지 마십시오. 2) 1일 기준치(%) 검증만 시안 표시량을 바탕으로 사칙연산을 수행하십시오. 3) 사유 마지막에 무조건 **'[Rule 21] ⚠️ 시험성적서 미제출로 시안 표기값 기준 역산함 (반올림 오차 존재 가능)'** 문구를 100% 강제 출력하여 확인 요망(⚠️) 판정으로 유도하십시오.\n"
                ...
                judgment_prompt_tab3 += "| 영양성분 | 성적서 환산값(A) | 시안 표시량(B) | 허용오차 커트라인 | 1일 기준치 % 검증 | 상세 사유 | 판정 (반드시 '✅ 적합', '🚨 부적합', '⚠️ 확인 요망' 텍스트 포함) |\n|---|---|---|---|---|---|---|\n\n"
                judgment_prompt_tab3 += "(※ 위 표 뼈대를 N번 복제하여 연달아 작성. 서류가 없다면 성적서 관련 칸에 '서류 없음' 기재.)\n\n"

                judgment_prompt_tab3 += "## [영양정보표 서식 및 오탈자 정밀 스캔]\n"
                judgment_prompt_tab3 += "🛑 [절대 명령]: 사전 연산 0의 순서 검증 결과와 Pass 1.8의 맞춤법 스캔 결과를 종합하여 아래 표를 작성하십시오.\n"
                judgment_prompt_tab3 += "| 검토 항목 | 발견된 오류 문구 및 교정 제안 | 판정 (반드시 '✅ 적합', '🚨 부적합', '⚠️ 확인 요망' 텍스트 포함) |\n|---|---|---|\n"
                judgment_prompt_tab3 += "| **영양소 법정 표시 순서 위반** | (※ 식이섬유, 칼슘 등 추가 성분의 위치가 틀렸다면 🚨부적합 처리하고 이동 위치 명시) | |\n"
                judgment_prompt_tab3 += "| **영양소 명칭 오탈자** | | |\n"
                judgment_prompt_tab3 += "| **숫자/법정 단위/기호 띄어쓰기 오류** | (※ Rule 82에 따라 비타민A(µg RAE) 등 현행 단위를 정확히 썼는지 픽셀 단위로 검증. µg RE 등 구형 단위 사용 시 🚨부적합 처리) | |\n\n"
...
| 검토 항목 | 관련 룰(Rule) | 시안 텍스트 | 상세 사유 (오탈자 무관용, <br> 태그로 줄바꿈 필수) | 판정 (반드시 '✅ 적합', '🚨 부적합', '⚠️ 확인 요망' 텍스트 포함) |
|---|---|---|---|---|
| **포장재질 및 세부 재질 검증** | [Rule 20, 73] | | | |
| **부정/불량식품 신고 (1399) 등 CS 3종** | [Rule 59] | | | |
| **액상 음료 및 우유류 주의문구 강제 스캔** | [Rule 74] | | | |
| **고카페인 함유 액상음료 의무 표시** | [Rule 105] | (※ 액상음료의 경우 어린이/임산부 등 섭취 주의문구 기재 여부 스캔) | | |
| **수치/단위/상태 결합 어순** | [Rule 103] | (※ 단위가 중간에 끊어지는지 스캔) | | |
...
"""
    ...
    with tab5:
        if st.button("AI 법무팀 자유 스캔 시작", key="btn_law"):
            with st.spinner("AI 법률 엔진 가동 중 (9대 핵심 법령 전문 심층 딥다이브)..."):
                free_style_prompt = """<pre_calc>
🚨 [최고 수준 시스템 룰 오버라이드 및 9대 법령 전수 감사 발동]: 
이 순간부터 당신은 기존에 부여받았던 [식품 QC 마스터 룰북 99개]의 족쇄에서 벗어나, 업로드된 **[9대 핵심 법령 PDF 원문 데이터]**를 무기로 삼아 시안 전체를 픽셀 단위로 감사하는 대한민국 최고의 '식품 전문 수석 변호사'입니다.
어떠한 경우에도 답변에 'Rule OO'이라는 단어를 쓰지 말고, 반드시 실제 법률 조항(예: 「친환경농어업법」 제O조, 「표시기준」 제O조)을 명시하십시오.

🕵️‍♂️ [특명: 9대 핵심 법령별 집중 수사 매뉴얼 (Deep-Dive Guidelines)]
다음 6가지 핵심 맹점을 무조건 100% 교차 검증하십시오:
1. **[친환경농어업법 70% 컷오프 무관용 적발]**: 
   - 서류상 유기농 원료의 총 배합비(%)를 반드시 계산하십시오.
   - 유기농 원료 합계가 70% 미만인 경우, **주표시면(앞면 하단 원료 함량 표시 포함)에 '유기농' 또는 '유기' 단어가 단 한 글자라도 노출되면 명백한 법률 위반(🚨부적합)**으로 즉시 적발하십시오. (오직 뒷면 원재료명 란 내부에서만 표기 허용)
2. **[숨은 당류 및 무가당 기만 추적]**: '무가당/설탕 무첨가' 강조 시 원재료에 덱스트린, 말토덱스트린, 물엿, 시럽, 올리고당 등 식품유형상 당류/엿류가 포함되었는지 스캔하여 적발하십시오.
3. **[신체 기능 및 소화 작용 오인 표현]**: "소화불편감을 줄인" 등 인체 효능·기능을 암시하는 문구는 부당광고법 제8조 위반(🚨부적합)으로 강력 제재하십시오.
4. **[분리배출 도안 및 재질 규정]**: 환경부 고시에 따른 분리배출 도안 마크 및 세부 재질 표시의 적합성을 스캔하십시오.
5. **[유전자변형(GMO) 및 원산지]**: 'Non-GMO' 오남용 여부 및 농수산물 원산지 표시법 준수 여부를 확인하십시오.
6. **[어린이 기호식품 및 이력추적]**: 인증 도안 오사용 및 법정 의무 기준을 대조하십시오.
</pre_calc>

## 5. [AI 법무팀 특별 감사 리포트 (9대 핵심 법령 전문 분석)]

### 챕터 1. 🔍 [패키지 구역별 마케팅·법률 심층 분석]
- 패키지를 4개 구역(주표시면, 정보표시면, 영양성분표, 기타면)으로 나누어, 발견된 모든 문구 및 디자인에 대해 법률 원문에 근거하여 상세히 브리핑하십시오.
- 🟢 **[합법/적합 사례]**: 법령에 부합한 우수 사례는 일반적인 글머리기호(-)로 브리핑하십시오.
- 🛑 **[기만/위법 리스크 적발 시 시각적 강조 강제 룰]**: 친환경농어업법 70% 룰 위반, 신체 기능 오인 문구, 숨은 당류 등 **위법/부적합 리스크를 지적할 때는 절대로 일반 텍스트로 적지 마십시오. 반드시 아래의 HTML 태그 구조를 그대로 사용하여 빨간색 경고 박스가 렌더링되도록 출력**하십시오.

<div style="border: 2px solid #ef4444; background-color: #fef2f2; padding: 15px; border-radius: 8px; margin: 15px 0;">
    <strong style="color: #ef4444; font-size: 1.1em;">🚨 [위법/부적합 적발]</strong><br>
    여기에 적발 내용, 위반한 관련 법령 원문, 그리고 상세한 판정 근거를 가차 없이 지적하십시오.
</div>

### 챕터 2. ⚖️ [리스크 적발 및 법률 위반 사항 총정리]
- 챕터 1에서 분석한 내용 중 **수정이나 확인이 필요한 법적 리스크(부적합 / 확인요망)**만 빠짐없이 표로 정리하십시오.

| 적발된 리스크 (Risk) | 발견 위치 및 문구 | 관련 법령 원문 및 판정 근거 (상세 사유) | 방어 및 수정 제안 |
|---|---|---|---|

### 챕터 3. 📜 [최종 법령별 적합성 판정 (9대 핵심 법규 전수 검증)]
🛑 [절대 명령]: 아래 9대 법령 목록을 단 하나도 누락하거나 생략하지 말고, 1번부터 9번까지 100% 전부 마크다운으로 출력하여 각각의 합격 여부와 1줄 사유를 명시하십시오.

1. **「식품 등의 표시·광고에 관한 법률 및 시행규칙」**: [✅ 적합 / 🚨 부적합 / ⚠️ 확인요망 중 택 1] (사유)
2. **「식품등의 부당한 표시 또는 광고의 내용 기준」**: [✅ 적합 / 🚨 부적합 / ⚠️ 확인요망 중 택 1] (사유)
3. **「식품등의 표시기준 고시」**: [✅ 적합 / 🚨 부적합 / ⚠️ 확인요망 중 택 1] (사유)
4. **「친환경농어업 육성 및 유기식품 등의 관리·지원에 관한 법률 (친환경농어업법)」**: [✅ 적합 / 🚨 부적합 / ⚠️ 확인요망 중 택 1] (※ 유기농 70% 미만 시 앞면 유기농 단어 노출 여부 필수 판정)
5. **「농수산물의 원산지 표시 등에 관한 법률 시행규칙」**: [✅ 적합 / 🚨 부적합 / ⚠️ 확인요망 중 택 1] (사유)
6. **「어린이 기호식품 품질인증기준 (식생활안전관리)」**: [✅ 적합 / ⚠️ 해당없음 / 🚨 부적합 중 택 1] (사유)
7. **「식품 등 이력추적관리기준」**: [✅ 적합 / ⚠️ 해당없음 / 🚨 부적합 중 택 1] (사유)
8. **「분리배출 표시에 관한 지침 (환경부 고시)」**: [✅ 적합 / 🚨 부적합 / ⚠️ 확인요망 중 택 1] (사유)
9. **「유전자변형식품등의 표시기준」**: [✅ 적합 / ⚠️ 해당없음 / 🚨 부적합 중 택 1] (사유)
"""
                cache_name = st.session_state.get("qc_cache_name")
                generation_config = genai.types.GenerationConfig(temperature=0.0, max_output_tokens=65536)
