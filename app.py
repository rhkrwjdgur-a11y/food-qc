...
# ==========================================
# 📚 3. 85대 마스터 룰북 원문 (V310.30 절대 축약 금지 원형 복구본)
# ==========================================
RULE_BOOK_FULL = """
...
🔥 **Rule 85. [식품첨가물 공전 명칭 사수 및 기호 창조 절대 금지 (범용 형식주의)]**
   - **[명칭 축약 엄격 금지]**: 식품첨가물은 어떠한 공간 부족을 이유로도 임의 축약을 금지함. 반드시 식약처 고시 원문의 [표 4, 5, 6]에 등재된 '정식 명칭' 또는 '법정 간략명'과 글자 단위로 100% 일치해야 합격.
   - **[임의 기호 창조 전면 금지]**: 원재료와 첨가물의 용도를 병기하거나 하위 성분을 전개할 때, 대한민국의 식품 법정 표준 기호인 **'괄호()'** 외에 작성자의 편의를 위한 임의의 특수기호**(슬래시 `/`, 대시 `-`, 플러스 `+`, 콜론 `:` 등)**를 사용하여 구조화하는 행위는 전면 금지(🚨).
"""
...
# 마스터 룰북의 모든 규정을 탭별 슬라이싱 매트릭스에 100% 누락 없이 장전
ALL_RULES_NUMBERS = list(range(1, 86))
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V310.30 - 범용 아키텍처 완결본)")
...
"""
    ...
    # ── TAB 5: AI 법률 자문 스캔 (★범용적 법리 교차 추적 모드) ──
    with tab5:
        st.info("💡 [AI 자율 스캔 모드] 특정 지침에 국한되지 않고, 사용자가 업로드한 식약처 법령 PDF 원문 전체를 절대적 팩트(Fact)로 삼아 패키지 전반의 위법성 및 부당광고 소지를 입체적으로 전수 추적합니다.")
        if st.button("▶️ AI 법률 자문 자율 스캔 시작", key="btn_law"):
            with st.spinner("【법령 PDF 전수 대조 및 부당문구 자율 스캔 중...】"):
                missions = [
                    "1. 업로드된 시안의 모든 면(주표시면, 정보표시면, 기타면/측면 등)을 종합적으로 스캔하여 마케팅 카피, 제품명, 강조 문구, 뱃지, 디자인 등 특정 규제를 받을 소지가 있는 표현을 남김없이 추출하십시오.",
                    "2. 추출된 시안의 모든 요소와 관련된 제한, 의무, 면제 조항을 업로드된 법령 PDF에서 검색하여 '법령명, 조항, 법적 원문'을 그대로 추출하십시오."
                ]
                
                judgment_prompt = """## 5️⃣ [AI 법률 자문 자율 스캔 리포트]
⭐ [환각 원천 차단 및 범용적 법리 교차 추적 명령] ⭐
1. Zero-Knowledge: 사전 학습 지식을 차단하고 오직 사용자가 업로드한 법령 PDF 파일만을 진리로 삼아 대조하십시오.
2. 조기 종료 금지: 특정 구역의 단어만 보고 판단하지 말고, 법령이 규정한 세부 단서 조항과 예외 조항(예: 제형별 유예, 도안 표시 조건 등)이 완벽히 만족되었는지 패키지 전면을 유기적으로 다면(多面) 교차 검증하십시오.
3. 임의 국한 금지: 특정 문구에만 갇히지 말고 소비자기만, 과대광고, 인증 도안 위변조 등 법령 PDF에 근거한 모든 잠재적 위법 리스크를 자율적으로 도출하십시오.

---

### 📋 [법률 스캔 결과 보고서]

(아래 구조화된 포맷을 사용하여 식별된 리스크 항목을 출력하십시오)
...
* **AI 법무팀 자문 의견 (위법 리스크):**
  ...
  * 🔍 **[다면(Double-Check) 교차 검증 결과]:** (시안 전체 구역을 대조하여 해당 법령의 요구 조건을 정확히 충족했는지, 추가 단서 조항 위반이나 누락이 있는지 다각도로 분석한 내용 기재)
---
"""
