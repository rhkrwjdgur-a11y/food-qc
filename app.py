...
🔥 **Rule 35. [🌟 범용 간략명/동의어 허용 및 N종 묶음 절대 금지]**
   ...
   - 단, 일반 원료를 '비타민 3종' 등으로 숫자로 묶는 단축 표기는 🚨부적합 처리.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V310.6 - 무결점 매트릭스 픽셀스나이퍼본)")
...
⭐ [출력 템플릿 강제]: 당신은 임의로 요약 보고서나 소설을 쓰지 마십시오. 아래 프롬프트에 제공된 표(Table) 양식의 뼈대를 단 1줄도 훼손하지 말고 그대로 출력하십시오.
...
| **'무가당' 및 당류 강조표시** | [Rule 19, 24] | ('무가당' 텍스트 발견 시 원재료명 인위적 당류 첨가 유무 및 열량 병기 여부를 교차 검증) | |
| **영양강조 컷오프(4대 조건)** | [Rule 21, 52] | (부적합 판정을 내리려면 반드시 100g, 100mL, 100kcal, 1회섭취량 4가지 수식을 모조리 나열하여 전부 미달함을 증명하십시오. 합격 시에도 최소 2개 조건 수식을 명시할 것) | |
| **극단적 픽셀 대조 (오탈자/공백)** | 전수 검사 | (의미 추론 영구 정지! 스페이스바 공백 개수와 위치까지 Byte 단위로 대조하여 다르면 🚨부적합 처리) | |
"""
...
| **'무가당' 및 당류 강조표시** | [Rule 19, 24] | ('무가당' 텍스트 발견 시 원재료명에 인위적인 당류가 없는지 반드시 확인) | |
| **영양강조 컷오프(4대 조건)** | [Rule 21, 52] | (부적합 판정을 내리려면 반드시 4가지 수식을 모조리 증명하십시오. 합격 시에도 2개 이상 명시할 것) | |
| **극단적 픽셀 대조 (오탈자/공백)** | 전수 검사 | (의미 추론 영구 정지! 스페이스바 공백 개수와 위치까지 Byte 단위로 대조하여 다르면 🚨부적합 처리) | |
"""
