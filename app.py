...
# ==========================================
# 📚 3. 55대 룰북 원문 (축소 금지! 100% 상세 풀-텍스트 전개판)
# ==========================================
RULE_BOOK = """
...
🔥 **Rule 14. [표 4 및 표 6 의무/예외 표기 마스터 룰 (묶음 표기 허용)]**
   ...
   - **[표 6 묶음 표기 적합성]**: 일반 첨가물 표기 시 구연산나트륨 등을 묶어서 시안에 **"영양강화제 2종"**처럼 표기하는 것은 완벽 적합(✅)입니다. (단, Rule 44의 혼합제제 내부는 예외)
...
✅ **Rule 17. ['無첨가' 마케팅 검증]**
   - 사용이 원천 금지된 첨가물을 배제했다고 강조한 경우(예: 보존료 무첨가) 기만광고로 부적합(🚨) 처리하십시오.
...
🔥 **Rule 29. [복합원재료 원산지 표시 한계]**
   - 복합원재료 자체의 원산지만 확인하십시오. 하위 성분 원산지까지 강제하지 마십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V69.0 - 선물세트 팩 vs 박스 전용 맵핑)")
    ...
    # ==========================================================
    # 탭 2: 정보표시면 검토 (V69.0 선물세트 모드 팩 vs 박스 전용 비교 및 필사 멈춤 방지)
    # ==========================================================
    with tab2:
        st.info("단품은 시안 vs 서류를 대조하며, 선물세트는 '서류 유무와 상관없이' 팩과 박스의 텍스트가 100% 동일한지 1:1 대조합니다.")
        if st.button("▶️ 정보표시면 원재료 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("시안 텍스트를 대조하여 1:1 표를 생성 중입니다... (최대 40초 소요)"):
                
                if inspection_mode == "선물세트 교차 검토 (팩 vs 박스 비교)":
                    prompt = """
                    [지시]: '선물세트 교차 검토' 모드입니다. 서류(배합비/한글라벨) 유무와 관계없이, 업로드된 **팩(내포장) 시안**과 **박스(외포장) 시안**의 원재료명이 토씨 하나 틀리지 않고 100% 일치하는지(순서, 누락, 띄어쓰기, 오타 등) 대조하는 표를 작성하십시오.
                    
                    🚨 [1단계: 사전 텍스트 스캔 (필사 절대 금지!)] 🚨
                    <thinking>
                    - 시스템 멈춤을 방지하기 위해 팩과 박스에 적힌 긴 원재료명을 여기에 절대 타이핑하지 마십시오. 단답형으로 "텍스트 스캔 완료"라고만 적으십시오.
                    </thinking>
                    
                    🚨 [2단계: 팩 vs 박스 교차 검증 표 렌더링] 🚨
                    ## 2️⃣ [팩(내포장) vs 박스(외포장) 원재료명 100% 일치 검증 (Rule 47)]
                    - 결론: (✅ 일치 또는 🚨 불일치)
                    
                    **(🚨 시안 원재료명 행 분리 절대 규칙 🚨): 표를 작성할 때 쉼표(,)를 기준으로 행을 나누되, 괄호 '(', ')', '{', '}', '[', ']' 안에 있는 쉼표는 절대 분리 기준이 될 수 없습니다!! 복합원재료의 괄호 안 내용이 아무리 길어도 무조건 하나의 덩어리로 묶어서 '1개의 행(줄)'에 표시하십시오.**
                    
                    | 팩(내포장) 원재료명 | 박스(외포장) 원재료명 대조 | 일치 여부 (순서/누락/오타) | 판정 |
                    |---|---|---|---|
                    | (예: 정제수) | 정제수 | 100% 일치 | ✅ |
                    | (예: A2단백원유(국내산)) | A2단백원유(국산) | 🚨 글자 불일치 (국내산 vs 국산) | 🚨 |
                    | (예: 식물성유지) | (누락됨) | 🚨 박스 시안에서 누락됨 | 🚨 |
                    
                    (위 양식에 맞춰 팩에 적힌 모든 원재료를 박스의 텍스트와 1:1로 끝까지 대조하는 표를 그리십시오.)
                    
                    ## 3️⃣ [알레르기 및 주의사항 교차 검증]
                    - 결론: (✅ 적합 또는 🚨 부적합)
                    - 팩과 박스의 '~함유' 알레르기 물질 표시 완벽 일치 여부:
                    - 주의사항(보관방법, 해동방법 등) 완벽 일치 여부:
                    """
                else:
                    prompt = """
