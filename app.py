...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.68 - 스나이퍼 족쇄 범용 강화판)")
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                # STEP 1: 마스터표 작성 (V311.68 패치: 출력하되 다이어트 및 강제 해체 족쇄)
                if has_any_doc:
                    if "무더기" in doc_mode:
...
⭐ **[마스터 DB 강제 해체 및 다이어트 족쇄 (토큰 절약 & 정확도 100%)]**:
1. [다이어트]: 서류에 기재된 '코드 번호', '제조원(국내)', '수입원(판매원)', '식품유형' 열은 토큰 절약을 위해 표에서 완전히 삭제하고 오직 아래 4개 핵심 열만 출력하십시오.
2. [강제 해체]: 여러 개의 향료나 영양강화제가 있더라도 절대 '향료 N종', '영양강화제 N종'으로 AI 임의로 묶어버리지 마십시오. 서류의 행(Row)에 적힌 0.01% 단위의 원료 하나하나를 무조건 1줄에 1개씩 독립된 개별 항목으로 전개하여 배합비 순위를 매기십시오.
| 서류상 원료명 (임의 묶음 금지) | 하위 전개 성분 | 원산지 | 배합비(%) / 순위 |
|---|---|---|---|
...
| 시안 표기 개별 원재료명 (1줄에 딱 1개씩만) | 매칭된 서류 원료명 (Rule 90 유연한 맵핑 적용) | 대조 검증 결과 및 사유 (단답형 절대 금지! 룰을 근거로 2~3문장 상세 서술) | 최종 판정 |
|---|---|---|---|
