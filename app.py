...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V214.0 - 시각/텍스트 환각 완전 통제판)")
...
- ⭐ 띄어쓰기 및 오탈자 적발: (글자 단위 1:1 대조. ⭐[전 구역 범용 띄어쓰기/오탈자 정밀 스캔 (AI 자동완성 절대 비활성화)]: 문맥을 파악해 알아서 띄어 읽는 '뇌내 보정(Auto-Correction)'을 엄격히 금지합니다. 픽셀에 공백(Space) 없이 글자들이 다닥다닥 붙어있다면(예: '본제품은', '부유물이나침전물'), 무조건 붙어있는 그대로 인식하여 🚨부적합(띄어쓰기 오류)으로 낱낱이 적발하십시오. 단, 똑같은 글자를 두고 오류라고 지어내는 기계적 환각을 절대 금지합니다.)
"""
...
- ⭐ [Rule 69] 비타민 아래첨자 스나이퍼:
- 🔍 [전 구역 공통] 오탈자 및 띄어쓰기 정밀 스캔: (표 내 오타 여부. ⭐[전 구역 범용 띄어쓰기/오탈자 정밀 스캔 (AI 자동완성 절대 비활성화)]: 문맥을 파악해 알아서 띄어 읽는 '뇌내 보정(Auto-Correction)'을 엄격히 금지합니다. 픽셀에 공백(Space) 없이 글자들이 다닥다닥 붙어있다면(예: '본제품은', '부유물이나침전물'), 무조건 붙어있는 그대로 인식하여 🚨부적합(띄어쓰기 오류)으로 낱낱이 적발하십시오. 단, 똑같은 글자를 두고 오류라고 지어내는 기계적 환각을 절대 금지합니다.)
"""
                else:
                    judgment_prompt = base_tab3_warning + """
## 4️⃣ [영양표시 및 % 기준치 검증]
