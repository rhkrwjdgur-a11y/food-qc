...
# 2. 통합 전문가 프롬프트 (Rule 1~45 무삭제 원문 + V6.27 신규 패치 적용)
...
---
# [식품 패키지 표시사항 QC 자동화 검수 시스템 룰북 6.27]
...
[무가당(당류 무첨가)]: 제조 시 당류(설탕, 시럽, 과일농축액 등)를 인위적으로 첨가하지 않았다는 뜻입니다. 우유의 유당 등 '천연 유래 당'이 존재하여 영양정보에 당류가 0.5g 이상(예: 5g) 표기되어 있더라도, 원재료명에 당류가 없다면 완벽한 **✅적합(합법)**입니다.

🚨[AI 오지랖 금지]: "무가당은 법규상 무당(0.5g 미만)과 동일하다"라고 지적하는 행위를 엄격히 금지합니다.

🔥 **Rule 20. [용기·포장재질 표기법 정밀 스캔 - 개정본]**
   - 식약처 표준 포맷인 **`재질명(포장부위)`** (예: 폴리스티렌(용기)) 또는 **`포장부위: 재질명`** (예: 내면재질: 폴리에틸렌) 형태를 권장합니다.
   - 🚨 시안에 "컵,리드(폴리스티렌)" 처럼 부위 명칭을 뭉뚱그려 비표준으로 표기한 경우, 추후 포장재질 변경 시 허위표시 리스크가 있으므로 "폴리스티렌(컵, 리드)" 또는 "용기: 폴리스티렌, 리드: 폴리스티렌" 등 명확한 형태로 분리 수정할 것을 🚨부적합(수정권고) 처리하십시오.
...
🔥 **Rule 25. [다중 포장 듀얼 컬럼(번들) 완벽 인식 및 분리 검증 - 개정본]**
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 (V6.27 - 듀얼 컬럼/재질 패치)")
    st.markdown("---")
    ...
    def process_qc(product_type, content_hashes):
        ...
        # [V6.27 업데이트]: 듀얼 컬럼 자동 분할 및 기호 강제 적용
        final_prompt = f"""
        [현재 검토 제품유형]: {product_type}
        
        🚨[출력 형식 강제 명령]🚨
        1. 모든 검토 항목의 결론에는 반드시 기호를 명시하여 가독성을 극대화하십시오. (통과: **✅적합**, 위반/오류: **🚨부적합**)
        2. 영양표시 검토(4번 항목) 시, 시안이 '1개당'과 '총 내용량당' 듀얼 컬럼(번들)으로 표시된 제품인 경우, 표(Table)를 2개로 나누어(예: [1컵 당 기준], [총 내용량 당 기준]) 두 기준의 데이터를 모두 독립적으로 검증하십시오.
