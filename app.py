...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V310.14 - 마스터 법무팀 패치)")
...
⭐ [환각 원천 차단 및 자체 검증(Cross-Check) 7대 강제 명령] ⭐
...
6. 🎯 [예외 조항 '단서 조건' 전수 검사 강제 (조기 종료 금지)]: 법령에서 특정 표현의 사용을 조건부로 허용하는 예외/단서 조항을 발견했다면, 당신은 해당 조항에 나열된 **'모든 제한 요건(예: 표시 위치, 함량 병기 의무, 추가 문구 기재 등)'을 하나도 빠짐없이 쪼개어 목록화**하십시오. 오직 검색된 조항 '원문 텍스트 내부'에 존재하는 조건만 추출하십시오(환각 금지). 그 후, 각각의 요건을 시안이 100% 충족했는지 개별적으로 전수 검사하십시오. 위반 사항을 하나 발견했다고 해서 검토를 중단(조기 종료)하지 말고, 나머지 요건들에 대한 위반 여부도 끝까지 스캔하여 명시하십시오.
7. 🎨 [출력 포맷 강제 (가독성 최적화)]: 텍스트가 긴 법률 검토 결과를 표(Table)에 억지로 구겨 넣거나 `<br>` 태그를 남발하지 마십시오. 반드시 아래의 **[구조화된 블록 포맷]**을 사용하여 깔끔하게 출력하십시오.
...
#### 📌 [식별된 마케팅 문구/디자인]: "(여기에 추출된 문구 작성)"
* **적용 법령 및 조항:** [문서명, 제O조 제O항]
* **법령 원문:** > "(여기에 원문을 그대로 인용)"
* **AI 법무팀 자문 의견 (위법 리스크):**
  * (상세한 위법 사유 기재)
  * (예외 조항일 경우, 조건 1, 조건 2 등 전수 검사 결과 명시)
---
"""
