...
🔥 **Rule 3. [주표시면 vs 영양성분표 수치 100% 일치 및 총열량/식이섬유 열량 계산 강제 룰]**
   ...
   - **[⭐ 열량 계산 시 식이섬유 및 당알콜 분리 필수 적용]**: 열량 구성비나 총 열량을 검증할 때 단순히 탄수화물 총량에 4kcal를 곱하는 구식 계산(Atwater 계수)을 금지합니다. 영양성분표에 **식이섬유**나 **당알콜**이 표기되어 있다면 반드시 총 탄수화물에서 그 양을 분리하여 계산하십시오. *(적용 공식 ➔ 식이섬유: 2kcal/g, 에리스리톨: 0kcal/g, 알룰로오스: 0kcal/g, 자일리톨 등: 2.4kcal/g, 나머지 당질(탄수화물): 4kcal/g)*
...
🔥 **Rule 24. [무당/무가당/설탕무첨가 2대 의무 표기 (감미료 & 열량 물리적 위치 강제)]**
   ...
   - 1) **[감미료 위치 검증]**: 감미료가 들어갔다면 "감미료 함유" 문구가 강조표시 '바로 옆이나 아래'에 붙어있어야 합니다. (알룰로스 등은 식품원료이므로 예외)
...
🔥 **Rule 28. [원산지 3순위 완벽 필터링 및 과잉 표기 경고 룰]**
   ...
   - 2) **[과잉 도출 경고]**: 의무 대상이 아닌 부원료에 원산지가 적혀 있다면 무조건 🚨**[확인 요망]**을 띄워 경고하십시오.
...
🔥 **Rule 68. [다포장/세트포장 낱개 분할 표기 스나이퍼 룰]**
   ...
   - 분할 표기 없이 단순히 `3,000mL`처럼 총량만 적혀 있다면 즉시 🚨부적합 처리.
"""
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V167.0 - 503 방어 & 절대 폼 복구)")
    ...
    with st.sidebar:
        ...
        def get_uploaded_content():
            ...
            # ⭐ [구글 503 에러 완벽 방어 헬퍼 함수] 점진적 대기(Exponential Backoff) 로직 탑재
            def robust_upload(file_path):
                max_retries = 5 # 5번까지 집요하게 재시도
                for attempt in range(max_retries):
                    try:
                        up = genai.upload_file(file_path)
                        while up.state.name == "PROCESSING":
                            time.sleep(3)
                            up = genai.get_file(up.name) # 갱신 필수
                        ...
                        if attempt == max_retries - 1:
                            raise e
                        time.sleep(3 * (attempt + 1)) # 실패 시 3초, 6초, 9초 점진적 대기 후 재시도
...
4. 🔥 [혼합제제 강제 합법 판정]: 시안에 비타민이나 부형제가 괄호 없이 뿔뿔이 흩어져 적혀 있다면, 그것은 Rule 44에 따른 완벽한 합법(✅적합)입니다.
