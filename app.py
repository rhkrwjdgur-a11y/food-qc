...
# ==========================================
# 📚 3. 55대 룰북 원문 (단 한 글자도 생략 불가 100% 풀버전)
# ==========================================
RULE_BOOK = """
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V60.0 - 55대 룰북 원상복구판)")
    ...
    with tab2:
        ...
        if st.button("▶️ 정보표시면 원재료 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("서류와 시안을 좌우에 놓고 1:1 정밀 대조 표 생성 중... (최대 40초 소요)"):
                prompt = """
                [지시]: 오직 아래의 원재료명 1:1 맵핑 표와 검증 리포트만 출력하십시오.
                
                🚨 [1단계: 초단기 전략 수립 (Thinking 강제)] 🚨
                여기는 오직 아래 내용만 단답형으로 적고 즉시 닫으십시오.
                <thinking>
                1. 서류 타입: (배합비 % 유무 파악)
                2. 시안 누락/오타 의심: (서류엔 있는데 시안에 없거나 글자가 다른 것 스캔, 예: 추정->주정)
                3. 혼합제제 묶음(Rule 44) 위반 스캔: (유화제5종 등 혼합제제 주용도 표기 여부)
                </thinking>
                ...
                **(🚨 텍스트 다이어트 필수 경고 🚨): 서류 매칭 원료 칸에 복합원재료의 기나긴 하위 성분(괄호)을 절대로 통째로 복사해서 붙여넣지 마십시오! 핵심 원료명, 원산지, 비율만 극단적으로 짧게 적으십시오. 원료명이 길어지면 시스템이 멈춥니다.**
                
                [🟢 올바른 표 작성 예시]
                | 시안 원재료명 (포장지) | 서류 매칭 원료 (서류 요약) | 배합비 룰 (3순위/5%/2%) | 규정/오타/누락/Rule 44 검증 | 판정 |
                |---|---|---|---|---|
                | 유함유가공품(프랑스산) | 유함유가공품 (프랑스 / 괄호생략) | 미상 | ✅ 적합 | ✅ |
                | 유화제5종 | 혼합제제 (하위성분생략) | - | 🚨 Rule 44 위반 (혼합제제는 개별명칭 전개 필수) | 🚨 |
                | 추정 | 주정 | - | 🚨 오타 적발 (추정 -> 주정) | 🚨 |
                
                (위 양식에 맞춰 시안에 있는 모든 원재료를 끝까지 1:1로 대조하는 표를 그리십시오.)
                
                - 🚨 [추가 지적 사항 (누락 검증)]: (서류에는 명시되어 있으나 시안에서 아예 '누락'된 원료를 지적하십시오. 단, Rule 5에 따라 5% 미만 복합원재료의 하위성분 생략은 합법이므로 지적 금지.)
                - [Rule 53 관련]: (제품명 연동 원료 함량/원산지 위반 사항 별도 서술. 가공품이면 원산지 면제 합법 처리.)
                
                ## 3️⃣ [알레르기 및 주의사항 교차 검증]
                - 결론: (✅ 적합 또는 🚨 부적합)
                - '~함유' 물질 원재료명 실존 여부:
                - 교차오염 경고 중복/모순 여부:
                - 법정 표시사항 누락 검증 (냉동식품일 경우 해동방법 누락 여부 등):
                """
        ...
        if st.session_state["result_tab2"]:
            ...
            if thinking_match:
                thinking_log = thinking_match.group(1).strip()
                report_content = result.replace(thinking_match.group(0), "").strip()
                with st.expander("🧠 1:1 매칭 전략 로그 보기"):
                    st.markdown(f"*{thinking_log}*")
                st.markdown(report_content)
            else:
                st.markdown(result)
    ...
    with tab3:
        st.info("영양성분표 이미지와 성적서를 대조합니다. 수치 오류 수정 시, 1일 기준치 비율(%)도 수정된 값으로 재계산합니다.")
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("영양성분 오차 수식 계산 및 % 재계산 표 렌더링 중..."):
                ...
                **(🚨 핵심 명령: 비율(%) 재계산 룰): 만약 실측값이 허용오차를 벗어나거나 Rule 23에 의해 시안의 표시 수치를 수정해야 한다면(예: 2mg -> 5mg 미만으로 수정 필요), 1일 영양성분 기준치 비율(%)도 '틀린 시안 수치'가 아니라 '수정될 올바른 수치(예: 5mg)'를 기준으로 재계산하여 올바른 %를 지시해야 합니다.**
