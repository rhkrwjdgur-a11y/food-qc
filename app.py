...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V81.0 - 초압축 암산 도입 및 셧다운 방어)")
    ...
    with tab2:
        st.info("비교 기준이 되는 서류를 바탕으로 팩과 박스 시안에 모든 원료가 100% 일치하는지 빠짐없이 대조합니다.")
    ...
    # ==========================================================
    # 탭 3: 영양성분표 검토 (V81.0 셧다운 완벽 차단: 초압축 암산 도입)
    # ==========================================================
    with tab3:
        st.info("특수영양식품의 25개+ 영양성분 오차를 검증합니다. 초압축 암산을 통해 정확도와 안정성을 동시에 잡습니다.")
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("초압축 수학 연산을 수행하고 전체 표를 렌더링 중입니다... (최대 40초 소요)"):
                prompt = """
                [지시]: '3번 탭: 영양성분표 검토' 모드입니다. 
                
                🚨 [1단계: 시스템 셧다운 방지 초압축 암산 (Diet Thinking)] 🚨
                정확한 오차 계산을 위해 `<thinking>` 태그를 사용해야 하지만, 여기서 문장(에세이)을 쓰면 글자 수 초과로 시스템이 뻗습니다! **절대 서술형 문장을 쓰지 말고, 오직 영양소 이름, 숫자, 기호(<, >), 적합(✅)/부적합(🚨) 여부만 아주 짧게 연속으로 작성하십시오.**
                
                [🟢 올바른 초압축 암산 예시]
                <thinking>
                열량: 140.5<174(✅), 나트륨: 146.6<216(✅9%), 탄수화물: 12.05>10.4(✅4%), 당류: 5.45<7.2(✅6%), 콜레스테롤: 4.99>2.4(🚨수정필요), 단백질: 5.04>2.4(✅), 비타민A: 200>150(✅) ... (모든 영양소를 이런 식으로 초압축하여 기호로만 적을 것)
                </thinking>
                ...
                **(🚨 Rule 41 % 표기 금지 및 수식 생략 🚨): 열량(kcal)과 트랜스지방은 시안 % 칸에 무조건 '-' 기호를 넣으십시오.**
                **(🚨 토큰 절약 특별 명령): 계산 결과가 올바른 정상(✅) 영양소는 '% 검증 및 재계산' 칸에 `(180/2000)*100` 같은 산수 수식을 절대 적지 말고 오직 "✅ 적합" 이라고만 짧게 적어 토큰을 아끼십시오! 오직 틀린(🚨) 영양소에만 수정 지시를 적으십시오.**
                
                [🟢 스마트 압축 표 작성 예시]
                | 영양성분명 | 성적서 실측값 | 시안 표시량 | 법적 허용오차 기준선 | 1일 기준치 | 시안 % | % 검증 및 재계산 | 판정 |
                |---|---|---|---|---|---|---|---|
                | 나트륨 | 146.60mg | 180mg | 216mg 이하 | 2000mg | 9% | ✅ 적합 | ✅ |
                | 콜레스테롤 | 4.99mg | 2mg | 2.4mg 이하 | 300mg | 1% | 🚨 시안 수정(5mg 미만) 필요. 재계산 시 2% 요망 | 🚨 |
                | 열량 | 140.51kcal | 145kcal | 174kcal 이하 | - | - | ✅ 표기 대상 아님 | ✅ |
                
                (위 예시처럼 수식을 극단적으로 압축하여 25개 이상의 모든 영양성분을 1개도 생략 없이 끝까지 작성하십시오.)
