...
🔥 **Rule 35. [🌟 범용 간략명/동의어 허용 및 N종 묶음 예외 보장 (강력 예외 룰)]**
...
🔥 **Rule 70. [내/외포장 원재료명 100% 일치 강제 범용 스나이퍼 룰 (V196.0 의미 해석 절대 금지)]**
...
# Rule 63 removed from COMMON_RULES to stop noise
COMMON_RULES = [36, 37, 42, 43, 45, 47]
RULES_TAB1 = "[탭1 주표시면 관련 핵심 룰 (Rule 63 1차 점검 포함)]\n" + get_sliced_rules([3, 9, 10, 15, 16, 17, 18, 19, 21, 24, 28, 40, 46, 47, 50, 51, 52, 53, 57, 58, 59, 60, 62, 63, 64, 68, 69] + COMMON_RULES)
RULES_TAB2 = "[탭2 정보표시면/원재료명 관련 핵심 룰]\n" + get_sliced_rules([1, 2, 5, 6, 7, 8, 12, 13, 14, 20, 25, 28, 29, 30, 34, 35, 38, 39, 44, 48, 52, 54, 57, 58, 59, 60, 61, 65, 68, 69, 70] + COMMON_RULES)
RULES_TAB3 = "[탭3 영양성분표 관련 핵심 룰]\n" + get_sliced_rules([3, 4, 6, 10, 11, 21, 23, 25, 26, 27, 31, 32, 33, 40, 41, 55, 59, 66, 67, 68, 69] + COMMON_RULES)
RULES_TAB4 = "[탭4 기타면/측면 관련 핵심 룰 (Rule 63 최종 점검 포함)]\n" + get_sliced_rules([7, 15, 17, 18, 20, 22, 24, 38, 56, 57, 59, 63, 64, 69] + COMMON_RULES)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V201.0 - 2번 탭 복붙 스나이퍼 탑재판)")
    ...
    with st.sidebar:
        ...
        st.info("💡 텍스트가 너무 빽빽해서 AI가 글자를 빼먹는다면, 여기에 디자이너 원본 텍스트를 복붙해 주세요. 100% 완벽한 대조가 가능합니다.")
...
2. ⭐ [누락/증발 텍스트 복구 강제]: 원본 이미지(또는 수동 텍스트)에는 분명히 적혀 있는데 누락된 단어가 없는지 글자 단위로 대조하여 완벽하게 복구해 내십시오.
3. ⭐ [미션 B 표 검증]: 미션 B의 결과물이 존재한다면 반드시 표(Table) 형태인지 확인하십시오.
...
3. ⭐ [초정밀 분해 지시 - 괄호 안 누락/증발 절대 금지]: 텍스트를 문장 단위로 대충 읽어오지 마십시오. 반드시 '쉼표(,)'를 기준으로 모든 성분을 잘게 쪼개서 번호를 매겨 추출해야 합니다.
...
| 타겟(박스) 시안 표기 원재료명 (순서대로 100% 나열, 중략 금지) | 매칭된 마스터 서류 원료명 | 원산지 산정 순위 | 대조 검증 (오탈자/순서/합법 간략화) | 판정 (상세 사유) |
|---|---|---|---|---|
...
## 🔍 [전 구간 공통: 수량 모순 및 오탈자 검증]
- ⭐ [Rule 68] 정보표시면 내용량 낱팩 복붙 스나이퍼: 박스(외포장) 시안인데 '내용량'이 낱팩 용량(예: 125mL)으로만 단독 표기되어 있다면 무조건 🚨부적합 처리하고, '낱팩 용량 x 수량(총 용량)' 형태로 수정 지시하십시오.
- ⭐ 포장 단위(수량) 2-Track 검증:
- ⭐ [Rule 69] 비타민 아래첨자 스나이퍼:
...
| 시안 표기 원재료명 (패키지 나열 순서 100% 나열, 중략 금지) | 매칭된 마스터 서류 원료명 | 원산지 산정 순위 | 대조 검증 (오탈자/순서/합법 간략화) | 판정 (상세 사유) |
|---|---|---|---|---|
| (시안 기준 100% 나열) | (서류 원료명) | (도출 사유) | (검증 결과) | (✅/🚨 판정 및 사유) |
...
## 2️⃣ [마스터 서류 vs 시안 법적 교차 검증 (Rule 35 동의어/간략명 허용)]
| 시안 표기 원재료명 (패키지 나열 순서대로 100% 나열, 중략 금지) | 매칭된 위 마스터 표 원료명 | 원산지 산정 순위 | 대조 검증 (오탈자/순서/합법 간략화) | 판정 (상세 사유) |
|---|---|---|---|---|
| (시안 기준 100% 나열) | (서류 원료명) | (도출 사유) | (검증 결과) | (✅/🚨 판정 및 사유) |
※ [법규 트랙]: 시안 명칭이 서류와 물리적으로 다르더라도, 표시기준 표5나 표6에 따른 합법적인 동의어/간략명(Rule 35)인 경우 ✅적합으로 판정하십시오. 단, 판정 란에 `🌟 [표5/6 치환 알림] 합법적 동의어/간략명 표기. 한번 더 확인해 보세요.` 라는 코멘트를 반드시 추가하십시오.
...
- ⭐ [Rule 63] 미드팩 190mL 기타면 내 '질소충전' 표기 누락 점검: (반드시 주의사항 구역에서 텍스트를 스캔하여 판정할 것)
