...
def main():
    ...
    st.markdown("<h1>Label Guard AI <span style='font-size:0.5em; color:#64748b;'>식품 패키지 무결점 검증 시스템 (V5.5 Ultimate)</span></h1>", unsafe_allow_html=True)
        ...
        if st.button("정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("원재료 1:1 매칭 매트릭스 연산 중..."):
                ...
                8. 🛑 [최고 수준 경고: 증빙 서류 역할 강제 분리 (Rule 48 적용)]: 
                   - [배합비/레시피 서류]: 오직 원재료의 **'표기 순서(중량순)'**를 검증하고, 5% 미만 복합원재료(Rule 5)나 원산지 98% 컷오프(Rule 1) 등 **'비율(%) 계산'**을 할 때만 사용하십시오.
                   - [한글라벨/원료 스펙]: 시안에 적힌 원재료의 **'법적 명칭'**과 **'하위 구성 성분'**을 1:1로 대조할 때는 반드시 **[한글라벨]**에서 추출한 데이터를 기준(정답)으로 삼으십시오.
                   - 대조표의 '서류 매칭 원료' 칸에 배합비에 적힌 공장 내부 식별명(가명)을 끌고 와서 명칭 불일치(🚨부적합)로 오판하는 행위를 엄격히 금지합니다.
                """
