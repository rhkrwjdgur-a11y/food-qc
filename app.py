...
**Rule 35. [범용 간략명/관용명 허용 및 혼합제제 괄호 내부 N종 은폐 금지 범용 룰]**
   ...
   - [혼합제제 괄호 내부 은폐 절대 불가]: 패키 시안에 `혼합제제(산도조절제 2종)`처럼 묶거나, 여러 혼합제제의 하위 성분들을 몰래 빼와서 임의로 `영양강화제 3종`처럼 묶어 은폐(블랙박스화)한 경우 명백한 위법(🚨부적합)으로 처리하십시오. 단, 애초에 레시피에 3가지 성분이 독립적으로 투입되어 그것들을 용도명으로 합법적으로 묶은 경우는 제외(적합)입니다.
...
def main():
    ...
    st.title("식품 표시사항 정밀 검토 시스템 (V4.5 Ultimate Master)")
...
"""
        ...
        for attempt in range(3):
            try:
                ...
                if "403" in str(e) or "404" in str(e) or "CachedContent" in str(e):
                    final_clean_text = "🚨 **[캐시 만료 안내]**<br>파일 연동 후 60분이 초과되어 데이터가 서버에서 지워졌습니다. 좌측 사이드바에서 **[전체 시스템 파일 연동 및 캐싱]** 버튼을 다시 한 일 눌러주세요."
                    break
    ...
    with tab3:
        if st.button("영양성분표 수치 자동 환산 및 대조", key="btn_nutri"):
            with st.spinner("영양성분 수치 역산 및 오차 매트릭스 검증 중..."):
                ...
                # 🔥 [V4.5 핫픽스: 다단 0표기 종속 법칙 최적 교정 제안 복구]
