...
# ==========================================
# 📚 3. 65대 룰북 원문 (V156.3 마스터표 강제 출력 완결판)
# ==========================================
RULE_BOOK_FULL = """
...
🔥 **Rule 52. [영양성분 강조표시(명칭 단독 기재 및 N종) 컷오프 엄격 검증 룰]**
   - 패키지 주표시면이나 측면 등 '영양성분 표시서식 이외의 위치'에 "비타민/미네랄 N종"이라는 그룹 명칭 또는 **"비타민C, 아연, 나이아신" 등 개별 영양소 명칭을 단독으로 기재/강조**하는 창의적인 마케팅 문구는 모두 식약처 법규상 **'영양성분 강조표시(함유/급원)'**로 간주됩니다.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V156.3 - 6열 마스터표 출력 강제판)")
...
- 사용자가 업로드한 '증빙 서류(한글라벨, 배합비 엑셀, 성적서 이미지 등)'를 샅샅이 분석하여, 시안 검토의 기준이 될 [절대 정답지 체크리스트]를 구축하십시오.
- ⭐ [필수 표 출력]: 서류 분석 결과는 반드시 아래의 컬럼을 가진 마크다운 표(Table) 형태로 정리하여 출력해야 합니다. 줄글로 요약하지 마십시오!
  | 증빙 서류명 | 원료 제품명 | 식품유형 | 하위 전개 성분 (100% 나열) | 원산지 |
- 특히 혼합제제의 하위 성분(예: 비타민D3혼합제제 안의 '가공유지', '팜유' 등)을 단 하나도 빠짐없이 표 안에 100% 추출하십시오. 
- ⭐ [가장 중요한 명령 1]: 이 마스터 정답지 표를 구축하는 동안, 시안 이미지는 절대 참고하거나 컨닝하지 마십시오. 오직 증빙 서류에 있는 사실만으로 완전 무결한 리스트를 만드십시오.
- 🔥 [가장 중요한 명령 2 (절대 요약 금지)]: 하위 성분이 20개든 30개든 귀찮아하지 말고 100% 전부 타이핑하십시오. "외", "등", "(하위 성분 다수)" 같은 축약어를 사용하는 순간 이 시스템은 폐기됩니다. 서류에 적힌 글자 그대로를 모두 추출하십시오.
...
(여기에 미션 B에서 지시한 5개의 열을 가진 마크다운 표 100% 출력)
"""
        try:
...
2. ⭐ [미션 B (서류) 표 검증]: 미션 B의 결과물이 반드시 표(Table) 형태인지 확인하십시오. 서류 내용 중 실수로 빼먹은 원료나 하위 성분(예: '가공유지' 등 구석에 숨은 부형제), 원산지가 없는지 샅샅이 뒤져서 표의 내용을 100% 꽉 채우십시오. "등", "외 다수" 라는 표현이 표 안에 있다면 원래 성분명으로 복구하십시오.
...
1. [마스터 표 작성 시 시안 컨닝 금지]: 1번 마스터표를 작성할 때 '시안 텍스트'는 절대 쳐다보지도 마십시오. 오직 '미션 B'에서 추출된 증빙 서류 정답지만을 기반으로 표를 구성하십시오. 만약 시안에만 있고 서류에 없는 원료가 있다면 '매칭되는 증빙 서류 없음'이라고 명시하십시오.
...
7. 🔥 [글자 단위 완전 일치 강제 명령 (오탈자 무관용)]: 당신은 글자의 '의미'를 이해하는 AI가 아니라, 글자의 '형태'만 대조하는 시각 판독기입니다. 서류 정답지와 시안의 글자를 1:1 대조할 때, "토코페일"과 "토코페릴"처럼 단 한 글자의 자음/모음이라도 다르면 🚨부적합(오탈자 적발) 처리하십시오.
"""
