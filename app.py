...
🔥 **Rule 41. [% 영양소 기준치 정밀 검증 및 예외 항목]**
   ...
   - **[🚨 열량 및 트랜스지방 % 표기 금지]:** 열량(kcal)과 트랜스지방은 % 비율을 표기하는 항목이 아닙니다. 절대 %를 계산하지 마십시오. (빈칸 또는 '-' 처리)
...
🔥 **Rule 44. [혼합제제 하위성분 '주용도 묶음표기' 절대 금지]**
   - 혼합제제의 괄호 안 하위 성분은 용도가 같아도 '산도조절제', '유화제' 등 주용도명으로 묶어서 표기할 수 없습니다. '구연산' 등 개별 명칭을 전개해야 합법입니다. 시안에 혼합제제(유화제5종) 처럼 묶어 썼다면 즉시 🚨부적합.

✅ **Rule 45. [선택적 누락 허용]**
🔥 **Rule 46. [제품명 숫자 강조 시 전개 확인]**
🔥 **Rule 47. [내/외포장 물리적 차이 예외 인정]**
   - 내포장(팩)과 외포장(박스)의 **'원재료명'과 '영양성분' 텍스트는 100% 일치**해야 합니다. (단, 소비기한 표기 위치 등 용기 특성에 따른 문구 차이는 합법 인정)
...
🔥 **Rule 53. [제품명 연동 원료 강제 추적 (🚨마카다미아 등 농산물 원물 한정)]**
   1) 함량 검증(공통): 주표시면에 해당 원료 함량(%) 표기. (누락 시 🚨 부적합)
   2) 원산지 검증(농산물 원물 강제 추적): 원료가 **'진짜 농수산물 원물(예: 마카다미아, 아몬드, 딸기, 고구마, 밤, 우유 등)'**인 경우, 해당 원료가 서류상 '페이스트', '농축액', '분말' 형태의 가공품으로 투입되었더라도 반드시 정보표시면에 그 원물의 원산지가 기재되어야 합니다. (마카다미아 원산지 누락 시 🚨 부적합)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V67.0 - 박스 교차검증 UI 추가)")
    ...
    with st.sidebar:
        ...
        # 🚨 [V67.0 패치] 박스 교차 검토 선택 시 UI 동적 변경
        ...
        # 라디오 버튼이 "선물세트"일 때만 박스 업로드 칸 렌더링
        box_info = None
        box_nutri = None
    ...
    with tab2:
        st.info("시안(팩)과 서류를 1:1로 맵핑하며, '선물세트 교차 검토' 모드일 경우 박스와 팩의 텍스트가 동일한지 대조합니다.")
        if st.button("▶️ 정보표시면 원재료 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("시안 텍스트 추출 및 교차 대조 중... (최대 40초 소요)"):
                prompt = """
                [지시]: 오직 아래의 원재료명 1:1 맵핑 표와 검증 리포트만 출력하십시오.
                
                🚨 [1단계: 시안 원문 텍스트 강제 추출 (Thinking)] 🚨
                표를 그리기 전에 시안 이미지에 적힌 '원재료명' 텍스트를 있는 그대로 추출하십시오. 
                
                <thinking>
                - 팩(내포장) 원재료명 텍스트 추출: 
                - 박스(외포장) 이미지가 제공된 경우, 박스 원재료명 텍스트 추출:
                </thinking>
                ...
                **(🚨 텍스트 다이어트 필수): 서류 매칭 원료 칸에 복합원재료의 하위 성분(괄호)을 절대로 통째로 복사하지 마십시오! 핵심 원료명, 원산지, 비율만 짧게 요약하십시오.**
                ...
                (위 양식에 맞춰 시안에 있는 모든 원재료를 끝까지 1:1로 대조하는 표를 그리십시오.)
                
                - 🚨 [추가 지적 사항 (누락 검증)]: (서류에는 명시되어 있으나 시안에서 아예 '누락'된 원료를 지적)
                - [Rule 53 관련]: (마카다미아 등 제품명 연동 농산물 원산지 누락 위반 사항 별도 서술)
                
                ## 3️⃣ [알레르기, 주의사항 및 박스 교차 검증]
                - 결론: (✅ 적합 또는 🚨 부적합)
                - '~함유' 물질 원재료명 실존 여부:
                - 교차오염 경고 중복/모순 여부:
                - 📦 [Rule 47 외포장 교차 검증]: 만약 박스(외포장) 이미지가 업로드 된 경우, 팩(내포장)의 원재료명 글자와 박스의 원재료명 글자가 토씨 하나 안 틀리고 100% 동일한지 반드시 대조하여 결과를 적으십시오. 박스 이미지가 없다면 "➖ 박스 미업로드 (대상 아님)"이라고 적으십시오.
        ...
        if st.session_state["result_tab2"]:
            ...
            if thinking_match:
                thinking_log = thinking_match.group(1).strip()
                report_content = result.replace(thinking_match.group(0), "").strip()
                with st.expander("🧠 시안 텍스트 1차 추출 및 교차 대조 로그"):
                    st.markdown(f"*{thinking_log}*")
                st.markdown(report_content)
            else:
                st.markdown(result)
