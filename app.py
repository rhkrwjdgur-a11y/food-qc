...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V310.60 - 마스터 DB 추출 부활 패치)")
...
"안녕하세요", "PASS 1 추출 결과", "종합 검토 결과" 같은 불필요한 서론/요약/인사말을 단 한 글자라도 맨 앞이나 뒤에 출력하면 시스템 에러로 간주됩니다. 당신의 응답 첫 글자는 무조건 "## "으로 시작해야 합니다. 지정된 탭의 검토 항목 외에 다른 영역을 침범하는 월권행위를 절대 금지합니다.
