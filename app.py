...
**Rule 23. [식약처 영양성분별 '0' 표시 절대 규정 및 다단 표기 종속 법칙]**
   - 영양성분별로 '0'으로 표기할 수 있는 절대 커트라인이 명확히 다릅니다. AI는 0표기 합법성을 검증할 때 반드시 아래의 해당 성분별 기준을 엄격히 적용하십시오. (다른 성분의 기준을 섞어서 오판하면 치명적 오류입니다.)
     * [열량]: 5kcal 미만
     * [나트륨]: 5mg 미만
     * [탄수화물, 당류, 단백질, 지방, 포화지방]: 0.5g 미만
     * [콜레스테롤]: 2mg 미만
     * 🚨 [트랜스지방]: 0.2g 미만 (절대 0.5g이 아님에 극도로 주의할 것)
...
- 예: '세균수 3만 개/mL 미만' 또는 '세균수 3만/mL 미만'은 완벽한 합법(✅) 및 권장 표기입니다. '3만 미만 개/mL'처럼 단위가 중간에 끊어지면 가독성 및 어법 오류로 지적(⚠️)하십시오.
...
# 탭별 맞춤형 룰 타겟팅
# ==========================================
TAB1_RULES = [3, 9, 12, 17, 18, 19, 21, 24, 46, 50, 52, 53, 57, 62, 63, 66, 68, 71, 72, 84, 86, 87, 88, 94, 95, 96, 99, 100, 103, 105, 106]
TAB2_RULES = [1, 2, 4, 5, 8, 9, 12, 13, 14, 16, 28, 29, 30, 34, 35, 38, 39, 44, 45, 47, 48, 51, 53, 54, 60, 61, 64, 65, 70, 85, 89, 90, 91, 95, 101, 106]
TAB3_RULES = [3, 6, 10, 11, 23, 24, 25, 26, 27, 31, 32, 33, 40, 41, 55, 68, 79, 80, 82, 83, 97, 98, 106]
TAB4_RULES = [7, 15, 20, 22, 36, 58, 59, 73, 74, 75, 76, 77, 78, 81, 92, 93, 103, 105, 106]
...
def main():
    ...
    st.markdown("<h1>Label Guard AI <span style='font-size:0.5em; color:#64748b;'>식품 패키지 무결점 검증 시스템 (V5.9.8 Ultimate)</span></h1>", unsafe_allow_html=True)
...
| **주표시면 하단 함량 표시 팩트체크 (증빙 대조 강제)** | [Rule 12, 21, 52] | (※ 앞면에 원료(%)나 영양성분 수치 기재 시 반드시 배합비/시험성적서와 대조할 것. 증빙이 없거나 산출이 불가하면 절대 임의 합격 처리 금지, 무조건 ⚠️ 확인 요망 판정) | |
