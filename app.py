...
def main():
    ...
    st.markdown("<h1>Label Guard AI <span style='font-size:0.5em; color:#64748b;'>식품 패키지 무결점 검증 시스템 (V5.9.2 Ultimate)</span></h1>", unsafe_allow_html=True)
    ...
    with tab1:
        if st.button("주표시면 분석 시작", key="btn_main"):
            with st.spinner("데이터 스캔 및 판정 중 (최적의 3분할 미션 스캔)..."):
                missions = [
                    "[미션 1. 마케팅 텍스트 전수 스캔]: '주표시면'으로 업로드된 **모든 이미지(주표시면 1, 2, 3 등)**의 텍스트를 각각 추출하십시오. 팩 모양에 의해 반쪽씩 나뉘어 찍힌 이미지라면 문맥을 합쳐서 하나의 면으로 인식하십시오. 양쪽 면(앞/뒤)의 의무 표기 내용이 일치하는지 비교할 준비를 하고, 마케팅 클레임 진위 여부(Rule 99)를 위해 뒷면의 '원재료명' 란도 전수 스캔하십시오.",
...
| **주표시면 강조 원료 구조적 무결성 (1:1 픽셀 대조)** | [Rule 100] | (※ 쉼표 등 기호 삽입으로 형태소가 분리되었는지 적발. 단, "먼지나 스캔 오류로 인한 기호 인식일 수 있으니 육안 확인 부탁드립니다."라는 멘트를 반드시 포함할 것) | |
    ...
    with tab2:
        if st.button("정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("원재료 1:1 매칭 매트릭스 연산 중..."):
                ...
                judgment_prompt += "(AI는 아래 5단계를 단답형으로 100% 명확히 작성하여 논리를 확정할 표기 확정할 것)\n"
