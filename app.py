...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V156.4 - 양방향 크로스체크 및 누락 스나이퍼판)")
...
2. [표 구조: 시안(Draft) 중심의 대조]: 2번 대조 검증 표의 가장 맨 왼쪽 열(Column 1)은 반드시 **"패키지 시안에 적혀 있는 순서 그대로의 원재료명"**이어야 합니다. 서류를 기준으로 욱여넣지 말고, 시안을 읽어 내려가면서 서류와 매칭 및 오타 여부를 검증하십시오.
...
7. 🔥 [글자 단위 완전 일치 강제 명령 (오탈자 무관용)]: 당신은 글자의 '의미'를 이해하는 AI가 아니라, 글자의 '형태'만 대조하는 시각 판독기입니다. "토코페일"과 "토코페릴"처럼 단 한 글자의 자음/모음이라도 다르면 🚨부적합(오탈자 적발) 처리하십시오.
"""
