...
# ==========================================
# 📚 3. 88대 마스터 룰북 원문 (V311.41 완결본 - 100% 룰 추가)
# ==========================================
RULE_BOOK_FULL = """
...
🔥 **Rule 11. [영양정보 단방향 허용오차 법칙 및 수식 증명 강제]**
   - **[하한선 그룹(비타민,단백질 등)]**: `(용량 환산 실측값) >= (시안 표시량 × 0.8)` 이면 합법.
   - **[상한선 그룹(열량,당류 등)]**: `(용량 환산 실측값) <= (시안 표시량 × 1.2)` 이면 합법.
   - ⭐ **수식 증명 강제**: 성적서가 주어지면 반드시 `(실측값/100g) × (1회제공량 환산비율) = 실측환산값`을 계산하고, 이 값이 80% 또는 120% 컷오프를 만족하는지 숫자로 증명해야 합니다.
...
ALL_RULES_NUMBERS = list(range(1, 89))
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.41 - 박스 모드 영양오차/수학역산 강제 및 100% 룰 픽스판)")
...
| 영양성분 | 성적서 환산값(A) (계산식 포함) | 시안 표시량(B) | 법적 기준선 (B의 80% 또는 120%) | 🎯 % 역산 검증 (수식 포함) | 판정 및 상세 사유 (오차/반올림 준수 여부) |
|---|---|---|---|---|---|
