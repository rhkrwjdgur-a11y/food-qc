...
🔥 **Rule 14. [첨가물 표 4, 표 5, 표 6 정밀 교차 검증 (고시 원문 대조 강제)]**
   - 사용자가 업로드한 「식품등의 표시기준 고시 전문」 PDF 파일 내의 [표 4], [표 5], [표 6]을 반드시 검색하고 대조하여 첨가물 표기의 합법성을 판단하십시오.
   - **[🚨표 4 (명칭+용도명 병기 의무)]**: 감미료, 보존료, 산화방지제 등 표 4에 속하는 첨가물은 반드시 "명칭+용도명"(예: 수크랄로스(감미료))으로 기재되어야 합니다. 누락 시 부적합(🚨). (단, 향료에 '(착향료)' 병기 지적은 절대 금지)
   - **[🚨표 5 (간략명 표기 허용 및 용도명 대체 금지)]**: 표 5에 속하는 첨가물은 용도명으로 뭉뚱그려 쓸 수 없으며 반드시 정해진 명칭이나 허용된 '간략명'(예: L-글루탐산나트륨)으로 적어야 합법입니다.
   - **[🚨표 6 (용도명 대체 100% 합법/선택)]**: 유화제, 산도조절제, 증점제, 팽창제, 영양강화제 등 표 6에 해당하는 첨가물은 개별 화학적 성분명을 생략하고 오직 용도명(예: '유화제', '산도조절제')만 표기하거나 "영양강화제 2종"으로 묶어 표기해도 완벽한 합법(✅)입니다. 이를 부적합으로 지적하는 환각을 엄격히 금지합니다. (단, Rule 44의 혼합제제 괄호 안은 예외로 개별 명칭을 전개해야 함)
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V122.0 - 영양성분 기준치 마스터 확장판)")
            ...
            with st.spinner("서류 유형 분석 및 고시 파일 교차 대조 중..."):
                ...
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        7. 🚨 [첨가물 고시 대조 스나이퍼 (중요!)]: 서류에 첨가물이 있다면, 업로드된 식약처 고시 파일의 [표 4], [표 5], [표 6] 목록을 스캔하여 해당 첨가물이 의무 병기 대상인지, 간략명 대상인지, '유화제' 등 용도명으로 대체 가능한 대상인지 스스로 대조(Cross-check)한 후 판정하십시오.
                        </thinking>
                        ...
                        4. 🚨 [사유 설명 강제]: 판정 열에 단순히 '불일치'라고 적지 말고, Rulebook에 의거하여 왜 틀렸는지 법적 사유나 수정 방향을 상세히 명시하십시오. (예: Rule 14에 따라 [표 6] 첨가물은 용도명 표기가 합법임)
                        
                        ## 1️⃣ [원재료명 정밀 교차 검증 (통합 서류 vs 박스 시안 vs 팩 시안)]
                        - 결론: (✅ 적합 또는 🚨 부적합)
                        
                        | 통합 서류 기준 원재료명 | 타겟(박스) 시안 표기 | 비교용(팩) 시안 표기 | 판정 (Rule 기반 상세 사유 필수) |
                        |---|---|---|---|
                        ...
                        3. 🚨 [사유 설명 강제]: 판정 열에 단순히 '불일치'라고 적지 말고, Rulebook에 의거하여 왜 틀렸는지 법적 사유나 수정 방향을 상세히 명시하십시오. (예: Rule 14 적용)
                        
                        ## 1️⃣ [원재료명 정밀 교차 검증 (통합 서류 vs 시안)]
                        - 결론: (✅ 적합 또는 🚨 부적합)
                        
                        | 통합 서류 기준 원재료명 | 시안 대조 결과 | 판정 (Rule 기반 상세 사유 필수) |
                        |---|---|---|
                        | (무조건 1행 1원료 원칙, 순서대로 나열) | ... | ... |
                    ...
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        | 위 마스터 표 기준 원재료명 | 타겟(박스) 시안 표기 | 비교용(팩) 시안 표기 | 판정 (Rule 기반 상세 사유 필수) |
                        |---|---|---|---|
                        | (무조건 1행 1원료 원칙, 순서대로 나열) | ... | ... | ... |
                        ...
                        | 위 마스터 표 기준 원재료명 | 시안 대조 결과 | 판정 (Rule 기반 상세 사유 필수) |
                        |---|---|---|
                        | (무조건 1행 1원료 원칙, 순서대로 나열) | ... | ... |
