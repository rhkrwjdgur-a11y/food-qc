...
🔥 **Rule 24. [감미료 14pt 의무 표기 및 다중 강조표시 규칙]**
   ...
   - **단, 당류 강조표시(ZERO, 무당 등)가 시안에 2회 이상 반복되어 있는 경우에는, 반드시 "가장 큰 강조표시 주위"에 감미료 함유 문구를 기재해야 합니다.** 조그만 뱃지나 구석에 적은 경우 위치 부적합(🚨)으로 적발하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V39.0 - 상세 룰북 100% 영구 보존판)")
    ...
    with tab1:
        ...
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("주표시면 텍스트 및 뱃지 정밀 대조 중..."):
                ...
                ## 1️⃣ [주표시면 및 마케팅 뱃지]
                - 결론: (✅ 적합 또는 🚨 부적합/확인요망)
                - [Rule 24] 다중 강조표시 및 감미료 문구 위치 적합성: (강조표시가 2개 이상일 때, 가장 큰 글씨체 옆에 감미료 함유가 적혀 있는지 검토할 것)
                - 100% 원액 강조 적합성: 
                - 마케팅 숫자(N종, N곡 등) 정합성: 
                - 제품명 연동 함량(%) 표기 여부: 
                - 기타 특이사항 (과장광고 등): 
