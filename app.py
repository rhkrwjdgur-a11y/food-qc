...
🔥 **Rule 44. [🌟 혼합제제 해체 병합 완벽 허용 및 '명칭 유지 병합' 전면 금지 룰]**
   ...
   - ⭐ **[명칭 유지 병합 절대 불가]**: 단독으로 투입된 복합원재료(예: 비타민E혼합제제)와, 다른 복합원재료(예: 비타민믹스)의 하위 성분으로 들어있는 동일한 이름의 복합원재료를 합칠 때, **"OO혼합제제"라는 복합 묶음 명칭 자체를 그대로 살려서 하나로 퉁치는 것은 명백한 위법(🚨부적합)입니다.** 반드시 괄호를 깨고 최하위 단일 첨가물 성분들(예: 비타민C, 구연산, 변성전분 등)로 낱낱이 전개하여 흩뿌려야만 통합이 가능합니다. 시안에 'OO혼합제제'라는 묶음 명칭이 병합 목적으로 기재되어 있다면 무조건 적발하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V311.76 - 2.5 Pro 복구 및 방탄 패치)")
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                # V311.76: 마스터표 원본 복사 + 에러 방패 구조 반영
                ...
                6. [순서 역전 정밀 검증]: 표를 채우며 윗줄과 아랫줄 원료의 2% 이상 배합비 순위를 대조하고, 서류 순서와 시안 나열 순서가 다를 경우(역전 현상) 즉시 🚨부적합 처리하십시오.
