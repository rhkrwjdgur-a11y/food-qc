...
**Rule 44. [🌟 혼합제제 하위 성분 전개 절대 원칙 및 범용성 룰]**
   ...
   - [부적합 판정 기준]: 서류가 '혼합제제'인데 시안에 단순히 `향료`, `합성향료(두유향)`과 같이 단일 명칭으로 축약하거나 일부 성분을 누락했다면 무조건 🚨부적합 처리하십시오. (사유: 서류상 혼합제제이므로 하위 성분 전개 필수)
...
def main():
    ...
    st.title("식품 표시사항 정밀 검토 시스템 (V4.6 Ultimate Master)")
    ...
    with tab2:
        if st.button("정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("원재료 1:1 매칭 매트릭스 연산 중..."):
                ...
                judgment_prompt += "4. **[Rule 89 타겟 락온]**: 해당 원료의 [제조사 국가]와 [원물 원산지 국가]를 대조하라. 만약 [제조사는 한국]인데 [원산지는 외국산]으로 불일치(Mismatch)한다면 국내 가공품 이중 표기 룰(Rule 89)을 즉시 발동하라.\n"
