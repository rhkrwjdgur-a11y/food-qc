...
# ✅ 복잡한 연산, 하위 전개 분기 처리, 대용량 출력을 위해 Pro 모델 적용
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V88.0 - 전 구간 OCR 태그 강제 & 3방향 영양 연산)")
    ...
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("오차 계산 및 100% 전체 영양성분 3방향 대조 렌더링 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    ...
                    <thinking>
                    1. 데이터 추출: [근거_서류(성적서/배합비)]의 영양성분 값, [비교용_정답지_시안(팩)]의 영양성분 값(함량 및 %), [타겟_시안(박스)]의 영양성분 값(함량 및 %)을 각각 정확히 추출하십시오.
                    2. 오차 연산: 서류 실측값 대비 시안의 표시량이 법적 허용 오차(Rule 11) 내에 있는지 계산하십시오.
                    3. 일치 여부 확인: 팩 시안과 박스 시안의 숫자(함량 및 %)가 100% 동일한지 대조하십시오. (예: 칼슘이 팩은 160mg, 박스는 240mg으로 다르면 불일치)
                    </thinking>
                    ...
                    |---|---|---|---|---|---|
                    | (예: 나트륨) | 130 mg | 140 mg (7%) | 140 mg (7%) | ✅ 100% 일치 | ✅ |
                    | (예: 칼슘) | 230 mg | 160 mg (23%) | 240 mg (34%) | 🚨 불일치 (수치 다름) | 🚨 |
                    | (예: 콜레스테롤) | 4.99 mg | 5 mg 미만 (1%) | 5 mg 미만 (1%) | ✅ 100% 일치 | 🚨 서류오차 초과 |
                    ...
                    <thinking>
                    1. 데이터 추출: [근거_시험성적서]의 실측값과 [타겟_시안]의 표시량(함량 및 %)을 각각 추출하십시오.
                    2. 오차 연산: 서류 실측값 대비 시안의 표시량이 법적 허용 오차(Rule 11) 내에 있는지 계산하십시오.
                    </thinking>
                    ...
                    |---|---|---|---|---|---|
                    | (예: 나트륨) | 146.6 mg | 180 mg (9%) | 216 mg 이하 | ✅ 기준 이내 | ✅ |
                    | (모든 영양성분을 생략 없이 기재할 것) | ... | ... | ... | ... | ... |
                    """
        ...
        if st.session_state["result_tab3"]:
            ...
            if thinking_match:
                thinking_log = thinking_match.group(1).strip()
                report_content = result.replace(thinking_match.group(0), "").strip()
                with st.expander("🧠 오차 계산 및 수치 대조 로그 보기"):
                    st.markdown(f"*{thinking_log}*")
                st.markdown(report_content)
            else:
                st.markdown(result)
