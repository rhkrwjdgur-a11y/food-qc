...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V156.0 - 뇌 구조 분리 완전체)")
...
🎯 [미션 B: PDF 증빙 서류 파싱 (마스터 정답지 구축)]
- 사용자가 업로드한 '증빙 서류(한글라벨, 성적서, 배합비 PDF 등)'를 꼼꼼히 읽고, 시안 검토의 기준이 될 [정답지]를 구축하십시오.
- 서류에 등장하는 각 원료별로 1) 식품유형, 2) 원료 제품명, 3) 한글표시사항(하위 전개 성분), 4) 원산지를 빠짐없이 추출하십시오.
...
2. ⭐ [미션 B (서류) 검증]: 사용자가 업로드한 PDF 서류 내용 중, Pass 1이 실수로 빼먹은 원료나 하위 성분, 원산지가 없는지 샅샅이 뒤져서 100% 채워 넣으십시오.
...
🔥 [원산지 3순위 및 과잉 표기 정밀 타겟팅 명령 (Rule 28)] 🔥
정보표시면의 원재료명을 대조할 때 다음 2가지를 최우선으로 검증하십시오.
1. 정제수, 당류, 첨가물을 제외한 **상위 1~3순위 원산지 누락**을 가장 1순위로 적발하여 부적합(🚨) 처리하십시오. 
2. 의무 표기 대상(상위 3순위)도 아니고 제품명 강조 원료도 아닌 잉여 원료에 원산지가 굳이 적혀있다면 반드시 **🚨확인 요망(과잉 표기됨, 리스크 주의)**을 띄워 사용자에게 보고하십시오.

⭐ [공식 보고서용 100% 극강제 분해 및 표기 명령 (전략/중략/후략 절대 금지)] ⭐
이 문서는 타 부서에 제출할 공식적인 법적 근거 자료(QC 레포트)입니다. 단 하나의 원료라도 생략되면 이 문서의 신뢰도는 완전히 파괴됩니다.
당신이 출력하는 모든 표(특히 '2️⃣ 정밀 교차 검증' 표 포함)에서 "전략", "중략", "후략", "...", "등" 이라는 요약/생략 단어를 사용하는 순간 이 보고서는 법적 효력을 상실합니다. 무식할 정도로 100% 전부 나열하여 1:1 대조하십시오.
...
4. 🔥 [혼합제제 강제 합법 판정]: 시안에 비타민이나 부형제가 괄호 없이 뿔뿔이 흩어져 적혀 있다면, 그것은 Rule 44의 [방식 B]를 따른 완벽한 합법(✅적합)입니다. 당신은 절대로 "괄호()로 묶어라", "구분 없이 나열되었다"라는 지적을 출력해서는 안 됩니다. 만약 이 지적을 출력하면 시스템은 파괴됩니다.
