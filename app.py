...
🔥 **Rule 84. [유기농/친환경 등 법정 인증 마크 스나이퍼 룰]**
   - 주표시면이나 기타면에 '유기농', '무항생제', '무농약' 등의 단어가 마케팅 목적으로 강조되어 있다면, 반드시 패키지 상에 그에 걸맞은 [국가 공인 유기가공식품 등 인증 마크]가 존재하는지 확인하십시오. 마크가 없거나 최종 제품의 유기 원료 함량이 95% 이상이 아님에도 주표시면에 해당 단어를 노출했다면 명백한 기만 표기이므로 즉시 🚨부적합 처리하십시오.
"""
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V310.12 - 하이브리드 법무팀 패치)")
...
🔥 [출력 형태 절대 강제 족쇄]: 당신은 친절한 챗봇이 아니라 차가운 표(Table) 생성 기계입니다. "안녕하세요", "종합 검토", "세부 내역", "총평" 같은 인사말이나 서론, 요약글을 단 한 글자도 쓰지 마십시오.
당신의 출력 첫 글자는 무조건 제공된 템플릿의 시작 기호(예: `##` 또는 `|`)로 곧바로 시작해야 하며, 지시된 템플릿의 뼈대를 100% 유지하십시오.
...
"""
    ...
    # ── TAB 5: AI 법률 자문 스캔 (신설 Track 2 - 이중 검증 로직 탑재) ──
    with tab5:
        st.info("💡 [AI 자율 스캔 모드] 기계적 룰북을 벗어나, 업로드된 법령 PDF 원문을 바탕으로 마케팅 카피의 위법성 및 부당광고 소지를 찾아냅니다. (자체 팩트체크 가동 중)")
        if st.button("▶️ AI 법률 자문 자율 스캔 시작", key="btn_law"):
            with st.spinner("【1단계: 법령 PDF 분석 및 2단계: 팩트 크로스체크 진행 중...】"):
                missions = [
                    "1. 업로드된 시안에서 마케팅 카피, 제품명, 강조 문구, 타겟 문구 등을 모두 추출하십시오.",
                    "2. 추출된 시안 문구와 관련된 제한 사항을 업로드된 법령 PDF에서 검색하여 '법령명, 조항, 원문'을 그대로 추출하십시오."
                ]
                
                judgment_prompt = """
## 🤖 5️⃣ [AI 법률 자문 자율 스캔 리포트]
⭐ [환각 원천 차단 및 자체 검증(Cross-Check) 6대 강제 명령] ⭐
...
3. 환각 완전 삭제: 만약 단어 하나라도 다르게 조작되었거나 존재하지 않는 조항(예: 가짜 법률, 상상한 조항)이라면, 그 지적 사항은 표에서 완전히 삭제(Drop)하십시오.
4. 근거 조항 강제 인용: 최종 팩트체크를 통과한 항목만 아래 표에 작성하되, 반드시 **[문서명, 제O조 제O항(또는 별표)]**을 명시하고 원문을 따옴표("")로 완벽하게 인용하여 증명하십시오.
5. 억지 지적 금지: 검증 결과 위법 소지가 명확하지 않다면 당당하게 "✅ 식별된 법적 특이사항 없음"이라고 출력하십시오.
6. 🎯 [예외 조항 '단서 조건' 100% 완벽 검증]: 법령의 [별표] 등에서 특정 마케팅 용어(예: 유기농 등)의 사용을 조건부로 허용하는 경우, 그 조항에 명시된 **'표시 장소 제한(예: 원재료명 표시란에만 표시할 것)'**과 **'함량 표시 의무(예: 백분율 % 표시)'** 규정을 시안이 100% 준수했는지 이중 검증하십시오. 주표시면 등 허가되지 않은 장소에 노출되었거나 함량(%)이 누락되었다면, 허용 조항이 있더라도 가차 없이 "🚨위법 리스크(조건 미충족)"로 적발하십시오.

[검토 요청 사항]:
자체 검증을 통과한 법적 팩트만을 바탕으로 '과대광고', '소비자 기만', '인증 마크 위반', '단서 조항 위반' 소지가 있는 항목을 리포트하십시오.

| 식별된 마케팅 문구/디자인 | 적용 법령 및 근거 조항 (자체검증 완료된 원문 인용 필수) | AI 법무팀 자문 의견 (위법 리스크) |
|---|---|---|
"""
