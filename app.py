...
🔥 **Rule 52. [논리적 모순 탐지 및 마케팅/원재료 N종 카운트 스나이퍼]**
   ...
   - **[🚨역산 조작 환각 금지]**: 시안에 '3종'이라고 적혀있다고 해서 서류 기준 열에 '[식품첨가물] 향료 3종'이라고 거짓으로 지어내어 합격시키는 데이터 조작을 시스템 폐기 수준의 중대 범죄로 간주합니다. 서류 있는 그대로 '향료 2종'이라고 팩트만 적고 불일치를 지적하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V115.0 - N종 카운트 스나이퍼 완결판)")
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("서류 유형 분석 및 창의성 배제 100% 교차 검증 중..."):
                ...
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        6. 🚨 [N종 카운트 스나이퍼]: 시안 원재료명에 '향료 3종', '혼합제제 5종' 등 숫자가 기재되어 있다면, 반드시 업로드된 서류에서 실제 개수를 카운트하십시오. 시안의 숫자에 맞춰 서류 기준 열의 데이터를 '3종'으로 거짓 조작(역산)하는 것을 엄격히 금지합니다.
                        </thinking>
                        ...
                        4. 🚨 [사유 설명 강제]: 판정 열에 단순히 '불일치'라고 적지 말고, Rulebook에 의거하여 왜 틀렸는지 법적 사유나 수정 방향을 상세히 명시하십시오. (예: 서류엔 2종인데 시안엔 3종으로 과장 표기됨)
                        
                        ## 1️⃣ [원재료명 정밀 교차 검증 (통합 서류 vs 박스 시안 vs 팩 시안)]
                        - 결론: (✅ 적합 또는 🚨 부적합)
                        
                        | 통합 서류 기준 원재료명 | 타겟(박스) 시안 표기 | 비교용(팩) 시안 표기 | 판정 (상세 사유 필수) |
                        |---|---|---|---|
                        ...
                        6. 🚨 [N종 카운트 스나이퍼]: 시안 원재료명에 '향료 3종', '혼합제제 5종' 등 숫자가 기재되어 있다면, 반드시 업로드된 서류에서 실제 개수를 카운트하십시오. 시안의 숫자에 맞춰 서류 기준 열의 데이터를 거짓으로 조작(역산)하는 것을 엄격히 금지합니다.
                        </thinking>
                        ...
                        | 통합 서류 기준 원재료명 | 시안 대조 결과 | 판정 (상세 사유 필수) |
                        |---|---|---|
                        | (무조건 1행 1원료 원칙, 순서대로 나열) | ... | ... |
                    ...
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        | 위 마스터 표 기준 원재료명 | 타겟(박스) 시안 표기 | 비교용(팩) 시안 표기 | 판정 (상세 사유 필수) |
                        |---|---|---|---|
                        | (무조건 1행 1원료 원칙, 순서대로 나열) | ... | ... | ... |
                        ...
                        | 위 마스터 표 기준 원재료명 | 시안 대조 결과 | 판정 (상세 사유 필수) |
                        |---|---|---|
                        | (무조건 1행 1원료 원칙, 순서대로 나열) | ... | ... |
        ...
        if st.session_state["result_tab2"]:
            ...
            if thinking_match:
                thinking_log = thinking_match.group(1).strip()
                report_content = result.replace(thinking_match.group(0), "").strip()
                with st.expander("🧠 기계적 텍스트 필사 및 분기 대조 로그 보기"):
                    st.markdown(f"*{thinking_log}*")
                st.markdown(report_content)
            else:
                st.markdown(result)
