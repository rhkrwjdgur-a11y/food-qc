...
🔥 **Rule 38. [⭐ 알레르기 22종 하드코딩 및 교차오염 완벽 검증 룰 (V338 패치)]**
...
🔥 **Rule 52. [단순 명칭 강조 및 '함유/급원' 4조건 완벽 방어 룰]**
   ...
   - **[대원칙]**: 4가지(100g당, 100mL당, 100kcal당, 1회섭취량당) 중 **단 하나라도 충족하면 합법(✅)**입니다. 
   - 제품에 **"무당", "당 ZERO", "당류 무첨가"** 등의 강조 표시를 하면서 법적 식품첨가물인 감미료(수크랄로스, 에리스리톨, 아스파탐 등)를 사용한 경우, 해당 강조 표시 주변에 반드시 **"감미료 함유"**를 병기해야 합니다. 누락 시 소비자 기만으로 🚨부적합.
   - ⭐ **[알룰로오스 예외 룰]**: 단, 서류상 식품첨가물(감미료) 없이 **'알룰로오스(식품원료)'만 단독으로 사용하여 단맛을 낸 제품**이라면, "감미료 함유" 문구를 적지 않아도 **완벽한 합법(✅ 적합)**입니다. 알룰로오스를 감미료로 오인하여 부적합 처리하지 마십시오.
...
🔥 **Rule 63. [⭐ 190mL 전용 질소충전 동적(Dynamic) 스캔 룰 (V338 패치)]**
...
🔥 **Rule 92. [부분 캡처 이미지 한계에 따른 누락 항목 조건부 보류(⚠️) 룰]**
   - 사용자가 업로드한 이미지는 패키지의 특정 구역만 자른(크롭한) 부분 이미지일 수 있습니다. 따라서 **'식품유형', '반품/교환처', '소비자상담실', '1399 신고문구'** 등의 일반/행정 정보가 현재 검토 중인 이미지에서 보이지 않는다고 해서 즉시 🚨부적합(누락) 판정을 내리지 마십시오.
   - 대신 **⚠️(확인 요망)**으로 판정하고, 사유에 "현재 업로드된 부분 이미지에서는 해당 항목이 확인되지 않습니다. 패키지의 다른 면(주표시면, 측면 등)에 표기되어 있는지 실무자의 확인이 필요합니다."라고 부드럽게 안내하십시오. (단, 원재료명 리스트 내부의 성분 누락 등은 이 예외를 적용하지 않고 엄격히 부적합 처리하십시오.)

🔥 **Rule 93. [수출 겸용 소재지 주소란 '대한민국' 표기 예외 룰]**
   - 시안의 제조원 또는 유통전문판매원 '주소지' 텍스트에 한하여 "대한민국"이라는 국가명이 추가로 표기되어 있다면 절대 부적합 처리하지 마십시오. (이는 Rule 94의 'MADE IN KOREA' 마케팅 강조 표기와는 다른 사안입니다.)
   - 무조건 ⚠️(확인 요망)으로 판정하고, 사유에 "⚠️ [수출 겸용 확인 요망]: 시안의 소재지에 '대한민국'이 표기되어 있습니다. 원칙적으로는 품목제조보고서와 일치해야 하나, 수출 제품(또는 수출/내수 겸용)일 경우 수입국 통관 요건에 따른 합법적인 추가 기재입니다. 수출용이 맞는지 실무 부서에 최종 확인하시기 바랍니다."라고 출력하십시오.

🔥 **Rule 94. ['MADE IN KOREA' 스마트 조건부 스캔 룰]**
   - 시안에서 'MADE IN KOREA' 또는 '한국산' 텍스트가 발견되면, 반드시 증빙 서류(배합비)의 원산지 목록과 교차 검증하십시오.
   - 🟢 [합법(✅적합)]: 서류상 모든 원료가 100% 국산이거나, K-MILK 인증 마크가 있는 제품인 경우 (대외무역관리규정 완전생산물품에 해당하므로 허용).
   - 🔴 [위법(🚨부적합)]: 서류상 '수입산(외국산)' 원료가 단 1개(1%)라도 존재하는 경우. (식품은 부가가치 51% 혜택에서 배제되므로 수입 원료 포함 시 원산지 오인 기만행위에 해당함).
"""
...
ALL_RULES_NUMBERS = list(range(1, 95))
...
# ==========================================
def main():
    # ⭐ [시스템 현재 시각 동기화] AI 환각 방지용 현재 연도 추출
    ...
    # ⭐ 상단 고정 헤더 영역
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V338.00 - 교차오염 & 질소충전 동적 패치 완료)")
                ...
                if not content:
                    ...
                    try:
                        ...
                        cache = genai.caching.CachedContent.create(
                            ...
                            ttl=datetime.timedelta(minutes=60) # ⭐ 비용 절약을 위해 1시간으로 완벽 단축
                        )
    ...
    with tab2:
        if st.button("▶️ 정보표시면 원재료 기계적 1:1 맵핑 시작", key="btn_info"):
            with st.spinner("【원재료 1:1 매칭 매트릭스 연산 중...】"):
                ...
                4. 🛑 [초강력 족쇄: 마스터 DB 환각 창조 절대 금지 (Zero-Tolerance)]: 
                   - '2️⃣-1 서류 기반 마스터표'는 오직 사용자가 업로드한 'PDF/이미지 서류(한글라벨)'만 100% 신뢰하여 작성해야 합니다.
                   - 시안(포장지)에 '약콩원액두유 99.6%'처럼 아무리 비중이 높은 원료가 적혀 있더라도, 수석님이 그 서류를 업로드하지 않았다면 2-1 표에 절대 임의로 상상해서(환각) 쑤셔 넣지 마십시오.
                   - ⭐ [물리적 수량 일치 법칙]: 업로드된 서류(한글라벨)가 딱 3장이라면, 2-1 표의 행(Row)도 무조건 딱 3개에서 끝내야 합니다. 서류 없는 원료를 지어내면 즉시 시스템 강제 종료(Error) 사유가 됩니다.
                5. ⭐ [투트랙 스마트 1:1 대조 및 억지 맵핑 금지]:
                   ...
                   - [Track 2. 핵심 원물 억지 치환 절대 금지]: 단, 과일류(과채농축액), 유가공품 및 균류(유산균, 원유 등), 곡류, 육류 등 제품의 본질을 결정하는 핵심 원물은 종류가 다르면 절대 Rule 90을 핑계로 억지 매칭하지 마십시오! (예: 서류의 `복숭아농축액`을 시안의 `유산균배양액`과 매칭하는 것은 최악의 오류입니다.)
    ...
    with tab3:
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("【영양성분표 수치 환산 및 교차 검증 중...】"):
                ...
                missions = [
                    "⭐ **[영양정보표 2단/다단 레이아웃 100% 전수 스캔 강제 명령]**: 영양정보표 이미지를 스캔할 때 중간에 표가 2단(좌/우)으로 나뉘거나 하단에 수십 개의 미량 영양소(인, 요오드, 비타민 등)가 나열되어 있다면, 절대 중간에 읽기를 멈추지 마십시오. 이미지의 최하단 픽셀까지 샅샅이 스캔하여 시안에 적힌 30~50개의 모든 영양소 텍스트 단 하나도 빠짐없이 100% 추출해 내십시오. (말줄임표 절대 금지)",
        ...
        if st.button("▶️ AI 법률 자문 자율 스캔 시작", key="btn_law"):
            with st.spinner("【법률 스캔 중: 마케팅 리스크 및 심층 추적...】"):
                missions = [
                    ...
                    f"업로드된 시안에서 '12년 연속 1등', '특허', '효능 표방', '미래 시점(현재 {current_year}년 기준 {current_year + 1}년 이후의 날짜) 포함 문구' 등 마케팅 카피, 제품명, 강조 문구, 뱃지 디자인만을 정밀 스캔하여 추출하십시오. (단, {current_year}년 이하의 과거 및 현재 연도는 미래 시점이 아니므로 절대 위법 리스크로 지적하지 마십시오.)",
...
(단순 주의문구 체크를 넘어서, 앞서 적용된 기계적 룰(1~94)이 놓칠 수 있는 패키지 전반의 법적 이상 징후를 업로드된 고시 원문을 바탕으로 자유롭게 찾아내십시오. 
예를 들어, ① 앞면의 제품명/이미지와 뒷면 배합비 간의 법적 모순(예: 향료만 썼는데 원물 이름을 쓴 경우), ② 식품유형과 어울리지 않는 부적절한 규격 표기, ③ 타겟 고객층(예: 영유아, 임산부, 환자 등)을 표방함에 따라 추가로 요구되는 법적 요건 누락 등 **전체적인 맥락에서의 위법 리스크**를 심층 스캔하십시오.)

#### 📌 [식별된 구조적 모순 리스크]: "발견된 맥락적 모순 및 법적 이상 징후 작성"
* **적용 법령 및 조항:** [문서명, 제O조 제O항 또는 별표 규정]
* **법령 원문:** > "PDF 원문을 그대로 인용"
* **AI 법무팀 자문 의견 (위법 리스크):**
  * 🚨 **[리스크 총평]:** (법령에 근거한 모순 지적)
  * 🔍 **[다면(Double-Check) 교차 검증 결과]:** (제품명 vs 배합비, 디자인 vs 식품유형 등 전체를 아우르는 입체적 교차 검증 결과)
---
"""
