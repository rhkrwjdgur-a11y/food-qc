...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V105.0 - 소수점 및 행열 스나이퍼 완결판)")
    ...
    with tab1:
        if st.button("▶️ 주표시면 분석 시작", key="btn_main"):
            with st.spinner("주표시면 기계적 텍스트 추출 및 정밀 대조 중..."):
                ...
                4. 🚨 [기계적 수량/오탈자 교차 검증]: 주표시면의 포장 단위를 파악하고 다른 면과 철자, 띄어쓰기까지 100% 일치하는지 비교하십시오.
                </thinking>
            ...
            with st.spinner("서류 유형 분석 및 창의성 배제 100% 교차 검증 중..."):
                ...
                if doc_type == "통합 엑셀/PDF 자료 (마스터표 생략)":
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 하단 구석의 '식품이력추적관리번호' 주변 수량을 추출하여 논리적 충돌을 적발하십시오.
                        </thinking>
                        ...
                        4. 🚨 [수량/오탈자 교차 검증]: 이미지 하단 구석의 수량을 추출하고 비교하십시오.
                        </thinking>
                    ...
                    if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                        ...
                        4. 🚨 [수량/오탈자 교차 검증]: 수량을 추출하고 비교해 충돌 여부를 확인하십시오.
                        </thinking>
                        ...
                        4. 🚨 [수량/오탈자 교차 검증]: 수량을 스캔하여 비교하십시오.
                        </thinking>
        ...
        if st.button("▶️ 영양성분표 오차 정밀 연산 시작", key="btn_nutri"):
            with st.spinner("오차 계산 및 100% 전체 영양성분 3방향 대조 렌더링 중입니다..."):
                if inspection_mode == "선물세트 박스(외포장) 교차 검토":
                    ...
                    5. 🚨 [수량/오탈자 교차 검증 절대 명령]: 총 내용량을 확인하여 다른 면의 수량과 충돌하는지 적발하십시오.
                    </thinking>
                    ...
                    4. 🚨 [수량/오탈자 교차 검증 절대 명령]: 영양성분표의 총 내용량을 확인하고 비교하십시오.
                    </thinking>
    ...
    with tab4:
        if st.button("▶️ 기타면/측면 분석 시작", key="btn_extra"):
            with st.spinner("기타면/측면 기계적 텍스트 검토 중..."):
                ...
                3. 🚨 [수량/오탈자 교차 검증 절대 명령]: 기재된 수량이 다른 모든 면과 충돌하는지 기계적으로 샅샅이 스캔하십시오.
                </thinking>
        ...
        if st.button("▶️ 최종 종합 리포트 생성", key="btn_summary"):
            if not any([st.session_state["result_tab1"], st.session_state["result_tab2"], st.session_state["result_tab3"], st.session_state["result_tab4"]]):
                ...
                with st.spinner("모든 분석 데이터를 병합하여 최종 수정 지시서를 작성 중입니다..."):
                    ...
                    ### 📌 [핵심 지적 사항 및 수정 지시]
                    (위 분석 데이터에서 '부적합(🚨)' 또는 '확인요망'이 나온 내용들만 뽑아서 1, 2, 3번 불릿 포인트로 요약하십시오. 특히 '포장 단위 충돌', '띄어쓰기 오탈자', '고형분 누락'은 가장 중요하게 다루십시오.)
