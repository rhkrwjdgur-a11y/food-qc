...
🔥 [오탈자 무관용 원칙]: 단어의 의미가 통하더라도 글자(자음/모음)가 단 하나라도 다르면 무조건 부적합 처리하십시오. (예: 토코페일 vs 토코페릴 = 완벽한 불일치). 단, 단순 띄어쓰기 차이로 억지 부적합을 남발하여 핵심 오타를 가리는 행위는 자제하십시오.
...
🔥 **Rule 35. [범용 간략명/동의어 허용 및 첨가물 임의 수식어(분말/액상) 금지 룰]**
   ...
   - **[🚨 첨가물 명칭 임의 조작 절대 금지 (가장 중요 범용 원칙)]**: 식품첨가물 공식 명칭 앞뒤에 실무자 마음대로 **'분말', '액상', '결정' 등 제형을 뜻하는 단어를 붙여서 축약/조작하는 것(예: 유성비타민A지방산에스테르 ➔ 분말비타민A, 액상수크랄로스 ➔ 액상수크랄로스 등)은 어떠한 경우에도 '통용 명칭'으로 인정되지 않습니다.** 이는 복합원재료(혼합제제)의 부형제를 숨기거나 단일 원료처럼 위장하려는 악질적인 기만행위이므로, 첨가물 명칭에 임의의 수식어가 붙어있다면 무조건 🚨부적합 처리하고 "공식 명칭을 적고 누락된 하위 부형제를 모두 전개하라"고 지시하십시오.
...
def main():
    ...
    st.title("🏭 식품 표시사항 정밀 검토 시스템 (V156.8 - 첨가물 임의조작 원천차단판)")
    ...
    def run_qc_3pass(tab_rules: str, judgment_prompt: str):
        ...
        # ── PASS 1: 순수 텍스트 추출 및 증빙 서류 마스터링 (배열 강제 고정 및 짬처리 금지 패치) ──
        pass1_prompt = """
[PASS 1 - 텍스트 추출 및 서류 파싱 전용 명령]
⭐ 이 단계에서는 판정이나 평가를 절대 금지합니다. 오직 아래 두 가지 미션만 수행하십시오.

🎯 [미션 A: 라벨 시안 이미지 추출 (배열 강제 추출 및 오타 교정 절대 차단)]
- 당신은 인간의 맥락 이해 능력이 없는 '바보 광학 스캐너(OCR)'입니다. 시안 이미지(JPG, PNG)에 오타(예: 토코페일아세티이트, 비타민B₁염산염, 향료5종 등)가 있더라도 절대 올바른 단어(토코페릴, 향료3종 등)로 교정하지 말고 기괴한 글자 그대로 100% 필사하십시오.
- ⭐ [배열(List) 강제 추출]: 시안의 원재료명 텍스트를 줄글로 뭉뚱그리지 말고, 쉼표(,)를 기준으로 완벽하게 쪼개서 [1. 원액두유, 2. 정제수, 3. 분리대두단백 ... 17. dl-α-토코페일아세티이트] 처럼 번호가 매겨진 세로 리스트 형태로 추출하십시오.
...
- 🔥 [가장 중요한 명령 2 (절대 요약 금지 및 영양강화제 짬처리 금지)]: 하위 성분이 20개든 30개든 귀찮아하지 말고 100% 전부 타이핑하십시오. "외", "등", "(하위 성분 다수)" 같은 축약어를 사용하는 순간 이 시스템은 폐기됩니다. 서류에 적힌 글자 그대로를 모두 추출하십시오. 또한 여러 비타민 제제를 묶어서 '영양강화제'라는 단어로 임의 창조하여 짬처리하지 마십시오. 오직 서류에 명시된 낱개 성분으로만 행(Row)을 구성하십시오.
...
검증 규칙:
1. ⭐ [미션 A (시안) 검증]: Pass 1 텍스트가 번호가 매겨진 리스트 형태인지 확인하십시오. 시안 이미지에 명백한 오타(예: 토코페일)가 있는데 AI가 마음대로 정답(토코페릴)으로 교정했다면 원본 이미지의 오타 그대로 다시 훼손시켜 복구하십시오. 배열 순서가 시안의 실제 흐름과 일치하는지 강제 정렬하십시오.
...
3. (Rule 35) '중조 ➔ 탄산수소나트륨' 처럼 공전 공식 명칭이나 널리 쓰이는 통용명, 또는 식약처 간략명(표 5, 표 6)으로 치환된 경우. 단, 첨가물 명칭 앞뒤에 임의로 '분말', '액상' 등을 붙이는 꼼수는 이 룰의 보호를 받지 못하며 무조건 부적합(기만표시) 처리합니다.
...
2. ⭐ [표 구조: 1열(Column 1) 절대 Lock]: 2번 대조 표를 그릴 때, 가장 맨 왼쪽 열(Column 1)은 반드시 **"[미션 A]에서 추출한 번호가 매겨진 시안 원재료명 리스트"**를 그대로 복사해서 순서대로 붙여넣으십시오. 서류에 존재하는 '영양강화제'나 '비타민D3혼합제제' 같은 묶음 단어가 시안 리스트(미션 A)에 없다면, 절대 1열에 임의로 적어넣지 마십시오. 1열은 오직 시안에 적힌 낱개 원료명들로만 구성되어야 합니다.
...
4. 🔥 [혼합제제 강제 합법 판정]: 시안에 비타민이나 부형제가 괄호 없이 뿔뿔이 흩어져 적혀 있다면, 그것은 Rule 44의 [방식 B]를 따른 완벽한 합법(✅적합)입니다.
...
7. 🔥 [글자 단위 완전 일치 강제 명령 (오탈자 무관용)]: 1열(시안)과 2열(서류)의 글자를 비교할 때, 당신은 글자의 '의미'를 이해하는 AI가 아니라, 글자의 '형태'만 대조하는 시각 판독기입니다. "토코페일"과 "토코페릴"처럼 단 한 글자의 자음/모음이라도 다르면 🚨부적합(오탈자 적발) 처리하십시오. AI의 자의적인 문맥 파악 및 자동 교정을 엄격히 금지합니다.
"""
